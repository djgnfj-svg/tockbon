extends Control
## 플레이어 HUD — 장착한 마법진 슬롯(진 미니 다이어그램) + HP·마나 막대 + 획득 토스트 + 돈 카운터.
## 마을과 보스방이 같은 HUD를 쓰고 **씬별 분기가 없다.**
##
## 🔴 위력을 **직접 계산하지 않는다** — `RingPower.power_display()`를 부른다. 복사해 두면 한쪽만
## 고쳐도 아무도 못 알아채고 갈라진다(HUD는 "위력 140"이라 적어 놓고 130으로 때리는 식으로).
##
## 🔴 CanvasLayer 위에 산다 — 카메라가 플레이어를 따라다녀서 월드에 그리면 HUD가 같이 흘러간다.
## 슬롯 내용을 캐시하지 않고 `_draw`가 매번 `GameState.ring_equipped`를 읽는다 — 그래서 갱신이
## `queue_redraw()` 하나로 끝나고 GameState와의 **연결 순서를 따질 필요가 없다.**

const RingPower := preload("res://src/core/ring_power.gd")
## 🔴 진 미니 다이어그램 기하(`jin_slot_dots`·`rune_slot_positions`)의 단일 소스 — static 순수 함수라
## 그대로 부른다. 식을 베끼면 판·Tab 다이어그램과 조용히 어긋난다.
...
# ── 톤 = 책 UI(양피지·먹·가죽). HUD는 게임 위 오버레이라 어두운 가죽 바탕 + 따뜻한 크림 글자 ──
...
const SLOT_EDGE_ON := Color(0.85, 0.55, 0.18)
...
const BAR_TEXT := Color(0.94, 0.90, 0.80)          # 채운 색·빈 색 위 모두 읽히게 밝게
...
# ── 우하단 상태창(C) 아이콘 ──
## 🔴 **시각 표시일 뿐 클릭은 안 받는다** — HUD가 `mouse_filter = IGNORE`라 조준·발사 클릭을 안 먹는
## 것이 계약이고, 클릭 진입까지 주면 그 계약이 깨진다.
...
# ── 획득 토스트 ──
## 🔴 **별도 씬·자식 Control을 만들지 않고 여기 `_draw`에 얹는다.** `hud.tscn`이 없어 노드로 만들면
## 씬 두 곳을 고쳐야 하고 **한 곳만 고치면 한쪽 무대에서만 토스트가 뜨는 조용한 갈라짐**이 된다.
## 덤으로 새 Control을 안 까니 「화면 덮는 Control이 좌클릭을 다 먹어 발사가 죽는」 함정에도 안 걸린다.
...
# ── 한 줄 안내문(say) 수명 ──
## 🔴 **안내문은 기본이 만료다** — 안 그러면 "마나가 부족하다" 경고 한 번이 목표 줄을 덮고 **씬이
## 끝날 때까지 상주한다**(이미 마나가 찼는데 부족하다고 하는 거짓 경고가 화면에 남는다).
## 계속 떠 있어야 하는 문구만 호출부가 `sticky := true`로 명시한다.
const SAY_LIFE := 4.5
const SAY_FADE := 0.6   ## 마지막 이만큼 동안 흐려진다
...
## 돈 총액과 톡톡 세기 — `_draw`는 tween을 못 쓰니 상태값을 직접 읽는다.
var _coin: int = 0
var _coin_punch: float = 0.0


func _ready() -> void:
	# 🔴🔴 IGNORE가 아니면 화면을 덮는 이 Control이 좌클릭을 다 먹어 **발사가 에러 없이 죽는다.**
	...
	# 🔴 **슬롯을 갈아 끼우면 다시 그린다** — `_process`의 redraw 조건은 마나 변화·토스트·안내문·
	# 돈 톡톡뿐이라 **마나가 만땅이면 아무도 redraw를 안 걸어** 미니 다이어그램이 바꾸기 전 진을
	# 계속 보여 준다(= 쏘는 것 ≠ 보이는 것).
	EventBus.equipment_changed.connect(_on_equipment_changed)
	EventBus.codex_unlocked.connect(_on_codex_unlocked)
	_coin = _coin_total()   # 시그널을 기다리지 않고 부팅 세이브의 잔액을 곧장 보여 준다
...
## 한 줄 안내문. warn=true면 붉게.
## 🔴 기본은 `SAY_LIFE` 뒤 자동 소멸이다 — **`sticky := true`는 목표·유효한 지시에만 써라**(경고를
## sticky로 띄우면 그게 목표 줄을 덮고 상주한다). 빈 문자열은 즉시 지운다.
...
## 새 조립 부품을 얻었다 — 발신처 셋(보스 클리어·공방 제작·두루마리 픽업)을 한 곳에서 받아야
## 문구가 갈라지지 않는다.
## ⚠ **획득물이 아닌 codex 키도 이 시그널로 온다** — 리졸버가 빈 문자열을 주면 조용히 넘긴다
##   (안 그러면 "chapter_clear_ch1 획득!"이 뜬다).
## ⚠ 보스방은 이 발신 **뒤에** 자기 클리어 안내를 sticky로 덮는다 — 발신 순서를 바꾸면 두 줄이 다툰다.
...
## 자원(창고·가방)이 변했다 — 돈 총액을 다시 세고 **늘었으면** 톡톡을 찍는다.
...
## 창고 coin + 가방 coin 합. `GameState.bag`은 [{id, count}] 배열이라 직접 훑는다.
...
## 바닥 픽업이 가방에 도착했다.
## 🔴 **같은 아이템은 새 줄을 만들지 않고 수량을 더한다** — 안 그러면 드롭 셋이 동시에 도착할 때
## 세 줄이 떠 화면이 도배된다. 🔴 합칠 땐 **그 줄을 맨 뒤로 옮겨라** — 안 옮기면 위치가 낡은 자리에
## 남고 FIFO가 방금 갱신된 줄을 "가장 오래됐다"며 밀어낸다.
...
## 🔴 공개 관측점 — `_say`를 밖에서 더듬으면 리팩터 때 조용히 죽는다. 만료됐으면 빈 문자열.
func say_line() -> String:
	return _say


## 토스트 스택 열람. 사본을 준다 — 밖에서 고쳐도 HUD가 안 흔들리게.
...
func _process(delta: float) -> void:
	if not is_equal_approx(GameState.mana, _last_mana):
		...
		queue_redraw()
	# 아래 셋은 살아 있는 동안만 매 프레임 다시 그린다 — 평소엔 redraw 비용 0.
...
func _draw() -> void:
	...
	# ── 좌상단: HP(위)·마나(아래) 막대 → 돈 카운터 → 안내문 순으로 아래로 쌓는다 ──
	var bar_y := MARGIN
	...
	# 만료 직전 SAY_FADE 동안 흐려진다. sticky 문구는 안 흐려진다.
	if _say != "":
...
## 획득 토스트 — 우측 하단, 슬롯 윗쪽(슬롯·막대가 전부 좌측이라 안 겹친다).
## 최근 줄이 맨 아래고 오래된 줄이 위로 밀린다.
...
## 우하단 상태창(C) 아이콘 — 사람 실루엣 위에 "C" 키. 🔴 시각 표시일 뿐 클릭은 안 받는다.
func _draw_status_icon(font: Font) -> void:
	...
	# 사람 실루엣 — 머리(원) + 어깨(아래로 볼록한 호). UI 아이콘이라 도형 드로잉 허용.
	var cx := pos.x + s * 0.5
...
## 선택된 슬롯의 이름·위력·점수 한 줄 — 잘 그린 진과 막 그린 진의 차이가 여기서 보인다.
func _draw_selected_detail(font: Font, at: Vector2) -> void:
	...
	if design == null:
		...
		return
	# 🔴 점수 반올림도 core가 판다 — 「퍼펙트」가 그 반올림으로 정의돼 있다.
	# 🔴 `size`(진 크기)를 실어야 Tab 마법진 탭·조립 리포트와 **같은 숫자**가 나온다 — 안 실으면
	#   HUD만 낮게 나와 어느 쪽이 맞는지 아무도 모른다.
	# ⚠ 변형형(확산·폭발)이 끼면 그 숫자는 **갈래 하나**의 위력이다.
...
## 돈 카운터 — 엽전 도트 + "N닢". `at.y` = 이 줄의 윗변(다른 좌상단 텍스트와 같은 관례).
## 엽전은 UI 아이콘이라 도형 드로잉 허용 — 바닥 픽업 스프라이트와는 별개다.
...
## HP 막대 — GameState가 원장이다(씬을 갈아타도 남는 값).
## 숫자를 막대 안에 넣는 이유 = 막대만 있으면 "한 대 더 맞아도 되나"를 못 읽는다.
...
func _draw_mana(font: Font, at: Vector2) -> void:
	...
	if frac > 0.0:
		draw_rect(Rect2(at, Vector2(MANA_SIZE.x * frac, MANA_SIZE.y)),
			# 🔴 「부족」 경계는 `GameState.cast_mana_cost()`다 — 지팡이 배수가 얹힌 값이라,
			# core를 직접 부르면 값싼 지팡이를 껴도 막대만 빨갛다.
...
func _draw_bar_label(font: Font, at: Vector2, bar: Vector2, text: String) -> void:
	var fs := 10
	var baseline := at.y + bar.y - 4.0   # 작은 막대라 세로 중앙은 눈대중 보정
	draw_string(font, Vector2(at.x, baseline), text,
		HORIZONTAL_ALIGNMENT_CENTER, bar.x, fs, BAR_TEXT)


## 슬롯 한 칸 — 번호 + 진 미니 다이어그램. 이름·위력은 여기 안 적는다(선택 슬롯 상세 한 줄이 진다).
...
## 진 미니 다이어그램 — 원 둘레 8칸(열린 칸 주황 · 문양이 놓인 칸은 더 밝게 · 닫힌 칸은 흐리게) +
## 중심 룬 색 점. 진의 "형태"가 손끝에서 읽힌다.
func _draw_jin_diagram(center: Vector2, design: RingDesign) -> void:
	draw_arc(center, DIAG_RADIUS, 0.0, TAU, 24, DIAG_RING, 1.0)
	# 🔴 진이 **여러 층**을 가질 수 있다 — 채움 판정은 core 단일 소스 `design.is_slot_filled(k)`가
	# 쥔다. 여기서 `rings[0]`만 보면 바깥 층 문양이 통째로 안 그려져 **HUD가 조용히 거짓말한다**
	# (`open`은 층들의 합집합이라 "열렸는데 비었다"로 보인다).
	...
	# ── 중심 룬 색 — 융합진(룬 2개)이면 점이 둘이다 ──
	# 🔴 목록은 반드시 `RingDesign.runes_of`를 거쳐라 — `design.rune`(첫 룬)만 읽으면 **융합진의 두
	#   번째 룬이 HUD에서 사라져** 두 슬롯의 중심 점 색이 똑같아진다(쏘는 것 ≠ 보이는 것).
	# 🔴 자리 좌표는 `rune_slot_positions`를 그대로 부른다 — 각도를 베끼면 판·책 셀·HUD가 어긋난다.
	...
	# 자리가 여럿이면 점을 줄인다 — 판의 룬 축소와 같은 규약.
