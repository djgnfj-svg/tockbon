...
## ⚠ **오른쪽 아래에 붙인다.** 캐릭터 시작 자리가 왼쪽 아래(48,480)라 거기를 비워야
##  「조립하는 동안 다가오는 것을 볼 수 있나」가 성립한다(GDD). `HUD/Stats`(~y 210)도 피한다.
## 🔴 크기가 단계 3보다 늘었다 — **두 페이지가 되면서 가로가 필요해졌다.** 마법진 페이지 하나가
##  진 지름을 담아야 하는데, 388px를 반으로 가르면 200px짜리 진이 안 들어간다.
const WINDOW_RECT := Rect2(430, 238, 514, 286)
