...
## 🔴 **이제 셰이더다** — `monster_flash.gdshader`. 그 파일 머리가 「왜 uniform 이 아니라
...
const MONSTER_FLASH_SHADER := "res://src/view/monster_flash.gdshader"
...
## 불꽃 하나의 바깥 반지름(px). 심은 이것의 절반이다.
const MONSTER_BURN_R_PX := 4.5
...
## ⚠ **다음에 화면을 보는 사람이 볼 것**: 세대0에서 그림 디테일이 사나 · 세대1이 배경에 안 묻히나.
##  🔴 **둘은 반대 방향이라 한 값으로 둘 다 만족 못 할 수 있다** — 그러면 세대별로 갈라야 한다.
