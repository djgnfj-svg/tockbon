...
const CHAR_BURN_PX := 2.0

## 🔴🔴 **쓰러진 실루엣 — 서 있는 것과 확실히 달라야 한다**(기획 「화면」).
##  ⚠ **색만 바꾸면 안 된다.** 무적 흐림도 불 테두리도 이미 색 축을 쓰고 있어서,
##   셋이 겹치면 무엇이 무엇인지 못 가른다. ⇒ **모양을 바꾼다** — 눕힌 납작한 상자다.
## 🔴 눕힌 높이는 원래 폭에서 나온다(`CHAR_DOWN_H_RATIO × W_PX`) — px로 박으면 캐릭터 크기를
##  바꾸는 날 실루엣만 안 따라온다.
const CHAR_DOWN := Color(0.62, 0.55, 0.58)
const CHAR_DOWN_H_RATIO := 0.4
...
##
## 🔴🔴 **유예가 끝났다** — 「캐릭터와 그 주변이 보인다」 계약이 **캐릭터가 다칠 수 있게 되면서
##  다시 걸렸다.** 근거였던 「몬스터도 체력도 없어서 못 보는 대가가 0이다」가 사라졌다:
##  조립 중에 **내가 불에 타고 있는지**가 안 보이면 안 된다.
##
## 🔴 **세로만 줄여 위로 붙인다**(사용자 판정, 2026-08-04). **가로 90%는 그대로다** —
##  대안이었던 「가로도 줄여 좌우에 여백」은 **안 골랐다.** ⇒ 창 좌우로는 캐릭터가 여전히 안 보이고,
##  **보이는 근거는 「창 아래 띠가 화면 폭 전체」라는 것 하나뿐이다. 세로를 다시 늘리면 그 근거가 죽는다.**
##  · 창 아래 끝 **384** · 캐릭터가 스폰 자리에 선 높이 **480** · 점프 정점 **426**
##    (= `JUMP_VY_PX²/(2·GRAVITY_PX)` = 54px 위) ⇒ **걷는 동안에도 뛰는 동안에도 안 가린다**
