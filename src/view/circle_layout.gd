...
## ⚠ **진이 없어도 참이다 — 다만 「이 함수 안에서만」이다.**
##  그래야 진을 뺀 사용자가 다시 놓을 수 있는데, 🔴 **호출부가 그 앞에서 막으면 그대로 갇히고
##  아무도 안 짖는다.** 「구조적으로 보장」이 아니다 — 여기가 참인 것과 **눌러서 놓아지는 것**은 다르다.
##  ⇒ 실제로 놓아지는지는 **verify-run이 눌러 봐야** 안다(그물은 `Control` 메서드를 못 부른다).
