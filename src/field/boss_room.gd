...
##  • 🔴🔴 방 크기 **1600×1000**(x −800~800 · y −500~500) — **세112 R2에 2400×2200에서 줄였다.**
##    ⚠ 방향이 뒤집힌 값이다: 세88엔 「깊이 들어간다」를 만들려고 1200×1040 → 2400×2200으로 **키웠고**,
##    세110 장르 전환이 **그 걸어 다니는 시간을 폐기 사유(템포)로 지목**해 화면 1.5~2배로 되돌렸다
##    (`room_loop_design.md` R2). 이제 「깊이」는 방 하나가 아니라 **방을 이어서** 만든다.
##    🔴 **크기의 소유자는 씬의 `Ground` rect 하나다** — `_fill_tiles`·`_clamp_camera_to_room`·
##    `_spawn_props`가 전부 거기서 파생하므로 다시 바꿀 땐 그 rect만 고친다(balance.tres 금지 —
##    소유자가 둘이 되면 어느 쪽이 이기는지 매번 확인해야 한다).
##    ⚠ **씬이 손으로 든 좌표 넷은 rect를 따라가지 않는다** — `Player`(입구)·`Exit`·`Lights/*`,
##    그리고 **`data/chapters/*.tres`의 스폰 좌표**다. 방을 또 옮기면 그 넷을 같이 옮겨라.
##    구역(어귀·중간·깊은)은 **코드 개념이 아니라 `mob_spawns` y좌표 관례**다 — 새 스키마 0.
...
##
##  • `share` = 격자 한 칸이 이 종류가 될 확률(백분위 · 누적으로 잘린다). 둘의 합 71 = 나머지 29%는 빈칸.
##    🔴🔴 **후보 수 ≠ 선 수다 — 이 표에서 제일 자주 오독되는 줄이다.** 「빈칸이 29%뿐이니 빽빽하겠다」는
##     틀렸다: 세112 R2 실측에서 후보 45%가 **실제로는 화면당 나무 3.2~3.9그루**로 떨어졌다(회피·겹침이
##     절반 넘게 쳐낸다). `share`를 조일 땐 **그물이 찍는 실제 개수(`test_prop_layout_auto [1]`)만** 보고
##     맞춰라 — 빈칸 비율로 밀도를 짐작하면 방 크기가 바뀔 때마다 틀린다.
...
##  ⚠ 되살릴 땐 옛 바위 `share`(31)를 그대로 더하지 마라 — 지금은 합이 이미 70이라 빈칸이 없어진다.
##   나무·수풀에서 나눠 받아야 하고, 그 뒤 화면당 나무 수를 `[1]`로 다시 재라.
##  차폐(`occlude` 반경)의 경위는 **`git show`(세105~107)로만 캘 수 있다** —
##  ⚠ 옛 이 줄이 가리키던 `src/core/vision.gd`는 **세112 R3에 시야와 함께 삭제됐다.**
##
## 🔴🔴 **세112 R2: `share`가 14/18 → 44/26으로 뛰었다 — 손으로 놓은 20그루가 없어졌기 때문이다.**
##  세101~111의 화면당 나무 수는 **절차 + 씬의 20그루**가 같이 낸 값이었다(20그루 / 화면 10.2장 = 2.0을
##  씬이 지고 있었다). 방 축소와 함께 그 20그루를 지우자 **절차 층만 남아 화면당 0.65그루** —
##  세100이 「들판」이라 부른 0.8보다 더 휑했다. ⇒ **절차가 그 몫까지 진다.**
##  🔴 **두 번째 하락이 더 컸다**: 챕터 스폰 좌표를 방 안으로 옮기자(같은 R2) 회피 원이 **진짜로 방
##   안에서** 자리를 먹어 다시 2.27까지 떨어졌다. 방이 3배 작아지면 같은 반경(몹 100 · 보스 260 ·
##   문 150 · 지점 220)이 먹는 **면적 비율**이 3배가 된다 — 축소의 진짜 비용은 크기가 아니라 이것이다.
## 🔴 조인 절차는 이 표 머리말이 시킨 그대로다 — **그물이 찍는 실제 개수(`[1]`)를 보고** 맞췄고,
##  착지값은 나무 3.56/4.54/5.51(ch1/2/3) · 수풀 12/15/10이다(사용자 확정 *"화면당 3~4그루"* 언저리).
...
## 🔴 나무만 씬의 `Trees` 아래로 간다 — `test_daylight_tree_auto`가 **그 홀더로** 나무 계약(물리 몸 0 ·
##  뒤에서 비침)을 재므로, 여기로 꽂아야 새 나무가 저절로 그 그물에 든다.
##  ⚠ **세112 R2에 그 홀더가 비었다** — 손으로 놓았던 20그루를 축소와 함께 지웠고 지금은 **절차 나무만**
##   들어간다. 🔴 그래도 홀더를 지우지 마라(비어 있는 게 아니라 매 판 채워지는 자리다).
...
##
## 🔴🔴 **세112 R2: 148/56 → 120/45.** 크기가 아니라 **칸 수**가 이유다 — 148 격자는 1000짜리 방에
##  가로 11 × **세로 6칸**밖에 안 놓여서, `[4]`(어귀·중간·깊은이 고르다)의 한 대역이 **격자 2줄**로
##  줄었다. 그러면 보스 무대 하나(반경 260 = 대역 높이 333의 1.5배)가 **한 대역을 통째로 비운다.**
##  ⇒ 격자를 조여 대역당 줄을 늘렸다(세로 8칸). 🔴 **비율(지터/격자 ≈ 0.375)은 그대로 옮겼다** —
##   그 비율이 「격자로 줄 서는 것」과 「이웃이 붙어 다 쳐내지는 것」 사이의 자리다.
## ⚠ **share와 같이 움직여야 한다** — 격자를 조이면 후보 수가 늘어 밀도가 같이 뛴다(그래서 44로 내렸다).
