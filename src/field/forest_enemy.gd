...
## ⚠ 파일 이름의 「forest」는 흔적이다 — **라이브 무대는 챕터 던전**이고 `boss_room`이 보스·잡몹을
## 둘 다 이 씬으로 스폰한다.
##
## 🔴 **적 노드 계약**을 지킨다(참고 구현 = 허수아비 `src/spell/dummy_target.gd`):
...
##  • layer 4가 곧 **맞는 몸**이다 — 캐리어·탄 마스크가 5(world+enemy)라 여길 본다. 기본 레이어 1로
##    되돌리면 world로 읽혀 마법이 **부딪히기만 하고 take_hit이 안 불린다.**

## 적이 죽었다(로컬). ⚠ **로컬 구독자는 0**이다 — 킬카운트·퀘스트·클리어는 전부 `EventBus.enemy_died`가
## 나른다(수신 = `juice`·`game_state`·`boss_room`). 처치 경로를 만질 땐 그쪽을 봐라.
signal died

## 🔴 드롭 굴림·낱개 스폰의 **단일 소스** — 아래 `_roll_drops`·`_spawn_loose`는 이걸 부르는 얇은
## 껍데기다. ⚠ **여기 로직을 되돌려 적지 마라** — 그 순간 규칙이 두 벌이 되고 에러가 0이다.
const DropRoll := preload("res://src/field/drop_roll.gd")


## 적 투사체(gale 연사) — 탄은 field/actors를 안 물어 preload가 안전하다.
const GaleProj := preload("res://src/field/enemy_projectile.tscn")

## 🔴 상태이상·원소 반응의 **규칙 단일 소스.** 규칙을 여기 베끼지 마라 — 복사하면 "진흙인데
## 안 묶인다" 식으로 조용히 갈라진다.
const SR := preload("res://src/core/status_rules.gd")
## 상태 **보유고** — 적·허수아비가 같은 물건을 쓴다.
const SH := preload("res://src/core/status_holder.gd")
## 지속·반경·틱 간격은 전부 balance가 쥔다(연출값이 아니라 밸런스다).
const BAL := preload("res://data/balance.tres")

## 🔴 히트 플래시 셰이더 — modulate 곱셈(어두운 픽셀이 안 하얘짐) 대신 mix-to-white.
## Shader **리소스** 공유는 안전하고, 갈라야 하는 건 uniform을 쥔 ShaderMaterial 쪽이다
## (`_ready`가 per-instance 생성 — 공유하면 한 마리의 플래시가 전원 플래시다).
const FLASH_SHADER := preload("res://src/actors/hit_flash.gdshader")
## 발밑 그림자 — 여기서 자동 부착하므로 **미래의 모든 적이 공짜**다(씬마다 노드를 요구하면
## 새 적마다 침묵 누락이 생긴다).
const ShadowScript := preload("res://src/actors/shadow.gd")
## 죽음 연출 — 몸이 마나로 터진다. 죽는 몸은 전부 이 파일 하나를 지나므로 잡몹·보스가 공짜로 따라온다.
const ManaBurst := preload("res://src/actors/mana_burst.gd")

## ⚠ 기본값이 **없는 id**를 가리키면 `Db.get_enemy`가 null을 돌려주고 **hp 0짜리 유령이 선다**(에러 0).
##  라이브 경로는 전부 `add_child` 전에 대입하지만, 기본값으로 스폰되는 길이 하나라도 생기면 터진다.
@export var enemy_id: StringName = &"hound"

@onready var _visual: AnimatedSprite2D = $Visual
## 맞는 몸(히트박스). 씬 기본 반경은 9.0이고 `params.hitbox_radius`가 덮는다(`_apply_hitbox`).
@onready var _shape: CollisionShape2D = $Collision

## 발밑 그림자 손잡이 — `_attach_shadow`가 담는다.
## 🔴 **`get_node`나 자식 인덱스 0으로 찾지 마라** — `move_child(shadow, 0)`이 지금은 0번이지만
##  트리 순서는 연출이 만지는 자리다(그러면 어느 날 조용히 엉뚱한 노드를 흐린다).
var _shadow: Sprite2D = null

## 피격 손맛 — 넉백/플래시/스쿼시 **연출값**이라 밸런스가 아니라 const다(사용자가 때려 보며 조인다).
## idle 애니 기본 박자 — `params.anim_fps`가 없을 때. ⚠ **옛 하드코딩 값 그대로**라
## 키를 안 준 .tres는 동작이 안 바뀐다(하위호환 계약).
...
const HURT_HOLD_SEC := 0.15       ## 1프레임 hurt 스트립의 홀드 시간 (함정은 `_play_hurt` 참조)
...
## 시트 없는 몸(테스트 스텁)의 그림자 크기가 안 바뀐다(하위호환 계약).
...
## 행동 갈래 = `params.ai`. 스키마를 늘리는 대신 params에 얹어 「새 적 = .tres 한 장」이 **행동까지**
## 포함하게 한다. 수치는 전부 `_param`으로 .tres에서 읽는다 — balance.tres가 아니다.
...
## 돌진 예고 범위 — **지연 생성**이라 charge가 아닌 적은 노드가 없다. 적의 자식이라 적이 죽으면
##  같이 죽는다 = 취소·소유·수명을 새로 안 만든다.
...
## 돌진 예고 범위 **연출값**(밸런스 아님). 색 = TAKBON 60의 붉은 램프 3단.
##  ⚠ **크기는 여기 없다** — 길이·폭은 `.tres`의 돌진 수치에서 파생한다(`_charge_reach_*`).
##
## 🔴🔴 **알파가 색을 정한다 — 실측으로 고쳤다.** 밝은 낮 풀밭(`#4A9E38`) 위에 붉은색을 옅게 깔면
##  초록과 상쇄돼 갈색·올리브가 된다(α0.55 → `#845632`). 같은 A53030도 α0.92면 `#9E3931`로
##  **빨갛게 읽힌다** — 차오르는 쪽은 거의 불투명해야 한다. 취향이 아니라 배경과의 산수다.
##  ⚠ 어두운 배경에서 고르면 뭘 깔아도 잘 보여 **도구가 거짓말한다.**
##  ⚠ 반대로 **아직 안 찬 쪽을 짙게 깔지 마라** — 「닿을 범위」와 「찬 만큼」이 같아 보이면 남은
##   시간이 사라진다. 그쪽 읽힘은 채움이 아니라 **불투명한 어두운 테두리**가 진다.
...
## 「보는 쪽」 — 좌우 반전(`_update_facing`)이 읽는 한 값(순수 시각 축이다).
## ⚠ 기본이 `RIGHT`인 근거 = `SPRITE_FACES_LEFT == false`(시트가 오른쪽을 본다).
var _look_dir: Vector2 = Vector2.RIGHT

## 🔴🔴 **이름이 `sight_range`지만 시야가 아니라 어그로·리쉬 거리다 — 지워도 에러가 0이라 위험하다.**
##  `_ai_chase`가 어그로를, `_ai_charge` APPROACH가 접근 거리를, 보스 셋이 리쉬를 전부 이 한 키로
##  읽는다 ⇒ 지우면 **모든 추격 적이 그 자리에 굳는데** 나오는 건 `push_warning` 하나뿐이라
##  `SCRIPT ERROR` grep에 안 걸린다.
## ⚠ 없으면 `aggro_range`로 폴백한다 — 옛 이름을 쓰는 그물 주입 몸들이 아직 있다.
const SIGHT_KEY := "sight_range"
const SIGHT_KEY_LEGACY := "aggro_range"

## 🔴 뱀 보스(boss_snake) 전용 상태 — charge 변수를 재사용하지 않는다(두 AI가 섞이면 조용히 깨진다).
## 위브 추격 → 텔레그래프 러시 → 회복 → 다시 위브.
...
## hp 절반 페이즈 전이 — 1회 플래그. **보스 공용**이다(뱀은 위브 진폭·러시 빈도, gale은 돌풍·연사
## 빈도·이동속도 배율이 오른다). 전이 판정 패턴도 두 분기가 같다.
var _phase2: bool = false

## 🔴 gale 보스(boss_gale) 전용 상태 — charge/snake 변수를 재사용하지 않는다(뱀과 같은 원칙).
## 거리 유지(DRIFT) → 붙으면 돌풍 윈드업 → 밀쳐내기.
...
## 돌풍 텔레그래프 링 **연출값**(밸런스 아님). 반경은 `gust_radius`(.tres)가 쥔다.
const GUST_RING_SEGMENTS := 24      ## 링 원 분할 수
const GUST_RING_WIDTH := 2.5        ## 링 선 굵기(px)
const GUST_RING_COLOR := Color(0.75, 0.95, 1.0, 0.85)  ## 옅은 바람색

## 상태이상 보유고 — 보유·틱·반응 해결은 전부 holder가 한다(여기 남은 건 몸의 일: 피해·확산·색).
## 뽑아 둔 이유 = 허수아비가 같은 물건을 써야 **연습장에서 반응을 시험할 수 있다.**
var _status: SH = SH.new()
## 몸 색조(`EnemyDef.params.tint`). 흰색 = 곱셈 항등 = 「색조 없음」.
## 대입처는 `_apply_look`(읽기)·`_refresh_tint`(곱하기) 둘뿐이다 — modulate 소유권 계약 참조.
var _base_tint: Color = Color.WHITE
## 🔴 죽음 1회 보장 — DoT·연쇄·즉발이 같은 프레임에 겹쳐 `_die()`가 두 번 돌면 드롭이 두 번
## 떨어지고 퀘스트가 두 번 센다(`queue_free`는 프레임 끝에야 반영된다).
var _dead: bool = false
## hurt 홀드 세대 토큰 — 연타 시 마지막 발동의 타이머만 idle로 복귀시킨다(`_play_hurt`).
var _hurt_gen: int = 0
...
func _ready() -> void:
	...
	# 🔴 히트 플래시 material은 **per-instance 생성**이다 — 같은 ShaderMaterial을 전원이 공유하면
	# 한 마리의 플래시가 전원 플래시다(.tscn에 박으면 `resource_local_to_scene`이 필요해진다).
	if _visual != null:
...
## ⚠ **`params.color`를 되살리지 마라** — 소비자가 0인 죽은 키다. 죽음 연출이 마나 푸른색으로
##  통일돼서, 죽음 색이 다시 적마다 갈리면 「무슨 마나인가」라는 **없는 축**이 생긴다.
##  몸을 물들이는 건 `params.tint`다(다른 키 — 섞지 마라).
## 🔴 `size`는 루트 scale이라 **덩치가 곧 히트박스**가 되지만, **바탕 반경은 `params.hitbox_radius`가
##  쥔다** — size만으로는 시트를 다시 그릴 때(32→64px) 몸만 커지고 판정이 안 따라와 **쏴도 통과한다.**
func _apply_look() -> void:
	if _def == null or _visual == null:
		return
	# 프레임은 정사각(높이=한 변) 가로 스트립이라 런타임에 SpriteFrames를 구워 붙인다.
	...
	if not is_equal_approx(s, 1.0):
		scale = Vector2(s, s)
	# **몸 색조**(`params.tint`) — 네임드를 「덩치 + 색」으로 알아보게 하는 축이다(오라가 아니다).
	# ⚠ 실제 적용은 `_refresh_tint`가 한다 — modulate 소유권 계약을 안 깨려고 여기선 값만 들고 있는다.
	if _def.params.get("tint") is Color:
		_base_tint = _def.params.get("tint")
		_refresh_tint()


## **맞는 몸도 .tres가 쥔다**(`params.hitbox_radius`) — 씬 고정 반경으로 두면 시트를 다시 그릴 때
## 보이는 몸통에 쏴도 탄이 그냥 지나간다.
## 🔴 **값은 「루트 scale 이전」의 로컬 반경이다** — `Collision`이 루트의 자식이라 `params.size`가
##  자동으로 곱해진다. 화면 px로 해석하면 `size`를 만질 때마다 두 숫자를 같이 고쳐야 하고,
##  한쪽을 잊으면 **에러 없이** 몸과 판정이 갈린다.
## 🔴🔴 **`shape`를 duplicate 없이 쓰지 마라** — `.tscn`의 SubResource는 **씬 인스턴스끼리 공유**돼서
##  radius에 직접 대입하면 **마지막에 스폰된 적의 반경이 전원에게 번진다**(에러 0).
## ⚠ 키가 없으면 씬 기본값 9.0 그대로다(하위호환).
...
## 가로 스트립 시트(프레임 = 정사각, 한 변 = 시트 높이)를 루프 "idle" 애니로 굽는다. 프레임 수 = 폭 ÷ 높이.
## `params.hurt_sprite`가 있으면 **같은 규약**으로 비루프 "hurt"를 얹는다(없으면 hurt 없이 선다).
func _setup_frames(tex: Texture2D) -> void:
	var frames := SpriteFrames.new()
	frames.remove_animation(&"default")
	# ⚠ hurt는 데이터를 안 본다 — 피격은 짧은 반응이라 성격 축이 아니고, 종마다 다르면 "맞았다"가
	#   일관되게 안 읽힌다(느린 적은 피격 반응도 느려져 손맛이 갈린다).
	...
	if hurt_path != "":
		if ResourceLoader.exists(hurt_path):
			...
			# 시트가 없으면 hurt 없이 선다 — 침묵 대신 경고.
			push_warning("hurt_sprite를 못 찾았다 (%s) — hurt 애니 없이 선다" % hurt_path)
	_visual.sprite_frames = frames
	_visual.play(&"idle")


## idle 박자(fps) — 「새 적 = .tres 한 장」이 애니까지 덮는다.
## 기준선: 떠도는 것 2~4 · 묵직 4~6 · 보통 6~8 · 빠른 짐승 10~14 · 식물 3~5.
## ⚠ **값은 F5로 조인다** — 박자는 헤드리스도 스샷도 못 잰다.
...
## 🔴 idle 첫 프레임의 **한 변**(px) — 이 몸의 크기를 재는 **단일 소스**다(그림자와 죽음 폭발이
## 같이 읽는다 — 각자 재면 같은 몸을 두 크기로 보게 된다).
## ⚠ 반환은 **루트 배율 이전의 로컬 값**이다 — 그림자는 자식이라 배율을 저절로 먹고, 씬에 따로
##  붙는 폭발은 `_body_px`가 직접 곱한다. 여기서 미리 곱하면 그림자가 두 번 먹는다.
...
## 발밑 그림자 자동 부착 — 루트 scale의 자식이라 `params.size`(=덩치)를 공짜로 추종한다.
## 🔴 z는 **0 유지 + `move_child(…, 0)`** — 음수 z는 Ground(z0) 뒤로 숨는다.
...
## 행동을 `params.ai`로 가른다. 각 갈래가 `velocity`(추격 의지)를 세우면 공통 꼬리가 넉백을 얹어
## `move_and_slide`한다. 접촉 피해는 각 갈래가 `_contact`로 부른다.
...
## 🔴 **감속과 「보는 쪽」은 여기 한 곳에만** 둔다 — 모든 AI가 이 통로를 지나므로 한 줄이 전부를
##  먹는다. 갈래마다 곱하면 **새 AI를 넣을 때 조용히 빠진다.**
## ⚠ 넉백에는 감속을 안 곱한다 — 넉백은 손맛(연출)이지 이동 의지가 아니다.
...
## "chase"(기본) — sight_range 안이면 다가오고 attack_range 안이면 때린다.
## 🔴 **폴백 수치를 넣지 마라 — `_param_req`가 맞다.** 예전엔 `slime.tres`의 실값을 베낀 폴백이
##  있어서, .tres 파싱이 죽어도 **슬라임과 똑같이 움직여 데이터 죽음이 행동에 안 드러났다**
##  (그물 16회 스폰이 전부 통과했다). 지금은 결손이면 적이 굳고 `push_warning`이 뜬다.
...
## 🔴 윈드업 동안 뜨는 **바닥 예고 범위**(`_show_charge_band`)를 켜고 끄는 자리는 `_set_telegraph`와
##  **같은 두 줄**이다 — 한쪽만 옮기면 몸은 식었는데 바닥이 남는다.
...
func _ai_hover(delta: float, player: Node2D, to_player: Vector2, dist: float) -> void:
	...
	# 🔴 leash — `sight_range` 밖이면 자기 자리를 지킨다. 없으면 hover는 거리와 무관하게 늘 다가와
	# **적이 전부 입구로 모이고** 구역 배치가 무의미해진다.
	# ⚠ **이동 대입만 감싼다** — `return`으로 끊으면 아래 `_contact`와 분산 타이머가 멀리서 멈춘다
	# (leash를 넣는 세 갈래가 전부 같은 이유로 이 모양이다).
...
## "boss_snake" — 세그먼트 몸통(snake_body.gd)을 **살리는** 이동이 핵심이다.
##  • **위브 추격**: 진행 벡터에 수직 방향 sin을 얹어 머리가 S자로 미끄러진다 → 몸통이 그 S를 물려받는다.
##  • **텔레그래프 러시**: 짧게 멈춰 달아오른 뒤 확 뻗는다. 방향은 윈드업 시작에 락(그 사이 피하면 흘린다).
##  • **hp 절반 페이즈**: 위브 진폭·러시 빈도·이동속도 배율이 오른다(1회 플래그).
## 🔴 머리 회전은 **`_visual.rotation`만** 돌린다 — 루트 rotation은 SnakeBody 자식 좌표를 꼰다.
func _ai_boss_snake(delta: float, player: Node2D, to_player: Vector2, dist: float) -> void:
	...
	match _snake_state:
		SnakeState.WEAVE:
			# 🔴 leash — hover와 같은 이유다. 위브 계산만 감싼다(`_contact`·러시 쿨다운·페이즈2는
			# match 밖에서 이미 돌고 있어 안 건드린다).
...
## "boss_gale" — hover형 거리 유지 + 근접 징벌(돌풍) + 원거리 압박(연사).
##  • **DRIFT**: hover_min~hover_max 거리 유지. ⚠ `_ai_hover`를 재사용하지 않는다 — 거긴 disperse가 얽혀 있다.
##  • **돌풍**: 쿨 소진 && 반경 안일 때만 윈드업한다 — 플레이어가 안 붙으면 낭비 텔레그래프가 없다.
##  • **연사**: DRIFT 위 오버레이라 **이동을 유지한 채** 쏘고, 매 발 **재조준**한다(움직이며 피하는 재미).
##  • **hp 절반 페이즈**: 배율은 **쿨 리셋 시점에** 곱한다 — 진행 중 타이머를 건드리면 전이 순간 이중 적용된다.
func _ai_boss_gale(delta: float, player: Node2D, to_player: Vector2, dist: float) -> void:
	...
	match _gale_state:
		GaleState.DRIFT:
			...
			var dir := to_player / dist if dist > 0.01 else Vector2.ZERO
			# 🔴 leash — 없으면 gale이 **자기 자리를 버리고 입구까지 내려와** 잡몹과 함께 만나진다.
			# ⚠ **이동 대입만 감싼다** — 쿨다운 감소·돌풍/연사 발동·페이즈2 전이는 멀리 있어도 계속
			# 돌아야 한다(`return`으로 끊으면 페이즈2가 안 열리는 조용한 고장이 된다).
			...
			if _gale_gust_cd <= 0.0 and dist <= _param("gust_radius", 90.0) and dist > 1.0:
				...
				_set_telegraph(true)             # 붉은 달아오름 재사용 (charge·snake와 같은 규약)
...
func _resolve_gust(player: Node2D, dist: float) -> void:
	...
	if dodging:
		return
	# source_pos = 내 위치 — player_hurt에 실려 방향성 카메라 킥이 쓴다.
...
## 탄은 적의 자식이 아니라 현재 씬에 붙는다 — 적이 죽어도 이미 나간 탄은 남는다(유언 탄).
...
## 돌풍 반경 텔레그래프 링 — 윈드업 동안만 보인다. **절차적 VFX라 도형이 맞다**(돌풍은 형태 없는
## 공기 흐름이다). 지연 생성이라 gale이 아닌 적은 노드 자체가 없다.
## 🔴 z_index 양수 명시 — 음수 z는 Ground(z0) 뒤로 숨는다.
...
## 돌진 예고 **바닥 범위** — 윈드업 동안만 뜬다. **절차적 VFX라 도형이 맞다**(예고는 생명체·프롭이
## 아니라 「빛·가이드선」이다). 지연 생성이라 charge가 아닌 적은 노드 자체가 없다.
##
## 🔴🔴 **어디에 붙이나 — 「Ground 위 · 적과 플레이어 아래」는 z에 없는 자리다**(셋 다 z 0).
##  있는 건 **트리 순서**뿐이라 씬에 붙이고 플레이어 바로 앞으로 옮긴다(`_place_on_floor_layer`).
##   ⚠ `top_level`은 트랜스폼만이 아니라 **그리기 순서까지 바꿔** 예고가 적 몸 위를 덮는다.
##   ⚠ 적의 자식으로 두면 적이 런타임 `add_child`라 씬 자식인 `Player`보다 나중에 그려져
##      예고가 **플레이어 위**를 덮는다(자기가 어디 서 있는지 안 보이면 예고의 목적이 무너진다).
##   ⚠ 음수 z도 답이 아니다 — Ground(ColorRect z0) 뒤로 숨는다.
## 🔴 **대가 = 수명을 직접 쥐어야 한다**(적 자식이었으면 공짜였던 것). `_die`·`_exit_tree`가 지운다 —
##  안 지우면 **죽은 늑대의 붉은 띠가 바닥에 영영 남는다.**
...
## 🔴 인덱스를 숫자로 박지 않는다 — 플레이어가 씬 바로 밑에 없을 수도 있어(`Actors` 같은 묶음 노드)
## **씬의 직계 조상까지 거슬러 올라가 그 조각의 인덱스**를 쓴다. 그래야 방 씬을 재편해도 따라간다.
...
## ⚠ **여기서 `_exit_tree`를 쓰는 건 아래 `_wire_status` 금기와 다른 종류다.** 그쪽 금기는
##  **영구 배선(콜백)을 끊지 마라**는 것이고(리페어런팅 시 영영 죽는다), 이쪽은 **한 판짜리 연출
##  노드**라 다음 윈드업에 다시 만들어진다. 안 지우면 반대로 주인 없는 붉은 띠가 화면에 남는다.
...
func _update_charge_band(progress: float) -> void:
	if _charge_band == null or not _charge_band.visible:
		return
	# 🔴 위치·방향을 매 틱 다시 세운다 — 씬 소유라 부모(=적)를 안 따라가고, 윈드업 중에 맞으면
	#  **넉백으로 몸이 밀린다**(안 따라가면 예고가 몸을 떠난다).
	# 🔴 **scale은 절대 안 물려받는다** — 돌진 거리·`attack_range`는 월드 px라 덩치(`params.size`)와
	#  무관하다. 적 자식으로 되돌리면 덩치 큰 몸에서 **범위만 같이 커져** 예고가 거짓말한다.
...
## 공개 관측점 — 헤드리스는 「보인다」를 못 재지만 「윈드업 동안 있고 돌진에 없다」는 잴 수 있다.
...
## 「바닥에 깔린다」를 z가 아니라 트리 순서로 만들었으므로 **이 숫자가 그 계약의 유일한 측정
## 가능한 형태**다(헤드리스는 픽셀을 못 본다).
...
## 🔴 **params에서 되계산하지 말고 폴리곤을 실측해라** — 되계산하면 범위가 잘못 그려져도 이 리더가
##  옳은 값을 돌려줘 그물이 **거짓 그린**이 된다(`hitbox_radius()`가 형상에서 읽는 것과 같은 이유).
...
## 머리만 회전 — `_visual.rotation`. boss_snake 분기에서만 부른다. 스프라이트는 진행 방향 +x 기준이다.
## ⚠ **좌우 반전(`_update_facing`)과 한 몸에서 같이 못 산다** — `_uses_rotation_facing` 참조.
...
## **좌우 반전 — 「보는 쪽」의 시각 갈래.** 방향을 정하는 자리는 `_update_facing` 하나이고 입력은
##  `_look_dir`이다. **부르는 자리를 늘리지 마라** — 그 순간 갈아끼울 자리가 둘이 된다.
##
## 🔴🔴 **`_visual`에만 건다 — 루트에 `scale.x = -1`을 걸지 마라.** 루트를 뒤집으면 그림자·돌풍 링이
##  자식이라 같이 뒤집히고 `Collision`까지 좌우가 바뀌어 **판정이 에러 0으로 어긋난다.**
##
## 🔴🔴 **부호는 원본 시트가 어느 쪽을 보나가 정한다** — 지금 시트는 머리가 오른쪽이라
##  `flip_h = false`가 오른쪽이고 플레이어와 부호가 같다.
##  ⚠ 이 상수가 한 번 반대로 들어간 적이 있는데 **늑대가 늘 뒷걸음질로 달렸는데도 전 스위트가
##   그린이었다**(그물의 기대치를 같은 오독 위에 세웠다). 잡은 건 헤드리스가 아니라 실게임 스샷이다.
##  ⇒ 지금은 그물이 PNG를 열어 픽셀로 검산한다 — **아트가 시트를 좌우 반대로 그리는 날 빨개진다.**
##  ⚠ 앞으로 올 시트가 왼쪽을 보면 이 상수를 몸마다 가르지 말고 **시트를 뒤집어 그려라**(부호가
##   몸마다 갈리면 읽는 자리가 둘이 된다).
const SPRITE_FACES_LEFT := false

## 반전 데드존 — 거의 수직으로만 볼 때 `x`가 0 근처에서 떨려 스프라이트가 **파르르 뒤집히는**
##  것을 막는다. 연출값이라 balance가 아니다(0.15 = ±8.6°).
## 🔴🔴 **단위는 「정규화 벡터의 x 성분」이다 — 입력과 짝을 갈라 두면 조용히 죽는다.** 입력을
##  `velocity`(px/s)로 되돌리면서 이 값을 그대로 두면 `absf(x) <= 0.15`가 거의 항상 거짓이 되고,
##  반대로 px/s 문턱(예: 12.0)을 정규화 벡터에 쓰면 **항상 참**이 되어 **반전이 통째로 죽는데
##  에러가 0이다.** ⇒ 입력을 바꾸는 사람은 이 상수도 같이 바꿔야 한다.
## 🔴 **돌진 윈드업은 이 값에 안 얹혀 있다** — 윈드업 동안 `_desired_look()`이 `_charge_dir`을
##  고정으로 돌려주므로 고개가 안 돈다(방향 락이 거짓말이 되지 않게).
...
## 🔴 **회전으로 방향을 내는 몸** — 거기에 `flip_h`를 겹치면 회전된 스프라이트를 로컬 x축으로
##  거울질해서 머리가 진행 방향의 **정반대**를 가리킨다. 회전과 반전은 한 몸에서 같이 못 산다.
## ⚠ **판별이 「보스면 제외」가 아니다** — 회전을 안 쓰는 보스는 반전해야 한다. 기준을 「보스」로
##  적으면 앞으로 올 **비대칭 보스가 조용히 굳는다.**
func _uses_rotation_facing() -> bool:
	return _ai == "boss_snake"


## 공개 관측점 — 지금 몸이 향한 쪽(-1 왼쪽 · +1 오른쪽 · 0 = 몸이 없다).
...
## 「보는 거리」의 **단일 소스** — `sight_range`이고, 없으면 `aggro_range`를 승계한다.
## ⚠ 갈래별 폴백을 **한 값으로 합치지 마라** — 그게 특정 `.tres`의 사본이 되어 데이터 죽음을 가려 준다.
...
## **필수 수치판**(`_ai_chase` 전용) — 둘 다 없으면 0을 돌려주고 한 번 경고한다 = 적이 굳는다(눈에 보인다).
## 경고 문구는 **새 이름**으로 낸다 — 지금의 정본 키가 그것이다.
...
## 고개가 계속 돌면 방향 락이 거짓말이 된다(옆으로 피하면 돌진을 흘리는 것이 계약이다).
...
## 🔴🔴 **돌진이 닿는 자리 — 판정과 그림이 여기 한 곳에서 나온다.**
## 그리는 쪽이 값을 베끼면 한쪽만 조였을 때 *"보이는 곳 밖에서 맞는다"*가 되고, **그건 예고가 없는
## 것보다 나쁘다.** 폴백까지 const로 뽑은 것도 같은 이유다 — 리터럴을 두 자리에 적으면 데이터가
## 빠졌을 때 **그림과 판정이 서로 다른 기본값**으로 갈라진다(에러 0).
##
## 기하: 판정은 `_contact`가 **중심 거리 ≤ attack_range**로 재고 몸은 `charge_speed × dash_sec`만큼
## 미끄러진다 ⇒ 닿는 자리 = 그 선분을 반경 attack_range로 부풀린 캡슐.
## ⚠ 이 캡슐은 상계다 — 쿨다운이 남아 있으면 그중 한 번만 든다. **좁게 그리는 쪽으로는 틀리지 않는다.**
...
## 🔴 구르는 중이면 흘린다(구르기 = 무적 계약) — `player.is_rolling()`이 유일 판정이다.
## `.call`로 부르는 이유: player는 Node2D 타입이라 정적으로 못 찾는다(공용 배우 계약 무변경).
...
## 재생 — `regen_per_sec > 0`이면 초당 회복(상한 = `_def.hp`). 죽은 뒤엔 회복 안 한다.
## 「빨리 몰아쳐 죽여야 하는」 표적을 만든다. 대부분 적은 키가 없어 no-op.
...
# 일뿐이다 — 어떤 조합이 무엇이 되는지·얼마나 가는지를 이 파일에서 판단하지 마라.


## holder의 콜백을 이 몸에 잇는다(`_ready`에서 한 번). 🔴 **콜백 경계**가 분리의 핵심이다 —
...
func _wire_status() -> void:
	_status.on_dot = func(amount: float) -> void:
		...
		# 피해 숫자·히트스톱·피격음이 물려 있다. 0.5초마다 쏘면 화면이 숫자로
...
## 🔴 **여기 `_exit_tree`로 콜백을 끊지 마라.** 끊을 순환이 없고(Callable은 소유자가 Node면 ObjectID만
## 쥔다), 끊어 두면 **`_wire_status`가 `_ready`에만 있어서** 노드를 뺐다 다시 넣는 순간(리페어런팅·풀링)
## 콜백이 영구히 죽고 **적이 상태를 하나도 안 받는데 에러가 안 난다.**
## ⚠ 위쪽 `_exit_tree`(돌진 예고)와 가르는 기준은 **「나갔다 들어오면 스스로 되살아나나」**다 —
##  **연출은 지운다 · 배선은 안 끊는다.**
...
## rune = 이 버스트의 정체 룬 — 자신·연쇄 대상 모두 `take_reaction_damage`에 그대로 넘긴다.
func _burst_damage(radius: float, amount: float, include_self: bool, result_status: int, rune: int) -> void:
	# 🔴 링이 반경을 폭로하므로 amount 가드 **앞**에 둔다(반응은 일어났으니 링은 늘 뜬다).
	...
	for node in get_tree().get_nodes_in_group("enemies"):
		...
		if node.has_method(&"take_reaction_damage"):
			# 감전(SHOCK)만 대상마다 번개 아크 — 증기(NONE)는 링만. 피해 전에 쏜다.
...
func _spread_statuses(statuses: Dictionary) -> void:
	if statuses.is_empty():
		return
	# 여러 상태를 옮겨도 아크는 **한 대상에 한 가닥**. 대표 상태는 holder 공개 API로 얻는다 —
	# 몸이 holder 내부 dict를 더듬지 않는다.
...
## 확산이 옆 적에게 상태를 옮길 때 쓰는 유일 경로(내부 필드를 남이 더듬지 않게).
...
## 공개 관측점 — 헤드리스가 상태를 **공개 API로만** 확인하게.
...
## 🔴 `rune` = 이 반응의 정체 룬(감전=BOLT·증기=WATER) — `enemy_hit`에 그대로 실어야 **소리가
## 반응과 맞는다**(하드코딩하면 감전 연쇄가 불 소리를 낸다).
...
## 🔴 **modulate 소유권 계약**: modulate는 **rgb = 몸 색조 × 상태 틴트 · a = 분산** 두 축뿐이고,
## 팝(흰 섬광)·텔레그래프(붉음)는 셰이더 uniform이 쥔다. **만지는 코드는 이 함수와 `_refresh_alpha`
## 둘뿐이어야 한다** — 3파전으로 되돌리지 마라.
## 🔴 rgb가 **곱**인 것이 요점이다 — `_apply_look`에서 `_base_tint`를 modulate에 직접 쓰면
##  **첫 상태이상 한 번에 몸 색조가 영영 지워진다**(에러 0).
## 🔴 `a`도 **파생이다**(`_alpha_now`) — 화면에서 되읽으면 대입자와 이 함수가 서로의 결과를
##  주고받는 왕복이 되어 순서 의존·부동소수 드리프트가 들어온다.
...
## 🔴 **알파의 유일한 진실원.** 지금 축은 분산 하나지만 **곱셈 모양은 그대로 둔다** — 축을 늘리는
## 쪽이 각자 `modulate.a`에 대입하면 한쪽이 `a = 1.0`을 찍어 다른 축을 지워도 **에러가 0이다.**
## ⇒ 축을 얹고 싶으면 **여기 곱을 늘려라. `modulate.a`에 직접 대입하지 마라.**
func _alpha_now() -> float:
	return DISPERSED_ALPHA if _dispersed else 1.0


## 🔴 **`modulate.a`에 대입하는 유일한 함수.** 축을 바꾼 쪽이 이걸 부른다.
## 🔴🔴 **알파를 거는 노드는 `_visual`과 그림자 둘이다 — 루트에 걸지 마라.** 「루트에 걸면 그림자도
##  자식이니 공짜」가 자연스러운 손인데, 그 한 줄이 **돌풍 링을 같이 죽인다**(링은 `_visual`의
##  형제다). 공격 예고가 안 보이는 건 아무도 의심하지 않는 종류의 고장이다.
## ⚠ 그림자 자신의 반투명은 텍스처 그라디언트 안에 있지 modulate에 없다 — 충돌 없음.
## ⚠ 허수아비는 분산이 없어 알파 축이 상수 1.0이다 — 파리티를 이유로 이 함수를 거기 옮겨 심지 마라.
...
## 🔴 셰이더 `telegraph_amount`가 팝의 `flash_amount`와 **갈라져 있어야 한다** — 하나로 합치면
## 팝 트윈이 0으로 감쇠하며 **윈드업 중에 맞으면 섬광이 텔레그래프를 지운다.**
...
## 분산 표시 — 분산 중엔 반투명(때리기 나쁨이 보인다). 경감 자체는 `take_hit`이 적용한다.
## 🔴 **자기 축만 갱신하고 대입은 `_refresh_alpha`에 맡긴다**(3파전 방지 — 위 머리말).
...
## 🔴 **필수 수치**용 — `_param`의 fallback은 「없으면 중립」인 값에는 맞지만, 없으면 안 되는 값에
## 쓰면 **그 폴백이 특정 .tres의 사본**이 되어 데이터 죽음을 가려 준다.
## 여기선 0.0을 돌려주되(적이 굳는다 = 눈에 보인다) 키마다 **한 번** 경고한다
## (매 프레임 경고하면 로그가 잠겨 진짜 경고가 묻힌다).
var _warned_params := {}
...
## 공개 HP 리더 — 그물이 공개 API로 확인할 유일 경로다(`_hp`는 리팩터 때 옮겨 다니는 물건 = 계약이 아니다).
func hp() -> float:
	return _hp


## 공개 페이즈 리더 — 1 = 기본, 2 = hp 절반 이후. 보스가 아닌 적은 항상 1.
func phase() -> int:
	return 2 if _phase2 else 1


## 공개 관측점 — **최종(화면) 히트박스 반경**(루트 scale까지 곱한 값).
## 🔴 **params에서 되계산하지 말고 실제 형상에서 읽어라** — 되계산하면 `_apply_hitbox`가 통째로
##  죽어도 이 리더가 옳은 값을 돌려줘 그물이 **거짓 그린**이 된다.
...
## 🔴 계약: `enemy_hit`는 **약점 배율·경감까지 반영한 최종 피해**로 발신한다.
func take_hit(damage: float, rune_type: int, status: int, status_power: float) -> void:
	if _dead:
		return
	# 🔴 직격 피해 0 = 융합진 보조 룬의 **상태 전용 히트**. 손맛·발신·넉백·죽음판정을 건너뛰고 상태만
	# 얹는다 — 안 그러면 한 발이 룬 수만큼 `enemy_hit`·팝·피해숫자 "0"을 도배한다.
	# ⚠ 반응 **버스트**는 `take_reaction_damage` 별도 경로라 이 가드와 무관하다.
	if damage <= 0.0:
	...
	# 피해 경감 — 방어·분산을 「막는 비율」로 곱한다. 0.95 상한이라 완전 무적은 못 만든다.
	dealt *= (1.0 - clampf(_param("armor_reduction", 0.0), 0.0, 0.95))
	if _dispersed:
		dealt *= (1.0 - clampf(_param("dispersed_resist", 0.0), 0.0, 0.95))
	# ⚠ hp를 깎는 자리는 **셋**이다: 여기(직격) · `take_reaction_damage`(반응·연쇄) ·
	#  `_wire_status`의 `on_dot`(DoT 틱).
	_hp -= dealt
	...
	_status.apply_incoming(rune_type, status, status_power)
	# 넉백 = 플레이어 반대쪽 — 탄이 플레이어→적 방향으로 오므로 그 근사다(take_hit 계약을 안 넓히고도
	# 맞는 방향으로 밀린다).
	var p := _player()
	if p != null:
...
## 죽으면 **드롭을 굴려 바닥에 떨군다** — 가방에 넣는 건 픽업이 플레이어에 닿을 때다
## (`add_to_bag` → 귀환 시 창고로 회수 · 죽으면 통째로 사라진다는 흐름은 그대로다).
## 🔴 룬 조각은 **`until_unlock` 관문 드롭으로만** 나온다 — 확률 줄로 되살리면 진행 그물이 빨개진다.
## 🔴 픽업 소리는 여기서 안 낸다 — 실제로 주울 때(픽업) 울린다.
func _die() -> void:
	if _dead:
	...
	if _def != null and scene != null:
		...
		if not rolled.is_empty():
			# 보스 포함 모든 적이 재료를 낱개로 떨군다 — 값어치는 픽업의 **등급 후광**이 알린다.
			_spawn_loose(scene, rolled)
	# 처치 순간 1회 — GameState가 KILL 퀘스트를 센다. enemy_id를 실어 특정 적 목표도 가능하다.
...
## 드롭 테이블을 굴린다 — **로직은 `drop_roll.gd`가 쥔다.** 여기 남은 일은 `GameState`를 찾아
## 넘기는 것뿐이다: static 안에서 오토로드 식별자를 쓰면 `-s` 테스트가 컴파일 단계에서 죽는다.
...
## 낱개 바닥 픽업 — **로직은 `drop_roll.gd`가 쥔다.** 여기 남은 일은 죽은 자리를 넘기는 것뿐이다.
##
## 🔴🔴 **지연 실행이 계약이다.** 여기까지 오는 길이 `projectile._on_body_entered`(= 물리 flush 중)라,
##  `CollisionShape2D`를 가진 픽업 씬을 그 자리에서 `add_child`하면 **드롭 하나당 엔진 에러 하나**가
##  뜬다(`Can't change this state while flushing queries`).
##  ⚠ 그 문구의 「monitoring」은 거짓 단서다 — 진짜 원인은 `CollisionShape2D`가 `ENTER_TREE`에서
##   `shape_owner_set_disabled`를 다시 거는 것이다.
## 🔴🔴 **헤드리스가 구조적으로 못 잡는다**: ⓐ 접두가 `SCRIPT ERROR`가 아니라 `ERROR`고
##  ⓑ 그물은 `take_hit`을 직접 불러 죽여서 **물리 콜백을 아예 안 지난다.**
...
## 팝 — 셰이더 플래시 + 스쿼시(피격 손맛). modulate를 **아예 안 만진다**(흰 섬광은 셰이더
## `flash_amount`가 진다 — mix-to-white라 어두운 픽셀도 하얘진다).
## 🔴 **scale은 `_visual`에만** 준다 — 루트 scale은 `_apply_look`이 쥔 덩치(=히트박스)라
## 건드리면 맞을 때마다 히트박스가 출렁인다.
func _pop() -> void:
	if _visual == null:
		return
	var mat := _visual.material as ShaderMaterial
...
## 피격 프레임 재생 — `params.hurt_sprite`가 있는 적만. `_pop`(플래시+스쿼시) 위에 얹는 층이라
## hurt가 없는 적은 no-op다.
func _play_hurt() -> void:
	...
	if not _visual.sprite_frames.has_animation(&"hurt"):
		return
	_visual.play(&"hurt")
	# 🔴 1프레임 스트립이면 `animation_finished`가 **즉시** 온다 — 그대로 두면 한 틱 번쩍하고 끝나
	# "안 보인다". 타이머로 홀드한 뒤 idle 복귀로 분기한다.
	# 🔴 세대 토큰 — 연타 시 **첫** 타이머가 뒤 hurt를 일찍 끊지 않게 마지막 발동만 복귀시킨다.
	# 람다가 아니라 bind인 이유 = 메서드 Callable은 노드가 free되면 소멸돼 안 불리지만,
	# 람다는 free된 self를 만져 에러가 난다.
...
## **죽음 = 몸이 마나로 터진다.** 연출 자체는 `ManaBurst`가 지고 여기 남은 일은 **몸에서 규격을
## 뽑아 넘기는 것**뿐이다 — 색·시간·모양을 여기 적으면 「무엇을 그리나」가 두 파일로 갈린다.
## 🔴 **보스 판별 = `params.ai`가 `boss_`로 시작하나** — 이미 데이터에 있는 축이라 새 키가 없다.
##  덩치만으로는 못 가른다(gale은 늑대와 같은 한 변이다).
## 🔴 적은 이 프레임에 `queue_free`되지만 폭발은 **현재 씬에 따로 붙어 살아남는다.**
## 🔴🔴 **플레이어를 안 넘긴다 — 파편은 플레이어에게 안 온다.** 여기서 `_player()`를 넘기려는
##  순간이 각하된 궤적을 되살리는 순간이다. **마나 수치도 여기서 절대 안 건드린다**(한 줄이라도
##  올리면 「몹을 잡아 마나를 회복한다」는 없는 축이 생긴다).
...
## 🔴 배율을 안 곱하면 덩치 큰 적에서 **시체보다 작은 폭발**이 터진다. 곱한 값은 그림자가 자식이라
## 저절로 얻는 크기와 같다(그림자와 폭발이 같은 몸을 가리키게 된다).
func _body_px() -> float:
