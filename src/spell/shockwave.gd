...
##
## ⚠⚠ **현재 미배선 — 이 파일은 아무도 호출하지 않는다 (죽은 코드).** 실제 발사 경로
## (`ring_spell_system._deploy_now`)는 발산→`_spawn_bolt`, 응집→`_spawn_pillar`로 **기둥을
## 직접 스폰**한다. 아래의 "충격파끼리 부딪히면 기둥이 선다" 메커니즘은 이 산 경로에 연결돼
## 있지 않다 — `shockwave.tscn`/`shockwave.gd`를 instantiate·preload하는 산 코드가 0건이다.
## (삭제 여부는 balance의 shockwave_* 필드 호환성 확인 후 리드가 결정 — 지금은 경고만.)
##
## ── 아래는 미배선 설계 자산의 원래 의도 (지금 도는 규칙이 아님) ─────────────────
...
##
## 🔴 **기둥을 여기서 "만들지" 않는다(설계 의도).** "수렴하면 기둥"이라고 적는 순간 창발이
## 아니라 규칙이 되고, **"몇 개부터 기둥인가"**라는 답 없는 질문이 따라온다. 충격파끼리
## **부딪히면** 기둥이 서고, 어긋나면 안 선다. 룬을 겨눈 화살표 8개는 **중심에서 저절로 만난다.**
##
...
func _on_area_entered(area: Area2D) -> void:
	# ⚠ (미배선) 충격파가 실제로 스폰될 때 기둥이 서던 자리 — 다른 충격파와 부딪혔다.
