...
func _spawn_projectiles(design: SpellDesign, origin: Vector2, aim_dir: Vector2) -> void:
	...
	for arrow: ArrowData in design.arrows:
		var world_angle := arrow.direction
		var offset := arrow.origin * design.circle_radius * balance.circle_radius_px
		if design.circle_type == Enums.CircleType.AIMED:
			var rotate_by := aim_angle - design.aim_axis
			world_angle = arrow.direction + rotate_by
			offset = offset.rotated(rotate_by)
