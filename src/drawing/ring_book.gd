...
const JIN_DESC := {
	...
	&"jin_fork": "세 갈래로 넓게 흩뿌린다. 가까울수록 겹쳐 맞는다.",
	&"jin_ring": "사방으로 동시에 퍼진다. 둘러싸였을 때.",
	&"jin_burst": "같은 곳에 시간차로 세 발.",
	&"jin_spray": "좁은 각도로 촘촘히 뿜는다. 멀리는 못 간다.",
	&"jin_seek": "가장 가까운 적을 스스로 겨눈다.",
	&"jin_spiral": "나선으로 훑으며 나아간다. 넓게 스친다.",
	&"jin_bird": "나갔다가 되돌아온다. 오는 길에 또 맞는다.",
}
