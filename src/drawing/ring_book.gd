...
## 문양을 골랐다 — 열린 칸을 이걸로 채운다 (RingBoard.G_*)
...
var _stage := TAB_TEMPLATE            # 문양본 탭을 먼저 편다
var _template_idx := 0                # 지금 삽입된 문양본 (하이라이트)
var _active := RingBoard.G_RADIATE    # 지금 고른 문양 (하이라이트)
var _tab_rects: Array[Rect2] = []
## 클릭 가능한 칸 — {rect, kind:"template"/"glyph", value}
...
func _draw() -> void:
	...
	match _stage:
		TAB_JIN:
			_draw_single_cell(font, top, "일반진", "동그라미 그릇", "jin")
		TAB_RUNE:
			_draw_single_cell(font, top, "불", "불 속성", "fire")
		TAB_TEMPLATE:
...
func _draw_single_cell(font: Font, top: float, name_text: String, desc: String, icon: String) -> void:
	var cw := 120.0
	var ch := 120.0
	var r := Rect2((size.x - cw) * 0.5, top, cw, ch)
	draw_rect(r, CELL_BG, true)
	draw_rect(r, CELL_LINE, false, 1.0)
	_draw_icon(icon, r.get_center() + Vector2(0, -10.0), 30.0, [])
	_text_center(font, r.position + Vector2(cw * 0.5, ch - 26.0), name_text, NAME_COLOR, 11)
	_text_center(font, r.position + Vector2(cw * 0.5, ch - 12.0), desc, DESC_COLOR, 8)
...
func _draw_glyph_cells(font: Font, top: float) -> void:
	var n := RingBoard.GLYPH_NAMES.size()
	var gap := 8.0
	var cw := (size.x - 16.0 - gap * float(n - 1)) / float(n)
	var ch := 120.0
	for g in n:
		var r := Rect2(8.0 + float(g) * (cw + gap), top, cw, ch)
		_cells.append({"rect": r, "kind": "glyph", "value": g})
		var sel := _active == g
		draw_rect(r, CELL_SEL_BG if sel else CELL_BG, true)
		draw_rect(r, SEL_EDGE if sel else CELL_LINE, false, 2.0 if sel else 1.0)
		_draw_icon("glyph", r.get_center() + Vector2(0, -14.0), 26.0, [g])
		_text_center(font, r.position + Vector2(cw * 0.5, ch - 30.0),
			"%s [%s]" % [RingBoard.GLYPH_NAMES[g], RingBoard.GLYPH_KEYS[g]], NAME_COLOR, 11)
		_text_center(font, r.position + Vector2(cw * 0.5, ch - 15.0), GLYPH_DESC[g], DESC_COLOR, 8)
...
func _draw_icon(kind: String, c: Vector2, s: float, data: Array) -> void:
	match kind:
		"jin":
			draw_arc(c, s, 0.0, TAU, 40, RingBoard.RING_LINE, 2.5, true)
			draw_arc(c, s * 0.5, 0.0, TAU, 32, Color(RingBoard.RING_LINE, 0.5), 1.5, true)
		"fire":
			var col := RingBoard.RUNE_COLOR
		...
		"glyph":
			var col2: Color = RingBoard.GLYPH_COLORS[int(data[0])]
			var inward := int(data[0]) == RingBoard.G_GATHER
			for k in 8:
				...
				draw_line(tail, tip, col2, 1.6, true)
				draw_line(tip, tip - hdir.rotated(0.5) * s * 0.28, col2, 1.6, true)
				draw_line(tip, tip - hdir.rotated(-0.5) * s * 0.28, col2, 1.6, true)
