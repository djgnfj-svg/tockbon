...
##
## 🔴 **채점 규칙을 바꿀 땐 여기만 연다** — 예전엔 채점 하나 고치려면 렌더·입력을 소유한
## 757줄 Control을 열어야 했다.
## ⚠ 그 명분이던 *"핵심 재미 = 그리는 것"*(memory `takbon-core-fun-drawing`)은 **세83 그리기
## 폐지로 뒤집혔다.** 🔴 폐지 모드(`balance.skip_drawing = true`)에서 **가이드 보관**
## (`set_guide`/`guide_points` — 판이 밑그림을 그린다)은 그대로 돌지만, **점수 출력**
## (`coverage`·`accuracy`·`piece_score`)은 **소비자가 없다** — 패널 `_score_now()`가
## `RingPower.assembled_score`로 갈라지기 때문이다. 스위치를 false로 되돌리면 그 축이
## 되살아나므로 **지우지 마라**(CLAUDE.md 「되돌릴 수 있어야 하는 것」).
##
...
## ⚠ **조각마다 점수를 매기고 [다음]으로 잠그던 per-piece 흐름은 세70에 죽고 세85 ⑨에 걷혔다** —
##   지금은 조립본 **한 장을 통째로** 긋고 `piece_score()` 하나를 읽는다(`RingBoard.combined_total`).
##
...
## 🔴 **이 밑그림을 그리려던 획인가**의 경계 (세션 23에 0.24 → 0.32). 이보다 멀면 가이드와
## 무관한 획으로 보고 **통째로 무시**한다(펜 뗌·판 여백 클릭).
## ⚠ 0.32라는 **값의 유래는 per-piece 시절**이다 — 8칸 고리의 이웃 칸 간격이 약 0.46R이라
## 그 안쪽에서 끊어야 남의 칸 획이 내 점수로 안 들어왔다. per-piece는 세85 ⑨에 걷혔지만
## 값은 그대로 살아 있다(통째 밑그림에선 「멀리 삐끗한 획」 필터로 계속 일한다).
...
## open = 열린 칸 목록 — 조립 상태기계(`RingAssembly.get_open()`)가 쥔 값이고 채점기는 모른다.
## ⚠ 「진이 칸을 연다」(`JinDef.glyph_slots`) 축은 세85 ⑦에 은퇴했다 — 라이브에서 열린 칸을
##   만드는 자리는 `ring_forge_panel.build_assembly()`(층에 놓인 칸의 합집합)다.
