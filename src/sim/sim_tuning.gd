...
##
## 🔴 **그래도 512를 유지한다.** 실제 호수는 깨어 있는 청크가 **≤36**이라 평소엔 안 닿고,
##  값을 내려도 **총 일의 양은 같다** — 느려지는 순간이 길어질 뿐이다.
##  ⇒ 내리는 것이 「빨라진다」가 아니라 **「느린 구간을 얇고 길게 편다」**이다. 어느 쪽이 나은지는
##   **화면으로만 답이 난다.** 🔴 **판정 7(FPS)이 볼 자리이고, 그 판정의 무게가 여기서 제일 크다.**
##
...
##  ⚠ verify-look이 화면에서 본 절벽(≈100청크)은 **N=1 때의 값**이라, N=3이면 **≈33청크**에서 같은
##   절벽이 온다. 🔴 **값을 아직 안 내렸다** — 얼마로 내릴지는 여전히 화면 판정이다(위 상자).
const MAX_CHUNKS_PER_TICK := 512

## 이 양 **이하**면 얕은 물이다(`cell_materials.FLAG_SHALLOW`).
##
## 🔴🔴 **순전히 화면 값이다. 멈춤과 아무 상관이 없다** — spec이 뒤집어 쟀다(2026-08-07):
...
##
## 🔴 **32는 계획이 자리를 채운 값이고 아무도 화면을 안 봤다**(255의 1/8).
##  올리면 갈리는 물이 늘고, `WATER_MAX` 에 가까워질수록 **물 전체가 얕아 보인다.**
