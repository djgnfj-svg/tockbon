...
## 🔴🔴 **무속성 — 「세상에 아무것도 안 한다」가 정의다.** 색만 다른 불이 아니다.
...
##
## 🔴🔴 **네 열 전부 32px 전환에서 ×2 됐다**(전: speed 10·6 / rd 12·6 / ignite_r 18·9 / rune_r 3·1).
##  단위는 셀인데 **뜻이 「화면에서 이만큼」**이라 위 `TILE_CELLS` 주석의 갈래에 걸린다 —
##  안 올리면 폭발 지름이 6타일 → 3타일, 화면에서 192px → 96px로 **절반**이 된다.
##  ⚠ 그래서 「문을 연다 / 긁는다」의 이름은 **24·12에 그대로 붙는다** — 타일로 읽은 값이 보존됐다.
## ⚠ `ignite_r` 는 **따라올 수밖에 없다** — `ignite_r > rd` 가 깨지면 「폭발은 되는데 불이 안 난다」다.
const SIM_SIZES: Array[Dictionary] = [
	{"speed": 20, "rd": 24, "ignite_r": 36, "rune_r": 6},
	{"speed": 12, "rd": 12, "ignite_r": 18, "rune_r": 2},
]
...
## 🔴 `rd` 가 12 → 24가 되며 **셀 면적이 4배**라 폭발 적용도 4배로 간다. **재지는 않았다**(추정 ~5,300μs).
##  ⇒ 예산 여유가 2.6배에서 1.6배로 준다. **알고 고른 것이다** — 마법이 화면에서 절반이 되는 쪽이 더 큰 손해다.
...
##  불을 막는 것은 연료여야 한다.
##
## 🔴🔴 **근거는 「격자 셀 수의 11%」다. 무대에서 뽑지 마라.**
##  전에는 4,096 / 36,864셀 = 11.1%였고, 격자가 147,456셀이 되며 **16,384 = 11.1%** 로 따라왔다.
