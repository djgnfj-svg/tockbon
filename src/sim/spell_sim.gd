...
##
## 🔴🔴 **왜 「지금 위치」가 아니라 구간인가.** 세대 0 탄은 틱당 20셀(80px)을 뛰고 캐릭터 상자는
##  **20px**(몬스터는 종류에 따라 20~44px)이다 ⇒ 틱 경계의 점만 맞대면 **도약이 상자를 통째로
##  넘는다.** 한 프레임이라 **눈으로 절대 못 보고** 「가끔 안 맞는다」로만 보인다 —
##  `_walk` 가 벽에 대해 같은 함정을 이미 한 번 적어 뒀다(「도착 칸만 검사로 짜지 마라」).
##  ⚠ **32px 전환에서 낡았던 값이다**(옛 값 40px·16px는 16px 세상 것) — `monsters-minimum`이
##  단계 3을 지나며 고쳤다.
##
...
func _walk(grid: CellGrid, i: int, x0: int, y0: int, x1: int, y1: int) -> bool:
	...
	#  판정 2가 통째로 죽는다. 폭발도 같은 점을 쓴다(1셀 차이는 반경 24셀 원에서 안 보인다).
	_hit_cx = x0
	_hit_cy = y0
...
## 착탄이 세상에 하는 일 **전부**. 🔴🔴 **순서가 계약이다: 파기 → 룬 흔적 → 문양.**
func _impact(grid: CellGrid, x: int, y: int, i: int) -> void:
	# ① 🔴🔴 **조건 분기가 없다.** 룬도 문양도 안 본다 — 아래 `_carve` 주석.
	_carve(grid, x, y, _gen[i])
	# ② 🔴 **룬 흔적이 문양보다 먼저다.** 흔적은 바닥값이고 문양이 그 위에 얹힌다.
	_rune_trace(grid, x, y, _element[i], _gen[i])
	# ③ 문양 파이프라인. 폭발이 여기서 ①보다 훨씬 크게 판다.
	_resume(grid, x, y, _glyphs[i], _element[i], _gen[i])


## 🔴🔴 **모든 착탄이 판다 — 문양이 없어도, 룬이 무속성이어도**(GDD 자연법칙 첫 줄
##  「지형이 파괴된다. 마법이 벽을 판다」).
##
## ⚠ **여기가 `_rune_trace` 안이 아닌 것이 요점이다.** 넣으면 무속성 갈래가 `return` 이라
##  **무속성만 안 파고**, 그게 「어떤 탄은 파고 어떤 탄은 안 판다」다 —
##  아래 `_rune_trace` 가 「모든 착탄이 흔적을 남긴다」로 이미 피해 둔 함정이고 같은 어법이다.
##
## **①을 ②보다 먼저 둔다** — `cell_grid._blast` 의 「파괴가 먼저다」와 같은 어법이다.
##  🔴🔴 **그런데 이 순서는 그물이 못 잰다. 둘이 따로 실측했다**(2026-08-05, builder · verify-run):
##   ①②를 맞바꾸고 전체를 돌렸더니 **1,371개가 전부 초록이고 stderr도 깨끗했다.**
##  ⚠ 왜 안 물리나 — 뒤집으면 `_write_cell` 의 `_unburn` 이 방금 붙인 불을 지우는 것은 맞다.
##   그런데 **바른 순서에서도 그 칸들은 애초에 안 붙는다**(파인 칸은 EMPTY = 연료 0이고
##   `_ignite_cell` 이 연료 0에서 멈춘다) ⇒ **두 순서의 격자 상태가 같다.**
##  🔴🔴 **그 등가성은 「`_write_cell` 이 `_unburn` 을 부르는 한」에 매달려 있다.**
##   그 호출을 빼면 그물 15개가 빨개진다(verify-read 실측) — 즉 `_unburn` 을 옮기거나 빼는 날
##   이 순서는 **조용히 다시 계약이 된다.** ⚠ 그때 이 문단을 안 고치면 아무도 모른다.
##  ⚠ 그리고 **같은 것은 격자 상태뿐이다** — `consume_changed()` 는 **13만큼 다르다**
##   (파기 먼저 113 · 점화 먼저 126). 지금은 아무도 그 값을 안 재고 화면 거동도 같다.
##   🔴 **그러니 이 줄을 「그물이 지켜 준다」로 읽지 마라.** 순서는 읽는 사람을 위한 것이다.
##
## 🔴🔴 **실제로 관측되는 계약은 `carve_r < rune_r` 하나다.** 안 서면 파기가 태울 연료를
##  통째로 지워 **불만 조용히 사라진다** — `cell_grid.blast_ignite_r` 의 함정과 같은 것이고,
##  뒤집어 재 봤다: `carve_r(0)` 을 `rune_r(0)` 과 같게 두니 **16개가 빨개졌다.**
##  ⇒ 「파이면서 탄다」의 실제 그림은 **고리**다: 안쪽 `carve_r` 은 파이고
##   `carve_r < r ≤ rune_r` 인 띠가 탄다. 부등식은 `net_tables` 가 세대마다 잰다.
##
## 🔴🔴 **통지를 안 낸다.** 파기는 폭발이 아니다 — 섬광도 흔들림도 안 뜬다.
##  ⚠ 구멍은 나는데 화면이 안 번쩍하는 것이 「이건 폭발이 아니다」를 말한다.
...
##  원반에 통째로 들어간다 ⇒ **관측 가능한 효과가 0이다.**
##  🔴🔴 **여기 「그래도 파면에 유령 인덱스가 남는다(실측 41%)」가 적혀 있었고, 그건 거짓이다.**
##   verify-read가 계기 셋(파면 길이 · 소속 주장 · 깃발)을 맞대 네 조합에서 다시 쟀는데
##   **「EMPTY인데 깃발」이 전부 0**이었다(문양없음 46/46/46 · 폭발 118 · 확산 62 · 확산+폭발 254).
##   ⇒ **`_write_cell` 이 `_unburn` 을 부르는 한 유령은 원리적으로 0이다.** 41%는 `_unburn` 이
##   붙기 **전**의 값으로 보인다. 파기가 들어와도 **키울 위험이 애초에 없다.**
##
## 🔴 **흔적 자신은 지형을 안 부순다. 점화만 한다.** 파는 일은 위 `_carve`(①)의 몫이다.
##  ⚠ **이 줄이 전에는 「착탄은 지형을 안 부순다」였고, 2026-08-05에 뒤집혔다.**
##   옛 근거는 「부수면 확산이 폭발을 겸해서 「확산→폭발」과 「폭발→확산」이 화면에서 뭉개진다」였다.
##   🔴 **뒤집은 근거: 뭉개짐은 「파느냐 마느냐」가 아니라 「크기」가 막는다.** 폭발 `rd` 8셀에
##   파기 2셀이면 **넓이가 16배** 차이다 — 오히려 지금 아슬아슬한 쪽은 `rune_r`(6)과 `rd`(8)이고
##   그건 `sim_tuning.SIM_SIZES` 가 「여유가 0.25에서 0.75로 줄었다」고 스스로 적어 뒀다.
##   ⇒ **뭉개져 보이면 첫 용의자는 `carve_r` 이 아니라 `rune_r` 이다.**
##  ⚠ 계기는 사용자 보고다 — 「일반 마법이랑 불마법이 벽을 한 칸도 못 줄인다 ·
##   **공격한 건지 아닌지도 구분이 안 간다**」. 옛 규칙에서 벽을 파는 것은 폭발 문양 하나뿐이라
##   문양 없는 마법진은 **돌벽에 완전히 무해했다**(불 룬은 점화만 하는데 돌은 안 탄다).
...
func _rune_trace(grid: CellGrid, x: int, y: int, element: int, gen: int) -> void:
	...
	#   지목했으므로 그게 의도다. (같은 문장이 `sim_tuning.ELEM_NONE` 에도 있다 — **두 곳이다.**)
