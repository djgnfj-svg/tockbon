...
##
## The board still belongs to the SLOT and not to the body: `Army` holds one `Loadout` for its whole
## life, so a soldier dying does not touch it — 「늑대에게 투구를 끼우면 모든 늑대가 낀다」.

## The boards, flat: `board[slot * Rules.ITEM_CELLS + cell]` is the ITEM in that cell, or -1.
...
func reset() -> void:
	...
	board.resize(Rules.summon_slot_count() * Rules.ITEM_CELLS)
	for i in board.size():
		board[i] = -1
	held = PackedInt32Array()


## The item in that cell, or -1 for an empty cell or an out-of-range slot/cell. Out of range is a
## refusal, not a fault.
func fitted_item(slot: int, cell: int) -> int:
	if slot < 0 or slot >= Rules.summon_slot_count():
		return -1
	if cell < 0 or cell >= Rules.ITEM_CELLS:
		return -1
	return int(board[slot * Rules.ITEM_CELLS + cell])


## The first cell on this slot with nothing in it, or -1 when the board is full. **The whole of what
...
func first_empty(slot: int) -> int:
	for cell in Rules.ITEM_CELLS:
		if fitted_item(slot, cell) < 0:
			return cell
	return -1


## Sums `Rules.item_bonus(item, col)` over the cells that are filled. ⚠⚠ This one function is the whole
## of the room set effects need — a set term is added here and no consumer moves.
func bonus(slot: int, col: int) -> float:
	if slot < 0 or slot >= Rules.summon_slot_count():
		return 0.0
	var sum := 0.0
	for cell in Rules.ITEM_CELLS:
		var item := fitted_item(slot, cell)
		if item >= 0:
...
## that a piece of equipment can move comes out of THIS call — the fight's and the dashboard's alike.
func stat_of(slot: int, col: int) -> float:
	var value := Rules.unit_stat(Rules.summon_type_of(slot), col) + bonus(slot, col)
	...
	# attack period negative — a body attacking backwards in time.
	if col == Rules.ITEM_COL_PERIOD:
		return maxf(Rules.PERIOD_FLOOR_SEC, value)
	return value


## Fits the held card at `held_index` into the first empty cell on `slot`. False and **changes nothing**
## on a bad slot, a bad index, or a full board.
##
