...
## ⚠ 세112: 여기 있던 **`vision_dir()` 파사드**(적이 *"내가 저 사람 눈에 보이나"*를 묻던 유일한 경로 ·
##  세104 N27)가 시야와 함께 죽었다. 소비자는 `forest_enemy._judge_seen`·`vision_overlay` 둘이었고
##  **둘 다 사라졌다.** 🔴 되살릴 땐 **한 줄 파사드로만** — 방향의 단일 소스는 `player_caster._aim`
##  하나이고, 여기서 `get_global_mouse_position()`을 다시 읽으면 지팡이·스프라이트 반전과
##  **세 곳이 같은 것을 각자 계산**하게 된다(T5). ✅ `is_rolling()`이 그 덕타이핑 관용구의 산 예다.
