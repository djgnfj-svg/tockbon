...
##
## ✅ **세105 착지: 40.0** — 0.40에서 20·26·32를 나란히 뽑아 보여 주고 사용자가 골랐다
##  (*"40이 괜찮네 그걸로 결정"*). 셋 중 가장 부드러운 32보다도 **더 편을 든 것**이라,
##  이 값의 방향은 **「원뿔이 또렷한 쪽」이 아니라 「경계가 안 보이는 쪽」**으로 확정됐다.
##  ⚠ 그러니 다음 사람이 *"그림자가 흐릿하다"*를 근거로 이 값을 낮추지 마라 — **일부러 그렇다.**
##  🔴 대신 원뿔이 **아예 안 읽히면** 낮출 게 아니라 `FOG_DARKNESS`(0.40)를 올려라
##   — 대비가 모자란 것이지 폭이 넓은 게 아니다(둘이 한 축이라는 위 문단).
##  비교본 = `scratch_shots/occl_d40_f20·f26·f32.png` + 착지 확인 = `occl_d40_f40.png`.
const SHADOW_FEATHER_PX := 40.0
