extends Sprite2D
## 📦 **방 보상 상자** (세112 1c) — **겉모습 · 바닥 접점 · 열림 판정**을 진다.
## 정본 = `docs/takbon-design/planned/room_loop_design.md` R5·R11 · 옛 지점 계약 = `dungeon_structure_design.md` §3·§10.
##
## 🔴🔴 **세112 1c: 이 파일은 「늑대 둥지」(`nest.gd`)를 이름만 바꾼 것이다 — 로직은 한 줄도 안 새로 썼다.**
##  R5(방 가운데 상자 하나)가 요구하는 기계가 **이미 여기 전부 있었다**: [E] 즉시 열기 · 재열기 가드 ·
##  2컷 교체(접지선 공유) · `opened` → 부모 굴림 · 세이브 무접촉 · 소진 뒤 프롬프트 끔.
##  ⚠ 사용자 원문도 애초에 **상자**였다 — *"e는 한번만 누르면 **상자가 열리는** 걸로"*(D10).
##  🔴 **아직 안 바뀐 것 셋**(리드 몫이라 여기서 못 한다 — 개명이 절반만 된 상태다):
##   ⓐ **텍스처가 여전히 `nest_wolf*.png`다**(PNG 개명엔 `--import`가 필요하다 · 아래 `TEX_*`)
##   ⓑ **데이터 id가 `lm_nest` · `LandmarkKind.NEST`다**(id는 `data/chapters/*.tres`로, kind는 core로 번진다)
##   ⓒ `zone_id`는 여전히 `&"landmark"`(`interact_zone.gd`의 열거와 짝이다)
##  ⚠ **그래서 옛 이름을 만나면 여기서 해소해라: `nest.gd`/`nest.tscn`/`Nest` → `chest.gd`/`chest.tscn`/`Chest`.**
##   「지점(landmark)」은 **아직 살아 있는 어휘**다(스키마·그룹 이름이 그대로다) — 지우지 마라.
##
## ⚠ **「마지막 적이 죽으면 방 가운데 세운다」(R5의 배치)는 이 파일이 아니다** — 단계 4에서
##  **방이** 한다(소유권은 아래 그대로: 여기는 겉모습·열림 판정만 진다).
##
...
##  인스턴스조차 안 되고 **방 전체가 못 선다.** `load` + 가드면 「상자만 안 보인다」로 **번짐이 멎고**,
##  무엇이 빠졌는지 로그가 말한다. ✅ 세99에 리드가 `--import`를 돌려 지금은 정상이다 —
##  **가드는 남겨 둬라**(에셋을 옮기거나 `.godot/`를 지우는 날 다시 이 자리로 온다).
##
## 🔴🔴 **감지 상자(`chest.tscn`의 `BehindSense`)의 치수 근거** — 세100에 `.tscn` 주석에서 **여기로
...
##  ⓑ 🔴 **아래로 안 내리는 게 계약이다** — 접지선보다 **아래**(남쪽)에 선 플레이어는 상자 **앞**이라
##     가릴 이유가 없다(그때 상자는 z 0으로 남아 원래대로 몸 뒤에 그려진다). 각도 계산이 필요 없는
##     이유가 이것이다: **위/아래가 곧 원근**이다.
##  ⓒ 🔴 **가지 끝(원점 위 140px)까지 안 덮는다** — 거기까지 덮으면 상자 북쪽을 지날 때마다 화면의
...
##   **감지 상자를 아래로 250px 늘려도 전 스위트가 그린이었다**(세100 N25ⓑ에 당겼다).
##  ⚠ **그림을 다시 그려 감지 상자를 키우거든 위로만 키워라** — 아래로 새는 순간 위 둘이 빨개진다.
##
...
##   (`chest.tscn`의 `BehindSense`) 각도 계산 없이 그 판정이 공짜로 나온다.
##  ⚠ 나무와 **코드가 닮았지만 사본이 아니다**: 감지 상자 크기·알파가 물건 크기에서 나온다(이 프롭은 178×121로
##   나무의 3배라 같은 값이면 화면 절반이 흐려진다). 🔴 **세 번째 프롭이 같은 걸 원하면 그때 뽑아라** —
##   지금 공용 부모를 만들면 소비자 둘짜리 추상이 되고, 나무 쪽 그물(`test_daylight_tree_auto`)까지
##   같이 흔들린다.
##
## ⚠ **물리 몸이 없다**(StaticBody2D 아님) — world 레이어에 두면 마법 캐리어(마스크 5)가 상자마다
...
##    저절로 다시 찬다**. `GameState`에 넣는 순간 **영영 빈 상자**가 되는데 화면은 정상이다.
...
## 🔴 상호작용 지점의 **타입**만 문다(씬은 `chest.tscn`이 인스턴스한다). 타입이 있어야
