...
## 캐릭터 크기. 🔴 **32px = 8셀 = 지형 타일과 정확히 같다**(GDD 격자).
##  캐릭터와 타일 두께를 같게 잡은 것이 「저 벽을 뚫으면 지나갈 수 있다」를 화면에서 세게 만드는 장치다.
## ⚠ **그 계약은 배율이 바뀌어도 그대로다** — 바뀐 것은 상자의 크기지 계약이 아니다.
## 🔴 **상자가 커지면 이 파일이 비싸진다.** `_box_free`·`_grounded`·`_standing_in_fire` 가
##  상자가 덮는 셀을 **GDScript로** 훑는데 **25칸 → 81칸**이 됐고, 이건 틱이 아니라 **60Hz**로 돈다.
##  ⚠ 실측(2026-08-04, 단계 0): `step()` 이 걷는 중 **77 → 249μs**(예산의 1.5%). 예산 안이지만
##   **평상시 프레임에서 제일 큰 단일 항목**이다 — 「셀은 안 변했으니 여기도 안 변한다」로 읽으면 틀린다.
const W_PX := 32
const H_PX := 32
...
##  **뜨면서 동시에 아프다.** 「위험과 기동이 같은 행동에서 나온다」가 이 조합의 값어치다.
##  ⚠ 넉백이 사라지며 **아픈 것만 남고 밀리는 것은 없어졌다** — 뜨는 것은 여전히 이 반동이 한다.
