...
# ── 💀 `extract_points`(추가 탈출구 좌표)를 **세112에 지웠다**
#    (`room_loop_design.md` R1 · genre_pivot D1 「출구 여럿 폐기」). 남은 출구는 씬 인라인 `$Exit` 하나다.
#
# 🔴🔴 **되살릴 때 반드시 아는 것 — 이 필드는 「좌표만 늘리면 안 되는」 필드였다.**
#  출구는 겉모습이 없는 `InteractZone`이라 **유일한 시각 표시가 흙길**(`_exit.position`에서 파생)이었고,
#  좌표만 늘리면 **2번째부터 화면에 아무것도 안 보였다.**
#  ⚠ **세112에 그 흙길 ⓐ줄기마저 죽었다** ⇒ 지금 ch2·ch3은 **나가는 길의 시각 안내가 0**이다
#    (실측: 지점이 없는 챕터는 흙길 0칸 + 출구 겉모습 0). **이게 설계 §8의 문 스프라이트가 필요한 이유의 실증이다.**
#  ⇒ 🔴 **다시 만들 때는 좌표가 아니라 「보이는 문」부터 세워라.**
