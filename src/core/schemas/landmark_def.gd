...
##  ✅ **세112에 `InteractZone.hold_sec`이 통째로 사라졌다** — 홀드 기계가 탈출과 함께 죽어(R1)
##   *"0이 아닌 값을 적으면 탈출 시간으로 돈다"*는 옛 함정은 **이제 도달 불가능하다.**
##  ⚠ 홀드로 뒤지는 안이 각하된 것은 그대로 유효하다(사용자 확정 *"e는 한번만 누르면 상자가 열리는 걸로"*).
##   🔴 **그 각하는 이제 이 파일의 유일한 근거다** — 홀드를 되살리려는 안이 나오면 여기부터 읽어라.
##  ⚠ **무방비 시간의 거취가 세112에 바뀌었다** — 옛 문장은 *"낱개를 걸어가 줍는 동안으로 옮겨간다"*였는데
##   **R11이 「상자를 열면 바로 가방」으로 그 시간마저 없앴다.** 지금 무방비는 **상자를 여는 순간**뿐이다.
##
