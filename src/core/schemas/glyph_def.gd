class_name GlyphDef
extends Resource
## 문양 정의 — 룬과 진 사이 고리 칸을 채우는 조각. data/glyphs/*.tres.
## 문양 어휘가 늘 때 코드가 아니라 .tres를 늘린다.
## ⚠ 문양엔 해금 게이트가 없다(책이 `Db.all_glyphs()`를 무필터로 띄운다) — 파밍 단위는
##  문양이 아니라 문양-고리(`GlyphRingDef`)다.

@export var id: StringName = &"radiate"
@export var display_name: String = "발산→"
## 발사 계약 코드 — ring_spell_system이 이 값으로 전개를 가른다.
## 🔴 정본은 `Enums.GlyphCode` 9값이다 — **값을 밀면 저장된 도안이 깨진다.**
@export var code: int = 1
## ⚠ 폐기 — 미사용. 문양은 오른쪽 셀을 클릭해서 고르므로 고르기 키가 없다.
## 기존 .tres 호환으로 필드만 남긴다 — **새 코드에서 읽지 마라.**
...
## ⚠ 기본값이 빈 값인 건 의도다 — 갱신을 빠뜨린 `.tres`가 「직진탄으로 잘못 동작」하는 대신
##  미등록으로 걸려 시끄럽게 죽는다.
...
##
## 「같은 알고리즘 + 다른 수치 = 다른 문양」이 이 축의 요점이다 — 「확산 46°」 옆에 「돌풍 120°」를
## `.tres` 한 장으로 세운다.
