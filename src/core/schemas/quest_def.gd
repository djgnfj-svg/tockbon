...
##
## 🔴 **퀘스트는 순수 오버레이다** — 목표는 이미 도는 EventBus 이벤트(적 처치·귀환·룬 해금)를
## 관찰해 **자동 진행**한다. 룬 해금 사슬(보스→조각→해독)을 전혀 안 건드리고 그 위에 얹혀,
## 흩어진 진행을 **보이는 사슬**로 만든다. 진행·완료 판정·보상 지급은 GameState가 쥔다
## (선례: 잉크 등급 리졸버가 스키마가 아니라 Db에 있는 이유 — 이 스키마는 순수 데이터).
##
## 🔴 **새 퀘스트 = .tres 한 장.** `requires`로 이전 퀘스트를 물면 그게 곧 스파인(사슬)이다.

## 목표 종류 — Enums.QuestGoal.
##  KILL   = 숲에서 적을 처치 (target 비면 아무 적이나, 특정 enemy_id면 그 적만).
