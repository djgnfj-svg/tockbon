...
## 보는 쪽 부채꼴의 **전체** 각도(도) — 120이면 좌우 60씩이다(반각이 아니다).
## 화면이 가로로 길어(960×540) 좌우로 넉넉해야 한다 — 90°면 조준을 조금만 틀어도 적이 사라져 「깜빡인다」.
@export var vision_fan_deg: float = 120.0
...
## 🔴 **내리지 마라**(V7): 내리면 네임드에만 주기로 한 「등 뒤」가 **바탕 늑대 전원에게 번진다**.
##   네임드(`hound_alpha` aggro 280)는 **일부러 이 밖**이라 60px를 등 뒤로 얻는다 — 그게 정체성이다.
@export var vision_near_radius: float = 220.0
