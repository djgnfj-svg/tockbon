...
## 🔴🔴 **세112: 둘 다 0으로 내렸다 — 차징을 걷었다**(사용자 지시 *"차징도 없애줘 바로발싸되도록"*).
##   `player_caster.fire()`의 `if duration <= 0.0: _release_cast()` 분기가 그 되돌림을 위해 남아 있던
##   자리라 **코드는 한 줄도 안 고쳤다.** ⇒ 이 두 값이 곧 스위치다: 0.15/0.75로 되돌리면 차징이 산다.
##   ⚠ **바닥 마법진도 같이 꺼진다** — `vfx._on_ring_cast_started`가 `duration <= 0`이면 원을 안 연다
##   (차징이 없으면 「채워지는 것」을 보여줄 대상이 없다). 되돌리면 그것도 같이 살아난다.
##
## 🔴 **대가를 정확히 적는다 — 위 「밸런스 계약」이 지금 죽어 있다.**
##   *"위력은 2배 오르는데 시전이 그보다 더 느려져 DPS가 떨어진다"*가 성립하지 않는다 ⇒
##   **강한 마법진이 그냥 우월해진다.** 슬롯 3개가 「속사용 하나 · 한방용 하나」로 갈리던 것도 같이 죽는다.
##   ⇒ 🔴 **강한 진에 다시 대가를 물리려면 시전 시간이 아닌 축을 골라야 한다**(마나·쿨다운·이동 배수).
##   ⚠ `cast_move_mult_*`는 **일부러 안 건드렸다** — 시전이 0이라 지금은 아무 일도 안 하지만,
##     차징을 되살리는 날 같이 살아나야 하는 짝이다.
@export var cast_time_plain_sec: float = 0.0
## 퍼펙트 진의 시전 시간 — 세112에 0. 옛값 0.75("공들인 것을 쓴다"가 몸으로 느껴지는 길이)
