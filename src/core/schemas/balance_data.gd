...
@export_group("던전 · 원정 (세99)")
## 🔴 **탈출에 걸리는 시간** — 출구에서 [E]를 꾹 눌러 버텨야 하는 초 (세99 D7, 사용자 확정).
## 익스트랙션의 마지막 긴장이 여기서 나온다 — *"달려와서 버틴다"*가 한 판의 마지막 장면이 된다.
## 정본 = `docs/takbon-design/dungeon_structure_design.md` §2-4.
##
## 🔴 **값의 소유자가 여기인 이유** = 출구가 여럿이라 씬 `@export`에 두면 **출구마다 한 벌**이 생겨
##  튜닝할 값이 흩어진다.
##  ⏳ **단계 1에 `InteractZone.hold_sec`(기본 0)이 붙는다 — 아직 없는 필드다.** 그게 생기면
##  그쪽은 **「0이냐 아니냐」 스위치로만** 쓰고 값은 이걸 읽게 해라.
##  ⚠ 그때 그 기본값 0이 **마을 책상·문이 안 바뀐다는 회귀 방어선 전부**가 된다(설계 §6 S8).
##
## 🔴 **홀드는 폴링(`_process`)으로 재라 — 「뗐나」를 이벤트로 받지 마라.**
##  `player_caster._tick_cast` 머리말의 실측 세 가지가 그대로 재현된다(전부 에러 0):
##  화면 덮는 Control이 뗀 이벤트를 먹고 · alt-tab이면 엔진이 액션만 풀고 · 모달 중엔 `_unhandled_input`이 안 온다.
##  ✅ 덤으로 `_process` 카운트다운은 **씬 전환에 구조적으로 안전**하다(노드와 함께 죽는다).
##
## ⚠ **잠정값이다 — 손맛은 F5가 정한다.** 취소 트리거는 넷: 피격 · 범위 이탈 · 모달 열림 · 씬 전환.
@export var extract_hold_sec: float = 3.0

@export_group("시야 (세104)")
# ── 🔴 판정은 `src/core/vision.gd`가 한다(순수 static). 여긴 **난이도를 직접 바꾸는 셋**만 든다.
#    ⚠ 페이드 시간·숨김 알파는 **여기 두지 마라** — 연출값이라 `forest_enemy`의 const다
#      (이 파일 머리말의 「연출값은 여기 두지 마라」 규약).
#    ⚠ 이름의 `vision_` 접두사를 떼지 마라 — 그냥 `fan_deg`가 되면 문양의 `spread_fan_deg`
#      (탄 확산각)와 **같은 말이 두 축을 가리킨다**.
#    정본 = `docs/takbon-design/vision_design.md` §4.
## 보는 쪽 부채꼴의 **전체** 각도(도) — 100이면 좌우 50씩이다(반각이 아니다).
## 화면이 가로로 길어(960×540) 좌우로 넉넉해야 한다 — 🔴 **90° 밑으로 내리지 마라**:
## 조준을 조금만 틀어도 적이 사라져 **「깜빡인다」**(세104에 그 하한을 실측으로 잡았다).
##
## ⚠ **세105: 120 → 100** (사용자 *"부채꼴도 조금만 줄여줘"*). 주변시 축소(220→110)와 **같은 방향**이다 —
##   시야를 좁혀 「보이는 것」을 줄이고, 그 자리를 **차폐**와 **표시(`?`/`!`)**가 맡는 쪽으로 옮기는 중이다.
##   🔴 **하한 90까지 여유가 10°뿐이다** — 더 좁히자는 말이 또 나오면 각도가 아니라
##   **사거리(`vision_fan_range`)**나 **경계 또렷하기**(`vision_overlay`의 `*_FEATHER_PX`)를 먼저 만져라.
@export var vision_fan_deg: float = 100.0
## 부채꼴 사거리(px). 화면 가로 반폭이 480이라 그보다 조금 넓게 잡아
## **화면 가장자리에서 적이 갑자기 나타나지 않게** 한다.
@export var vision_fan_range: float = 560.0
## 🔴🔴 **주변시 반경(px) — 이 값은 임의값이 아니라 「바탕 종 aggro의 최댓값」에서 나온 파생값이다.**
## 그래서 *"나를 쫓기 시작한 적은 반드시 보인다"*가 성립하고, 안 보이는 적 = **아직 나를 모르는 적**이 된다.
## 긴장은 「저 너머에 뭐가 있나」에서 오고 짜증(「안 보이는 데서 맞았다」)이 그 자리를 안 뺏는다.
##
## ⚠ **바탕 종이 `hound`(220) 하나뿐이라 이 값은 취약하다** — 잡몹을 늘리는 날 aggro가 이걸 넘으면
##   보장이 **조용히** 깨진다. `test_vision_auto`의 데이터 불변식이 그날 빨개진다(리터럴 220을 박지 마라).
##
## 🔴🔴 **세105: 220 → 110. 위 「내리지 마라」(V7)를 사용자가 뒤집었다** — *"주변시 줄여줘 내가 볼때
##   나무가 걸리면 그 뒤방향이 안보여야하는데"*(2026-07-28).
##
## ⚠🔴🔴 **세107: 위 문단이 들던 이유가 통째로 사라졌다 — 값만 살아남았다.**
##   세105엔 *"주변시는 차폐 면제라 크면 나무 뒤도 보여서 차폐가 무의미해진다"*가 근거였는데,
##   **세107에 차폐가 은퇴했다**(사용자 *"구지 나무나 돌맹이에 시야가 가려지지 않도록"*).
##   🔴 그런데 **이 값은 일부러 안 되돌렸다** — 같은 세션에 되돌릴지 물었고 사용자가 *"지금 값 유지"*를
##   골랐다. ⇒ **110은 이제 「차폐를 살리려고 좁힌 값」이 아니라 「좁은 게 좋아서 좁은 값」이다.**
##   ⚠ **그러니 *"차폐가 없어졌으니 220으로 되돌리면 된다"*고 혼자 판단하지 마라 — 사용자 결정이다.**
##   경위 = `docs/DECISIONS.md` 「비주얼」 첫 항목 · `src/core/vision.gd` 머리말.
##
## 🔴 **대가를 정확히 적는다 — `vision_design` §4-1의 보장이 죽었다.**
##   *"주변시 ≥ 감지거리 ⇒ 나를 쫓기 시작한 적은 반드시 보인다"*가 이제 **성립하지 않는다**
##   (`hound`의 `sight_range` 220 > 110). 즉 **안 보이는 채로 쫓기는 구간이 생긴다.**
##   ⇒ 🔴 **그 자리를 「몸이 보인다」가 아니라 「표시가 뜬다」가 맡는다** — 인지 표시(`?`/`!`)가
##   선택이 아니라 **필수**가 된 이유다. 정보 채널이 옮겨간 것이지 사라진 게 아니다.
##   ⚠ 그래서 **표시를 약하게 만들거나 조건을 좁히면 이 값과 함께 무너진다.**
##
## ⚠ **잠정값이다 — F5가 정한다.** 너무 작으면 「코앞만 보인다」가 된다.
##   🔴 **상한을 재는 잣대가 세107에 없어졌다** — 옛 기준은 *"프롭 격자 148px이라 110이면 나무 한
##   그루 너머는 안 보인다"*였는데 **나무가 이제 아무것도 안 가린다.** 지금 이 값을 키울 때 걸리는
##   것은 위 「보장이 죽었다」 문단뿐이다: **220까지 올리면 `hound` aggro와 같아져 그 보장이 되살아나고,
##   대신 「저 너머에 뭐가 있나」의 긴장이 준다.** 그 교환이 이 손잡이의 전부다.
@export var vision_near_radius: float = 110.0

@export_group("몬스터 인지 (세105)")
# ── 🔴 **여긴 「소리가 얼마나 멀리 가나」 하나만 든다.** 적별 인지 수치(`sight_range`·`sight_angle`·
#    `turn_rate`·`alert_sec`·`patrol_*`·`ambush_mult`)는 **종의 성격이라 `EnemyDef.params`**다
#    (「새 적 = .tres 한 장」). 그 둘을 여기 섞으면 늑대를 조이려다 전 종이 같이 움직인다.
#    정본 = `docs/takbon-design/enemy_perception_design.md` §5·§7.
#
## 🔴🔴 **마법 소리 반경(px) — 위력이 클수록 멀리 간다**(사용자 확정 ⓔ).
## 이 두 값이 *「한 방에 잡고 조용히」* 대 *「다 불러내고 광역으로」*를 **조립 단계의 선택**으로 만든다
## — 지금까지 위력은 전투 안에서만 의미가 있었고, 이게 그 첫 바깥 축이다.
##
## 🔴 **입력은 `RingPower.quality_t(score)`이지 raw `score`가 아니다.** 그 함수 머리말이 이미
##   경고한다 — 조립 점수의 실사용 범위는 0.70~1.0이라 **raw를 lerp에 넣으면 하위 70%가 통째로 죽고**
##   맨 진↔퍼펙트 반경비가 **1.43배**밖에 안 나온다(설계 C4). 그러면 위 문단이 성립하지 않는다.
##
## 🔴🔴 **왜 「계수 하나」가 아니라 둘인가**(설계 §13은 `@export` 1개로 적었다):
##   하나만 두면 **최소↔최대 비율이 코드 const로 내려간다.** 그런데 그 비율이 정확히
##   설계 §10 검증 3이 재는 값(*"1.43배보다 크다"*)이라 — **그물이 흔들어야 할 손잡이를
##   그물이 못 닿는 곳에 두는 꼴**이 된다. `vision.gd` 머리말이 같은 이유로
##   *"수치는 반드시 밖에서 주입한다"*고 못 박았다(static const 폴딩 함정).
##
## ⚠ **잠정값이다 — F5가 정한다.** 설계 §11이 *"가장 먼저 조일 값 셋"*에 소리 계수를 넣었다
##   (*"쏘면 늘 들키나"*). 기준점: 주변시 220 · 부채꼴 사거리 560 · 방이 2400×2200.
##   맨 진은 「내 둘레만」, 퍼펙트는 「화면 밖까지」가 의도다.
@export var percept_sound_radius_min_px: float = 240.0
@export var percept_sound_radius_max_px: float = 700.0
