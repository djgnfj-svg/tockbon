...
## 사용자 확정: "65퍼 이하면 터지고". 지금 등급으로는 「거침」(<55)과 「무난」(<75) 사이를 가른다
@export var ring_stability_min: float = 0.65
## 기준선을 겨우 넘긴 마법진(≈65점)의 위력 배율 — 살아남았지만 시원찮다
@export var ring_power_min: float = 0.7
## 만점(100점) 마법진의 위력 배율. 기준선~만점 사이를 선형 보간한다 —
## "점수가 높을수록 성능이 좋아"(사용자)가 **연속적**이어야 1점 차이가 의미를 갖는다
