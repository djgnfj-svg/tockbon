...
##
## **왜 뒤집혔나 — 차폐가 들어오면서 이 값의 뜻이 바뀌었다.** 주변시는 **차폐 면제**다
##   (`vision.gd`의 그 순서가 계약이다 · `vision_occlusion_design` §2-3 ①). ⇒ **주변시가 크면
##   나무 뒤도 주변시로 보여서 차폐가 통째로 무의미해진다.** 220은 화면 가로 반폭(480)의 절반에
##   가까워, 그 안에서는 무엇을 세워도 안 가려진다. **차폐와 주변시는 한 몸이고 서로를 갉는다.**
##
...
##
## ⚠ **잠정값이다 — F5가 정한다.** 기준점: 프롭 격자 `PROP_GRID` 148px이라 110이면
##   **「나무 한 그루 너머는 안 보인다」**가 대략 성립한다. 너무 작으면 「코앞만 보인다」가 되고,
##   키우면 차폐가 다시 무의미해진다.
