...
##  ⚠ **세100에 `color` 키 자체가 은퇴했다** — 세45에 스프라이트로 갈아타며 `_visual.color`가 사라져
##  **죽음 퍼프 색으로만** 살아 있었는데, 그 퍼프가 마나 폭발(푸른색 통일)로 바뀌며 **소비자가 0**이
##  됐다(감사 T3 「거짓 손잡이」). `.tres` 세 장에서 줄을 걷었다. 몸을 물들이는 건 **`tint`**(세99 신설)이고
