...
func seed_codex_unlocks() -> void:
	...
	# 🔴🔴 **세88 사냥 흐름 — 시작은 룬 1 · 진 1 · 고리 1뿐이다** (사용자 결정 ④:
	# *"나머지 전부 획득 대상으로 회수"*). 세78~83의 **임시 시드 10키를 걷었다** — 그 줄들에
	# *"경로를 붙이는 세션이 이 줄을 지운다"*고 적혀 있었고, **이 세션이 그 세션이다.**
	#
	# 걷은 10키가 지금 어디서 나오나 (정본 = `docs/takbon-design/hunt_loop_design.md` §5·§4):
	#   물·바람·풀 룬  → 챕터 보스 **첫 처치**(`ChapterDef.reward_unlock` — ch1·ch2·ch3)
	#   번개·흙 룬     → 공방 제작(`RecipeDef.reward_unlock`)
	#   진 2종·고리 3종 → 공방 제작 + 잡몹 **두루마리 드롭**(`DropEntry.unlock_id`)
	#
	# 🔴🔴 **되살리지 마라.** 시드가 있으면 보상·제작·드롭이 전부 **소급 완료로 덮여 조용히 죽는다**
	# (세58에 실제로 그랬다 — 관문을 붙였는데 시드가 이미 해금해 둬서 아무 일도 안 일어났다).
	# ⚠ 이 회수는 **F8 「새로하기」로만 검증된다** — 기존 세이브엔 codex가 이미 실려 있어 룬 6종이
	#   그대로 보인다(= 아무것도 회수되지 않은 것처럼 보이는 거짓 초록불).
	codex[&"rune_fire"] = true      # 시작 룬 = 불 하나
	codex[&"jin_single"] = true     # 시작 진 = 단발진(1겹·룬 1자리)
	# 🔴 발산×5 — ch1 보상이 **물 룬으로 바뀌면서** 이 고리가 시작 지급으로 이관됐다(세88).
	# 시작부터 고리 하나는 있어야 「밴드에 끼운다」를 배울 대상이 존재한다(온보딩이 이걸 가르친다).
	codex[&"gr_radiate5"] = true
	# ⚠ 문양(GlyphDef)은 시드가 아니다 — **해금 게이트 자체가 없다**: GlyphDef에 unlock_id 필드가
	# 없고 책도 `Db.all_glyphs()`를 무필터로 띄운다(ring_forge_panel._inject_defs). 옛 `glyph_thrust`
	# 시드는 소비자 0인 유령이었다. 문양에 해금 축을 세우면 그때 시드·판정을 같이 만든다.
	# ⚠ gr_gather3는 세87까지 획득 경로가 없었다 — 세88에 공방 제작(`craft_gr_gather3`)이 받았다.


## 🔴 시작 퀵슬롯 미리 장착 — **슬롯 1에 불 마법진 한 장뿐이다**(세88 사냥 흐름 §6-E-2).
## 조립을 거치지 않고 완성된 도안을 심는다 — 단발진(jin_single)에 빈 고리(문양 없음)+불 룬,
## 점수 1.0(안정, 펑 안 남). 빈 고리도 캐리어 몸이 착탄 피해를 주므로 볼만 날리는 데 충분하다.
##
## 🔴🔴 **세88에 세 장 → 한 장으로 줄였다. 여기가 시드 회수의 진짜 급소였다.**
## 세78~87은 불·물·바람 **완성 도안 3장**을 슬롯 0·1·2에 직접 꽂았다. `seed_codex_unlocks`에서
## 물·바람 **codex만** 걷으면 *"책에서 물 룬을 못 고른다"*일 뿐이고 **1·2·3 키로 물볼·윈드볼이
## 그대로 나간다** — 발사는 `to_assembly()`가 도안을 그대로 읽고 **codex를 안 본다**.
## 그러면 ch1 보상 「물 룬」이 **첫 판부터 쏘던 것**이 되어 보상 지도가 통째로 무너진다.
## ⚠ 빈 슬롯 발사는 세59에 **거부 + 안내 + 마나 무소모**로 이미 안전하다(소프트락 없다).
##
## ⚠ Db 미준비 시점(GameState._ready가 Db._ready보다 먼저)이라 정적 값만 쓴다 — Db 조회 금지.
## 기존 세이브는 load_game이 이 시드를 덮으므로(옛 슬롯 유지), 이 한 장은 **새 게임**에서만 뜬다.
func _seed_starting_rings() -> void:
	ring_designs.clear()
	_reset_equipped()
	var d_fire := _make_seed_ring(Enums.RuneType.FIRE, "불 마법진")
	ring_designs.append(d_fire)
	ring_equipped[0] = d_fire   # 슬롯 1 = 파이어볼. 🔴 슬롯 2·3은 **비운다**(위 주석 참조)
...
##  🔴 목표에 닿아도 여기서 완료하지 않는다 — 문에 [E]로 말 걸어 claim_ready_quests()로 정산해야 완료된다(세95).
...
## 목표를 채웠나 (완료와 별개 — 완료는 문에서의 정산에서만 일어난다, 세40·세95).
...
## 🔴 문에서의 정산 (세40 · 세95에 길잡이NPC→문) — 달성한(claimable) 퀘스트를 실제로 완료(보상·다음 개방)한다. **정산은 여기서만.**
...
##  세션40: 이제 claim_ready_quests()(문에서의 정산)에서만 불린다 — advance_quests는 부르지 않는다.
...
##  이미 조건이 충족된 채 열린 퀘스트는 claimable로 파생돼 물음표로 뜨고, 문에서의 정산에서 완료된다.
