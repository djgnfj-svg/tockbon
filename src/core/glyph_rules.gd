extends RefCounted
## 🔴 문양 **알고리즘 표**의 단일 소스. class_name 없음 — `const GR := preload(...)`.
## 문양을 늘릴 때마다 계열 판별·효과 분기·수치가 코드 여기저기로 흩어지는 걸 구조로 막는다.
##
## 🔴 `Db`를 참조하지 않는다 — 순수 static 표라 `GlyphDef`를 받아서 답한다. `Db`를 더듬으면
##   static 캐시가 테스트의 in-memory 주입 관행(`db.glyphs[...] = ...`, 아무도 reload()를 안 부른다)과
##   충돌해 **조용히 자명 통과하는 그물**이 된다. `code → GlyphDef` 조회는 `Db`가 쥔다.
##
## 새 알고리즘 = `BEHAVIORS`에 줄 하나 + `ring_spell_system`에 함수 하나. 하지만 대부분의 새 문양은
## 여기를 안 건드린다 — 같은 알고리즘에 수치만 다르면 `.tres` 한 장이다.
...
##
## 전개형/변형형이 곧 층(밴드) 해석의 뼈대다 — 층은 안→밖이 연산 순서이고, 변형형은 그때까지
## 쌓인 명령 목록을 함수처럼 변환한다. 계열이 문양 데이터에 딸린 성질이라 상수가 아니라 이 표에 둔다.
const BEHAVIORS := {
	&"bolt": {"modifier": false},
...
## `bolt.effect` 문자열 → `Enums.GlyphType` — 문양 데이터가 자기 효과를 들고 온다.
## ⚠ `&""`를 여기 넣지 마라 — 효과 없음은 빈 사전(`{}`)으로 떨어져야 한다.
##  `GlyphType.BASIC`(=0)을 실으면 `projectile._setup_effects`가 BASIC을 안 읽어 지금은 무해하지만
##  새는 씨앗이 된다.
const EFFECTS := {
...
## ⚠ `.tres`의 사전 키는 String으로 저장되지만 StringName으로도 조회된다(둘은 Dictionary 키로
## 상호 호환) — 그래서 키 타입을 호출부가 신경 쓰지 않아도 된다.
...
## 효과 없음이면 빈 사전이다(위 `EFFECTS` 주석 참조).
