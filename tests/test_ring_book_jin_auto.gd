extends SceneTree
## 책 진 탭이 진 8종을 갈라 보여 주는가 자동 검증 — 헤드리스 실행:
##   ./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_ring_book_jin_auto.gd
## 🔴 왜 있나: 항목이 3→N으로 늘 때 한 줄 격자가 셀을 쪼그라뜨리고 설명 배열이 인덱스 초과로
##  잠복 크래시를 냈다. 여기서 지키는 계약: ① 셀이 다 생기고·책 밖으로 안 나가고·서로 안 겹친다
##  ② 아이콘 획이 서로 다르다(색만 다른 N지선다가 아니다) ③ 늘어도 인덱스 초과로 안 죽는다(폴백).
## ⚠ 헤드리스가 못 잡는 것: 셀이 실제로 보이는지·클릭이 닿는지 — 실게임 확인이 대체되지 않는다.
...
func _test_all_jins_present(jins: Array) -> void:
	# 카탈로그가 1종까지 비워질 수 있다 — 되살릴 때마다 늘어난다.
	_check(jins.size() >= 1, "진이 1종 이상이어야 한다 (실제 %d)" % jins.size())


## 🔴 격자 계약 — 8칸이 다 생기고, 책 안에 들어가고, 서로 안 겹친다.
## 뮤테이션: _draw_jin_cells를 한 줄 배치(cw = 전체폭/n)로 되돌리면 셀 폭이 30px로 쪼그라들어
##  이름이 넘친다 — 여기선 높이가 안 늘어 아래 desc 줄과 안 겹치므로 폭 하한(48px)으로 잡는다.
func _test_cells_fit_and_dont_overlap(jins: Array) -> void:
	var top: float = _BookScript.body_top()
	# Db 진이 적어도 격자 계약(8칸 배치·겹침 없음)은 합성 개수 8로 계속 잰다 —
	#  jin_cell_rects는 순수 함수라 Db와 무관하고, 진이 되살아나도 이 검사는 그대로 유효하다.
...
## 🔴 아이콘이 진마다 갈리는가 — 색만 다르면 N지선다다. 갈리는 축 = 진 모양(guide_shape) × 룬 자리 수(rune_slots).
## ⚠ pattern·motion은 안 잰다 — 죽은 축이 아니라(ring_spell_system이 여전히 읽는다) 셀이 답하는 질문이 아니게 된 것이다.
## 뮤테이션: jin_icon_paths가 shape를 무시하고 늘 원을 내면 도형 조합이 대량 실패하고,
##   rune_slots를 무시하고 늘 1자리면 아래 [자리 수] 검사와 지문이 같이 실패한다(실측 확인).
func _test_icons_differ(jins: Array) -> void:
	# Db 진이 적어 실데이터만으론 「서로 다르다」를 못 잰다 — 합성 조합으로 검출력을 유지한다.
	var combos: Array = []
	...
	for c in combos:
		...
		# 🔴 뮤테이션이 잡은 그물 구멍: 자리 좌표를 전부 중심으로 고정해도 위의 [자리 수]·지문이
		#   전부 그린이었다(원이 한 점에 포개질 뿐 개수는 그대로다) — 융합진 셀에서 두 자리가 겹쳐
		#   기능 자체가 죽는데 아무도 안 빨개진다. 그래서 판의 단일 소스와 같은 좌표인가를 직접 잰다.
...
## 🔴 층 탭 미리보기 아이콘이 판 정본에서 나오는가 — 옛 아이콘은 판을 베끼고 있었다. 판이 층 선을
##  문양 바깥으로 밀자 미리보기만 옛 그림에 남아 「본 것 ≠ 끼운 것」이 됐다.
## 그물이 지키는 계약 셋 — 전부 관계식이라 판이 여백·모티프 크기를 조여도 거짓 빨강이 안 난다:
##   ① 🔴 모티프 점열이 RingBoard.glyph_ring_subpaths 정본과 모양이 같다(로컬 계산 = 빨감)
##   ② 🔴 선이 모티프를 침범하지 않는다 — 모든 모티프 점이 띠 두 선 사이(옛 그림 = 빨감)
##   ③ 바깥 띠선이 아이콘 반지름에 앉는다(= 카드 밖으로 안 샌다) · 빈 소켓은 띠만
## ⚠ 겉보기(톤·굵기·「읽히나」)는 헤드리스가 못 본다 — 실게임 스샷이 최종 판정.
## 아래 헬퍼는 모티프를 자기 중심·크기로 정규화한다 — 어디에 얼마나 크게 놓였는지를 지우고 「모양」만
##  남긴다(아이콘은 판을 확대·맞춤 배율로 옮겨 그리므로 좌표는 달라도 모양은 같아야 한다).
...
func _test_ring_icon_from_board() -> void:
	...
	for bi in [0, 1]:
		for combo in [[1, 5], [6, 3], [7, 1], [0, 8]]:
			...
			# ① 🔴 판 정본과 모양이 같다. ⚠ 아이콘은 판의 픽셀 축소판이 아니다 — 판 비율 그대로면
			#   모티프 지름이 아이콘 반지름의 24%(≈4px)라 「폭발 ×1」이 점 하나로 사라져서,
			#   ICON_MOTIF_ZOOM이 모티프와 띠를 같은 배로 벌린다(「선 사이」는 유지).
			# 🔴 그래서 계약은 「점 좌표 동일」이 아니라 정규화한 모양이 같은가다(사본을 만들면 갈라진다).
			# ⚠ 배치 각도는 여기서 안 잰다 — 판을 임의 반경(100)으로 만들어 아이콘의 작은 반경과 비교하면
			#   모티프 평균 방향에 반경 의존 편차가 생겨 축 비대칭 문양이 거짓 빨강이 났다(검사 자체가 틀렸다).
...
## 🔴 층 탭 소켓·목록 레이아웃 — 카드를 키운 대가로 목록이 사라지지 않는가.
## 카드를 키워 아이콘 겉보기를 되찾은 대신 스크롤 없이 보이는 고리가 3장으로 줄었다 — 그 거래의
##  하한을 여기서 못 박는다(더 키우면 목록이 1~2장으로 무너진다).
...
## (설명 배열이 인덱스 초과로 잠복 크래시를 냈던 자리와 같은 종류의 계약.)
