extends SceneTree
## 🔴🔴 `rings`(정수 층 배열) ↔ `band_defs`(GlyphRingDef 배열) **왕복** 검증 (세98). 헤드리스 실행:
...
##
## 정본 = docs/takbon-design/spell_cast_visual_design.md §2
##   발밑 바닥 마법진은 **매핑표 없이** `RingBoard.compose_guide_paths(...)`를 불러 「조립한 그림
##   자체」를 그린다. 그런데 `RingDesign`은 `.tres`에 **정수 배열만** 저장하고 GlyphRingDef 참조를
##   저장하지 않는다 → `RingBoard.band_defs_of`가 그걸 되돌린다.
##
## 🔴🔴 **이 파일이 존재하는 이유 = 사본 위험(T5)이다.**
...
##   있는지를 매 케이스에서 같이 못 박는다(세61 리셋 때 그물이 검사 0건으로 그린이던 전례).
##
## 🔴🔴 **이 그물이 못 잡는 것 — 정방향 배치 규칙의 변경**(세98 뮤테이션 실측):
...
##      같은 M4 뮤테이션에 8개가 빨개졌다). 여기서 그 리터럴을 **또 적지 마라**(사본이 는다).
##
...
## 🔴 대조군이 증발하면 아래 케이스들이 **검사 0건으로 그린**이 된다 — 세50 바람 룬이 두 세션
## 죽어 있는데 전 스위트가 그린이던 자리와 같은 종류다.
...
func _test_null_band_keeps_index() -> void:
	...
	# (M4 뮤테이션 실측: 정방향 배치를 칸 1..로 밀었더니 생성 입력 케이스는 전부 그린이었다).
...
# ── [4] 🔴 옛 8칸 한 겹(세78까지)도 왕복한다 — 층 판별을 **core 단일 소스로** 거치는가 ──
...
## 세79 이전 `flatten_bands` 도안은 한 겹에 여러 모티프가 섞여 있는데 `GlyphRingDef` 하나는
## 모티프를 하나만 담는다. 🔴 **발사는 멀쩡하고 그림만 거짓말한다**(발사는 `rings`를 직접 읽는다).
## 여기서 그 한계를 **명시적으로 못 박는다** — 한계가 조용히 바뀌면(혹은 해소되면) 이 줄이 빨개져
## `band_defs_of` 주석과 설계문서 ⓙ를 같이 고치게 만든다.
