extends Control
## 고리 조립 마법진 — **프로토타입**. 기존 게임과 분리된 실험 씬이다(오토로드·모듈 의존 없음).
##
##   • 진이 **고정 칸**을 준다: 고리 N겹 × 8칸. 칸을 클릭해 글자를 끼운다
##   • **고리 = 실행 단계**: 중심(룬 발화) → 안 고리 → 밖 고리 순서대로
...
# 팔레트 — 지금 어휘는 3종. 도형 하나하나가 힘을 갖고, 조합이 목적.
...
## 🔴 문양 고리는 **룬과 진 사이**에만 있다. 진은 바깥 그릇(경계)이라
...
## 진 = 바깥 그릇. 두꺼운 경계 + 단계(고리 수)만큼 스파이크.
