...
## ⚠ **세107: 세105에 늘었던 셋(`occluders`·`occluder_count`·`shadow_feather`)이 은퇴했다** —
##  차폐가 통째로 빠졌다(`src/core/vision.gd` 머리말). 되살리면 여기에 다시 올려라.
...
func _test_uniforms_match_balance() -> void:
	...
	# ⚠ 세105의 그늘 uniform 셋(`shadow_feather`·`occluder_count`·`occluders`)을 재던 자리다 —
	#  세107 차폐 은퇴로 걷었다(`REQUIRED_UNIFORMS` 주석).
	_free_room()
...
func _test_shader_has_no_hardcoded_shape() -> void:
	...
	# 🔴🔴 **세107 재발 감지 — 차폐가 은퇴했으니 셰이더에 그 흔적이 없어야 한다.**
	#  누가 그늘 루프를 되살리면 여기가 빨개진다(T2: 은퇴를 선언 없이 두면 아무도 못 알아챈다).
	#  ⚠ 판정 쪽 짝 = `test_vision_auto [12]` · `test_perception_auto [15]`(라이브).
	for gone in ["MAX_OCCLUDERS", "occluder_count", "shadow_feather"]:
		_check(not code.contains(gone),
			"🔴 셰이더 코드에 '%s'가 없다 = 차폐 그늘이 은퇴한 채다 (되살리려면 판정·화면·그물을 같이 열어라)"
				% gone)
...
## ⚠ **세107: 여기 있던 [8]·[9]·[10]이 은퇴했다** — 차폐 그늘([8] `lit_at` ↔ 그늘 대조),
##  엄폐물 상한 넘침([9]), 「언제 굽나」([10])를 재던 셋이다. 재던 대상이 통째로 사라졌다
##  (`src/core/vision.gd` 머리말). 🔴 **되살리려면 판정·화면·그물을 셋 같이 열어라** —
##  지금 그 재발을 잡는 건 [3]의 셰이더 소스 스캔 + `test_vision_auto [12]` ·
##  `test_perception_auto [15]`(라이브)다.
