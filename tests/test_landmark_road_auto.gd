extends SceneTree
## 지점(랜드마크) + 입구→지점 흙길 자동 검증. 헤드리스 실행:
...
## ⚠ 못 잡는 것 = 겉보기 판단 전부(둥지가 떠 보이나 · 길이 「길」로 읽히나 · 알파가 적당한가) — 스샷·F5 몫이다.
## 주의: -s 모드는 오토로드 전역 등록보다 먼저 컴파일된다 — 오토로드 식별자·모듈 preload 금지.
## 첫 프레임 후 load()·/root 접근. 지역 변수는 의도적으로 동적 타입.

## 🔴 타일 소스 id·벌판 좌표는 명시 상수로 박는다 — `boss_room`의 const에서 파생하면 둘 다 밀려도 통과한다.
## ⚠ `test_extract_hold_auto`가 같은 값을 든다 — 사본이 아니라 「손으로 든 기대치」다(시트를 바꾸면 두 곳이 같이 빨개지는 게 맞다).
const TILE_SRC_GROUND := 2
const GRASS_ATLAS: Array[Vector2i] = [Vector2i(1, 4), Vector2i(2, 4), Vector2i(3, 4)]

## 🔴 둥지 스프라이트의 접지 계약 — 텍스처 (95,140) = 굴 입구의 x 중심이자 흙더미가 바닥에 닿는 줄.
##  `centered=false`면 offset이 그 음수다.
## ⚠ 씬은 `chest.tscn`으로 개명됐는데 텍스처 상수 둘은 그대로다 — PNG 개명은 `--import`가 필요하다.
...
## 「둥지 앞에 선다」의 프로브 거리 — 접지선(프롭 원점)에서 이만큼 아래.
## 🔴 작아야 그물이다 — 260px이던 시절엔 감지 상자를 아래로 250px 늘려도 프로브가 밖이라 전 항목 그린이었다.
## ⚠ 기준이 스프라이트 치수가 아니라 원점(=접지선)이라 그림을 키워도 안 낡는다. 플레이어 몸 반경(10px)만큼은 띄운다.
...
func _test_data_chain() -> void:
	...
	if ch == null:
		return
	# 🔴 「비어 있지 않다」가 이 항목의 심장이다 — 데이터가 비면 코드는 멀쩡한데 방엔 지점이 0개고 전 스위트가 그린이다.
...
## [3] 입구에서 지점까지 흙길이 끊김 없이 이어진다 + 꺾이는 자리(모양).
## 🔴 연결을 값으로 잰다 — 「길 칸이 몇 개 있다」로 재면 방 어딘가에 흙 얼룩만 있어도 통과한다.
## 🔴 연결만으로는 모자라다 — `_road_between`의 인자를 뒤집어도 전 항목 그린이었다(뒤집힌 길도 이어져는 있다). `_check_road_shape`가 그 자리를 잰다.
## ⚠ 「길이 예뻐 보이나」는 안 잰다 — 여기서 재는 건 꺾이는 자리뿐이다.
...
## 🔴 길의 모양 = 「어느 열로 들어가나」 — 길은 입구 열로 곧게 들어가다가 지점 행에서 꺾인다.
## 즉 ㄱ자의 꺾이는 자리가 (입구 x, 지점 y)다. 인자를 뒤집으면 (지점 x, 입구 y)가 돼 방 아래쪽이 통째로 흙 밭이 된다.
...
## 🔴 ⓑ가 대소 비교인 이유: 「몇 칸 이하」로 박으면 방·길 폭을 조일 때 거짓 빨강이 난다(`test_jin_layers_auto [4]`와 같은 판단).
## ⚠ 지점이 입구와 같은 열이면 ⓑ가 자명 통과라 SKIP으로 뺀다(자명한 줄을 PASS로 찍으면 「재고 있다」는 착각이 남는다).
...
## 이 둘 중 하나만 바뀌면 그림이 95/140px 튀어 떠 보이거나 파묻히는데 에러는 0이다.
## ⚠ 「떠 보이나」는 스샷이 정한다 — 여기서 재는 건 그 값이 계약대로인가 하나다.
...
## [5] 🔴 프롭 계약 — 물리 몸이 없다. StaticBody2D로 만들면 world 레이어라 마법 캐리어(마스크 5)가 지점마다 터진다.
## ⚠ **재귀로 훑는다** — 자식 하나에 몰래 붙어도 잡힌다.
## 🔴 Area가 둘이라 「전부 layer 0」이 아니라 「world·enemy 비트 없음」을 잰다:
##  ⓐ `BehindSense` = 겹침 감지 — layer 0 · monitorable false · 아래 변이 접지선 위(넷 다 안 무른다).
##  ⓑ `OpenZone`(`interact_zone.gd`) = 여는 지점 — layer 64(interaction)가 그 스크립트의 계약이라 「전부 0」이면 거짓 빨강이 된다.
## 🔴 진짜 불변식은 「캐리어가 안 터진다」 = `layer & 5 == 0`(마스크 5 = world 1 + enemy 4)이라 넓혀도 검출력이 안 준다.
## 🔴 mask는 둘 다 2(player)다 — 4(enemy)를 더하면 늑대가 스칠 때마다 프롬프트가 제멋대로 뜨고 사라진다.
## ⚠ 아래 변 검사는 `BehindSense`에만 건다 — `OpenZone`은 남쪽에서 걸어오는 몸을 잡아야 해 접지선 아래로 내려온다.
func _test_prop_has_no_physics_body() -> void:
	...
	for a in areas:
		...
		# 🔴 둘 다 플레이어만 본다 — 적까지 넓히면 늑대가 스칠 때마다 판정이 돈다.
	...
	if sense != null:
		...
		# 🔴 감지 상자가 접지선(원점) 아래로 안 내려간다 — 아래로 새면 둥지 앞에 선 플레이어까지 뒤로 잡힌다(에러 0).
		#  [9]ⓑ가 같은 계약을 실제로 걸어 본다.
	...
	# ── ⓑ 여는 지점 — 자세한 동작은 `test_nest_open_auto`가 잰다.
	...
	if open_zone != null:
		...
		# 🪦 `hold_sec == 0` 검사는 지웠다 — 홀드가 폐기돼 그 프로퍼티가 없다. 「그 프레임에 열리나」는 `test_chest_open_auto` ⓐ가 잰다.
...
## 🔴 「둘러싸고 퍼져 있다」를 값으로 잰다: 반지름 대역 안 + 동서남북 네 방향에 다 있다 — 개수만 재면 한 줄로 세워 놔도 통과한다.
## 🔴 입구에서 곧게 뻗는 줄기에서 늑대가 aggro 밖이어야 지점을 「고르는」 것이 되지 통행세가 안 된다.
...
func _test_wolf_pack() -> void:
	...
	if hound == null:
		return
	# 🔴 정본 키는 `sight_range`이고 옛 이름 `aggro_range`는 폴백이다.
...
## 🔴 딱지가 아니라 스위치다 — `--import`를 돌리는 순간 같은 줄이 자동으로 측정으로 바뀐다. PNG 파일 자체가 없으면 그건 지금도 실패다.
...
## 보스방엔 y-sort가 없어 둘을 같이 해야 한다: ⓐ `z_index`를 플레이어·지팡이 위로 올리고 ⓑ `modulate:a`를 내린다.
## 🔴 하나만 하면 「아무 일도 안 난다」(z만 빠짐)거나 「몸이 통째로 사라진다」(알파만 빠짐)인데 에러는 0이라 둘을 같이 잰다.
## 🔴 접지선 아래(남쪽)는 안 건드린다도 같이 잰다 — 없으면 감지 상자를 아래로 늘려도 그물이 안 짖는다.
## 🔴 프로브는 접지선 바로 아래(`FRONT_PROBE_PAD`)다 — 260px이던 시절엔 상자를 250px 늘려도 전 항목 그린이었다. 정적인 짝은 [5]가 형상에서 잰다.
## 🔴 z 기대치를 리터럴로 안 든다 — 살아 있는 노드에서 읽는다. 리터럴이면 플레이어 z를 올리는 날 그물도 같이 눈을 감는다.
## ⚠ 고치는 건 그물이지 구조가 아니다 — `tree.gd`/`nest.gd`의 공용 부모를 만들지 마라(소비자 둘짜리 추상).
## ⚠ 「보기 좋은가」(알파 0.5가 적당한가)는 못 잰다 — 여기서 재는 건 방향뿐이다.
func _test_seen_through_when_behind() -> void:
	...
	# 🔴 급소: 접지선에 붙여 서야 감지 상자가 조금이라도 아래로 새는 순간 빨개진다 — 멀리 세우면 상자를 늘려도 안 짖는다.
...
## [10] 🔴 루트가 Node2D가 아닌 지점 씬 — 방은 계속 서고 그 자리만 빈다.
## `packed.instantiate() as Node2D`는 루트가 Control이면 null을 내고 다음 줄이 그 null에 써 SCRIPT ERROR로 판이 통째로 안 선다.
## 🔴 진짜 검출자는 `SCRIPT ERROR` grep이다 — 그래서 나쁜 슬롯을 앞에 꽂는다: 가드가 없으면 거기서 루프가 죽어 뒤의 진짜 둥지가 안 선다.
## ⚠ Control 루트 씬은 살아 있는 것을 빌린다(`damage_number.tscn` = Label) — 테스트용으로 새 씬을 만들면 「누가 쓰나」 없이 리포에 남는다.
...
## 🔴 한 칸에서 한 방향으로 끊김 없이 이어지는 흙길이 몇 칸인가 (길 모양의 측정자).
