extends SceneTree
## 방 재입장(`_enter_room`) 자동 검증 — 통과하면 "TEST_ROOM_REENTER_OK".
## 🔴 `_ready`를 「판 1회」와 「방마다」로 가르는 것이 이 계약이다. **「두 번 불러도 안 깨지나」**를
##  여기서 안 재면 방을 잇는 순간 **에러 0으로** 깨진 채 굴러간다.
## 재는 것:
##  [1] 전제 — 방이 서고 프롭·적이 실제로 있다(0이면 아래가 전부 자명 통과다)
##  [2] 🔴 **HP·마나가 방마다 안 회복된다** — 심장. 옮기면 「방을 이어서 깬다」의 소모가 사라지는데 에러 0
##  [3] `Props` 홀더가 안 쌓인다 — `add_child`를 방마다 하면 둘이 되고 `get_node_or_null`은 **첫 번째만** 준다
##  [4] 프롭·나무·지점·적이 안 쌓인다 + EventBus 연결이 안 겹친다(연결은 판 1회다)
##  [5] 🔴 **방 클리어 판정** — 다 잡으면 열리고 재입장하면 닫힌다. 죽은 몸이 한 프레임 그룹에 남으므로
##      **그냥 세면 영영 안 열린다**
##  [6] `_leaving`은 **판 수명**이다 [7] 지점 길이 지점에서 파생한다 [8] 방 번호가 배치를 가른다
##  [9] 보상 상자가 서고 **[E]가 실제로 뜬다** [10] 열면 가방이 늘고 **낱개 픽업은 안 는다**
## [11] 두루마리 해금이 **정확히 한 번**(값이 아니라 **횟수**) [12] 테두리 흙길이 안 쌓인다
## [13] **문이 판을 나른다** [14] 고른 문이 다음 방 상자 종류를 정한다 [15] 마지막 방이 판을 끝낸다
## ⚠ 못 잡는 것: **엔진 `ERROR:`**(스크립트가 못 읽는다 — 러너가 자식 출력에서 센다) ·
##  지연/직접 스폰 구분 · 겉보기(방 2가 방 1과 달라 보이나·헌 프롭이 깜빡이나)는 F5 몫이다.
## 🔴 `_enter_room`·`_leaving`을 **이름으로** 부른다 — 보통은 공개 API만 쓰지만 이 둘은 이름이 곧 계약이다.
...
## `reset_player_hp`·`restore_mana_full`·`in_expedition`을 `_enter_room` 쪽으로 흘리면 **방을 넘을**
## **때마다 완전 회복**이라 「방을 이어서 깬다」의 소모가 통째로 사라지는데 **에러도 경고도 0이다.**
## 🔴 값을 깎아 두고 재입장해 **안 돌아오는지**를 잰다.
func _test_hp_mana_not_refilled() -> void:
	...
	#  실제로 밟았다: `is_equal_approx(mana, spent)`가 **거짓 빨강**을 냈고 출력엔 `30 ↔ 30`으로
	#  찍혀(`%.0f` 반올림) 원인이 안 보였다 — 프레임 두 개만큼 회복돼 있었다.
	#  **재는 것은 「그대로인가」가 아니라 「만마나로 안 찼나」**다. ⚠ 내가 만든 검사도 틀릴 수 있다.
...
##
## 🔴 세 가지가 각각 다른 이유로 쌓인다:
##  ⓐ `Props` 홀더 — `_spawn_props`가 `add_child`를 한다
...
##
## 🔴 **판정이 `enemy_id == boss_enemy_id`가 아니라 「살아 있는 적이 0」이다**(보스를 안 세우는 방이 있다).
...
## 내리면 귀환·사망 도중에 방이 하나 더 서서 **씬을 두 번 갈아탄다.**
...
## ⚠ 흙길의 출처가 **둘**이다(지점 길 · 문 길). 「비우면 0칸」으로 두면 문 길이 남아 **거짓 빨강**이다.
...
## [12] 🔴 **`_fill_tiles`의 `clear()`가 재어지는 유일한 자리.**
## 🔴 전엔 이 방어선을 재는 것이 0이었다(뮤테이션으로 지워도 전부 그린). 문 x가 **방마다 흔들리므로**
##  테두리 위 길 칸이 방마다 갈리고, `clear()`가 없으면 **헌 토막이 테두리에 눌어붙는다**(에러 0).
## 🔴 **개수로 잰다** — 방마다 문 수가 같으니 테두리 길 칸 수도 같아야 한다. 쌓이면 늘어난다.
## 🔴 **자명 통과 방지**로 「두 방의 테두리 길 자리가 실제로 다르다」를 먼저 잰다 — 안 다르면
##  쌓여도 개수가 같아서 이 검사가 아무것도 안 잰다.
...
## [8] 🔴 방 번호가 배치를 가른다 — 「방마다 다른 배치는 공짜다」.
...
## [9] 🔴 **방을 깨면 보상 상자가 방 가운데에 서고, 그 상자가 실제로 작동한다.**
...
##  ⚠ 물리 콜백에서 붙여도 노드는 붙고 Area도 감지한다 — 즉 **지연/직접을 가르지는 못한다.**
...
## [11] 🔴 **해금 호출이 정확히 한 번.**
## 🔴 두루마리의 `unlock_id` 해금행을 `drop_pickup._grant_unlock`이 혼자 지고 있었는데 상자 경로는
##  그 노드를 **아예 안 지난다** ⇒ 그대로 두면 상자에서 나온 두루마리만 **조용히 해금이 안 걸린다.**
## 🔴 **횟수를 센다 — 「해금됐나」가 아니다.** 값만 보면 두 벌 경로가 **둘 다 돌아도** 통과한다.
##  이중 해금은 해금음·UNLOCK 퀘스트가 두 번 반응하는 것으로만 드러나고 **에러가 0이다.**
## ⚠ 실데이터엔 해금행이 없어 **임시 `DropEntry`를 주입했다 되돌린다.**
...
## [13] 🔴 **문이 판을 실제로 나른다** — 이 파일에서 제일 무거운 항목이다.
...
##  ⓒ 🔴 **HP·마나가 안 회복된다.** ⚠ [2]는 `_enter_room`을 **직접** 불러 쟀다 — 여기선 **문을 눌러**
##     같은 것을 다시 잰다. 전환이 실제로 도는 경로에서 안 지켜지면 [2]가 그린이어도 게임은 깨진 것이다.
##  ⓓ `_leaving`은 **판 수명**이라 방을 넘어도 안 내려간다
## 🔴 **`interacted`를 직접 emit하지 않는다** — 실제 입력을 민다.
...
## [14] 🔴 **고른 문이 실제로 다음 방의 보상을 정한다** — `tag` 인자의 존재 이유.
## 🔴 **이 항목이 없으면 `chest_id`가 거짓 손잡이다.**
## 🔴 **지금 데이터로는 못 잰다**: 쓸 수 있는 상자 종류가 하나뿐이라 문 셋이 같은 보상을 가리키고,
##  `chest_id`를 통째로 무시해도 결과가 같다. ⇒ **두 번째 종류를 주입했다 되돌린다.**
## 🔴 **관측점 = 다음 방 상자를 열었을 때 가방에 들어온 물건**이다. 내부 필드를 보면 라우팅은 됐는데
##  상자가 그걸 안 읽는 경우를 놓친다.
func _test_door_choice_actually_routes() -> void:
	...
	#  ⚠ 그 대입 하나로 이 함수가 중간에 끊겼는데 **`TEST_..._OK`가 그대로 찍혔다**(`failures`가 0인 채
	#   함수만 죽는다). 그래서 아래 `_check`들이 **함수 끝에 모여 있는 것**이 이 항목의 방어 형태다.
...
## [15] 🔴 **마지막 방을 깨면 판이 끝난다 — 그리고 반드시 `_extract()`를 지난다.**
## 🔴 **`extraction_success`가 이 그물의 심장이다.** 안 쏘면 **가방→창고 이관 · 자동 저장 ·
##  EXTRACT 퀘스트가 전부 조용히 멎는다.** 🔴 `test_quests_auto`는 그 시그널을 **직접 emit해서** 재므로
##  이 경로가 통째로 죽어도 **그린인 채로다** — 여기가 유일한 실경로다.
## 🔴 **「실눌림」 그물이다** — 리포에서 [E]를 진짜 눌러 나가는 그물이 한때 0이 됐다(남은 것들은
##  `interacted`를 직접 emit해 정산 채널만 잰다). 여기선 플레이어를 문 안에 세우고 `Input.action_press`
##  + `push_input`으로 진짜 [E]를 민다 ⇒ `interact_zone._unhandled_input`을 실제로 지난다.
## ⚠ **그래도 못 잡는 것 둘 — 알고 남긴다**: ① 화면을 덮는 Control이 입력을 먹는지(렌더가 없어 히트
##  테스트가 실제와 다르다) ② [E] 안내가 실제로 **보이는지**. 🔴 그래서 이 항목이 그린이어도
##  **실게임 확인이 대체되지 않는다.**
...
## 🔴 `_enter_room`을 **이름으로** 부른다 — 이름째 못 박은 계약이다(머리말 참조).
...
## 🔴 뒷정리 = 계약이다 — 장비·창고를 안 만졌어도 **오토로드 상태는 되돌린다**(테스트끼리 세이브를 공유한다).
func _cleanup() -> void:
	_gs.pending_chapter = &""
