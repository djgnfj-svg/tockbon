extends SceneTree
## 🔴 적 히트박스 = 「보이는 몸에 쏘면 맞는다」 자동 검증 — 헤드리스:
##   ./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_enemy_hitbox_auto.gd
## 왜 생겼나: 잡몹이 32px → 64px로 다시 그려졌는데 히트박스가 반경 9.0 고정이라 몸통 대비 비율이
##  0.56 → 0.16이 됐다. 쏴도 탄이 지나가는데 에러 0 · 전 스위트 그린이었다(몸과 판정을 같이 재는 그물이 없었다).
## 🔴 재는 계약은 하나다: 그림의 몸통 안쪽으로 쏘면 맞고, 몸 밖으로 쏘면 안 맞는다.
##  ⓐ만 재면 「반경 9999」가, ⓑ만 재면 「반경 0」이 통과한다 — 둘 다 있어야 그물이다.
## 🔴 진짜 탄으로 쏜다 — 점 질의로 형상만 재면 레이어·마스크·take_hit 계약이 빠져 「형상은 큰데 안 맞는다」를 놓친다.
## 🔴 몸통 크기는 PNG를 직접 읽어 잰다 — 숫자를 여기 베끼면 아트가 다시 그려질 때 거짓 빨강/거짓 그린이 된다.
## ⚠ 못 잡는 것: 「커 보이나」·「때리는 맛이 나나」 — 반경은 손맛 값이라 F5가 정한다(여기선 대역으로만 잰다).
## 주의: -s 모드는 오토로드보다 먼저 컴파일된다 — 오토로드 식별자·모듈 preload 금지. 첫 프레임 후 load()·/root 접근.

## 🔴 히트박스를 반드시 .tres로 쥐어야 하는 적 — 명시 열거다(개수 검사로 바꾸면 장치가 죽는다.
##  test_save_auto의 시드 집합 선례). ⚠ gale·snake_boss·slime_elite는 일부러 빠져 있다.
...
## 코드가 안 읽어도 통과하는 죽은 손잡이가 된다. 그래서 공개 리더로 되읽는다.
...
## [4] 🔴 형상 리소스 공유 함정 — .tscn의 SubResource는 씬 인스턴스끼리 공유된다. _apply_hitbox가
## duplicate() 없이 radius에 대입하면 마지막에 스폰된 적의 반경이 전원에게 번진다(에러 0).
##
## 🔴 재는 법이 핵심이다: 먼저 선 적의 반경을 기억해 두고, 다른 종을 세운 뒤 다시 읽는다.
##  「둘의 반경이 다르다」만 재면 순서에 따라 우연히 통과할 수 있다.
## ✅ 덤으로 씬 원본이 안 더럽혀졌다도 잰다 — 트리에 안 넣은 인스턴스는 _ready가 안 돌아 씬 파일의
##  기본 반경을 그대로 들고 있어야 한다(그 값이 보스 등의 하위호환 경로다).
## 🔴 살아 있는 적 둘의 로컬 반경이 같으면 이 그물의 검출력이 0이다(공유돼도 앞 몸의 값이 안 변한다) —
##  그래서 다른 로컬 반경을 쥔 몸을 Db에 주입한다(공유면 앞 몸이 끌려가 빨개진다. 그 뮤테이션으로 확인했다).
...
## 🔴 새 네임드가 오면 여기 짝 한 줄을 더해라 — 짝이 하나뿐이어도 계약은 그대로다.
...
## [2]·[3]은 디스크 PNG를 기준으로 잰다(아트가 정본이라서). 그런데 게임이 실제로 그리는 건
##  .godot/imported/*.ctex이라 --import를 안 돌리면 옛 32px 시트가 그대로 화면에 남는다 →
##  히트박스만 두 배가 된 채 몸은 작아서, 이 파일 전체가 그린인데 몸 밖을 때린다.
## 🔴 그래서 런타임 프레임 한 변 == 디스크 프레임 한 변을 여기서 못 박는다.
## ⚠ 빨개지면 코드 버그가 아니다 — --headless --import를 돌리면 된다.
