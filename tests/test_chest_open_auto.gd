extends SceneTree
## 지점 열기 + 우두머리 자리 자동 검증 — 헤드리스 실행:
##   ./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_chest_open_auto.gd
## 전 항목 통과 시 "TEST_CHEST_OPEN_OK" 출력 후 종료 코드 0.
## 🔴 이웃 셋은 「밟기 전」만 잰다(`test_landmark_road_auto` = 지점이 서 있나 · `test_chapter_auto [1d]` =
##  산출 데이터 · `test_mob_roll_auto` = 굴림 기계) — **밟으면 무슨 일이 나나**는 여기뿐이다.
## 🔴🔴 **`interacted`를 직접 emit하지 않는다** — 그렇게 재면 **홀드가 아예 안 돌아도 그린**이다.
##  전부 `Input.action_press` + 이벤트 주입 + 프레임 진행 + `action_release`로 민다(선례 = `test_spell_cast_auto`).
## ⚠ 못 잡는 것: 프롬프트가 **읽히나** · 낱개가 **겹쳐 보이나** · 「열렸다」의 손맛 — 전부 스샷·F5 몫이다.
...
## ⚠ **값이 `lm_nest`인 것은 오타가 아니다** — 개명은 씬·스크립트까지이고 **데이터 id는 그대로 뒀다**
##  (바꾸면 `data/chapters/*.tres`와 테스트 넷으로 번진다).
const CHEST_ID := &"lm_nest"
## 🔴 실재하는 재료여야 한다 — 없는 id면 픽업이 마름모 폴백으로 떨어진다.
const DROP_ITEM := &"mat_hound_fang"
## 🔴 **절대 안 나와야 하는 줄** — chance 0.0. 이게 나오면 굴림을 안 지나고 통째로 뿌린 것이다.
const NEVER_ITEM := &"mat_beetle_shell"

## 상자 앞(남쪽)에 서는 거리 — 🔴 **접지선 기준**이라 그림을 다시 그려도 안 낡는다.
...
func _run() -> void:
	create_timer(90.0).timeout.connect(func() -> void:
		print("TEST_CHEST_OPEN_TIMEOUT — 90초 초과")
		quit(1))
	await process_frame
	...
	if failures == 0:
		print("TEST_CHEST_OPEN_OK — 전 항목 통과")
		quit(0)
	else:
		print("TEST_CHEST_OPEN_FAIL — %d건 실패" % failures)
		quit(1)


## [1] 🔴🔴 **[E] 한 번에 즉시 열린다 — 그리고 열린 표시가 화면에 남는다.**
## 네 가지를 **한 판에서** 잰다(따로 재면 「열렸는데 겉모습만 그대로」 같은 반쪽 고장이 통과한다):
##  ⓐ 즉시(홀드 아님) · ⓓ 비워진 컷 · 프롬프트 꺼짐 · ⓒ 낱개가 상자 자리에.
...
func _test_open_is_instant() -> void:
	print("[1] [E] 한 번 = 즉시 열림 · 비워진 컷 · 프롬프트 꺼짐 · 낱개가 상자 자리에")
	_inject_quiet_chapter()
	_inject_drops(2, 2)
	await _fresh(&"ch1")

	var chest = _one_chest()
	if chest == null:
		_check(false, "상자가 섰다 (전제 — 안 서면 아래가 전부 자명 통과다)")
		return
	var zone = _open_zone_of(chest)
	_check(zone != null, "🔴 상자에 여는 지점(zone_id=%s)이 있다" % LANDMARK_ZONE)
	if zone == null:
		return
	# 🪦 `hold_sec == 0` 검사를 지웠다 — 홀드 기계가 폐기돼 그 프로퍼티가 없다.
	#  계약(**[E] 한 번에 즉시**)은 아래 ⓐ가 **행동으로** 잰다.
	await _stand_in_front(chest)
	_check(zone.player_in_range(),
		"🔴 상자 앞 %.0fpx에서 범위 안이다 (전제 — 밖이면 아래가 전부 「안 열렸다」로 죽는다)" % FRONT_STAND)
	_check(not chest.is_broken(), "열기 전엔 온전하다 (전제)")
	_check(chest.open_prompt_visible(), "🔴 다가가면 [E] 안내가 뜬다 (전제)")
	var intact_tex = chest.texture
	...
	_press_interact()
	# 🔴🔴 **`await` 전에 먼저 본다 — 「문구를 직접 내리는 것」의 유일한 측정 창이다.**
	#  프레임을 흘리면 `monitoring = false`의 deferred가 적용되며 `body_exited`가 와서
	#  `InteractZone`이 **스스로** 문구를 내린다 → 직접 내리는 줄을 지워도 검출이 **0**이 된다.
	# 🔴 화면 사정이기도 하다: 그 프레임에 안 내리면 「비워진 상자 + [E] 안내」가 한 프레임 같이 보인다.
	_check(not chest.open_prompt_visible(),
		"🔴🔴 **누른 그 프레임에** [E] 안내가 내려간다 (다음 프레임으로 밀리면 소진된 상자에 안내가 한 프레임 남는다)")
	await process_frame
	_release_interact()
	_check(chest.is_broken(),
		"🔴🔴 ⓐ [E] **한 번**에 그 프레임에서 열린다 (안 열렸으면 홀드가 끼었거나 배선이 끊긴 것이다)")
	# 🪦 `zone.is_holding()` 검사도 지웠다 — 그 API가 홀드 기계와 같이 사라졌다.

	# ── ⓓ 비워진 겉모습 — 「한 번만」이 **화면에 보이는** 규칙이어야 한다
	_check(chest.texture != null and chest.texture != intact_tex,
		"🔴🔴 ⓓ 열면 **다른 컷**으로 갈아 끼운다 (같으면 열었는지 아닌지 화면에서 구분이 안 간다 — S20)")

	# ── 프롬프트 — 안 끄면 「눌렀는데 아무 일도 안 난다」가 된다
	_check(not chest.open_prompt_visible(),
	...
	_check(not got.has(NEVER_ITEM),
		"🔴🔴 chance 0.0 줄(%s)은 **안 나온다** = 굴림을 실제로 지난다 (나오면 drops를 통째로 뿌린 것이다)"
			% NEVER_ITEM)
	for d in drops:
		_check(d.global_position.distance_to(chest.global_position) < 96.0,
			"🔴 낱개가 **상자 자리**에 쏟아진다 (실제 %.0fpx — 멀면 원점이나 플레이어 자리에 뿌린 것이다)"
				% d.global_position.distance_to(chest.global_position))
	_restore_all()


## [2] 🔴🔴 **한 판에 한 번뿐이다** — 이 파일의 심장.
## 🔴 게이트를 안 걸면 다 치운 안전한 자리에서 **무한 파밍이 최적 전략**이 된다.
##  🔴 즉시 열기라 **연타로 재현되고 에러는 0**이다.
## 🔴🔴 **연타를 「프레임 경계 없이」 해야 한다 — 뮤테이션으로 실측했다.** 사이에 `await`를 끼우면
##  `monitoring = false`의 `set_deferred`가 적용되고 `body_exited`가 와서 `_player_in_range`가 내려가
##  **두 번째 [E]가 애초에 안 들어온다** = 소진 처리가 가드를 가려 재열기 가드를 하나도 안 잰다.
## → **한 판은 한 번, 다른 판은 `await` 없이 세 번**을 눌러 **같은 프레임 안**에서 비교한다
##  (그 프레임엔 deferred가 안 돌아 `_player_in_range`가 참이라 **가드만이 유일한 방어선**이다).
...
func _test_second_press_gives_nothing() -> void:
	...
	_check(chest.is_broken(), "비워진 상태가 유지된다")
	_check(not chest.open_prompt_visible(),
	...
	#  → 「[E]가 떴는데 눌러도 아무 일도 안 난다」가 되고 **에러가 0**이다 — 그래서 왕복을 실제로 걷는다.
	var player: Node2D = _room.get_node("Player")
	player.global_position = chest.global_position + Vector2(0, 900.0)
	for i in 6:
		await physics_frame
	await _stand_in_front(chest)
	_check(not chest.open_prompt_visible(),
...
## 넣는 순간 **영영 빈 상자**가 되는데 **화면은 정상**이라 아무도 못 알아챈다.
...
func _test_open_state_does_not_survive_the_run() -> void:
	...
	await process_frame
	_release_interact()
	_check(chest.is_broken(), "한 판에서 열었다 (전제)")

	# 🔴 다시 들어간다 — `_fresh`가 방을 통째로 새로 세운다(= 재입장).
	await _fresh(&"ch1")
	var again = _one_chest()
	_check(again != null, "다음 판에도 상자가 선다")
	if again == null:
		_restore_all()
		return
	_check(not again.is_broken(),
		"🔴🔴 다음 판의 상자는 **다시 차 있다** (비워진 채면 연 상태가 오토로드·세이브로 샌 것이다 — 영영 빈 상자)")
...
## [4] ⓔ 🔴 **우두머리가 지점 좌표에 선다** — 그리고 **잠금이 아니다**(살아 있어도 열린다).
## 🔴 **둘을 같이 재는 게 핵심이다** — 「선다」만 재면 「먼저 잡아야 열린다」를 얹어도 그린인데, 그건 각하된 설계다.
...
func _test_named_stands_at_landmark() -> void:
	...
	var lm_at := _slot_position(ch, CHEST_ID)
	var free_at := Vector2(-900.0, 500.0)
	_check(lm_at != Vector2.ZERO and lm_at != free_at,
		"ch1의 %s가 %s에 있고 자유 위치 %s와 다르다 (같으면 아래가 자명 통과다)" % [CHEST_ID, lm_at, free_at])
	ch.named_pool = _typed_named([_named(&"hound_alpha", 1.0, free_at, CHEST_ID)])
	await _fresh(&"ch1")
...
## [5] 🔴🔴 ⓕ **같은 `landmark_id` 슬롯이 둘이면 각각에 뜬다.**
## 🔴 `LandmarkSlot`이 id + 좌표라 **상자 2채가 문법상 가능**한데, 구현자가 「첫 번째」로 정하면
##  **둘째 상자엔 영영 우두머리가 안 뜨고 에러가 0**이다.
## 🔴 **열기도 같이 잰다** — 둘째 상자를 열어도 산출이 나와야 한다(노드↔id 짝이 첫 번째로 굳으면
...
func _test_named_at_every_slot_with_same_id() -> void:
	...
	_inject_drops(1, 1)
	var ch = _db.get_chapter(&"ch1")
	var first_at := _slot_position(ch, CHEST_ID)
	var second_at := Vector2(-first_at.x, first_at.y)
	ch.landmarks = _typed_slots([_slot(CHEST_ID, first_at), _slot(CHEST_ID, second_at)])
	ch.named_pool = _typed_named([_named(&"hound_alpha", 1.0, Vector2(0, 300), CHEST_ID)])
	await _fresh(&"ch1")

	var chests := get_nodes_in_group("landmarks")
	_check(chests.size() == 2, "상자가 두 채 섰다 (실제 %d — 전제)" % chests.size())
	...
	# 🔴 둘째 상자를 연다 — 노드↔id 짝이 첫 번째로 굳으면 여기서 아무것도 안 나온다.
	var second = null
	for n in chests:
		if n.position == second_at:
			second = n
	_check(second != null, "둘째 상자 노드를 찾았다 (전제)")
	if second != null:
		await _stand_in_front(second)
		_press_interact()
		await process_frame
		_release_interact()
		_check(second.is_broken(), "🔴 둘째 상자도 열린다")
		_check(_pickups().size() > 0,
			"🔴🔴 둘째 상자도 **산출을 준다** (실제 %d개 — 0이면 노드↔id 짝이 첫 채로 굳은 것이다)"
				% _pickups().size())
...
## 🔴 지점 산출을 **in-memory로** 심는다 — `lm_nest.tres`의 `drops`는 **비어 있고**
...
func _inject_drops(lo: int, hi: int) -> void:
	var lm = _db.get_landmark(CHEST_ID)
	if lm == null:
		_check(false, "🔴 %s가 Db.landmarks에 실재한다 (전제)" % CHEST_ID)
		return
...
## 🔴 **`interacted`를 직접 emit하지 않는다** — `Input.action_press` + 이벤트 주입.
##  선례 = `test_spell_cast_auto`.
...
## 상자 **앞(남쪽)**에 선다 — 🔴 접지선(프롭 원점)에서 `FRONT_STAND`px 아래.
## ⚠ 물리 프레임을 여러 번 흘려야 `body_entered`가 실제로 온다(한 프레임이면 감지가 안 끝난다).
func _stand_in_front(chest) -> void:
	var player: Node2D = _room.get_node("Player")
	player.global_position = chest.global_position + Vector2(0, FRONT_STAND)
	for i in 6:
		await physics_frame
	await process_frame
...
func _open_zone_of(chest):
	for z in get_nodes_in_group("interact_zones"):
		if z.zone_id == LANDMARK_ZONE and z.get_parent() == chest:
			return z
	return null
