...
##  아무도 안 잰다 — 세99에 상자가 *"밟을 방법도, 주는 것도 없는 예쁜 이정표"*로 서 있던 자리다.
##
...
##      ⓒ **낱개가 상자 자리에 쏟아진다**(D12) + 🔴 **굴림을 실제로 지난다**(chance 0 줄은 안 나온다)
...
## ⚠ **값이 `lm_nest`인 것은 오타가 아니다** — 세112 1c의 개명은 **씬·스크립트까지**이고
##  **데이터 id는 그대로 뒀다**(바꾸면 `data/chapters/*.tres`와 테스트 넷으로 번진다 · `chest.gd` 머리말 ⓑ).
...
func _test_open_is_instant() -> void:
	...
	if zone == null:
		return
	# 🪦 **`hold_sec == 0` 검사는 세112 R1에 지웠다** — 홀드(D7) 자체가 폐기돼 `InteractZone`에
	#  `hold_sec` 프로퍼티가 **없다**(읽으면 런타임 에러다). 🔴 재던 계약(**[E] 한 번에 즉시**)은
	#  아래 ⓐ가 **행동으로** 그대로 잰다 — 프로퍼티를 안 봐도 「그 프레임에 열렸나」가 더 센 측정이다.
	...
	#  🔴 그리고 이건 그물 사정이 아니라 **화면 사정**이다: 누른 그 프레임에 안 내리면 「비워진 상자 +
	...
	# 🪦 `zone.is_holding()` 검사도 R1에 지웠다 — 그 API가 홀드 기계와 같이 사라졌다.
