extends SceneTree
## 🔴🔴 문 기둥 콜라이더 = 「보이는 돌은 막고, 보이는 통로는 지나간다」 — 헤드리스:
...
##
## 🔴🔴 **기대치를 리터럴로 들지 않는다 — PNG에서 파생시킨다.** 그림이 바뀌면 기대치가 따라 움직여야
##  한다(리터럴 기대치는 우연히 큰 값이면 영영 안 빨개진다). 어긋나는 형태는 둘 다 에러가 0이다:
##  콜라이더가 그림 밖이면 **허공에 막히고**, 그림 안쪽으로 파고들면 **통로가 좁아진다**.
## 🔴 기둥을 그림에서 뽑는 법: 돌기둥은 완전 불투명·포탈 수면은 반투명이라, 열마다
##  「a >= 0.99인 픽셀의 최장 연속 세로 run」을 재면 갈린다. **비율이 애매해지면 [1]이 빨개진다.**
## 🔴 **양방향으로 잰다**: ⓐ「기둥으로 걸어가면 막힌다」만 재면 **문 전체가 벽**이어도 통과하고,
##  ⓑ「통로로는 지나간다」만 재면 **콜라이더가 통째로 없어도** 통과한다 — [4]가 둘 다 잰다.
## 🔴 **진짜 플레이어로 걷는다** — 형상 질의로 재면 레이어·마스크 계약이 빠져 「형상은 맞는데
##  안 막힌다」를 놓친다.
## ⚠ **못 잡는 것**: 문이 「지나갈 만해 보이나」·[E] 문구가 읽히나. 통로가 체감상 넉넉한지는 F5가 정한다.
...
## 정적 대조 허용 오차(px) — 콜라이더 중심·크기의 반올림 여유. 실제 어긋남은 17~27px이라 넉넉히 잡힌다.
const EDGE_TOL := 2.0
...
## [6] 🔴 **레이어 계약** — 형상이 맞아도 레이어가 어긋나면 **에러 없이** 통과해 버린다
...
## 「문이 있는데 못 연다」가 된다(기능이 돈다 ≠ 거기 갈 수 있다).
...
func _test_village_does_not_resize_the_gate() -> void:
	...
	for path: String in stages:
		...
		for i in st.get_node_count():
			...
			#   (부모가 인스턴스 이름·위치를 오버라이드한다). 막는 건 **크기·각도**뿐이다.
			_check(bad.is_empty(),
