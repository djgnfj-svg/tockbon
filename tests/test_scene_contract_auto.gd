...
##     노드 프로퍼티를 읽는 것으로 잡힌다 — 렌더가 필요 없다.
##   → 즉 **1차 방어선(값)은 여기, 2차 방어선(도달)은 F5**다.
##
...
##     (`hud.gd`가 IGNORE로 세운다)이 **둘 다** 한 그물에 걸린다 — 그래서 인스턴스만 하지 않고 붙인다.
...
## **새 씬을 만들 때마다 되살아난다**. 대신 `src/`를 훑어 **플레이어를 인스턴스하는
...
func _run() -> void:
	...
	# 🔴🔴 **자명 통과 방지 앵커** — 「0행은 PASS가 아니다」. 위 루프는 검사 대상이
