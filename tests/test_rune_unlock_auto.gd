...
##
## 🔴🔴 **이 파일을 통째로 지우지 마라.** 아래 ④-b 「룬 6종 로드 + 6종 개별 확인」이 .tres 침묵사의
## 유일한 감지기**다 — `.tres`의 `Color`를 3인자로 쓰면 파서가 리소스 전체를 거부하고 `Db`가
## **말없이 건너뛰는데**, 그때 빨개지는 곳이 여기뿐이다(바람 룬이 죽어 있는 동안 전 스위트가
## 그린이었다). 합계만 재면 하나 죽고 하나 살아도 6이라 **개별 확인이 같이 있어야** 한다.
##
## 🔴 남은 계약 = **해금이 조립 책의 룬 목록에 흐른다**(codex → forge_panel._unlocked_runes 규약).
##  해금의 주체는 **퀘스트 턴인**(`reward_unlock`)이라 어느 주체가 codex를 심든 이 흐름이
##  성립해야 한다 — 그래서 아래 ③은 **codex를 직접 심어** 통로만 잰다.
##
## ⚠ 조각(FRAGMENT) 경로는 걷혔다 — `data/items`에 kind 6이 0장이라 늘 빈 목록이었다.
##  ⑤(unlock_id 오타 스캔)·⑥(조각 관문 드롭 감시)는 **남긴다**: 조각이 되살아나면 자동으로
##  다시 가동되고, 지금은 0행이라 SKIP으로 찍힌다.

var _pass := 0
var _fail := 0
## 🔴 0행 스캔은 PASS가 아니라 SKIP으로 찍는다 (조각 .tres가 0장인 동안 ⑤⑥은 잠들어 있다).
var _skips := 0
...
## `_visited`는 순회가 정본 전량을 훑었는지의 그물.
var _visited := 0
...
func _run() -> void:
	# 🔴 워치독 — `_run`이 중간에 죽으면 `quit()`에 못 닿아 프로세스가 영구 hang한다.
	...
	# 🔴 흙 룬을 **미해금 상태로 되돌려** 아래 ③이 「해금 전 → 해금 후」를 실제로 대조하게 한다
	#   (흙은 시작 지급이라, 안 지우면 ③이 「이미 켜져 있던 것」을 재는 자명 통과가 된다).
	#   ⚠ 실데이터 룬 자체는 **건드리지 않는다** — 주입 뒤 `erase`가 진짜 흙 룬을 지워 ④-b가
	#   6→5로 떨어진 사고가 있었다. 여기서 만지는 건 GameState.codex뿐이다.
	...
	# 🔴 손으로 적은 `[0, 2, 3, 5]`가 아니라 **정본 `Enums.RUNE_TYPES` 순회**로 잰다 —
	#   7번째 룬이 와도 「책 목록에 흘러드나」를 재게. 손 사본이면 새 룬이 그물에서 빠진다.
	...
	# 🔴 물·바람은 **챕터 보스 첫 처치 보상**이라 시작엔 잠겨 있다 (시작 = 룬1·진1·고리1).
	# 🔴 이 반전이 곧 **재발 감지기**다: 시드가 돌아오면 여기가 빨개진다 — 그리고 시드가 돌아오면
	#   ch1·ch2 보상이 **소급 완료로 덮여 조용히 죽는다**.
	...
	# ── ④-b 🔴 룬이 실제로 로드된다 (.tres 파싱 침묵사 그물) ──
	# rune_wind.tres가 파싱에 실패해 룬이 통째로 죽었는데 전 스위트가 그린이던 자리다.
	# 실데이터 룬 = **6종 전부**(불·물·바람·번개·흙·풀). 룬을 되살릴 때마다 이 기대치를 갱신.
	...
	# 🔴 개별로도 세운다 — 합계만 재면 「하나 죽고 하나 살아」도 6이 될 수 있다.
	...
	# 🔴 위반을 찾을 때만 `_check(false)`를 부르면 **위반이 없을 때 단정이 0개**가 된다 —
	#   드롭 스캔이 파손돼도 통째로 침묵. 그래서 ①훑은 줄 수를 세고 ②위반 목록을 한 번의
	#   단정으로 낸다.
	var drops_scanned := 0
	...
	# 🔴 실패하면 종료코드 1 (인자 없는 quit()은 실패해도 0이다).
