...
func _hp_carries_by_identity(t) -> void:
	...
	# 「수 보상은 그 자리에서 붙고 런은 지도로 돌아간다」 — and ⚠⚠ **the floor as well**:
	# 「`finish_island(true)` 는 `island_index` 를 혼자 안 옮긴다」. Mutation: put `island_index += 1`
	# back into `_advance`, and the map becomes a picture the run ignores.
	t.eq(r.state(), Run.State.MAP, "첫 칸을 이기면 지도로 돌아간다 — 다음 섬으로 혼자 안 넘어간다")
	t.eq(r.island_index, was_index, "그리고 섬 번호를 혼자 안 옮긴다 — 어느 칸으로 갈지는 손이 정한다")
	t.eq(r.map.at(), 0, "서 있는 칸도 0번 그대로다")
	t.ok(r.begin_island() == null, "지도로 돌아왔으니 전투도 안 열린다")
...
func _rewards(t) -> void:
	...
	# 「부리를 달고 나면 지도로 돌아간다」
	t.eq(r.state(), Run.State.MAP, "고르고 나면 섬이 아니라 지도로 돌아간다")
