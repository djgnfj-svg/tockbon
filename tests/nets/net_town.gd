...
func _wired_root(t) -> Node:
	...
	for pair: Array in [["_hud", "HUD/Stats"], ["_hp_view", "HUD/HpBar"],
			...
			#  `_char_view.clear()` unconditionally. Left out, `reset_stage()`/`enter_town()`/`_leave_town()`
			#  all crash on a null mid-call — the same risk this file's own `_gate_view` box above names.
