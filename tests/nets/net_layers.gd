...
##  **문법이 깨져도 그물이 전부 초록이다** — 게임을 띄워야만 드러나고, 그건 이 리포의 대표 침묵사다.
##
## ⚠ **`load()`가 모든 걸 잡지는 못한다.** `.gd`는 파스 에러에 null을 주지만
##  **셰이더는 컴파일이 실패해도 null이 아니다**(실측) — 그 구멍은 `net_render`가
##  `get_shader_uniform_list()`로 따로 메웠다. 여기 셰이더 줄은 **「파일이 읽힌다」까지만** 잰다.
