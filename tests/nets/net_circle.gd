...
##
## 🔴 **바닥값의 근거는 여전히 「199px에서 눈이 통과시켰다」 하나다** — 지금 지름과 두 배 차이지만
##  **그 근거로는 안 올렸다.** verify-look이 더 큰 화면을 통과시키면 **그때 그 새 관측을 근거로 올려라.**
##  ⚠ 근거 없이 올리면 숫자만 남는다 — 이 리포가 계속 잡아 온 바로 그 모양이다.
##
## 🔴🔴 **다만 32px 전환에서 180 → 360으로 갔고, 그건 「올린 것」이 아니라 「같은 값」이다.**
##  위 표의 px은 전부 **월드 px**이고, 화면 배율이 2배 → 1배가 됐다:
## ```
##  전: 바닥값 180 월드px × 2배 = 360 화면px    지름 280.1 × 2 = 560.2 화면px
##  후: 바닥값 360 월드px × 1배 = 360 화면px    지름 560.2 × 1 = 560.2 화면px
## ```
##  ⇒ **눈이 보는 크기가 한 픽셀도 안 바뀌었다.** 여유도 200.2 화면px 그대로다.
## ⚠ **위 표는 16px 세상의 관측이라 그대로 둔다** — 월드 px로 읽어라. 고쳐 적으면 관측이 아니게 된다.
##
