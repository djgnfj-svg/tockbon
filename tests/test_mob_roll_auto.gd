extends SceneTree
## 몹 풀 굴림 · 네임드 자동 검증 (세99 단계 2 — D5·D6) — 헤드리스 실행:
##   ./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_mob_roll_auto.gd
## 전 항목 통과 시 "TEST_MOB_ROLL_OK" 출력 후 종료 코드 0.
##
## 검증 대상 = **굴림 기계가 실제로 도나** (정본 `docs/takbon-design/dungeon_structure_design.md` §4·§6).
## 짝이 되는 그물 = `tests/test_chapter_auto [1c]`(실데이터 `.tres`가 성립하나) — **둘이 있어야 한다**:
## 기계만 재면 데이터가 비어도 그린이고, 데이터만 재면 코드가 굴림을 안 해도 그린이다.
##
## 🔴🔴 **시드를 고정하지 않는다 — 확률·가중치의 양끝을 주입해 잰다.**
##  지금 리포의 랜덤은 전역 `randf()`/`randi_range()` **하나뿐**이라 테스트가 전역 `seed()`를 잡으면
##  `forest_enemy._spawn_loose`의 각도 굴림 등이 **같은 스트림을 소비**해 순서가 조금만 바뀌어도
##  기대값이 어긋난다(= flake). 그래서:
##   • chance 0.0 → **절대 안 뜬다** / chance 1.0 → **반드시 뜬다** (양끝은 결정적이다)
##   • weight 0 → **절대 안 뽑힌다** (양끝)
##   • 기본 확률 → **여러 번 돌려 0 < 개수 < N** (「굳지 않았다」만 잰다 — 비율은 F5가 정한다)
##
## 🔴 여기서 헤드리스가 **실제로 잡는** 것:
##   • [1] `pool_tag`가 빈 자리는 `enemy_id` 그대로 선다 = **회귀 0**
##   • [2] `pool_tag`가 있는 자리는 **풀 안의 적만** 서고, `weight <= 0`은 절대 안 나온다
...
##
## 🔴 **주입한 것은 반드시 되돌린다** — `Db.get_chapter`는 **공유 리소스**를 돌려주므로
##  안 되돌리면 뒤 항목·뒤 테스트(같은 프로세스)가 오염된다(test_dungeon_schema [2] 선례).
##
## 공개 계약으로만 검증한다: 그룹 `"enemies"` · `enemy_id` · `ChapterDef` 필드.
...
func _run() -> void:
	...
	_restore_all()
	# 🔴 뒷정리 — boss_room이 실제 세이브를 쓰는 경로(extraction/bag_lost)에 물려 있다.
	#  여기선 안 쏘지만 `_fresh`가 방을 세우며 자동 저장이 돌 수 있어 test_chapter_auto와 같이 지운다.
...
## [1] 🔴 `pool_tag`가 비면 **세98까지와 한 톨도 안 다르다** — 회귀 0의 실증.
...
## [2] 🔴 `pool_tag`가 있으면 **그 풀 안의 적만** 선다. 자리는 그대로다(D5 — 좌표는 안 굴린다).
...
## [3] 🔴🔴 **`weight <= 0`은 절대 안 뽑힌다** — `MobWeight.weight` 주석이 *"임시로 빼려면 지우지
## 말고 0으로"*라고 약속한 손잡이다. 안 지키면 **거짓 손잡이**(감사 T3)가 되고, 빼 둔 적이 그대로 나온다.
##
## 🔴🔴 **ⓑ가 검출력의 심장이다 — 세99에 실측했다.**
##  ⓐ만 두고 `weight <= 0` 필터를 뮤테이션으로 지웠더니 **그린이었다**: 누적 뽑기 알고리즘이
##  0짜리를 「폭 0의 구간」으로 만들어 **우연히** 안 뽑기 때문이다(계약이 지켜진 게 아니라 알고리즘이
##  그렇게 생겼을 뿐이다). 진짜로 관측되는 고장은 **음수가 합계를 깎아 풀 전체를 죽이는 것**이다:
##  `slime 2 + vine (−3) = −1` → `total <= 0` → **그 자리들이 통째로 빈 채 서고 에러가 0**이다
##  (= 설계 §6 S1). 그래서 ⓑ가 필터의 유일한 측정자다. **ⓐ를 지우지는 마라** — 구현이 균등
##  뽑기 등으로 바뀌는 날 ⓐ가 계약을 다시 잡는다(지금은 자명 통과라는 사실을 여기 적어 둔다).
...
## [5] 🔴🔴 **보스 id 제외 가드** (설계 §6 S9) — 이 단계에서 가장 큰 위험이다.
##
## `boss_room._on_enemy_died`의 클리어 판정이 **`enemy_id` 일치 한 줄**이라, 풀·네임드에
## `boss_enemy_id`가 섞이면 **잡몹 한 마리를 잡았을 뿐인데 `chapter_clear_*` + 보상 룬이 전부
## 조용히 나간다(에러 0)**. 세98까지 안 겹친 건 `mob_spawns`를 손으로 박아 **우연히 보장**되던 것뿐이다.
...
## [7] 🔴 네임드는 **항목마다 독립**이다 (설계 §4) — 「하나도 안 뜸」이 정상 결과이려면
...
## 설계 §6 S3이 지목한 「항상 뜸 / 절대 안 뜸」으로 굳는 고장을 잡는다. ⚠ 비율은 **안 잰다**(F5 몫).
...
## [9] 🔴🔴 **네임드는 「생김새」로 구별된다** (D6 · 세99 사용자 확정 — *"몸에서 빛이나는거 까진 별로임"*).
##
## 재는 축 둘 — **덩치**(`params.size` = 루트 scale)와 **몸 색조**(`params.tint` = `_visual.modulate`).
## 🔴 **바탕이 되는 잡몹과 같은 프레임에서 비교한다** — 절대값을 박으면 F5로 조일 때 거짓 빨강이 되고,
...
##
## 세103까지 위 셋은 `modulate`를 **통째로** `Color.WHITE`와 비교했다. 세104에 시야가 들어와
## **알파가 두 번째 주인을 얻자** 그 비교가 깨졌다(늑대가 부채꼴 밖이라 알파가 페이드했다).
## ⚠ **세112에 시야가 은퇴해 그 주인은 사라졌다 — 그래도 rgb만 보는 이 모양을 되돌리지 마라.**
## 알파 축엔 **분산**이 남아 있고(`_alpha_now`), 통째 비교로 되돌리면 아래 경고가 그대로 살아난다.
##
...
##
## ⚠ 알파 축을 재는 그물은 따로 있다 — 분산 = `test_enemy_ai_auto [3b]`(세112에 시야가 은퇴해 그 하나뿐이다).
