...
## [3] ⓑ 플레이어 스폰·**모든 탈출구**에서 떨어져 있다 — 시작 시야와 [E] 찾기.
## 🔴 출구를 **씬에서 찾는다**(`zone_id == "exit"`) — `extract_points`만 읽으면 씬의 남쪽 `$Exit`가 빠지고,
##  좌표를 박으면 출구를 옮길 때 그물만 제자리에 남는다.
func _test_away_from_gates(id: StringName) -> void:
	print("[3] ⓑ 입구(플레이어 스폰)·모든 탈출구에서 떨어져 있다")
...
##
## 🔴 **왜 중요한가**: 출구는 겉모습이 없어서(보라색 차원문을 안 세우기로 한 결정) **보이는 안내가
##  흙길뿐**이다(설계 §6 S12). 길 위에 나무가 서면 「나가는 길이 막혔다」로 읽히는데 **에러는 0**이다.
