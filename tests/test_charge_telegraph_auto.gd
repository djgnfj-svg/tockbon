extends SceneTree
## 돌진 예고 범위 자동 검증 (세99 — 늑대 「빨간 예고 + 닿을 범위」) — 헤드리스 실행:
##   ./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_charge_telegraph_auto.gd
## 전 항목 통과 시 "TEST_CHARGE_TELEGRAPH_OK" 출력 후 종료 코드 0.
##
...
##   좌표를 뽑아 실제로 맞아 본다**. 그림과 판정이 갈리는 순간 양쪽에서 빨개진다.
##
## 🔴 **공개 API로만** 검증한다 (takbon-verify §3 · `hp()`·`hitbox_radius()` 선례):
##   `charge_band_visible()` · `charge_band_reach()` · `global_position` · `GameState.hp`.
##   내부 필드(`_charge_state`·`_charge_band`)는 리팩터 때 옮겨 다녀 계약이 아니다 — 안 더듬는다.
##
...
##
## 왜 이 줄이 있나: 예고는 적의 자식이라 그냥 두면 **루트 scale을 그대로 탄다.** 그런데 돌진 거리도
## `attack_range`도 **월드 px라 덩치와 무관**하다 ⇒ `size 1.35`인 몸에서 **범위만 1.35배**로 그려진다
## (에러 0 · 화면만 거짓말). 설계 §5-D-2는 `top_level`로 막으라고 했는데 그건 **그리기 순서까지
## 바꿔서 예고가 몸 위로 올라온다**(세99 `vfx_shot -- charge` 실측) — 그래서 역스케일을 직접 곱한다.
## 🔴 **실데이터로 잰다** — `hound_alpha`가 지금 라이브에서 `size 1.35`를 쥔 몸이다.
...
## [6] 🔴🔴🔴 **그린 것 = 맞는 것** — 이 그물의 심장.
##
## 수치를 대조하는 대신 **그려진 범위에서 좌표를 뽑아 실제로 맞아 본다.** 세 자리를 잰다:
##   ⓐ 그린 폭 **안** → 맞는다      (예고가 실제보다 **크게** 그려지면 여기서 빨개진다)
##   ⓑ 그린 폭 **밖** → 안 맞는다   (예고가 실제보다 **작게** 그려지면 여기서 빨개진다)
##   ⓒ 그린 앞끝 **밖** → 안 맞는다 (길이 쪽 같은 판정)
## 🔴 방향 락은 **윈드업 시작**에 걸리므로, 스텁을 락 뒤에 옮기면 「예고를 보고 걸어 피한다」와
##  같은 상황이 된다 — 이 게임에서 회피 수단이 그것 하나뿐이다(설계 결정 ⑥ 구르기 off).
## ⚠ ⓒ의 여유(+24px)는 **한 물리 프레임분 초과 주행**을 덮는다: 돌진은 `ceil(dash_sec/delta)`
##  프레임을 달려 그린 길이보다 최대 한 틱(여기선 ≈4px) 더 간다. 여유를 그보다 좁히면 그물이
##  프레임률에 따라 흔들린다(값이 아니라 이유로 정한 여유다).
...
##
## ⓐ **바닥에 깔린다는 건 z가 아니라 트리 순서다** — Ground·적·플레이어가 전부 z 0이라
##   「Ground 위 · 플레이어 아래」라는 z 값이 **존재하지 않는다**(설계 §5-D-2가 캔 사실).
##   그래서 예고는 씬에 붙어 **플레이어보다 앞 인덱스**로 들어간다. 헤드리스가 픽셀은 못 봐도
##   **이 순서는 잰다** — 「바닥에」의 유일한 측정 가능한 형태다.
##   ⚠ 이게 없으면 실무대에서 조용히 깨진다: 적은 런타임 `add_child`라 **씬 자식인 `Player`보다
##    나중에** 그려진다 → 적에 붙인 연출은 **플레이어 위**에 얹힌다(붉은 후드가 붉은 띠에 묻혔다).
##
## ⓑ 씬 소유로 옮긴 **대가**가 수명이다 — 적 자식이었으면 공짜였다. 안 지우면 **주인 없는 붉은 띠**가
##   바닥에 영영 남는다(가리는 것보다 나쁘다). 윈드업 **한가운데서** 몸을 없애 그 자리를 짚는다.
