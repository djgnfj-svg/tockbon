...
## (Ground/모달 mouse_filter 함정) — 그건 에디터 실게임 push_input으로 따로 검증한다.
...
func _run() -> void:
	# 🔴 워치독 — `_run`이 중간에 죽으면(런타임 에러로 함수 중단) `quit()`에 못 닿아
	# 프로세스가 **영구 hang**한다.
	...
	# 🔴 계약은 "펜"이 아니라 "장비(equip 카테고리)"다 — 공방은 지팡이·로브·부적·모자도 만든다.
	# 🔴🔴 공방은 **조립 부품(고리·룬·진)도 만든다.** 그건 아이템이 아니라 **codex 해금**이라
	# `output_id`가 비어 있고 `get_item`이 null이다 — **아이템 레시피에만** 장비 계약을 적용한다.
	...
	# 🔴 명시 숫자 — 스캔만 하면 「레시피가 통째로 사라져도 0/0 통과」가 된다.
	_check("공방에 해금 레시피 8장이 있다 (실제 %d)" % unlock_recipes, unlock_recipes == 8)

	# 🔴🔴 목록 순서가 **결정적**인가: `Db.all_recipes`가 `keys.sort()`를 쓰면 키가
	# `StringName`이라 **사전순이 아니라 인터닝 순**이고 **실행마다 순서가 바뀐다**(실측 버그).
	# 뮤테이션: `sort_custom` → `sort`로 되돌리면 여기가 빨개진다.
	...
	# ── ⑤ 부위 효과 — 파생 getter가 실제로 바뀐다 (모자·부적·로브·지팡이) ──
	# 🔴 이 넷은 착용만 되고 효과가 죽어 있던 적이 있다 — 여기가 "껴도 아무 일 없다"의 회귀
	# 가드다.
	...
	# 🔴 옛 검사(`wand_pattern() == MULTI`)는 게터가 옳은 값을 줘도 **발사가 그 값을 못 받는**
	# 상태를 못 잡았다 — 실제로 두 지팡이는 게임에서 `wand_basic`과 성능이 완전히 같았다.
	# 여기선 두 축이 **맨손과 다른가**만 재고(장착 라운드트립), 「발사에 도달하나」는
	# `test_ring_spell_auto[12]`가 실제 발사로 잰다.
	...
	# 🔴 **세 지팡이가 서로 다르다** — 재료를 태운 제작이 순수 손실이 아니라는 계약.
	...
	# ── ⑤ 🔴🔴 해금 레시피 — 재료를 태워 **아이템이 아니라 codex**를 받는다 ──
	# 🔴 **라이브 패널 경로로 잰다**(`_recipe_title`·`_inputs_text`·`_craft`). 순수 함수만 직접 부르면
	#   「패널이 실제로 그 함수를 쓰는지」는 무측정이다 — 검사가 정렬 함수를 직접 불러서 패널이
	#   정렬을 안 해도 그린이던 전례가 있다.
	...
	await process_frame
	# 🔴 행 제목이 `RecipeDef.display_name`에서 나온다. 해금 레시피는 output_id가 비어서,
	# 폴백에 걸리면 제목이 정확히 **" ×0"**이 된다.
	...
	# 조일 때 거짓 빨강이고 ⓑ **앞선 실행이 남긴 재료가 누적돼** "6/5"가 된다(테스트 세이브가
	# 실행 사이에 살아 있어 `add_item`이 쌓인다).
