extends SceneTree
## 마나 폭발(죽음 연출) 자동 검증 — 헤드리스 실행:
##   ./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_mana_burst_auto.gd
## 전 항목 통과 시 "TEST_MANA_BURST_OK" 출력 후 종료 코드 0.
##
## 🔴 **헤드리스가 못 재는 것과 재는 것을 갈라 놨다.** 못 잰다 = 「푸른가 · 터졌다로 읽히나 ·
##  밀도가 맞나」 → `tools/vfx_shot.gd -- death`와 **F5**의 몫이다.
## 🔴 **[4]·[8]은 「일부러 안 하는 것」을 지키는 그물이다** — 파편이 플레이어에게 안 가고, 처치가
##  마나를 안 준다. 일부러 안 하는 것은 그물이 없으면 조용히 뒤집히고, 뒤집혀도 **에러가 0**이다.
## 🔴 [5]도 같이 있어야 한다 — [4]만 재면 **파편이 제자리에 굳어도 초록**이다(안 가면 통과니까).
## 🔴 **공개 API로만** 검증한다(`shard_count()`·`burst_radius()`·`shard_positions()`·`progress()`) —
##  내부 필드는 리팩터 때 옮겨 다녀 계약이 아니다.
## ⚠ **시간은 물리 프레임으로 센다** — `_process`였다면 `-s`의 delta가 제멋대로라 「얼마나 흩어졌나」를 못 잰다.
...
##  50을 올리면 반응 링·감전이 폭발 **아래**로 들어가 「박혔다 → 퍼졌다」 순서가 뒤집힌다.
...
##  그물이 거짓 빨강이 되고, 무엇보다 「파생」이 아니라 「그 숫자」를 재게 된다.
...
## [4] 🔴🔴 **파편이 플레이어에게 「안」 간다** — 「마나로 돌아간다」의 상대는 플레이어가 아니라 **세계**다.
...
##  처음엔 죽은 자리로부터의 **평균 거리**로 쟀는데 `SHARD_DRIFT_FRAC = 0`을 넣어도 **초록이었다** —
##  떠오르기만 해도 거리는 늘기 때문이다. 즉 ⓑ를 두 번 재고 ⓐ를 **한 번도 안 재고 있었다.**
...
## [7] 🔴 **광역 마법으로 여럿을 같은 프레임에 죽였을 때** — 늑대 무리가 실제 무대에 선다.
...
## [8] 🔴🔴 **적을 죽여도 마나가 안 는다.**
## 🔴 **「지금 코드가 안 준다」를 확인하는 검사가 아니다** — 나중에 누가 「연출이 있으니 수치도 주자」로
##  되돌리는 걸 막는 그물이다. 일부러 안 하는 것은 뒤집혀도 **에러가 0**이라 아무도 못 본다.
