extends SceneTree
## UI 문구·표시 계약 자동 검증 — 헤드리스 실행:
##   ./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_ui_text_auto.gd
## 🔴 왜 필요한가: 이 프로젝트의 반복 실패가 표시부다 — 축이 1→N으로 늘 때마다 표시부가 뒤처지고,
##  헤드리스는 렌더를 못 봐 F5까지 안 드러난다. 그래서 렌더가 아니라 순수 함수·레이아웃 계약만 잰다.
## 🔴 헤드리스가 못 잡는 것: 룬 점 2개가 실제로 보이나·겹치지 않나 · 색 띠 두 토막이 읽히나 ·
##  「… 외 N개」 줄이 잘리지 않고 뜨나 · 안내문 페이드가 자연스러운가. _draw는 렌더가 없으면 안 불린다 —
##  여기서 재는 건 좌표·문자열·상태뿐이고 실게임 확인이 대체되지 않는다.
...
## [3] 🔴 심장 — 표시부가 design.rune(첫 룬)만 읽지 않는다. 우회하면 발사는 두 룬을 쏘고 화면은
##  하나만 보여 주는 「쏘는 것 ≠ 보이는 것」이 된다. 옛 도안(룬 1개)이 색 하나로 그대로 도는지도 같이 잰다.
...
## 같은 함수를 부른다. 각도를 베끼면 규약이 바뀔 때 넷이 조용히 어긋난다.
...
## [5] 🔴 장비 효과 문구 = src/core/item_text.gd 하나. 사본이 포맷 문자열까지 같아서, 값을 하나
##  더하면 고친 곳만 맞고 나머지는 빈 문자열이 됐다.
## 🔴 사본 재발 감지 스캔 — 두 패널에 옛 함수 정의가 다시 생기면 여기서 빨개진다
##  (문구는 갈라져도 에러가 안 나는 종류라 필요하다).
func _test_item_text_single_source() -> void:
	...
	# ⚠ 배수 값을 박지 않는다 — .tres를 조여도 거짓 빨강이 되지 않게 「어떤 축을 적는가」만 잰다.
	#  지팡이 3종 전부 최소 한 축은 적어야 한다(전부 1.0이면 빈 줄 = 그 자체가 버그).
	...
	# 🔴 pattern_label·wand_pattern_text는 은퇴했다 — 소비자 0 함수를 남기면 「있는 기능」 행세를 한다.
	#  되살아나면 지팡이가 다시 발사 형태를 말한다는 뜻이라 여기서 잡는다.
...
## [5b] 🔴 codex 해금물 이름·안내 단일 소스(CodexText). 조회를 한 카탈로그로만 하면 종류가 늘 때
##  ① 폴백이 걸려 원시 id가 화면에 노출되고 ② 안내가 「밴드에 끼워라」 고정이라 거짓 지시가 된다.
##  발신처가 셋(보스 클리어·공방 제작·두루마리 픽업)이라 각자 조회를 복제하면 사본이 셋이 된다.
## 🔴 종류마다 안내가 실제로 갈리는지가 이 그물의 핵심이다 — 셋이 같은 문구면 리졸버가 있으나 마나다.
func _test_codex_text_resolver() -> void:
	...
	# 🔴 챕터 보상 3종이 전부 리졸버에 걸리나 — 하나라도 새면 그 챕터 클리어 문구가 거짓이 된다.
...
## [5c] 🔴 재료 진행 표시 「보유/필요」 단일 소스 (ItemText.count_text).
## 공방·정제대가 글자까지 같은 사본을 들고 있었고 둘 다 인자가 거꾸로였다(5개 필요한데 3개면 「5/3」).
##  한쪽만 고치면 다른 쪽이 계속 거꾸로 남아서 core로 뽑았다.
## 🔴 스캔형이라 네 번째 사본이 생기면 그때 빨개진다.
...
## [6] 🔴 퀘스트 탭 행 캡 — 퀘스트 여섯이 대화 한 번에 연쇄 완료되면 6행이 되고, 캡이 없으면
## 6번째 행이 패널 아랫변을 뚫고 진행 바가 화면(540) 밖으로 나간다(스크롤 없음).
...
## [7] 🔴 소지품 격자 행 캡 — 이 함수가 클릭 rect의 단일 소스라, 안 그린 카드를
...
## [8] 🔴 안내문 수명 — 옛 say()는 대입만 하고 타이머·비우기 호출자가 하나도 없어
...
## [9] 🔴 제작대 버튼 이름은 씬이 아니라 모드가 판다. 씬에 박힌 문구는 코드가 채우는 버튼과 다른
##  물건이라 아무도 못 알아챈다 — 있지도 않은 조작을 적는 건 그 자체가 버그다.
## 🔴 두 겹으로 잰다: ⓐ 라이브 버튼이 지금 모드에 맞는 이름을 달고 있나 ⓑ 씬에 문구 사본이 다시
##  생기지 않았나(스캔 — [5]의 사본 재발 감지와 같은 수법. 문구는 갈라져도 에러가 안 난다).
...
## [10] 🔴 리포트 「조립:」 줄이 카드 폭 안에 든다 — 넘치면 뒷부분이 통째로 안 보인다.
## 층·문양이 늘수록 길어지는 줄이라 조이면 또 넘친다.
...
## [11] 🔴 책 진 셀 설명이 JinDef에서 파생된다(band_count·rune_slots). 은퇴한 축을 말하던 옛 문구가
## 그대로 남으면 화면이 거짓말을 한다.
## ⚠ 아래 스캔 문자열을 이 주석에 그대로 적지 마라 — 이 파일이 자기 스캔에 걸린다(실제로 밟았다).
## 새 문장을 또 베껴 적으면 다음 축 변경 때 같은 일이 난다 → 숫자를 데이터에서 뽑는지를 잰다.
