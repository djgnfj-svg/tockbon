extends SceneTree
## 발밑 바닥 마법진 + 착탄 등급 반영 자동 검증 — 헤드리스 실행:
...
## **「조립한 것이 실제로 그림에 들어갔나」**다. 손맛·색감은 실게임이 판다.
...
func _test_cancel_and_release() -> void:
	...
	# 🔴 겹치기: 취소·발사 없이 새 시전이 들어와도 원이 쌓이면 안 된다
...
## 🔴🔴 다 차도 **발사·취소가 올 때까지** 원은 열린 채 머문다. 그래서 `duration`을 수명으로
## 재면 **오래 들고 있을 때 원이 에러 없이 사라진다**.
...
## 🔴🔴 즉발이면 발밑 원을 **안 연다** (`vfx._on_ring_cast_started`의 `duration <= 0` 가드).
