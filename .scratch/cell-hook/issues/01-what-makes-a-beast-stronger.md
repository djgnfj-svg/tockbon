...
## ⚠⚠ 2026-08-23 저녁 — 버린 셋 중 둘이 돌아왔다. 위 답은 지우지 않는다

**사용자가 짚었다**: ***"그렇게 하면 티켓 일도 바뀌는 거 아니야? 티켓 일에는 또 다르게 명시돼 있을 거
아니야?"*** **맞다.**

### 살아 있는 것 — **답의 바닥은 그대로다**

**「종이다」는 그대로 산다.** 어떤 짐승으로 무리를 채우느냐가 여전히 빌드의 바닥이고,
**화면에 그대로 보인다**는 것과 **그림 값이 감당된다**는 것도 그대로다.

### 뒤집힌 것 — **버린 근거가 사라진 것이지, 그때 틀렸던 것이 아니다**

| 버렸던 것 | 그때의 근거 | 지금 |
|---|---|---|
| **장비 — 부위에 붙인다** | 조합마다 그림이 필요하다 | ⚠⚠ **보통 장비는 외형을 안 바꾸므로 그림이 0장이다.** 레전더리만 몇 장 든다 (2026-08-23, 사용자) |
| **무리 전체 성질** | 정비 화면에 놓을 것이 없다 | ⚠⚠ **칸 줄과 짐승 도감이 생겼으므로 놓을 곳이 있다** (티켓 02) |

⇒ **답이 뒤집힌 게 아니라 넓어졌다: 「종」이 바닥이고 그 위에 「장비」가 얹힌다.**
**장비는 짐승 종류마다 붙는다** — 늑대에게 투구를 끼우면 모든 늑대가 낀다.

### ⚠ 위 답에서 고쳐야 할 두 줄

1. **「카드가 짐승 종류를 준다」** → **캐릭터와 장비 둘 다 준다. 둘 다 등급이 있다** (2026-08-23)
2. **「칸이 지금도 종을 담게 되어 있다」** → 담는 쪽은 맞는데 **찾는 열쇠가 슬롯이라 종으로 바꿔야 한다**

### ⚠ 2026-08-23 밤 — 「부위」라는 말이 이 티켓 안에서 죽었다

**티켓 02가 장비 칸에서 이름을 뗐다.** 위 글 곳곳의 **「부위 여섯」과 「머리·가슴·배·팔·손·다리」는
이제 옛말이다.** 지금은 **이름 없는 빈 칸 여러 개**이고, 거기에 **아무 장비나 꽂는다.**

⇒ **이 티켓의 답은 그대로 산다** — 「종이 바닥이고 그 위에 장비가 얹힌다」는 안 바뀌었다.
**바뀐 것은 장비가 앉는 자리의 생김새뿐이다.**

---

## ⚠⚠ 덧씀 (2026-08-24) — **장비가 성질도 싣고, 진화가 셋째 축으로 붙었다**

**이 티켓은 「종이 바닥이고 그 위에 장비가 얹힌다」로 닫혔다. 티켓 06 이 두 군데를 넓혔다.**

1. **장비는 숫자만 올리는 것이 아니라 성질도 싣는다.** 짐승이 패시브를 타고나고 **장비가 그것을
   키운다** — 출혈 장비를 늑대에게 몰아주면 출혈 늑대 무리가 된다. 사용자: *"짐승이 기본적으로
   가지고 있는 패시브가 있고, 패시브를 살리는게 있고"*
2. ⚠ **셋째 축이 붙었다 — 진화.** 장비와 겹치지 않게 **되돌릴 수 없는 갈림길**로 뒀다:
   장비는 뺐다 꽂았다 하는 것, **진화는 한 번 정하면 끝인 것.** **겉모습이 바뀐다.**
   엘리트를 잡으면 뜬다

⚠ **「레전더리만 외형이 바뀐다」는 진화에는 안 걸린다** — 진화는 등급과 무관하게 겉이 바뀐다.
**형태 하나에 그림 두 장(좌·우)이라 값이 싸다는 것을 코드에서 확인했다.**
