...
**2026-08-26 에 방향이 뒤집혀 열렸다.** ⚠⚠ **앞 지도들은 같은 날 사용자가 지우라고 했다 — 그런데 삭제가 권한에서 거절되어 파일은 아직
남아 있다.** 각 파일 맨 위에 죽었다는 표시를 붙여 뒀고, **지우는 명령은 사용자가 한 번 직접 실행해야
넘어간다.** 거기서 정해진 것 중 살아남은 것은 이 지도와 티켓 셋으로 옮겨 왔고, **여기가 유일한
...
| 무엇 | 어떻게 |
|---|---|
| **한 층** | 고원과 계단을 뺐다 — ***"정말 단순해도 돼. 층이 없어도 돼"***. 층 글자는 살아 있어서 글자 하나로 돌아온다 |
...
| **2층에 농사와 건물이 올라간다** | ⚠ **넓이가 문제가 된다.** 지금 고원은 4x4 칸이고 본채가 그중 넷을 먹어 열둘이 남는다 |
...
⚠ **그물은 빨간 채로 멈춰 있다** — 첫 섬이 26x20 에서 16x12 로 줄었는데 그 섬을 재던 검사가
옛 숫자를 들고 있다. ⚠ **「커밋도 안 됐다」는 거짓이었다** — 그날의 작업은 커밋됐다.
...
**스무 장 중 네 장만 초록이고, 통과 1159 · 빨강 열여섯 장이다.** 도는 데 약 2 초 걸린다.
...
⚠ **남은 빨강은 「16x12 섬 하나를 재는 숫자」와 「늑대가 플레이어이던 시절의 준비 코드」 둘이다.**
...
**2026-08-26 에 열렸다.** ⚠⚠ **앞 지도들은 같은 날 사용자가 지우라고 했고, 삭제는 아직 막혀 있다** — 거기서 정해진 것 중 살아남은
...
1. ⚠⚠ **해안 띠를 어떻게 그리나** — **다음 세션의 첫 일.** 블렌더의 반투명 바다에서 쓰던 「물 아래
   밝은 띠」가 게임의 불투명한 바다에서는 안 보이고, 물 위로 올리면 흰 판때기가 된다
2. **성장 카드를 언제 주나, 선택의 폭을 어떻게 주나** — ⏸ **사용자가 직접 더 생각하겠다고 했다**
3. **건설의 무엇을 · 어디에 · 무슨 값으로**
4. **웨이브가 언제 몇 척 오나**, 그리고 **보스 시계가 몇 초인가**
5. **사용자가 섬을 직접 만드는 자리가 어디인가** — 지금은 블렌더 스크립트 안의 글자표이고,
