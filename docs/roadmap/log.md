...
## ✅ **절벽 위를 걷던 몸 — 2026-09-03 밤, 03-21**

**사용자가 게임을 켜고 찾았다.** 이 세션은 그 한 문장에서 시작해 세 라운드를 돌았고, 코드는 한 파일과
그물 한 장이다.

> ***"Sometimes when they go, there is a bug where they cross onto the second storey. They climb on top
> and just go — they just walk across the cliff."***
> (「가끔씩 갈때 2층으로 넘어가는 버그 있음 넘어가서 그대로 뚫고감」 → 「뚫고가는게 아니라 위로 올라타고
> 감 그냥 절벽을 지나감 ㅇㅇ」)

⚠ **첫 마디는 「뚫고 간다」였고 사용자가 스스로 고쳐 「올라타고 간다」로 바꿨다.** 두 낱말이 정반대의
진단을 가리킨다 — 앞엣것은 그림이 바위에 묻히는 것이고 뒤엣것은 한 층 뜨는 것이다. **뒤엣것이 참이었다.**
...
**실측: `src/` 전체에 `CharacterBody3D` · `StaticBody3D` · `CollisionShape` · `NavigationRegion` ·
`NavigationAgent` · `RayCast` 가 0 건이다.** 섬은 메시 하나고 몸은 매 프레임 자리를 받아 적는
`Sprite3D` 다. **엔진 탓이 아니라 이 프로젝트가 엔진의 3D 이동을 하나도 안 쓴 것이고**, 격자는 죽은 세포
게임에서 왔으며 **8/24 의 3D 이사가 옮긴 것은 화면뿐이었다.** 다시 정한 적이 없다.

**바깥을 찾아봤다** (`docs/reference/2026-09-03-grid-sim-under-a-3d-mesh.md`) — **이 구조가 이 장르의
표준이다.** 배드노스 개발사 인터뷰(「가능성의 공간이 이산적이어야 배치의 결과를 예측하기 쉽다」) · XCOM 2
(경로·엄폐·시야가 전부 격자에서) · Divinity OS2 의 AI 그리드(높이를 미리 굽고 「이것이 절벽 벽면으로 달려
나가는 것을 막는다」) · Trudy's Mechanicals(칸마다 정수 높이). **반대로 간 곳은 Stonehearth 하나고 이유가
정확성이 아니라 속도였으며 몇 달 걸렸다.** ⚠ 배드노스의 내비메시 줄과 Stonehearth 인용은 페이지를 못 열고
검색 색인만 봤다.

### 2. 원인 — **밀린 점으로 지형에 높이를 물었다**

**걷는 몸은 자리 밀림(최대 한 조각)을 들고 그려지고, 발 높이를 그 밀린 점 밑에서 읽었다.** `surface_h` 가
조각으로 반올림하므로 **한 조각 밀린 점은 옆 조각을 짚는다.**

| | |
|---|---|
| 손 섬에서 뜨던 조각 | **46**, 뜨는 양은 **정확히 한 층** |
| 연속으로 걸리던 줄 | **(8,3) ~ (12,3)** — 사용자가 본 그 줄 |
| 생성된 섬 100 개 | **전부**, 섬마다 평균 **33.2 조각** |
| 눈금이 섞인 칸 | **0** (손 섬 195 칸 · 시드 100 개) |

⇒ **섬도 생성기도 무죄고, 쉬는 몸은 안 뜬다** — 자리는 제 칸 안에 있고 칸은 한 층이다.

### 3. 세 라운드 — **각 라운드가 앞 라운드의 무엇을 무너뜨렸나**

**1 라운드.** 원래 계획은 「발 높이를 시뮬레이션 점에서 읽는다」였고 `adversary` 가 넷으로 무너뜨렸다 —
그림자도 발밑 지형 높이를 입고 · 이동선 첫 점도 같고 · 죽는 몸이 새로 튀고 · **46 조각 전부에서 몸이
바위에 파묻힌다(확률이 아니라 100 %)**. **사용자가 다른 모양을 골랐다** — 「그려지는 점이 절벽을 안 넘는다」.

**2 라운드.** 지었는데 **사용자가 화면에서 「아직도 넘어가네」**라고 했다. `verify` 가 세 구멍을 찾았고
그중 하나가 그것이었다 — **쉬는 몸이 자리로 미끄러지는 직선이 아예 안 막혀 있었다.** 손 섬 70880 중 224
건, 최악이 **조각 (8,3)** 에서 1.90 조각 미끄러짐의 23 %, 약 0.15 초 동안 한 층 위.

⚠⚠ **그리고 더 나쁜 것**: 고침을 통째로 지우고 전체를 돌리니 **3993 통과 · 3 실패, 한 글자도 안 달랐다.**
그물이 함수를 직접 부르고 **게임이 그것을 쓰는지는 아무것도 안 쟀다.** 배선을 모는 줄이 그때 섰다.

**3 라운드.** 첫 규칙(밀림 방향을 따라 걸어 나가기)이 **없던 결함을 만들고 있었다** — 계단 어귀에서
그려지는 점이 한 프레임에 **0.960 조각** 순간이동. `can_step` 이 허용하는 걸음만 남기고 재도 그대로였고,
섬마다 168 자리. **「그냥 받아들이자」가 제안됐고 이 숫자로 물렸다** — 고침 전에는 완벽하게 이어져 있었고,
**절벽에 뜨는 것을 계단에서 튀는 것으로 바꾸는 것은 고친 게 아니다.**

⇒ **길이 상한**: 밀림을 **눈금이 바뀌는 가장 가까운 조각까지의 거리**로 자른다. 방향을 안 보므로 선 양쪽에서
답이 같다. **튐 0.0403 조각(한도 0.08, 산술 천장 0.06 에서 유도)** — 스물넷 배. 값은 손 섬 + 시드 열에서
4340 중 880 조각의 자리가 당겨지고 **0 이 되는 것은 하나도 없다** — 가장 짧은 것이 0.4990 조각.
...
⚠⚠ **계단의 무엇이 아쉬운지 안 물었다.** 후보가 넷이고 (03-21 의 0.0403 · 계단 어귀의 당김 · 8/28 부터
있던 발판 튐 0.167 조각 — **이게 네 배 크다** · 무관한 것) **어느 것인지 모른다.** 03-22 가 `grilling` 으로
섰다.

### 5. 이 라운드가 저 자신에 대해 잰 것

- ⚠⚠ **`verify-look` 을 없앨까가 논의됐다** — 사용자: 「저 벌이 피 시간이 좀 길어서」. **기록을 찾아 답했다**:
  잡은 것 일곱 건 중 **몸 잡는 네모가 2.2 배였던 것은 헤드리스가 구조적으로 못 잡는다** — 누름이 전부 몸
  한가운데에 떨어져서다. **없애면 밀린 눈 여덟에 더 쌓인다.** 사용자가 두기로 했고, 이 세션에서는 그 뒤
  **사용자가 직접 보겠다고 해서 껐다** — 「내가 눈으로 볼게 빠르게하고 다음작업하려고」
- ⚠⚠ **`verify` 첫 판이 `git checkout -- src/view/field_view.gd` 로 커밋 안 된 작업을 잠깐 날렸다.**
  떠 둔 diff 로 복원했고 blob 해시와 지문이 앞뒤로 같다. **에이전트에게 그 명령을 금지하는 줄이 아직 없다** —
  둘째 판에는 프롬프트로 금지했지만 문서에는 없다
- **`verify` 둘째 판이 돌던 중에 마무리가 불렸다.** 대각선 움직임의 산술 천장 · `_ledge_reach` 의 조기
  종료 · 약해진 줄이 아직 빨개지나 — 셋 다 안 확인됐다

### 6. 넷

**세션 시작 3965 · 3 · 그물 25 · 22.5 초 → 끝 4014 · 3 · 그물 26 · 23.5 초.** 빨강 셋은 서 있던 그대로
(`net_boats` 1 · `net_fx_view` 2) 이고 이 세션은 그 파일을 하나도 안 건드렸다.
...
found on screen the same day. **The branch was merged as PR 19 on 2026-09-03** — this session ran as a background
job in a worktree, as the 03-02 session did.
...
✅ **Merged as PR 15 on 2026-09-02.** It ran on branch `worktree-drop-auto-muster`.
...
measurement rather than a plan: **there is no Godot on this machine.** The repository's engine is a
Windows executable that git does not carry, the official download hosts answer nothing through the
proxy, and GitHub is scoped to this repository alone. **So the nets could not run, the game could not
open, Blender was not there, and nothing could be looked at.** The user's answer settled the round:

> ***"You just write the code ahead of time and I will actually merge and test it in the morning."***
> (「네가 코드만 미리 만들어놓고 놓고 내가 아침에 이제 실제로 합쳐보면서 테스트할 거여 가지고」)

**Two things were asked for, in the user's own count:** ***"one, tidy the code with ultracode; two,
write whatever can be written."*** (「울트라 코드 한 번 정비하기를 하고 그다음에 좀 짤 수 있는 거 짜 놓기」)

### What was built — seven tickets, every one still open

**03-14 defect 2** — the spill is pinned to the storey the press aimed at. ⚠⚠ **An existing net asserted
the exact opposite** (「the surplus walks down the stair」) and had to be inverted with it: the rule it
encoded was the user's until 2026-09-02, when they chose the refusal instead.

**08-01** — `IslandGen.board(seed)` answers the board a run walks on. **The algorithm was written and run
in Python FIRST**, with a mirror of `Grid.can_step` inside it, because the stair rule cannot be reasoned
about: a stair's axis comes from a mouth chosen by lowest 조각 index then west/east/north/south, so **a
stair with ground on three sides can come out with an axis across its own climb — a door that opens and
leads nowhere, while the island stays walkable through some other stair and the count still says three.**
400 seeds, 0 failures, boards repeat. **The GDScript asks the real `Grid` rather than a mirror.**

**05-08 · 05-07 · 05-09 · 05-10 · 09-02** — the 창고, 허기, coast fishing, gathering from a resource 칸,
and the 바리케이트. **One loop closed**: something now puts food into the 창고, which until this round
nothing anywhere did — 허기 was a death sentence with no answer.

### What the user decided, mid-round

> ***"So the trees should be generated by 칸 unit too ... press it and they go to gather, and being
> there is gathering. Hover and the outline lights up, press and it gathers — a bit like Warcraft. Or:
> harvest all the trees in a particular block, that kind of feeling."***
> (「그러면 그 나무들도 블록 단위로 생성되게 해야 되는데 ... 딱 눌렀을 때 채집하러 갔을 때 잘 갈 거 아니야.
> 거기 가면 채집이다 ... 살짝 워크래프트처럼 아니면 특정 블록에 있는 나무를 다 캔다 뭐 이런 느낌처럼」)

**The press was built.** ⚠⚠ **The last clause collides with 2026-09-02's 「계속 나와야 될 거 같아」** — a 칸
that runs out and a 칸 that never runs out cannot both be true, and the code stands on the older answer.
**The hover outline was not built**: it is a picture, and nothing could look at it.

### What the tidying round actually found

**Thirteen agents surveyed every non-game zone read-only, and each candidate was handed to a skeptic told
to refute it. Seventy-two of seventy-five proposed deletions were refused**, most of them with the
repository's own words. **The 170 MB the survey wanted is out of scope by the user's own answer**,
recorded in ticket 02-07 on 2026-09-01: 「the 24 pictures only. The screenshot pile, the 시안 folder and
the 프로토타입 folder are not touched this day」. **One proposal had its line numbers wrong by 28 lines and
would have cut live code.**

**What survived was three things and they were all lies rather than weight**: a `_fx` array nothing had
written to since 2026-08-29 with a comment beside it saying it was 「still filling every frame」, a net
comment naming two things that do not exist, and four orphan `.uid` sidecars.

### What this round measured that lasts

- **`tools/blender/island_build.py` is gone and that is correct** — deleted 2026-08-31 once the `.blend`
  originals existed. **`src/` still cited it in fourteen places**, one of them an assert message telling
  the player to run it. All fourteen were repointed at `blend/island.blend`. ⚠⚠ **Four rules in `src/`
  that used to pair with lines in a Python bake now pair with the way a part is CUT** — the stair's
  mouth order, its six treads, the height of one 눈금, and the 2.0-wide piece `Grid.block_of` rests on.
