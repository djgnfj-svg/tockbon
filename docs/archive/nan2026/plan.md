...
![개발 중인 조립창. 왼쪽이 조립한 마법진, 오른쪽이 팔레트다. **가진 것만 뜬다** — 이 판에서 얻은 문양이 둘이라 둘만 있다](img/circle-window.png)
...
![왼쪽에서는 불이 나무를 타고 번지는 중이고, 오른쪽은 마법이 판 구덩이와 그리로 흘러 들어가는 물이다. 실제 게임 화면이다](img/world.png)
...
확산 다음 폭발이면, 맞은 자리에서 여덟 갈래로 흩어지고 그 여덟이 **각자 닿는 곳에서** 터진다.
터지는 자리가 여덟 군데로 갈라진다.

![확산 다음 폭발. 맞은 자리에서 여덟 갈래가 흩어지는 순간이다. 실제 게임 화면](img/order-spread-blast.png)

폭발 다음 확산이면, 맞은 자리에서 **먼저 크게 터지고** 거기서 여덟 갈래가 흩어진다.
한 군데가 크게 터지고 불씨가 퍼진다.

![폭발 다음 확산. 크게 터진 그 자리에서 갈래가 흩어지는 순간이다. 실제 게임 화면](img/order-blast-spread.png)

두 그림의 왼쪽을 견주면 층이 뒤집혀 있다 — 안쪽 층이 먼저 읽히는 문양이다.
...
217×48 타일 한 장이 끊김 없이 이어진다. 로딩도 문도 없다.
...
물속에서는 점프가 무제한이라 물은 장애물이자 이동 수단이다. 물 룬으로 직접 부어서 만들 수도 있다.
...
설치가 필요 없다. 위의 플레이 링크를 열면 약 45MB를 내려받고 바로 시작된다.
...
- **문양은 17개를 설계했고 4개가 들어갔다.** 확산, 폭발, 응축, 그리고 위력만 곱하는 「더미」.
  더미는 이름이 없어서 더미다 — 변형 자리에 무엇이 들어갈지 정해지면 그때 이름이 붙는다
