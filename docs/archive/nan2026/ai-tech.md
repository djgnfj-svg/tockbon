...
**코드 구조보다 하네스에 집중해서 개발했다.** 중간중간 바뀌었지만 위 그림이 최종본이다.
...
정지 프레임과 UI는 **ComfyUI**로 뽑는다. **개인 PC의 GPU에서 도니 비용이 0이고,
그래서 한 번에 여러 장을 뽑아 그중 하나를 고른다.**
애니메이션은 **PixelLab**을 쓴다 — 로컬 워크 LoRA가 사람용이라 네발짐승 걷기는 로컬 경로가 없다.

**말로는 안 갈린다.** 후보를 한 장에 붙여 놓고 사람이 고르는 단계가 이 파이프라인의 중심이다.
프리셋 하나가 문체·LoRA 강도·크기를 함께 들고 있어 스타일이 갈라지지 않는다.
**참고 이미지는 넣지 않았다** — 넣으면 그 그림 자체를 거의 복제한다.

### 사운드는 파일이 아니라 코드다

**저장소에 오디오 파일이 하나도 없다.** 효과음은 부팅 때 코드가 PCM을 직접 합성한다 —
발사·폭발·점프·착지·피격 전부.

**검증도 따라 바뀐다.** "무슨 파일을 재생했나"를 물을 수 없는 대신 **"울렸는가"를 센다.**
재생 호출 수를 헤드리스 넷이 읽는다.
