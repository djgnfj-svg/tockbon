...
| **AI 활용** | 별도 제출물 「AI 활용 기술 문서」가 전담한다 |

탁본은 새겨진 것을 종이에 떠내는 일이다. 이 게임은 마법진을 새기고, 그것이 세계에 찍히는 것을 보는 게임이다.

## 개요

장르는 2D 횡스크롤 로그라이크다. **1인이 9일 만에 만들었다** — 첫 커밋 2026-08-02, 제출 2026-08-10.
...
![확산 다음 폭발. 실제 게임 화면이다](img/order-spread-blast.png)

폭발 다음 확산이면, 맞은 자리에서 먼저 크게 터지고 거기서 여덟 갈래가 흩어진다.
한 군데가 깊게 파인다.

![폭발 다음 확산. 실제 게임 화면이다](img/order-blast-spread.png)
...
```
싸운다 → 레벨업 → 3택 → 마법진 빌드 → 싸운다 → 보스 → 3택 → 마법진 빌드 → 클리어
```

한 판 안에서 마법진이 자라는 축이다.

3택에 뜨는 것은 **현재는 문양이다.** 진과 장비도 같은 문으로 들어올 자리다.
문양은 확산, 폭발, 응축 세 종류에 일반·희귀·유일 세 등급이 있어서,
이미 가진 문양의 상위 등급도 후보로 뜬다.

주운 것을 넣어둘 곳은 없다. 받는 순간 어느 층에 올릴지 정하고, 층이 찼으면 무엇을 밀어낼지 정한다.
셋 다 마음에 안 들면 안 받아도 된다.

세계는 조립하는 동안 멈추지 않는다. 그래서 조립은 안전한 자리를 찾아서 하는 일이 된다.
...
| 마우스 좌클릭 | 발사. 반동으로 옆으로 밀린다 — 이동 수단이자 함정이다 |
| Tab | 조립 창. 마법진의 층을 바꾼다. 세계는 멈추지 않는다 |
| P | 레벨업 3택 창. 레벨업 표시가 떴을 때 |
| F | 마을에서 설비를 쓴다 — 연구대, 조립대, 출발문 |
| R | 판을 처음부터. 스스로 판 구덩이에 갇혔을 때의 탈출구다 |
| ESC | 열린 창을 닫는다 |
...
**1인 개발, 9일이다.** 그 안에 넣을 수 있는 것을 넣었고, 아이디어는 있었으나 시간이 모자라
못 넣은 것을 여기 적는다. 숨기고 빌드를 열게 하는 것보다 낫다.

- **소리가 하나도 없다.** 무음으로 돌아간다
- **스테이지는 1개다.** 스테이지 2는 물을 이동 수단으로 쓸 자리이고, 그 문법은 스테이지 1의 물속 무한 점프에 이미 들어가 있다
- **번개 룬이 없다.** 물에 젖는 것은 구현돼 있고, 젖은 것을 타고 흐르는 번개가 그 짝으로 들어갈 자리다
