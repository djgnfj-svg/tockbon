...
**검사와 짐승이 0.9 초마다 서로 때린다.** 한 번 봐서는 안 갈리고 **연달아 볼 때 갈리기** 때문에
자동으로 친다. **왼쪽 한 쌍은 스위치를 안 받는 대조군**이다.

⚠⚠ **`B` 를 누르면 여덟이 한꺼번에 때린다.** 19 번(효과 누적 막기)이 무엇을 막는지는 그때만 보인다 —
안 켜면 화면이 발작한다.

**스물여섯 개 중 열다섯이 붙어 있다.** 소리(8 · 20 · 21 번)와 진동은 안 붙였다.

**↑↓ 로 고르고 스페이스로 켠다. A 전부 켜기, Z 전부 끄기.**
⚠⚠ **스위치 15 개면 볼 수 있는 화면은 32768 가지다.** 그 화면들은 만드는 게 아니라 켜는 것이고,
코드는 15 덩어리다.
