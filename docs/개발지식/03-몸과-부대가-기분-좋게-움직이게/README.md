...
**아홉이 한 부대다. 왼쪽 클릭으로 갈 자리를 찍는다.** `T` 를 누르면 네 자리를 자동으로 돈다.

⚠⚠ **아무것도 안 켜면 아홉이 한 조각에 겹쳐 서고 일정한 속도로 미끄러진다.** 그게 지금 게임의
모습이고, 11 번(도착 자리)을 켜는 순간 부대로 보이기 시작한다.

**`S` 로 한 명을 세워 두고 그 위로 부대를 보내 보라** — 20 번(서 있는 몸은 안 밀린다)이 없으면
세워 둔 몸이 밀려난다.

**스물여섯 개 중 열셋이 붙어 있다.**

**↑↓ 로 고르고 스페이스로 켠다. A 전부 켜기, Z 전부 끄기.**
⚠⚠ **스위치 13 개면 볼 수 있는 화면은 8192 가지다.** 그 화면들은 만드는 게 아니라 켜는 것이고,
코드는 13 덩어리다.
