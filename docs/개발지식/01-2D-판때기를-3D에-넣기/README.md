...
| # | 기법 | 안 하면 | 어떻게 하나 | 지금 이 게임 | 크기 |
|---|---|---|---|---|---|
| **12** | **기준점을 발에** | 그림 한가운데가 땅에 박혀 몸이 반쯤 묻힌다 | **가운데 정렬은 그대로 두고 위로 반 높이만 올린다.** ⚠ 아래 실측 | ✅ 됨 | 작음 |
...
거기서 높이만큼 또 내리니 **몸이 땅 아래로 통째로 사라진다.**
**어떻게 쟀나**: 스위치 열여섯을 하나씩만 켜서 열아홉 장을 찍었더니, **12 번만 켠 장에 몸이 한 명도
없었다.** 그림자 원판은 제자리에 있었으므로 세계가 아니라 몸만 사라진 것이다.

⚠ **맞는 코드는 가운데 정렬을 그대로 두고 위로 반 높이만 올리는 것이다** —
`centered = true` · `offset = (0, 높이 / 2)`. 그러면 그림의 아래끝이 원점에 앉는다.

⚠⚠ **게임 코드는 안 틀렸었다.** `field_view` 는 발 높이를 따로 계산하고 그 계산은 그물이 지키고
있었다. **틀린 것은 이 실험대뿐이었고**, 그래서 이 폴더의 규칙에 「실험대가 이번 라운드에 화면에
떴어야 한다」가 있는 것이다 — **12 번 스위치는 켜진 적이 없었다.**
