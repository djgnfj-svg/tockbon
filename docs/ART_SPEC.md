...
## 🔴 입체화 재조명 후처리 (2026-07-23 세69) — 스프라이트 PNG는 relight 필터를 거친다

사용자: *"그림들이 호떡 눌린 것처럼 납작하다, 입체적으로."* → 실루엣 기반 **재조명 필터**를
전 캐릭터·적·프롭·재료·아이템·건물 PNG에 적용(45장). 왼쪽 위 광원으로 형태 음영(밝음/그늘 밴드)
+ 색 외곽선(림)을 얹어 둥근 부피를 만든다. 강도=풀(1.0), 건물만 약(0.5). 필터·목록·제외 규칙 정본 =
`tools/relight_sprites.lua`.

🔴🔴 **이건 익스포트 이후 후처리다** — aseprite 원본은 "납작한 밑그림"으로 남아 있고 필터가 PNG를
덮어썼다. **aseprite에서 스프라이트를 재익스포트하면 음영이 사라진다** → 재익스포트한 PNG에
`doPng("<경로>", 1.0)`을 **다시 돌려라**(입력=납작 PNG면 됨). 제외 = 타일·UI 패널·룬 아이콘·이펙트
(이유는 relight_sprites.lua 헤더). 되돌리려면 git(세69 커밋 직전).
