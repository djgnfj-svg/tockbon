...
> 2026-07-12 작성. GDD §10.5 규칙 기반. Aseprite MCP 설치 후 P1부터 순서대로.
> 현재 게임은 전부 ColorRect/Polygon2D 플레이스홀더 — 스프라이트가 준비되는 대로 교체.
...
- 숲 타일셋 16×16: 바닥 3종·풀·나무(2타일 높이)·바위·보스 존 냉색 바닥
- 채집 노드: 주간 2종·야간 발광 2종 (16×16)
- 출구 게이트(32×48), 탁본 잔류물(무늬 바닥, 32×32, 발광 2프레임)
- 거점 인테리어: 작업대·창고·연구소·침대·이젤·문 (각 32×32~48), 바닥·벽 타일

## P4 — UI (한지·먹 질감)

- 패널 9-slice (한지 질감, 48×48 원본), HP/마나/하루 바 프레임, 장착 슬롯 프레임(24×24)
- 아이템 아이콘 18종 16×16: 잉크 3(병 색 차이)·종이 3·조각 2·재료 7·장비 3

## P5 — 이펙트

- 투사체 4종 16×16 (룬 색: 주황/노랑/파랑/연두 — 현재 코드 색 유지), 2프레임 회전
- 피격 플래시(셰이더로 대체 가능), 돌풍 텔레그래프 링(64×64, 코드 렌더 유지 가능 — 보류)
