...
세부(체력·피해·무적·밀림·쓰러짐)는 **`character-damage-minimum`** 이 기준이다.
...
🔴 왜 안전한 쪽을 안 골랐는지, 복구가 왜 필수인지, 체력·피해·무적·밀림이 얼마인지는
...
- **화면 확대율** — 지금 코드는 **기본 창(1920×1080)에서 2배**다(뷰포트 960×540, 필터 Nearest).
  🔴 **상수가 아니라 창 크기의 함수다** — `stretch/aspect="expand"` 이고 전체화면 키가 있어서,
  2560×1440이면 2.67배 같은 **비정수 배율**이 나온다. Nearest에서 비정수 배율은
  픽셀 크기가 들쭉날쭉해져 16px 캐릭터의 실루엣을 무너뜨린다.
  ⚠ **정수 배율을 강제할지가 미정이다.** 세부는 **`character-sprite`**
