...
| 결함 | 무엇이었나 | 얼마나 틀렸나 |
|---|---|---|
| **카메라가 180° 돌아 있었다** | `_place_camera` 의 `back.z` 가 음수라 카메라가 표적의 -z 쪽에 섰다. +z 를 보는 고도 카메라는 제 +x 가 세계 **-x** 를 가리킨다 | 물 위 **평균 18.6칸** |
| **눕힌 각의 단축률이 코사인이었다** | 피치는 **수평선에서 잰 각**이라 땅의 단축은 **사인**이다. 90°면 안 눕고(sin 90 = 1) 0°면 완전히 눕는다(sin 0 = 0). 코사인은 양끝이 다 거꾸로다 | 세로 **19% 오차**, 화면 아래에서 **2칸** |
| **누른 자리를 평면으로 풀었다** | 땅에 높이가 생겼는데 변환은 y=0 평면 하나로 답했다. `h` 타일 위에 선 것은 `h / tan(pitch)` 만큼 화면 위에 그려진다 | 땅 위 **평균 2.8칸, 최대 4칸** |
...
1. **계단을 몸들의 진행선에 두었더니** 길찾기가 얼어붙는 띠를 **우회**시켜서 아무도 안 얼었다 — **초록.**
   ⚠ **띠는 목적지에 있는 것이 아니라 위층 몸의 평면 사거리 안에 있는 모든 낮은 칸이다.**
...
상륙이 몸을 벽 위에 올려놓는 결함에 보호막 **둘**을 넣었다(벽을 가로질러 탐색하지 않는다 · 착지 칸과
같은 층만 수집한다). 주석에 **「둘 중 하나만 빼도 구멍이 다시 열린다」**고 적었는데, **실측하니 하나씩
빼서는 아무것도 안 빨개졌다** — 고원 칸에 대해서는 **둘이 서로를 가려 준다.**

⇒ **각 보호막만 답할 수 있는 판을 하나씩 따로 지어야 한다.** 계단 칸은 탐색이 지나가야 하는 칸이라
...
그림자맵 한 장이 4000 칸에 펼쳐지면 **텍셀 하나가 반 칸** — **나무 한 그루보다 넓다.**
