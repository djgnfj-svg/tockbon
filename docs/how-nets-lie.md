...
## 2026-08-27 — **표가 비면 그 표를 읽던 검사가 항상 참인 부등식으로 변한다**

**밀치기 표(`Rules.SPECIES_SHOVE`)가 2026-08-26 에 비었다.** 다람쥐와 소가 유닛 표에서 빠지면서
행이 둘 다 사라졌고, `shove_tiles_of` 는 그날부터 모든 몸에 대해 `0.0` 을 돌려주었다.

**그 표를 읽던 검사들은 빨개지지 않았다.** 밀치기를 재던 세 덩어리가 2026-08-27 에 지워질 때
`net_battle` 의 통과 수가 37 에서 21 로 떨어졌다. **열여섯 개가 통과 중이었다는 뜻이고, 잰 것은
아무것도 없었다.**

무엇이 어떻게 통과했는지, 한 줄씩:

```gdscript
var pull_tiles := Rules.shove_tiles_of(Rules.SWORDSMAN)   # 0.0 이 된다
...
t.ok(sb.enemy_pos[0].distance_to(before) >= pull_tiles - 0.01,
    "서브스텝을 열 번 더 돌려도 원래 자리로 안 되돌아간다")
```

⚠⚠ **`pull_tiles` 가 `0.0` 이 되는 순간 이 줄은 `거리 >= -0.01` 이 된다.** 거리는 언제나 0 이상
이므로 **어떤 입력에도 참이다.** 「끌었다가 한 프레임 반짝하고 마는 것이 아니다」라는, 이 검사가
존재하는 이유 그 자체가 항상 참인 부등식으로 바뀌어 있었다.

같은 덩어리에서 함께 통과하던 것들:
- 「끌려간 자리가 걸을 수 있는 칸이다」 — 몸이 안 움직였으니 원래 서 있던 칸이고, 당연히 통과한다
- 「다람쥐를 지나쳐 넘어가지 않는다」 — 안 움직였으니 통과한다
- 「다람쥐는 매번이다 (자가 점검)」 — `not shove_once_of(...)` 인데 빈 표는 `false` 를 돌려주므로
  통과한다. **바로 윗줄의 `t.ok(shove_once_of(...))` 와 같은 값에 참과 거짓을 동시에 요구하고
  있었고**, 한 줄은 빨갛고 한 줄은 초록인 채로 나란히 서 있었다

⚠ **부호를 요구하던 두 줄만 빨갰다** (`> 0.0`, `< 0.0`). 그래서 그물은 「실패」로 찍혔고, **그
실패 하나에 가려서 초록 열넷이 아무것도 안 재고 있다는 사실이 보이지 않았다.**

⇒ **규칙: 어떤 값의 크기에 기대는 검사는 그 값이 0 이 될 수 있는지 먼저 묻는다.** `>= x - eps` 는
`x` 가 0 이면 항상 참이다. 이 저장소가 이미 적어 둔 *바닥 없는 천장* 의 같은 얼굴이고, 이번에는
바닥이 **표에서** 빠져나갔다.

⇒ **그리고: 표가 비었으면 그 표를 읽는 검사부터 지운다.** 「기구는 배선돼 있고 한 줄이면 돌아온다」는
근거로 빈 표를 남기면, 그 표를 재는 검사도 함께 남고 **그 검사들은 아무것도 못 재는 채로 초록을
쌓는다.** 배선이 도달 불가능하면 그 배선을 재는 그물도 도달 불가능하다.

## 2026-09-03 — **그물 줄이 고정한 입력을 시뮬레이션이 만들 수 없었다 (티켓 12-01)**

**너무 느슨한 단언이 아니다. 정확한 단언인데 도달 불가능한 입력에 걸려 있었다.**

웨이브 예고가 열리는 순간의 남은 시간을 화면이 「3:00」으로 찍는지 재려고 그물이 이렇게 썼다:

```gdscript
[180.0, "3:00"],
```

**초록이었다. 그리고 화면에는 「3:00」이 한 프레임도 안 떴다.**

`Battle.elapsed` 는 `Rules.SIM_SUBSTEP_SEC` (1/60) 을 더한 합이라 **어떤 정수 눈금에도 안 앉는다.**
예고가 열리는 서브스텝의 실제 값은 `179.999999999944` 고, 잘라 쓰면 179 → 「2:59」다.
**웨이브 여덟 개 전부 그랬다.** 그물은 시뮬레이션이 절대 못 내는 `180.0` 을 스스로 손에 쥐고
그 답이 맞는지만 물어봤다.

**같은 결함이 반대쪽 끝에도 있었다.** 올림으로 고친 뒤 「0:00」이 웨이브 1~4 에서 안 떴다 — 잔차가
웨이브마다 고정이다 (`1.08e-10` · `5.44e-10` · `9.81e-10` · `1.42e-09`). 새로 넣은 `[0.0, "0:00"]`
줄도 **정확히 같은 이유로 이것을 못 잡았다**: 시뮬레이션은 정확한 `0.0` 을 웨이브 5~8 에서만 낸다.

⇒ **규칙: 표의 입력을 손으로 쓰기 전에 「시뮬레이션이 이 값을 정말 내는가」를 묻는다.**
누산된 시계에 대해 그 답은 거의 항상 「아니오」다. **재현해서 실제 값을 뽑아 그것을 고정하라.**

⇒ **그리고: 라벨의 서식이 값을 반올림하면 도달 불가능한 값을 두 번째로 불러들인다.** `%.1f` 로 찍힌
줄 이름이 「180.0초」였고, 그것이 못 만드는 숫자를 다시 문서에 심었다.

⚠ **화면 검증이 잡았지 그물이 잡은 게 아니다.** 그물 넷은 셋 다 초록이었다.
