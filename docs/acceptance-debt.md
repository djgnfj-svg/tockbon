...
| 2026-08-24 | **직접 실행한 게임** (`play.bat`) | *"이게 왜 렉이 걸리지? 이딴게임하는데? ㅋㅋㅋㅋㅋㅋ"* · *"렉이 아닌가"* ⚠⚠ **이 저장소가 3D 판을 직접 플레이해서 받은 첫 판정이고, 불합격이다.** ⚠ **재본 값은 이 말과 어긋난다** — 같은 기계에서 계측은 120fps 에 최악 프레임 8.6ms 였다. **원인 미상** |
