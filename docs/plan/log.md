...
| **단위는 칸이 아니라 2x2 조각** | ***"너무 많으가 라는 생각이 있네 지금 4개가 합쳐서 하나잖아"*** |
| **마음대로 자란 덩어리는 아니다** | ***"칸으로 가야하는데... 약간 네모네모 해야할듯한데... 완전 네모 말고"*** |
| **땅 밖으로 나가면 안 된다** | ***"삐져나가는 문제가 있네"*** · ***"태두리를 넘어가는데?"*** |
| **마우스도 조각 단위로** | ***"이번에 깐 4칸짜리를 기준으로 마우스 올리면 동작하게 해줘"*** |
| **섬이 너무 크다** | ***"음 칸이 너무 많네? ... 캐릭터부터 섬도 줄여야할듯"*** |
| **계단에는 마당 없음** | ***"계단에는 칸을 안만들어야하는데"*** |

⚠⚠ **덮개가 조각 한가운데에 안 오던 이유가 이 라운드의 배움이다.** 덮개는 도형을 얹는 것이 아니라
**땅을 깎고 남은 것**이라, 바다 쪽 한 방향만 더 깊게 깎이는 해안 조각에서는 남은 것이 안쪽으로 밀린다.
사용자가 그것을 정확히 짚었다 — ***"이거 4개합친게 4개의 가운데에 있는거 맞음?"***. **가장 깊게 깎이는
쪽만큼 사방을 똑같이 깎게** 고쳐서 해결했고, 그 대가로 해안 마당이 안쪽 것보다 작아진다.

⚠ **레퍼런스와 아직 다른 것 셋**: 마당이 조각의 절반밖에 안 차고(굽는 쪽 `COAST_WOB` 이 0.26 이라
0.45 칸을 물러나야 한다), 2 층에 설 자리가 둘뿐이며, **노란 모래에 흰 자국이라 자국이 땅보다 세게
...
28 점으로 1 등이었는데, 같은 검토의 정찰이 **배드노스가 구역이 아니라 칸 단위**라는 개발자의 말을
찾아왔다 — ***"게임에서 당신의 주된 행동은 부대에게 격자 칸 하나로 가라고 말하는 것이다"***. 사용자가
칸을 택했다: ***"칸단위 부대는 따로 없음 아직"***. **격자로 안 보이는 이유는 지형 조각을 칸보다 크게
만들어 칸 경계를 넘겨 깔았기 때문이다** — 격자를 없앤 것이 아니라 가린 것이다.
⚠ **그 검토가 남긴 사실 셋은 지금도 유효하다**: 플레이어 부대가 영원히 하나라는 것, 배를 「저기에
대라」로 못 보낸다는 것(소환 가능한 바다 칸이 19 개뿐), 그리고 지정된 몸이 적을 섬 전체에서 골라서
자리를 그냥 나간다는 것.
...
| **도구** | `tools/blender/send.py`(블렌더에 코드를 보낸다) ⚠ **여섯 각도로 찍던 `turntable.py` 는 2026-08-27 에 지웠다** — 부르는 곳이 없었고, 조각을 눈으로 보는 일은 게임의 빛 아래에서 도는 조각 뷰어가 맡는다 |
