...
⚠ **모니터에서 한 번 보고 확정해야 한다.** 그림은 `tools/shot/out/water/water_*.png` 에 남아 있고,
...
| **섬이 넓어졌다** | 16x12 → **20x16.** 땅 136 칸, **1층이 96 칸** (계단 4, 2층 36). 「2층이 비싸지만 유리하다」가 선택이 되려면 1층이 넓어야 한다 |
...
## ✅✅ **조각 방식이 되살아났고 이번엔 게임까지 갔다 — 2026-08-27 밤**
...
| 무엇이 정해졌나 | 사용자의 말 |
|---|---|
| **조각으로 간다** | ***"아닌데 좋은데 블록?"*** · ***"너무 좋다"*** |
| **옛 조각 열 개는 버린다** | ***"네가 뽑은 건 개 병신처럼 병신 따따구르만 하고"*** — 원인은 도구가 아니라 **색이 하나도 없고 윗면만 있었던 것** |
...
## ⚠⚠ **구역을 정했다가 같은 날 접었다 — 2026-08-27 밤**

***"한번 여러에이전트로 검토해서 제일좋은방법 찾아봐줘"*** (사용자)

**에이전트 열둘로 돌렸다**: 정찰 셋(밖·판 데이터·규칙) · 설계 넷 · 심사 넷 · 종합 하나.

| 안 | 점수 |
|---|---|
| **손으로 칠한 지역판** — 섬 모양 글자판 옆에 지역판을 하나 더 두고 굽는 김에 같이 내보낸다 | **28** |
| 블렌더가 같이 굽기 — 섬 글자와 지역 글자를 한 글자로 합친다 | 24 |
| 불러올 때 자동으로 나누기 — 데이터를 안 늘리고 걸을 수 있는 땅을 자른다 | 23 |
| 구역을 먼저 그리고 칸을 깔기 — 섬을 다각형으로 그리고 칸을 생성한다 | 16 |

⚠⚠ **그런데 정찰이 찾아온 것이 순위를 무의미하게 만들었다.** **배드노스는 구역 단위가 아니라 칸
단위다.** 개발자가 직접 말했다 — ***"게임에서 당신의 주된 행동은 부대에게 격자 칸 하나로 가라고 말하는
것이다"*** · ***"가능한 자리가 딱딱 끊어져 있어야 내 배치의 결과를 이해하고 예측하기 쉽다"***
(TheSixthAxis 2018-04-25 · Nintendo UK 2018-04).
**격자로 안 보이는 이유는 지형 조각을 칸보다 크게 만들어 칸 경계를 넘겨 깔았기 때문이다** — 격자를
없앤 것이 아니라 가린 것이다 (Boris the Brave, WFC tips and tricks).

⇒ **사용자가 칸 단위를 택했다**: ***"칸단위 부대는 따로 없음 아직"***. **구역은 접혔다.**
⚠ **양쪽을 다 남겨 두는 이유**: 뒤집힌 기록이 다음에 같은 자리에 설 때의 근거다.

**이 결정이 지운 일 둘**: 지역판을 손으로 치는 것, 덮개를 구역 단위로 다시 짜는 것.

⚠ **검토가 밝혀낸 것 셋은 결정과 무관하게 그대로 살아 있다.**
1. **플레이어 부대는 영원히 하나다** — 검사 하나뿐이고 두 번째 종류가 생길 수 없다. 어떤 단위로
   명령하든 전군이 한 덩어리로 움직인다. **사용자가 부대를 지금은 안 만들기로 했으므로 유효한 제약이다.**
2. **배를 「저기에 대라」로 못 보낸다** — 소환 가능한 바다 칸이 섬 전체에 19 개뿐이고, 일곱으로 나눈
   구역 중 둘은 0 개였다. 동쪽 해안 전체가 그렇다.
3. **지정된 몸이 자리를 그냥 나간다** — 적을 섬 전체에서 고르므로, 어디를 지키라고 해도 밖에 적이
   있으면 나간다. **걷기 명령을 붙일 때 조준 쪽도 같이 봐야 한다.**

⚠⚠ **그리고 이 라운드가 내 거짓말 하나를 잡았다.** 나는 사용자에게 「덮개 코드는 어떤 모양이든
따라가므로 다시 안 짜도 된다」고 말했는데, 코드는 타일 0 을 기준으로 한 두 칸짜리 격자에 못 박혀
있다. **구역을 넘겨줘도 바둑판만 그린다.** 판단의 근거로 쓰인 말이 틀렸던 것이라 여기 적는다.
...
**0.26** 이다. **판이 평지마다 흙 속에 0.26 만큼 잠겨 있었다.** 해안 칸에서만 물 쪽으로 삐져나온
...
| **단위는 칸이 아니라 2x2 조각** | ***"너무 많으가 라는 생각이 있네 지금 4개가 합쳐서 하나잖아? 그걸 하나 취급하는것도 좋을꺼같아"*** |
...
⚠ **거리장을 층마다 따로 잰다.** 2 층 칸과 그 옆 평지 칸은 둘 다 걸을 수 있어서, 하나로 재면 그
사이의 낭떠러지가 마스크에 안 보이고 덮개가 허공에 걸린다. **층이 바뀌는 자리도 땅 끝이다.**

## ⚠⚠ **다음은 「칸」이 아니라 「구역」이다 — 2026-08-27 밤에 정함**

***"이런 느낌으로 되야하는데 이게 3D 모델링 부터 되야할까?"*** (사용자, 배드노스 화면을 놓고)

**일곱 가지로 갈라서 재 봤고, 다섯이 블렌더였다**: 섬 윤곽(조각 계단 대신 비스듬한 다각형) · 색(채도가
...
우리는 칸이 520 개이고 넷씩 묶어도 130 이다. **덮개를 어떻게 칠하든 이 숫자로는 그 느낌이 안 난다.**

**사용자가 추천 셋을 다 받았다**: **구역 다섯에서 여덟** · **명령은 구역 단위** · **칸 격자는 남긴다**
(걷고 서는 자리와 계단 규칙이 전부 칸 위에 얹혀 있어서, 격자를 없애면 화면이 아니라 규칙을 다시 짜는
일이 된다).
...
⚠⚠ **`tools/blender/pieces.py` 는 2026-08-27 에 지워졌다.** 조각 열 개는 `assets/terrain/pieces.glb`
...
예순넷을 붙여** 섬을 만든다. 조각을 파일로 굽고 게임이 놓는 것이 아니라 블렌더가 붙여서 통째로
...
**[티켓 01 — 한 조각이 무엇인가](tickets/01-what-one-piece-is.md).** 조각을 여섯 번 뽑아 여섯 번 다
...
| 2026-08-27 | ✅✅ **섬은 블렌더 안의 판에서 면을 올려 그린다. GridMap 은 접혔다** — 면 하나가 반 칸, 0 아래는 바다 |
...
5. **계단이 오르는 길로 안 읽힌다** — 메시에는 반 칸짜리 턱으로 있고 전투 계산도 계단만 넘게 되어
