...
## 목~일 절반이 지어졌다 — 2026-08-30 — **그리고 이 티켓은 안 닫힌다**

**짐승이 배에서 내려 몸이 되고, 성채로 걷고, 때리고 죽고, 성채 체력이 0 이면 진다.**
**그물 통과 1048 → 1234.** `net_fight` 은 새로 선 그물이다.

### ⚠⚠ 왜 닫지 않나 — **성채가 1층 땅에서 탄다**

**해변 61 곳 중 24 곳에서 계단을 안 밟고 이기고, 그쪽이 세 배 빠르다.** 티켓 **57** 이 든다.
**끝나는 조건이 「막아 내거나 본채가 타서 진다」인데, 막을 이유가 없는 판은 그 조건을 만족하지 않는다.**

### ⚠ 안 만든 것 — **있는 척하지 않는다**

- **진 화면** — 상태만 있고 화면에 그린 것이 없다. 사용자: 「그냥 엔딩씬 하나 있을듯」
- **체력바도 성채 피해 표시도 없다** — 성채가 얼마나 탔는지 화면에 안 나온다
- **짐승이 검사를 안 쫓는다** — 성채로 걷다가 사거리에 든 것만 때린다
- **범위 공격 · 예고 동작 · 감지 반경** 없다

### ⚠ 안 정해진 셋 중 둘이 아직 그대로다

**검사 4 로 시작 · 부활 20 초.** 둘 다 짠 쪽이 고른 자리표시이고 상수에 그렇게 적혀 있다.
**셋째 「졌다는 것이 화면에서 어떻게 보이나」는 답이 나왔다** — 엔딩씬 하나.

### 사용자가 화면을 보고 한 말 — **그날 그대로**

- 「조작이 좀 ㅈ같이 바뀌였는데」
- 「늑대 인식잘 안되고 조각마다 한마리가 아니라 좀더 컷으면 좋겠고 한칸에 좀 9마리 까지는 가능해야하고(조각아님 칸임)」
- 「마우스 휠은 그대로 확대 축소가 맞음 마우스가 외각으로 가면 그쪽으로 카메라 이동되는걸로하고 오른쪽 누르면 카메라 회전으로 이해함」
- 「그 왜케 좌우로 흔들리니? 캐릭터랑 늑대랑...」 · 「막 흔들리는 느낌임 이건좀 제거가 필요」
- 「쫀득하게 위아래로 있는건 맞는데 이건 좀 아니고 병사들 그림자가 없다」
- 「너무 띠용거린다」
- 「늑대가 내가 골랐던 H 늑대가 아닌거야? 그떄 골랐던 늑대가 아니네? 그리고 너무 너무 작아서 눈에 안띔」
- 「왜 바닥에 마름모로 뭐가 뜨지?」
- **마지막으로 켜 보고: 「잘봤고」**

### 그날 고친 것

**휠이 확대·축소로, 오른쪽 드래그가 회전으로 되돌아갔다** (아침에 사용자가 정반대로 정했던 것을
화면 앞에서 뒤집었다) · **몸 크기 0.45 → 0.80** · **한 조각에 셋까지 선다** · **좌우 흔들림을 껐다** ·
**그림자 원판이 그려진 그림 기준이 됐다** (몸의 5 분의 1 이던 것이 절반쯤으로) ·
**늑대가 H 를 입고 1.70 배로 섰다** · **띠용거림 0.20 → 0.1125**.
