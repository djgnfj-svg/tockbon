Status: ⚠⚠ **결론은 유효하고 그릴 자리가 없어졌다.** 2026-08-28 에 사용자가 6번(색으로 그리는 마루)을 골랐는데, 그날 저녁 셰이더가 통째로 갈리면서 물주름이 화면에서 사라졌다. (티켓 05 였다)
...
## ⚠⚠ 2026-08-28 저녁 — **이 티켓이 고른 물주름은 화면에서 사라졌다**

**여기서 고른 것은 마루를 흰 선으로 직접 그리는 방식이었고, 그것을 그리던 셰이더가 그날 저녁에 통째로
교체됐다.** 해안선을 그리는 방법 일곱 벌을 조각 하나 위에 세워 놓고(`.prototypes/shoreline/`)
사용자가 **가장 적게 하는 것** — 평평한 바다에 경계선 하나 — 을 골랐기 때문이다.

⇒ **지금 바다에는 물주름이 없다.** 상수는 `look.gd` 에 「주차」 표시를 달고 그대로 있고, **아무것도 그
값을 안 읽는다.** ⚠ **이 티켓의 결론은 틀린 것이 아니라 적용될 자리가 없어진 것**이다. 바다에 다시
결을 넣기로 하면 여기서부터 읽는다.
