# TECH_SPEC — 탁본 (TAKBON) 기술 명세 · **상세 계약 (아카이브)**

> 🔴 **현재 유효한 계약 요약은 `TRUTH.md` §3이다.** 이 문서는 그 **상세판**(정확한 상수·gdscript
> 필드 주석·인식기 판정표·먹선 렌더 함수 목록)을 보존한다. 변경 이력·반전된 판단은 `CHANGELOG.md`.
> **스키마·시그널을 바꿀 때는 TRUTH §3을 먼저 갱신**하고 여기 상세를 맞춘다.
>
> 2026-07-12 신설 · 병렬 개발용 아키텍처·계약 문서. 스키마/시그널 변경은 리드만 수행한다.
...
### 4.0-b **v2.1 — 문양 = 착탄 충격파의 방향** (사용자 확정, 2026-07-15)

> **"화살표라는 문양은 적과의 히트 이후의 충격이 어디로 가는가를 정하는 거임. 그래서 그
> **충격파끼리 맞으면 기둥이 되는 거임.** 한쪽 방향으로 싹 다 그리면 히트 이후 그 방향으로만
> 충격파가 가는 느낌. **화살표 하나하나가 효과를 가지는 거지.**"** — 사용자

**세션 11이 "🔴 맞은 다음 → 비어 있다"고 남긴 빈칸이 채워진다.** 두 층이 갈라진다:

| 층 | 무엇이 정하나 |
|---|---|
| **날아가는 동안** | 팅김⚡ · 유도∿ · 관통‖ (v1.9~) |
| **맞은 다음** | 🔴 **곧은 화살표 = 충격파 하나** (v2.1 — 이 절) |

#### 🔴 기둥은 규칙이 아니라 **결과**다

**규칙은 하나뿐이다: "화살표 = 충격파 하나가 가는 방향."**
...
> **"별 진 = 전방위"를 글자로 외우게 하지 않는다 — 별을 그리면 별처럼 퍼진다"**(GDD §4.1)와
> **같은 원리다.** 가르칠 게 없다. 그린 대로 충격파가 간다.
> ⚠ **"수렴하면 기둥"이라고 코드에 적지 말 것.** 그 순간 창발이 아니라 규칙이 되고,
> "몇 개부터 기둥인가"라는 답 없는 질문이 따라온다. **만나면 만나는 거고 어긋나면 안 만난다.**

#### 🟢 v2.0에서 "소비 중단"한 두 필드가 **부활한다**

**탄이 곧 진**이므로(§4.0-a), 적에 닿는 순간 **진이 히트 지점에 놓인다.** 그러면 진 위에
그려진 화살표들이 **각자 제자리에서 제 방향으로** 충격파를 뿜는다 — 종이 위의 배치가
그대로 착탄 그림이 된다.
...
| `SpellDesign.aim_axis` | 소비 중단 | **여전히 소비 중단** — 단, 아래 변환의 상수(`UP_AXIS`)로 개념이 살아난다 |

#### 방위 기준 = **탄이 가던 방향** (사용자 확정: *"탄 기준이 맞기는 한네"*)

**종이 위쪽 = 앞 = 탄이 가는 쪽.** 발사각은 여전히 지팡이의 것이고(§4.0-a), 이건 **충격파 축**이다.
...
- **왜 탄 기준인가**: 종이(절대) 기준이면 같은 도안이 **적 위치에 따라** 끌어오기도 하고
  밀어내기도 한다 — **내 통제 밖이다.** 탄 기준이면 위(앞)로 그으면 **언제나 밀어내고**,
  아래(뒤)로 그으면 **언제나 나에게 끌어온다**
- 🔴 **세션 7의 "90도 틀어짐"이 정확히 이 공식에서 났다** — `design_builder`가 상대각을
  저장하는데 `spell_system`이 절대각으로 알고 **또** 빼서 `aim_axis`가 두 번 빠졌다.
  **`direction`은 캔버스 절대각이고, 회전은 발사 때 한 번만 건다.** 두 번 걸지 말 것
- ⚠ **방위 가이드(앞·뒤·좌·우)를 되살린다** — v2.0에서 "종이에 방위가 없어졌다"며 지웠는데,
  **충격파가 그 자리를 다시 채웠다.** 다만 의미가 다르다: ~~발사 방향~~ → **충격파 방향**

#### 미정 (억지로 정하지 않는다 — v1.8의 "팅김"이 그래서 잘 풀렸다)

- **팅김·유도·관통 획도 충격파를 뿜는가** (사용자: *"아직 미정"*).
  **일단 곧은 화살표(BASIC)만 뿜는다** — 지금 **아무 일도 안 하는 값**이라 죽은 값이 살아난다.
  손으로 만져 본 뒤 정한다
- **증폭·응집**은 이 설계가 흡수했을 수 있다 (모아 그으면 기둥 = 응집 / 밖으로 = 폭발 = 증폭).
  이름만 남기고 **따로 만들지 않는다**

---

### 4.0-a **v2.0 축 — 방향은 지팡이가 정한다** (사용자 확정, 2026-07-14)

> **"마법 발사를 그냥 지팡이로 대체하자. 결국 보는 방향으로 발사가 맞고, 지팡이에 따라
> 여러 발이 나가거나 내 주변에서 전체 방향으로 나가거나가 정해짐."** — 사용자
>
> **바뀐 건 한 줄이다: 방향이 문양에서 지팡이로 갔다.** 나머지는 그 한 줄의 **연쇄**다.

| 부품 | v1.9 (옛 계약) | **v2.0 (이 계약)** |
|---|---|---|
| **지팡이** | 기본 약공격만 (`wand_bolt`) | 🔴 **방향**(=에임) · **발수** · **기점** · 산포 |
| **진** | 규모 (+ 모양은 미구현) | **투사체의 모양** + 규모 (위력·크기·기준 사거리) |
| **룬** | 속성 + 농도 | **그대로** |
| **문양** | 발동 방식 — **1개 = 1발** | 🔴 **효과 부여** — **N개 = 한 탄에 효과 N개** |

#### 🔴 방향이 사라지면 "문양 1개 = 1발"이 자동으로 무너진다

문양이 방향을 안 정하는데 문양마다 탄을 만들면 **N발이 같은 자리에 겹쳐 나간다.**
그래서 **발수는 지팡이가 갖고, 문양은 그 한 탄에 얹히는 효과가 된다.**
팅김⚡ + 관통‖ + 증폭을 한 장에 그리면 **튕기면서 뚫는 강화탄 하나**가 나간다.

| 값 | v1.9 소비처 | **v2.0** |
|---|---|---|
| `ArrowData.direction` | 발사각 (`+ (에임 − aim_axis)`) | 🔴 **소비 중단** — 발사각은 **에임 하나**. 필드는 렌더·기록용으로 남는다 |
| `ArrowData.origin` | 발사 기점 오프셋 | 🔴 **소비 중단** — 기점은 지팡이가 정한다 |
| `SpellDesign.aim_axis` · `circle_type` | 도안 전체 회전 | 🔴 **소비 중단** — 회전이 없다 (구세이브 호환 필드로만) |
| `ArrowData.glyph` | 탄 1발의 **발동 방식** | **효과 종류** — 한 탄에 **여러 개가 동시에** 얹힌다 |
| `ArrowData.reach` | 그 탄의 세기 + 사거리 배율 | **그 효과의 세기.** 사거리는 아래 규칙 |

**같은 글자를 여러 개 그리면 세기가 합산된다** (`reach` 합 → `reach_t`). 팅김을 둘 그으면 더 튕긴다.
**사거리 배율은 문양 중 `reach` 최댓값**이 정한다 — *가장 멀리 뻗은 획만큼 날아간다*.
합이 아니라 최댓값인 이유: 합이면 효과를 하나 더 얹을 때마다 사거리가 공짜로 늘어나 **"문양은 무조건 많이"**가 유일한 정답이 된다.

#### 🔴 진 = 투사체의 모양 (사용자: *"투사체의 모양이 진이라고 끝인데?"*)

**날아가는 것이 문양 획에서 진(+룬)으로 바뀐다.** 세션 11이 *"아무도 안 정하는 것"*으로
남겨 둔 **히트박스 모양**(언제나 반지름 5의 원 — 그린 획은 그 위에 얹힌 **그림일 뿐**이었다)의
빈칸을 **진이 채운다.**

| | v1.9 | **v2.0** |
|---|---|---|
| 날아가는 먹선 | `InkRender.tail_line(arrow.path)` — **문양 획** | **`InkRender.build_design(roles: CIRCLE_ROLES)`** — 진 + 룬 |
| 히트박스 | `CircleShape2D` 반지름 5 × `size_scale` | **진 반지름에 비례** (`design.circle_radius`) |
| 탄의 회전 | 발사각으로 회전 (획이 진행 방향을 봄) | 🔴 **회전 없음** — 마법진은 그린 자세 그대로 날아간다. 빙글빙글 돌면 진이 아니다 |

⚠ **`RayCast2D`(팅김 반사)가 로컬 +X = 진행 방향을 전제했다** — 탄이 회전을 안 하므로
`target_position`을 **속도 방향으로 직접** 넣어야 한다 (안 그러면 반사가 조용히 엉뚱한 데를 본다).

#### 지팡이 (v2.0 — **스키마를 만들지 않는다**)

사용자: *"일단 지금은 그냥 지팡이만."* **`WandDef` 리소스를 지금 만들면 과잉이다.**
`spell_system._wand_shots(aim_angle) -> Array[float]`(발사각 목록) **함수 하나**로 두고 **단발 고정**.
다발·전방위는 **이 함수 안에서만** 열린다 — 호출부는 안 바뀐다.

```gdscript
## 지팡이가 발사각 목록을 준다. 지금은 단발 — 에임 하나.
## 다발이면 [aim-θ, aim, aim+θ], 전방위면 [0, 60°, 120°, …]. **호출부는 그대로다.**
func _wand_shots(aim_angle: float) -> Array[float]:
    return [aim_angle]
```

#### 아직 안 정한 것 (다음 단계)

- **증폭·응집 문양** — 응집(산포↓)은 **지팡이가 다발을 쏠 때만** 의미가 생긴다.
  단발뿐인 지금 넣으면 **아무 일도 안 하는 문양**이 된다. 지팡이 패턴과 **함께** 온다
- **룬 상극 = 증발** (사용자: *"불과 물의 룬을 가지고 있으면 쏘는 순간 증발… 그 자리에 수증기가
  남는 느낌"*) — **벌인지 보상인지 미정. 전투에서 확인한다.** 룬 N개가 오기 전엔 발생할 수 없다
- **진 모양의 종류** (별·용) — `$P` 인식기 전제. **다만 v2.0에서 별 진의 근거였던 "전방위"가
  지팡이로 갔다** — 진 모양은 이제 **탄의 생김새**이지 출구의 지형이 아니다

#### 🟢 v2.0이 없앤 선행 조건 — `$P` 교체가 급하지 않다

$P가 **전제**였던 유일한 이유는 **별·용의 진**(한붓그리기로 원리적 불가)이었고,
별 진의 존재 이유는 *"꼭짓점이 출구 → 전방위"*였다 (GDD §4.1). **전방위를 지팡이가 가져간
지금, 진 모양은 "탄이 어떻게 생겼나"일 뿐이라 원 하나로도 이번 주 목표가 굴러간다.**

#### 문양 축 — `reach`의 **정의**는 v1.9 그대로다 (소비처만 바뀌었다)
...
@export var glyph: Enums.GlyphType = Enums.GlyphType.BASIC
    ...
    # **v2.0: "이 탄의 방식"이 아니라 "탄에 얹히는 효과"다** — 한 탄에 여러 개가 동시에 붙는다
@export var reach: float = 1.0
    # **뻗은 거리** ÷ 진 반지름. **rune_fill과 정확히 대칭.**
    # v2.0 소비: **그 효과의 세기** (같은 글자 여럿이면 합산) + **사거리 배율은 최댓값 하나**
```

🔴 **~~"기본 1.0 = 기준 배율 (구세이브 불변)"~~ — 이건 거짓이었다** (세션 12 실측으로 발각).
`reach = 1.0`의 사거리 배율은 **1.0이 아니라 0.85다**:

```
t = inverse_lerp(0.6, 3.0, 1.0) = 0.167  →  range_mult = lerp(0.7, 1.6, 0.167) = 0.85
배율이 정확히 1.0이 되는 reach는 **1.4**다.
```

- **v1.9가 `reach`를 도입하면서 샘플·구세이브 도안의 사거리를 조용히 0.85배로 줄여 놨다.**
  문서(과 STATUS)는 *"구세이브 도안이 예전과 똑같이 날아간다 — test_spell_auto가 실측으로
  못 박음"*이라 주장했지만, **그 테스트는 사거리를 v1.8 값과 비교한 적이 없다.**
- **고치지 않는다** (사용자 결정 대기). `reach`가 없는 도안 = **문양을 안 그린 도안**이므로
  "짧은 문양" 취급(0.85배)이 오히려 규칙에 맞다. 실제 플레이 도안은 `design_builder`가
  `reach`를 계산하므로 **영향받는 건 샘플 도안·구세이브뿐**이다
- ⚠ **기본값을 "중립"이라고 부르지 말 것.** 숫자 1.0이 배율 1.0을 뜻하지 않는다 —
  이 착각이 문서에 두 번(TECH_SPEC·STATUS) 적혔다. `test_spell_auto`가 이제 이 값을
  **경고로 출력한다** (실패가 아니라 리드가 눈으로 보라고)

🔴 **뻗은 거리(`InkRender.arrow_extent`)이지 총연장(`path_length`)이 아니다** (세션 11 수정).
총연장으로 재면 *"얼마나 멀리 뻗었나"*가 아니라 **"얼마나 구불구불한가"**를 잰다:

| 똑같이 0.40을 뻗은 문양 | 총연장 기준 reach | 뻗은거리 기준 (현행) |
|---|---|---|
| 기본 (직선) | 2.00 | 2.00 |
| 팅김 진폭 크게 | **3.00 (상한 포화)** | 2.00 |
| 팅김 촘촘히 8꺾임 | **3.00 (상한 포화)** | 2.00 |

**짧게 그려도 진폭만 키우면 최대 사거리·최대 반사를 공짜로 받았고**, 팅김⚡·유도∿가 기본보다
구조적으로 유리했다. 세션 9의 `rune_fill`(AABB → 무게중심 최원거리) 수정과 **같은 성격**이다 —
정의가 불변량이 아니면 숨은 최적 플레이가 생긴다. 회귀: `test_glyph_auto._test_reach_shape_invariant`
...
- 🔴 **v2.0: 탄은 도안당 (지팡이 발수)개다 — 문양 개수가 아니다.** 그래서 `compute_arrow_lifetime`이
  **`compute_design_lifetime`으로 바뀐다**: `compute_lifetime(진 축)` × `range_mult(**max** reach)`.
  **사거리도 이제 도안당 1회다** — 한 탄에 모든 문양이 얹히므로 탄마다 갈라질 것이 없다
- `spell_system.compile_effects(design) -> Dictionary` — **{glyph: Σreach}**. 같은 글자를 여럿
  그으면 세기가 합산된다 (팅김 둘 = 더 튕긴다). BASIC은 효과가 없으므로 **사거리에만 기여**한다
- `design_builder`: 마나에 `glyph_reach_mana_mult × Σt` — **그대로 둔다.** 효과를 얹을수록 비싸야
  한다 (공짜면 "문양은 무조건 많이·길게"가 유일한 정답이 된다)
...
#### 미구현 스키마 (룬 N개) — **`angle`(배선)이 v2.0에서 죽었다**
...
# ~~@export var angle: float~~  🔴 **v2.0에서 폐기.** 아래를 읽을 것
```

#### ~~배선 규칙 (v1.9) — 룬 N개 × 문양 N개~~ 🔴 **폐기**

v1.9의 배선은 **`argmin | angdiff(rune.angle, arrow.direction) |`** — *"왼쪽에 둔 룬이
왼쪽으로 나가는 문양을 지배한다"*였다. **`arrow.direction`이 없어졌으므로 성립하지 않는다.**
문양이 방향을 안 정하는데 "각도상 가장 가까운 룬"을 찾을 대상이 없다.

**대신 (사용자 확정):** *"룬은 하나의 진 안에 들어가 있음. 진 안에 고유한 룬은 하나만 가능한
걸로 하고… 만약에 불과 물의 룬을 가지고 있으면 쏘는 순간 증발하게 할 거 같아."*

- **한 진 안에 같은 룬은 하나뿐** (종류가 유일하다). 같은 룬을 두 번 그려 농도를 뻥튀기할 수 없다
- **탄은 하나이고 룬들이 그 하나에 다 얹힌다** — 배선할 방향이 없으므로 나눌 것도 없다
- **상극(불↔물)이면 증발** — 🔴 **벌인지 보상인지 미정.** 전투에서 확인한다 (사용자:
  *"그 자리에 수증기가 남는 느낌일 듯한데?"*). **룬 N개가 오기 전엔 발생할 수 없는 상황이다**
- **위력 계수는 `fill` 가중 평균** (룬 1개일 때 지금과 **정확히 같다** → 하위 호환은 유지)

#### 남은 정합성 항목

- **적의 `_statuses`는 이미 Dictionary라 상태이상을 여러 개 동시에 갖는다** — 안 바꿔도 된다
- **세이브 마이그레이션 1회 필요** (`rune_fill` 스칼라 → `runes` 배열). v1.7에서 이미 세이브에 들어갔다.
  `arrows`는 **필드 추가라 기본값으로 흡수된다** (`glyph=BASIC`, `reach=1.0`) — 마이그레이션 불필요.
  ⚠ ~~구세이브 도안이 **예전과 똑같이** 날아간다~~ — **거짓이었다.** `reach=1.0`은 사거리
  **0.85배**다 (위 참조). 위력·크기는 같지만 **사거리는 v1.8보다 짧다**
...
- 🔴 **v2.0 순서가 바뀌었다**: **방향→지팡이 · 문양→효과 · 진→투사체 모양**이 먼저다 (이번 주).
  그다음이 **지팡이 패턴(다발·전방위) + 증폭·응집 문양** — 이 둘은 **함께** 와야 한다
  (응집은 다발이 없으면 아무 일도 안 한다). **`$P`·진 모양·룬 N개는 그 뒤로 밀렸다** —
  더 이상 **선행 조건이 아니기 때문이다** (위 "🟢 v2.0이 없앤 선행 조건")

---

### 4.0 역할 축 계약 (**v2.0**) — **그리기는 조준이 아니라 조합이다**

**부품은 서로 다른 질문에 답한다. 한 부품이 남의 축을 먹으면 그 부품은 의미를 잃는다.**

| 부품 | 답하는 질문 | 정하는 값 | 소비처 |
|---|---|---|---|
| **지팡이** | **어디로 · 몇 발** | **에임 각도** · 발수 · 기점 · 산포 | `player`, `spell_system._wand_shots` |
| **진** (원) | **얼마나 큰 마법인가 · 어떻게 생겼나** | `circle_radius` → **위력 · 탄 크기 · 기준 사거리** + **탄의 먹선·히트박스** | `spell_system`, `projectile` |
| **룬** | **무엇으로 때리는가 · 얼마나 진하게** | `rune_type` → 피해 계수 · 상태이상 종류 / **`rune_fill` → 상태이상 세기** / `rune_accuracy` → 순도 | `RuneDef`, `spell_system` |
| **문양** | **어떤 효과가 붙나** | `glyph` → **효과 종류** (한 탄에 N개) / `reach` → 그 효과의 세기 · 사거리 배율 | `spell_system.compile_effects`, `projectile` |

- **규모는 진의 것이다.** 큰 진 = 크고 아프고 멀리 가는 마법. 문양을 길게 그어도 **세지지 않는다**
  (길게 그으면 그 **효과**가 세지고 사거리가 늘 뿐이다)
- 🔴 **방향은 어느 도안 부품의 것도 아니다 — 지팡이의 것이다** (v2.0). `ArrowData.direction`·
  `origin`·`SpellDesign.aim_axis`·`circle_type`은 **더 이상 소비되지 않는다** (렌더·기록·구세이브용)
- **`ArrowData.magnitude`는 여전히 전투 스탯이 아니다.** 전투에 쓰는 건 진 대비 비율인 `reach`다
...
@export var direction: float             # rad. **캔버스 절대각** — strokes·origin과 같은 좌표계.
                                         ...
                                         # 🔴 값은 **InkRender.arrow_heading**(시작점 → 최원점)이다.
                                         # **끝점이 아니다** — 세션 11 참조 (아래)
...
3. **화살표 (파라미터 추출)**: 방향·세기는 인식기 없이 기하로 — 단 **글자(glyph)는 $1으로 읽는다**(v1.9).
   기점 = 시작점, **곡률 = `path`**(먹선 렌더 전용). 룬과의 구별은 직진성이 아니라 **탈출 판정**이다(§6.1).

   🔴 **방향·세기는 `InkRender`의 두 함수가 소유한다 — 끝점도 총연장도 쓰지 않는다** (세션 11):

   | | 정의 | 왜 |
   |---|---|---|
   | `arrow_tip()` | 시작점에서 **가장 먼 점** (지터 대비 상위 3% 평균) | 방향·세기가 **같은 점**을 겨눈다 |
   | `arrow_heading()` → `direction` | 시작점 → tip의 **각도** | ~~끝점~~: 관통‖은 뒤로 꺾인 **화살촉**으로 끝나 끝점이 축을 벗어난다. 실측 손그림 200회 — 끝점 기준이면 관통만 **평균 14.6도·최대 22.6도** 틀어지고 96%가 10도 초과 |
   | `arrow_extent()` → `reach` | 시작점 → tip의 **거리** | ~~총연장~~: "얼마나 멀리 뻗었나"가 아니라 "얼마나 구불구불한가"를 재게 된다 (위 표) |

   **먹선 로컬 변환(`arrow_local_points`)·투사체 머리(`tail_line`)·책자와 종이의 화살촉이 전부 같은
   함수를 쓴다.** 정의가 갈라지면 **그린 그림과 날아가는 방향이 어긋난다** — 그게 이 게임의 정체성이다.
   회귀: `test_glyph_auto._test_heading` (오차 상한 8도)
...
- ⚠ **탈출 판정은 여전히 `points[-1]`(끝점)을 본다** — `arrow_tip`이 아니다. **의도된 것이다**:
  "진을 뚫고 나갔는가"는 붓이 실제로 종이 어디서 떠났는가의 문제이고, "어디를 겨눴는가"(tip)와는
  다른 질문이다. 관통‖의 촉은 뒤로 꺾이지만 끝점은 여전히 진 밖이라 판정이 산다 (스펙상 촉 길이 ≤ 축의 1/3)
- 화살표 `direction`은 곡선이어도 **시작 → 최원점**(`arrow_heading`) — 발사 초기각·에임 회전 기준축.
  ~~시작→끝 벡터~~는 v1.9에서 관통‖을 14도 빗나가게 했다 (§6 3번 표). 곡률은 `path`(먹선 렌더 전용)에 담긴다
