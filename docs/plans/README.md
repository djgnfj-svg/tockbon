...
| Folder | Contents |
|---|---|
| `2.active` ⚠ **PLANNED, NOT STARTED — not one line built** | [parts on a board](2.active/refit-board.md) — **six cards after a won fight, take two; lay them into a slot's 3×2 board; and the board's numbers reach the fight.** Seven stages. ⚠⚠ **The largest piece is not either new screen**: every stat is read as `Rules.<stat>_of(TYPE)` today, so **two soldiers of one type cannot differ**, and those reads move to per-soldier lookups on `Army` — the shape the beak already uses for range. A soldier is **born to a summon slot** and carries that slot's board, so `Rules.SUMMON_SLOTS` becomes a three-column table and `Army.add` is **renamed** to `recruit` (`CELL_MELEE` is 0 and slot 0 is 0, so a call site left behind would keep producing the right answer today). **The chest node and `HEAL` leave — every node is a fight.** ✅ **Four OPEN questions were sent in one batch on 2026-08-20 and three came back**: both existing rewards stay and the six cards land on top, the ex-chest node pays cells on a shared grid, refit opens once after the pick. ⏳ **One is still with the user** — does a living soldier's CURRENT hp rise when a 체력 part is fitted? Built against **no**, isolated to five lines in one function, **because the chest left and there is no recovery path in this game at all any more.** ⚠ **Sets are OUT and the room they need is exactly one function** (`Loadout.bonus`); occupancy is deferred, so **the board asks 「did you collect it」 and nothing else and the round cannot be judged on whether it is interesting** — the design says so out loud. ⚠ **Acceptance includes RUNNING the probe and reporting whether a route wipes before the boss** |
