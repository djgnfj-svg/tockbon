...
🔴🔴 **직전 세션 = 49 「룬 상태이상·원소 반응 — 룬 축이 비로소 실체를 얻었다」** (정본 STATUS「49」+ memory `takbon-status-reactions`):
세34~48까지 `forest_enemy.take_hit`이 `_status`를 **밑줄로 버려** 불·물·바람의 실질 차이가 **색 + 데미지 ±15%**뿐이었다. 이제 실제로 걸린다.
🔴🔴 **설계 원칙 (사용자 확정): 「단독은 약한 바탕, 조합에서 폭발한다」.** 단독을 다 없애면 한 발이 밋밋하고, 단독이 다 세면 조합할 이유가 없다. **규칙 단일 소스 = `src/core/status_rules.gd`**(반응 표가 곧 기획서 — **반응 추가 = 줄 하나**).
🔴 **룬 3→6**: 불(화상)·물(젖음)·바람(**상태 없음 = 확산자**)·번개(감전)·흙(**취약 = 밑작업**)·풀(덩굴).
**반응**: 젖음+흙=**진흙**(사용자 아이디어) · 젖음+번개=**감전 연쇄** · 젖음+불=증기 · 화상+풀=**산불** · 화상+물=꺼짐.
🔴 **바람·흙은 자체 효과를 억지로 만들지 않았다** (원신 Swirl/Crystallize 선례를 찾아 대조): 혼자선 밋밋하고 **조합에서 최고**가 되는 자리. 사용자 직감(*"흙이나 바람은 좀 힘들듯"*)이 맞았다.
🔴 **룬 전용 문양은 각하** (사용자 확정): 문양은 전부 공통 = **문양6 × 룬6 = 36조합**. 외울 건 12개인데 결과는 36가지 — 어휘를 늘리는 게 아니라 **곱셈이 는다**.
🔴 **다음 세션 = ⓐ 사용자가 직접 쏴 보기**(진 8종 손맛 + 반응이 터지는 리듬 — 헤드리스도 스샷도 못 잼) → ⓑ **임팩트 표현**(사용자: *"재미는 보장된 시스템이고 임팩트를 어떻게 표현할 문제"* — 지금은 색 틴트뿐, 반응이 **터지는 순간의 연출**이 없다) → ⓒ 아래 남은 빚.
⏸ **보류 = forest_t2(숲2·티어 하강)** — 사용자 확정(세48): *"숲2는 아직 필요없음."* 딸린 **「하강 시 회복 스킵」도 같이 보류**(`src/field/forest.gd:131` 주석).

🔴 **세션49가 남긴 빚** (다음에 손댈 때 같이):
- **`status_power`의 의미가 상태마다 다르다** — 화상=초당피해(3.0)·바람=밀림(60.0)·젖음=감속(0.35)인데 **감전·취약·덩굴은 아예 안 쓴다**(세기가 balance에만 있다). 🔴 **특별잉크 증폭이 곱해지는 대상이 `status_power`라, 지금 구조면 번개·흙·풀은 특별잉크가 안 통한다** = 세28~29 경제가 룬 절반에만 산다. 고칠 방향 = `status_power`를 **「세기 배율」로 통일**하고 기본값을 balance로 옮기기(룬 데이터가 6장뿐인 지금이 제일 싸다).
- **허수아비(`dummy_target`)는 상태를 못 받는다** — `"enemies"` 그룹인데 `apply_status`가 없다. **베이스 연습장에서 반응을 시험할 수 없다**(마법 만들고 바로 쏴 보는 자리인데). `status_rules`를 core에 둔 이유가 공용인데 아직 적만 쓴다.
- 새 룬 3종(번개·흙·풀)은 **획득 경로가 없다** — codex 해금이 안 돼 게임에서 못 쓴다(검증은 exec로 열어서 했다). 조각→해독 경로(`decode_panel`)에 붙이는 게 자연스럽다.
