...
**곁가지 ② 룬 6종 복원**(`9ad6bef`, 사용자 *"룬을 일단 다 살려줘 6개로"*): 🔴 **기계는 처음부터 전량 살아 있었고 빠진 건 데이터 3장뿐**이었다(`rune_guide_verts` 6갈래·`status_rules` 반응표·`RUNE_TYPES`). 값은 세49 원본을 git(`43937c8^`)에서 그대로 꺼냈다. → **「반응이 물+불=증기뿐」이던 게 통째로 풀렸다**(젖음+번개=감전연쇄·젖음+흙=진흙·화상+풀=산불·풀=속박·흙=취약). 🔴 **복원이 테스트의 낡은 전제를 무너뜨렸다**: `test_decode_auto`가 흙을 「아직 실데이터가 없는 빈 자리」로 빌려 쓰다 `erase`로 **진짜 흙 룬을 지웠다**(로드 6→5, 그물이 잡음) → 원본 보관 후 되돌리기. ⚠ **룬이 전부 실재하는 이상 「빈 자리를 빌려 쓴다」는 수법은 못 쓴다**(진·문양 복원 때 같은 자리를 밟는다).
