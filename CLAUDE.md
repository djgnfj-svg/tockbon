...
🔴🔴 **직전 세션 = 61 「콘텐츠 카탈로그 리셋 — 각 1종만 남기고 사용자 큐레이션 체제」** (정본 STATUS「61」+ PROGRESSION.md):
사용자 확정 *"일반진+화염룬+확산문양 빼고 다 지워줘, 내가 하나씩 추가할게"* → **진 1(jin_single)·룬 1(rune_fire)·문양 1(radiate)**만 남기고 .tres 27장 삭제(진7·룬5·문양5·조각5·퀘스트 q06~q10) + 보스 3종의 관문 드롭 줄 제거. **기계는 전부 유지** — 발사 pattern/motion 8종 분기·문양 효과(BOLT_EFFECTS)·반응표·해독대·관문 판정·balance 필드가 데이터 없이 잠들어 있고, .tres가 돌아오면 그대로 산다. 🔴 **복원 레시피 = PROGRESSION.md 「콘텐츠 복원 레시피」절** — 진 복원 땐 **`glyph_slots`를 진마다 다르게 직접 정한다**(사용자 확정) · 「진마다 룬 갯수 다양하게」는 복합 룬 발사(`scratch_nested_design.md` ⑧)와 한 몸이라 방향만 기록(rune_slots를 미리 파면 「안 켠 배선」). 시드 = `rune_fire`+`jin_single` 2종(🔴 문양 시드는 안 넣었다 — **문양엔 해금 게이트 자체가 없다**: GlyphDef에 unlock_id 없음·패널 무필터라 어떤 문양 시드도 소비자 0 유령. D3 착수 때 스키마·판정·시드를 한 세트로). 옛 「룬 5줄 삭제 = 관문 가동 스위치」 과제는 리셋으로 자연 소멸 — 이제 관문은 만들면 즉시 산다. 세이브 호환 = 크래시 경로 없음(지워진 id 도안은 지팡이 폴백 패턴+기본 피해로 발사, 가드 안 심음 — 세50 원칙).
✅ 검증: 전 스위트 **22종**(목록 21+ring_book_jin) 그린+SCRIPT ERROR 0(리드 재실행) · 뮤테이션 검출(dev 3종: 관문 분기·BURST delay·해독 소비 + 리드 재현 1종) · 실게임 MCP 전 루프(새로하기→책 진 1셀 클릭→진·룬·문양 손그림→분석 100점→주입·자동 장착→실클릭 발사·마나 소모, 에러 로그 0) · 리뷰 치명 0. ⚠ **실게임 스트로크 주입 요령**(이번에 밟음): ① freeze(트리 pause) 중엔 `_gui_input`에 입력이 안 배달된다 — thaw 상태에서 push해라 ② 한 exec의 연속 모션 이벤트는 `Input.use_accumulated_input=false` 없이는 병합돼 획이 뭉개진다 ③ 보드 `guide_points()`는 모델 좌표 — 화면은 ×1.5(Control scale).
🔴 테스트 수술: 지워진 데이터의 기계 커버리지는 **in-memory 주입**으로 전량 유지(spell=진 7종·progression=관문·decode=조각·quests=사슬 — Db 레지스트리가 평범한 Dictionary라 주입→검증→제거). ⚠ **progression·decode 스캔 그물 3곳은 지금 자명 통과(검출력 0)** — 콘텐츠 복원 세션에서 뮤테이션으로 재점화 확인해라.
🔴 잔여: snake_boss `counter_rune=6`(GRASS) 사장(살아있는 약점은 gale FIRE뿐) · wand_fork/ring 지팡이 폴백으로 MULTI/NOVA 발사 형태는 진 없이도 열림(장비 축이라 삭제 범위 밖 — 큐레이션 때 인지) · q05가 퀘스트 사슬 끝 · 손맛 F5(세58-B 것) 이월.
