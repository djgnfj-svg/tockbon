...
- 🔴 **① 즉시 = 세83이 남긴 작은 것 셋** (전부 「파일 한 장」짜리, 순서 무관): ⓐ **번개·흙·풀 볼 애니 3장**(`takbon-art` → `ring_carrier.BALL_ANIM` 3줄 — 지금은 번개를 쏴도 **불덩이가 난다**. 세78 워터볼·윈드볼과 같은 방식 = 92×48 혜성 지오메트리에 색·질감만) · ⓑ **그 셋의 피격음 wav**(세56 이월 빚, `audio.gd` match 3줄) · ⓒ **사용자 세이브 codex 패치**(`rune_bolt`·`rune_earth`·`rune_grass` — 시드는 **새 게임에만** 걸린다. 🔴 **게임을 끄고** 넣어야 자동 저장이 안 덮는다. 백업 = `%APPDATA%/Godot/app_userdata/tockbon/save_BACKUP_s83`).
