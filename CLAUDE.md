...
🔴 **직전 세션 = STATUS.md 최상단**(세89 「세계관이 생겼다 — 망한 마법 마을에 문 하나」). 서사·이월·검증 기록은 전부 거기다.
...
- **모듈**: `src/menu`(타이틀) · `src/base`(마을·상점·공방·정제대) · `src/field`(챕터 보스방·적) · `src/actors`·`src/hud`(**공용** — 마을과 보스방이 같이 쓴다) · `src/props`(프롭 씬) · `src/drawing`(고리 조립) · `src/spell`(발사) · `src/core`(스키마·규칙 — 리드 전용)
  ...
  - 🔴 **`src/field`**: `boss_room.tscn`(**챕터 = 2400×2200 열린 숲**, 세88에 단칸방에서 넓혔다 — 유일한 원정 무대이고 **씬은 여전히 한 장**이다. 3장으로 늘리면 `mouse_filter`·`z_index`·레이어 함정을 그 수만큼 다시 밟는다) · `forest_enemy`(범용 적 몸, 이름만 옛 것) · `enemy_projectile` · `snake_body`/`snake_boss.tscn`. **적 수치는 전부 `data/enemies/*.tres`(EnemyDef) — 새 적 = .tres 한 장**이고 드롭은 `forest_enemy._die`가 굴린다.
    ...
    🔴 **방 크기·프롭은 `ChapterDef`가 안 나른다** — `Ground` rect·`Player` 스폰·나무 20·횃불 8·`BossGlow`가 **씬 하드코딩**이다(`ChapterDef`는 `mob_spawns`·`boss_spawn`·색·보상만). 방을 또 키우면 그 다섯을 같이 옮겨야 한다. ⚠ **나무는 적을 가린다** — 잡몹 스폰과 100px 이상, 플레이어 스폰·출구와 150px 이상 떨어뜨린다(y-sort가 없다).
  ...
  - 🔴 **`src/base` 마을은 「폐허 → 되돌림」이다**(세89): 건물이 잔해로 서 있다가 **퀘스트를 깨면 온전해진다**.
    ...
    🔴 **잠근 건물엔 그걸 여는 퀘스트(`QuestDef.reward_unlock`)가 반드시 있어야 한다** — 없으면 **영영 잔해**인데
    **기존 세이브엔 옛 값이 남아 F5로는 안 드러난다**(새 게임에서만 죽는다). 그물 = `test_quests_auto`의 전수 대조.
...
./Godot_v4.7.1-stable_win64.exe --headless --path . -s res://tests/test_base_auto.gd            # **베이스캠프 발사 배선** (세션 24 · 세83): 과녁 사거리 · 🔴**물리 레이어 계약**(내 몸/책상이 world면 진이 총구에서 죽는다 — 에러 없이 조용히) · [8] 숲길 · 🔴**[6]은 `balance.skip_drawing` 모드로 갈린다**(세56 「두 몸 복제 계약은 그물도 두 개」): 그리기 모드=미달 거부가 침묵 아님 · **폐지 모드=조립만으로 맺힘·펑 없음·부품 점수가 기준선 위** + 🔴**버튼 경로**([마법진 완성 ✦]이 리포트를 띄우나 — close() 자동 맺기만 재면 **버튼이 조용히 죽은 것**을 못 잡는다, 세83 뮤테이션 실측) · 🔴🔴**세89 [9] 문의 맥동**(빛 텍스처 실제 로드 + `energy`가 4초 창에서 출렁임 + **한 호흡 뒤에도 계속 돎**. ⚠ **한 점씩 비교하지 마라** — 두 샘플이 마루/골을 사이에 두고 대칭이면 값이 같아 「멈췄다」로 오독한다. 🔴 뮤테이션 실측 급소 둘: `set_loops` 제거해도 **진폭 검사는 통과**하고, 텍스처 경로를 깨도 **맥동 검사는 통과**한다 — 셋을 따로 재는 이유다) · 🔴🔴**세89 [10] 건물 게이트**(미해금이면 `monitoring == false` · `codex_unlocked` **시그널 경로로** 열린다 · `Dusk` 색조가 진행도로 움직이되 **성분 ≥0.85**(밝기 유지). 표를 베끼지 않고 `STATION_UNLOCKS`를 라이브로 읽어 전수 순회) · 🔴**허수아비 5개가 스폰에서 사거리(≈390px) 안**(세89에 캠퍼스를 통째로 옮겼다 — 스폰을 옮기면 허수아비도 같이 옮겨야 한다)
...
**F5 = 타이틀(`src/menu/title.tscn`) → 이어하기/새로하기 → 마을(`src/base/base.tscn`).** 마을: 책상 **E** → 고리 조립 책(오른쪽에서 진·룬·문양을 고르면 왼쪽에 밑그림) → **[마법진 완성 ✦]** → 리포트 → 맺으면 `GameState.ring_designs`로 들어간다. 연습장(허수아비 5): 마우스=조준 · **좌클릭=발사**(Space 아님) · **1·2·3 = 슬롯**(`EQUIP_SLOTS = 3`). 숲길 게이트 **E** → 챕터 선택 → **챕터 숲**(2400×2200 · 남쪽 입구에서 시작, 북쪽 끝에 보스). 잡몹이 재료·두루마리를 떨구고 **남쪽 출구 [E]로 언제든 귀환**한다(보스를 잡으면 그 자리에 포탈도 열려 되돌아갈 필요가 없다). 죽으면 `bag_lost` 후 마을 복귀. 모은 재료는 **공방(E)에서 조립 부품**으로 만든다.
