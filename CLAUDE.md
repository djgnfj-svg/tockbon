...
🔴🔴 **직전 세션 = 63 「손맛·아트 개편 — 히트 플래시 셰이더·피격 애니·그림자·먼지·카메라」** (정본 STATUS「63」):
사용자 *"아트하고 손맛을 잡고 싶어"* → 기획 4결정(피격 애니=플레이어+보스만 도트·효과음 이번 제외·그림자 전원·발사 반동 포함) → architect 설계(scratch, 읽고 삭제) → **art∥dev 병렬 → animator → reviewer**. **① 히트 플래시** = `src/actors/hit_flash.gdshader`(mix-to-white — modulate 곱셈은 어두운 픽셀이 안 하얘진다) + 🔴 **modulate 소유권 3파전 청산**: 팝·텔레그래프가 셰이더 uniform(`flash_amount`·`telegraph_amount`)으로 이사 → modulate는 **rgb=상태 틴트·a=분산 2축만**("복귀 목표=_status_tint" 곡예 소멸). 🔴 함정 둘 = 셰이더에서 TEXTURE 직접 샘플 금지(상태 틴트 침묵사)·ShaderMaterial은 **per-instance**(공유하면 한 마리 플래시=전원 플래시). 허수아비도 동일 이식(세56 두 몸 파리티 — 그물도 [10]으로 따로). **② 피격 애니** = 신설 `EventBus.player_hurt(amount, source_pos)`(발신=damage_player 한 곳 — juice·audio의 "hp 직전값 비교" 사본 청산 + 🔴 hp 0 뒤 무발신=사망 연출 스팸 방지) · 플레이어=시트 144×192 오른쪽 덧대기(3열 hurt, 🔴 기존 좌표 불변이 걷기 애니 생명줄)+`_face_mouse`가 매 프레임 덮는 성질 역이용(가드만, 복구 코드 0) · 보스=`params.hurt_sprite` 별도 스트립("새 적=.tres 한 장" 유지, 1프레임=타이머 홀드+세대 토큰). **③ 그림자** = `shadow.gd`(GradientTexture2D radial — 절차적 VFX 예외) forest_enemy **자동 부착**(새 적 공짜)+플레이어+뱀 마디 18 · z 음수 금지, 몸 아래=트리 순서. **④ 먼지** = `dust.gd` juice 형제(부모 공개 API 폴링 — player.gd 무수정), 걷기 주기 퍼프+구르기 부채꼴 버스트. **⑤ 카메라** = juice 확장(trauma² 감쇠+방향성 킥 5px+발사 반동 2px — `ring_cast_requested`의 aim이 이미 실려 있어 core 무변경), 픽셀 스냅 안 함(1~2px 킥이 계단 됨).
✅ 검증: 전 스위트 **23종**(+test_feel_auto 그물 10) 그린+SCRIPT ERROR 0 · 뮤테이션 10종 검출 실증(emit 제거·material 공유·telegraph 되돌리기·z 음수·킥 부호·사망 가드·허수아비 플래시…) · 실게임 MCP 전 항목(플래시가 어두운 외곽선까지 하얘짐·텔레그래프 오렌지·hurt 프레임·마디 그림자 18개 Ground 위=세54 자리 통과) · 리뷰 치명 0 · 사용자 확인(*"그림자 보임 먼지보임"*).
🔴 **실게임이 잡은 것 2**(헤드리스·테스트 전부 그린이었다): ① 플레이어 그림자가 로브에 **통째로 가려짐**(r9·y4 → r12·y10로 굽음) — "존재"는 테스트가 재도 "보인다"는 못 잰다 ② 먼지 퍼프가 겹쳐 한 점(반경·퍼짐·수명 키우고 **축소형→팽창+페이드형**으로 뒤집음).
잔여: 연출 수치 전부 const — 세부 손맛(히트스톱 길이·킥 크기·텔레그래프 0.6 세기·hurt 홀드 0.15)은 사용자 F5로 조임 · 허수아비·NPC는 그림자 스코프 밖(원하면 shadow.gd 한 줄) · 뱀 마디 플래시 리플=설계 §A-4에 있으나 의도적 미구현(폴리시).
