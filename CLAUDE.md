...
🔴 **직전 = 80 「M1 F5 확인 + 밀린 잔재 4덩어리 커밋」** (정본 STATUS「세션 80」): 사용자 *"m2진행하자"* → 착수 킷대로 **F5가 먼저**라 게임을 띄웠고 확인 = *"이거 되긴하네"*(층 순서가 실게임에서 실제로 다른 마법을 낸다) → *"커밋하고 다음세션에서 m2"*. 세74~78 미커밋 잔재를 세션별 4커밋으로 청산(전 스위트 25/25 확인 후). **코드 변경 0 — 아래 세79 상세가 여전히 현행이다.**

🔴🔴 **직전 구현 세션 = 79 「진별 해석 M1 — 층(밴드) 순서 = 연산 순서」** (정본 STATUS「세션 79」 · 설계 `docs/takbon-design/jin_interpretation_design.md` · memory `takbon-jin-layers-m1`):
사용자 *"m1부터 개발진행"*. **조립이 진짜 「수식」이 된 세션** — 같은 재료라도 **어느 순서로 감쌌나**가 마법을 통째로 바꾼다.
**한 문장**: 진의 층(밴드)이 **안→밖 = 연산 순서**가 됐다. 전개형 문양(발산·응집)은 전개 명령을 **만들고**, 새 **변형형** 문양(확산⋔·폭발∗)은 안쪽 층의 결과를 **받아 바꾼다** → `폭발(확산(불))` ≠ `확산(폭발(불))`.
🟢 **이행이 얕았다 — 층은 이미 있었다**(세78 architect 실측이 맞았다): `flatten_bands`가 밴드를 8칸 하나로 뭉개 **순서를 버렸을 뿐**이고 `RingDesign.rings`는 이미 배열의 배열, 세70 밴드는 이미 데이터·기하·UI로 층을 갖고 있었다 → **스키마 변경 0**. 🟢 `ring_carrier`는 `_ring`을 **해석 안 하고 나르기만** 해서(payload) 다겹을 그대로 실어도 **캐리어 무변경**.
**확정 규칙(사용자)**: **확산 = 복제 산개**(각 갈래를 n개로 복제해 부채꼴·갈래마다 세기↓·합은 ↑) · **폭발 = 융합 광역**(안쪽을 하나의 큰 폭발로 합침). 🔴 **폭발 반경이 안쪽 갈래 수에 비례**(`blast_radius_per_branch`)하는 게 **순서를 눈에 보이게 만드는 자리**다 — ❌ "각 갈래를 각각 터뜨림"으로 두면 두 순서가 둘 다 "작은 폭발 여럿"이 돼 **순서 실증이 실패**(뮤테이션으로 실증). **씨앗** = 감쌀 안쪽이 없으면 문양이 **룬 자체**를 감싼다(탄 1발) — 변형형이 있을 때만 생겨 **빈 진은 여전히 전개 0**. **세기 = 그 층에서 그 문양이 차지한 칸 수**.
**한 일**: `Enums.GlyphCode` SPREAD=6·EXPLODE=7 **끝에만 덧붙임** + 🔴 계열 판별 단일 소스 `MODIFIER_GLYPHS`/`is_modifier_glyph()` · `ring_spell_system` 층 해석기(`_as_layers`·`_deploy_now` 층 루프·`_apply_layer`·`_apply_modifier`·`_spread`/`_explode` → 명령 목록 → `_spawn_cmd`) · 신설 `RingBoard.layer_rings`(flatten 대체) · `build_assembly` 다겹 · `RingDesign.filled_count` 다겹 · balance 변형형 7개 · **위임**: `glyph_guide_pts` 확산·폭발 갈래(takbon-dev)·신설 `blast.gd`/`.tscn` 원샷 광역(takbon-dev) · `.tres` 5장(문양2·문양-고리2·🔴`jin_plain_g2` **band_count=2 = 2등급**).
✅ 검증: 전 스위트 **25종** 그린+SCRIPT ERROR 0(신설 `test_jin_layers_auto`) · 🔴 **뮤테이션 5/5 검출+전부 원상복구 diff 확인**(폭발 융합 제거 2건·층 루프를 `rings[0]`만 3건·씨앗 제거 6건·빈 밴드 건너뜀 2건·`build_assembly`를 flatten으로 1건) · 🔴 **회귀 근거 = "밴드 1개면 `layer_rings` 층0 == `flatten_bands`"**(칸 단위) — 살아있는 진이 전부 `band_count=1`이라 이게 곧 **저장 도안 발사 무변경**의 증명.
🔴 **함정(실제로 밟음)**: `_as_layers` 정규화가 **진입점 두 곳**(`_on_ring_cast`·`_deploy_now`)에 다 있어야 한다 — 한쪽만 두면 **기존 테스트가 옛 8칸 인자로 `_deploy_now`를 직접 부르다 `Invalid cast`로 죽는다**(세23 「테스트가 내부 API를 옛 인자로 부른다」 그대로 재발). · **빈 밴드도 층 자리를 지켜라**(건너뛰면 감쌈 깊이가 조용히 밀린다) · **1등급(1겹)은 감쌀 순서 자체가 안 생긴다**(그래서 2등급 진이 M1 전제) · `blast`는 **warmup 1프레임** 없으면 빈 overlap을 때리고 조용히 사라진다(원샷은 두 번째 기회가 없다).
**곁가지 = Tab 「마법진」 탭**(`fc8f5b5`): 사용자 *"내 마법진을 볼 수 있는 시스템이 있나?"* → **없었다**(HUD 다이어그램은 8칸 점 원 하나라 층을 못 보여주고 책은 그리는 동안만, 탭은 스텁). 순서 신호 4중(동심원·수식 `폭발(확산(불))`·층 목록·라벨)·넘침은 **층 줄 수 캡**으로 구조적 차단(스크롤=새 입력 경로=세25 자리 증가라 각하) · core `RingDesign.layer_summary()`/`has_modifier_glyph()`(⚠문양 이름·색은 core에 안 담는다=모듈 경계) · 🔴HUD가 위력에 `size`를 안 실어 탭과 **숫자가 갈라져 있던 것 정정** + 변형형이면 두 곳 다 "갈래당 위력" · 신설 그물 [9]=수식(뒤집기 뮤테이션 3건 빨감 — 표시가 뒤집히면 **자기 도안을 반대로 이해한 채 조립한다**).
🔴 **다음 = M2 (룬 2개 + 진 규칙 ②)** — **착수 킷이 `docs/takbon-design/jin_interpretation_design.md`「M2 착수 킷」에 실측으로 박혀 있다**(합의할 결정점 4·손댈 파일:줄 표·재활용 자산·위험·깨지면 안 되는 그물). M1과 달리 **스키마 변경 필요**(룬이 지금 정수 하나: `ring_assembly.gd:36`·`ring_design.gd:17`). 🔴 **M2 전에 M1을 F5로 보는 게 순서** — 감싸진 룬 뭉치가 ②의 입력이라 M1 문제를 M2가 **곱해서** 물려받는다.
잔여: ✅ **F5 동작 확인됨(세80)** — 사용자 *"이거 되긴하네"*. 층 순서가 실게임에서 실제로 다른 마법을 낸다. ⚠ **세부 소감은 미수집**(2겹 긋기가 지겨운가·순서 차이가 눈에 띄는가 — 그래서 M2의 「자리마다 층 따로 vs 공용」이 여전히 미결). 🔴 **세80 실측: 두 순서의 총 세기가 1.53으로 정확히 같다**(`_explode`가 합을 보존·`_spread`가 나눠서) — 순서는 「어느 쪽이 센가」가 아니라 **「어떻게 생겼나」만 바꾼다**. 덮는 범위는 오히려 흩어진 쪽이 넓다(98px vs 73px)라 문서의 *"폭발(확산)=넓은 광역 한 방"* 서술과 **수치가 반대**다. balance 7개 전부 **시작값** · **획득 경로 미설계**(사용자 *"일단 만들기만 하면 됨"* — 새 3종은 `GameState._seed_starting_unlocks` **임시 시드 3줄**, ⚠ ch1이 `gr_radiate5`로 차 있고 codex 해금 경로가 **챕터당 하나**라 자리 부족. 경로 붙이는 세션이 그 3줄을 걷는다) · `flatten_bands`가 F6 슬라이스 전용 레거시로 남음(**로직 두 벌** = 갈라짐 씨앗) · M2(룬 2개+·진 규칙②) 미착수.
