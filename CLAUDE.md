...
🔴 **다음 세션 = ⓐ 사용자가 직접 쏴 보고 그려 보기**(연발 간격·부메랑 선회·나선 진폭·큰 진 발수 — 헤드리스도 스샷도 못 잼) → ⓑ **세45 숙제가 아직 남아 있다**: forest_t2 실제 기획(별도 손맵·🔴하강 시 회복 스킵 = 지금 `forest._ready`가 진입 때 만HP라 "깊이=위험"이 약함) → ⓒ **룬 상태이상 배선**(아래 참조). ⚠ 성급히 코드 얹지 말고 플레이부터.
