...
🔴🔴 **직전 = 85 「감사 결정 15건 확정 + 덜어내는 세션 + 검증 수단 전환」** (정본 STATUS「세션 85」 · 🔴🔴 **착수 킷 = `scratch_decisions_s85.md` — 다음 세션이 그대로 쓴다, 지우지 마라**):
...
🔴🔴 **다음 세션 = (b) 신설 4건 + 문제 3건** (사용자 확정). **착수 킷에 좌표까지 박혀 있다 — 다시 조사하지 마라**: ① **슬롯 교체 UI**(오늘 실증 — 맺은 도안 6장인데 슬롯 그대로 = 「맺었는데 못 쓴다」) · ⑤a **「무성함」 전용 상태**(⚠ `Enums.Status`는 연속 0~9라 **끝에 추가**) · ⑫ **`inventory_changed`**(창고와 가방이 한 신호라 저장 트리거로 못 쓴다) · ⑭ **완성 연출**(세70 이후 **15세션째** 판 연출이 0개였다).

🔴 **지지난 = 84 「코드베이스 전면 감사 + 「조용히 죽던 자리」 청산」** (정본 STATUS「세션 84」 · 감사 보고서 `scratch_audit_s84.md` — **결정은 세85에 전부 소진됐다**):
