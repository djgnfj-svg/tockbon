...
- **모듈**: `src/menu`(타이틀) · `src/base`(마을·상점·공방·정제대) · `src/field`(챕터 보스방·적) · `src/actors`·`src/hud`(**공용** — 마을과 보스방이 같이 쓴다) · `src/props`(프롭 씬) · `src/drawing`(고리 조립) · `src/spell`(발사) · `src/core`(스키마·규칙 — 리드 전용)
  ...
  - 🔴🔴 **세90: 마을은 「캐릭 + 마법문 + 마법 제작대(책상)」 셋이다**(사용자 확정). 남은 노드 = `Player`·`ForestGate`·`Desk`·`Npc`(길잡이)·`Monument`(영구 잔해)·`Dummy1~5`(연습장). **빠진 것** = 공방(`Craft`)·정제대(`Refine`)·매점(`Shop`) + 장식 22개(나무·램프·벤치·울타리).
    ...
    🔴 **배치 계약이 세89와 정반대가 됐다**: 세89는 *"온전한 건물은 첫 화면 **밖**"*(설계 §4)을 쟀는데, 마을이 셋으로 줄자 유일한 제작대가 1200px 밖이면 「빈 공터」가 된다 → **모으는 쪽**으로 뒤집었다. 지금 계약 = 「문·책상·길잡이가 첫 화면에 **통째로** 든다」(`test_base_auto [11]`).
...
🔴🔴 **세90: 마을에 있는 것 = 문 · 책상(서고) · 길잡이 · 기념비 · 허수아비 5. 그게 전부다** — 공방·정제대·매점은 **마을에서 빠졌다**(패널·레시피는 살아 있고 여는 [E]가 없다). 그래서 아래 *"모은 재료는 공방(E)에서"*는 **지금 F5로는 못 한다**. 다섯이 **첫 화면에 통째로** 든다.
