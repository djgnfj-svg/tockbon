...
>
> ✅ **세션 42·43 = STATUS 최상단·memory가 정본** (여기 최상단엔 안 옮김). 42 = 인벤 장착 5부위(지팡이·부적
> 되살림+모자+소지품 탭 클릭 장착, memory `takbon-equipment-5-slots`). 43 = **[!] 새 목표 마크를 중간
> 게임에서도 살림** — 세션40 턴인 모델에서 [!]가 q00·옛세이브에서만 뜨던 걸, **접수(seen)=정산이 아니라
> 시트 열람**으로 옮겨 정산 후 [!]가 남아 [Tab] 유도되게(신설 `EventBus.quests_seen`·`tab_panel` 열람 접수·
> base 정산 분기가 시트 안 열고 [!] 남김) + **마우스 조준 향함**(`player._face_mouse` — 몸이 커서를 봄, 손맛 확정 남음).
>
