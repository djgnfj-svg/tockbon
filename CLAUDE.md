...
> ⚠🔴 **세83 이후 GDD는 「그리기」에 관해 낡았다** — 사용자가 **그리기 폐지**를 확정했는데(아래 직전 세션) GDD는
> 아직 탁본·손 긋기를 코어로 서술한다. **개정하려면 허락을 받아야 하고, 폐지가 굳을 때까지 일부러 미뤘다.**
> 그때까지 「그리는 것 = 심장」 서술은 **현재 상태가 아니다**(나머지 서술은 유효).
...
> ⚠ **숲 원정은 세58-B에 은퇴했다**(챕터 보스방으로 대체 — `src/field/forest.tscn`은 남아 있지만 진입 경로가 없다).
> 🔴 **기록 규칙: 직전 세션만 상세, 그 전은 아래 「한 줄 지도」로 내려보낸다** — 이 절이 길어지면 정리 신호.

🔴🔴 **직전 = 86 「세85가 결정만 하고 안 만든 것을 전부 만들었다 + 층 = 띠」** (정본 STATUS「세션 86」):
사용자 *"이번 세션에서 하려고 했던 거 진행해줘"* → 세85 착수 킷의 **(b) 신설 4건 + F5 문제 3건**을 집행하고, 중간에 사용자가 **마법진 그림을 새로 기획**해(*"그 선에 문양이 있는 게 아니라 선 사이에 있었으면"*) 그것까지 설계→구현→F5로 착지시켰다. ✅ **착수 킷은 전부 소진돼 삭제했다**(`scratch_decisions_s85.md`·`scratch_audit_s84.md`).
**신설 4건**: ① **슬롯 교체 UI** + `GameState.equip_design(slot, design)`(세85까지 슬롯 3칸이 차면 **그 뒤 도안은 영원히 못 썼다** — Tab「마법진」탭에서 보관 행 클릭 → 슬롯 클릭·1·2·3) · ⑤a **「무성함」(`Enums.Status.OVERGROWTH`)** 전용 상태(젖음+풀. 지속 5.0 > 진흙 3.5 > 덩굴 2.0 · `무성함+불=산불`도 정함 — 비우면 **불이 아무 신호 없이 덮는 함정**) · ⑫ **`EventBus.inventory_changed`** = 자동 저장 트리거(`resources_changed`는 `add_to_bag`도 쏴 못 썼다) · ⑭ **완성 연출 `RingBoard.play_finish()`**(세70 이후 **15세션째** 0개였다 — 금빛 파도가 안→밖으로 훑어 **M1의 「층 = 연산 순서」를 맺는 순간에 복습**).
🔴 **⑥ codex clear는 착수 킷대로 하면 안 됐다** — clear만 넣으면 시드가 세이브에 없을 때 그대로 사라져 **빌드가 시작 해금을 늘려도 옛 세이브에서만 룬이 증발**한다. `clear() → seed_codex_unlocks() → 세이브 얹기` 순서로 만들고 그 순서를 그물로 박았다. **착수 킷도 코드에서 의도를 확인하고 집행해라.**
🔴 **「층 = 띠」**(사용자 기획): 문양이 동심원 선을 가로지르던 걸 **선을 문양 바깥으로 밀어** 해결. `band_lane`(띠 경계, **모티프 크기에서 파생**) + `band_edge`(**실제로 그리는 선 = 층을 가르는 칸막이 하나씩** — 둘 다 그렸더니 원이 5개가 돼 사용자가 *"선이 너무 많음"*). ⚠ **세81에도 같은 지적이 있었고 그땐 「그릴 때 선을 끈다」로 피했다**(주석에 남아 있었다). 미리보기 아이콘도 같은 병이었다 — **아이콘이 판을 베끼고 있었다**(각도 루프 사본 + 모티프 크기 `radius*0.42` = 판의 3배) → `ring_icon_geom`이 정본을 부르게. 🔴 비율을 정직하게 맞추자 모티프가 **4px 점**이 돼 안 읽혀서 `ICON_MOTIF_ZOOM`으로 **모티프와 띠를 같은 배로** 벌렸다(띠는 **확대한 문양의 실제 점 범위**에서 파생 = 자기교정).
🔴🔴 **이번 세션의 교훈 셋**: ① **착수 킷도 그대로 집행하면 안 된다**(⑥) · ② 🔴 **뮤테이션은 「적용됐는지」부터 확인해라** — python 치환이 조용히 실패했는데 확인을 안 해 **"검출 0"이라는 거짓 결론**을 냈다(세85의 *"빨간불도 도구가 틀리면 근거가 안 된다"*가 **같은 세션 안에서 재현**). 이후 `grep -c` 적용 확인을 붙이자 6건 검출 · ③ 🔴 **내가 만든 검사가 틀릴 수 있다** — 아이콘 「배치 각도」 검사가 반경 의존 편차로 거짓 빨강을 냈다(검사 자체가 틀렸다). ⚠ 반대로 UI 갈래 뮤테이션은 **그물이 헐거워** 통과했다: 배열에 직접 대입해도 값이 같아 보였고 **`equipment_changed` 발신 횟수**를 재고서야 빨개졌다 — **「결과 값이 같다」는 「같은 길로 왔다」가 아니다.**
✅ 검증: 전 스위트 **28종 그린 · SCRIPT ERROR 0 · 검증 목록 기계 대조 완전 일치**(**새 테스트 파일 0** — 전부 기존 파일에 얹었다) · 리드 뮤테이션 **core 4/4 · 무성함 4 · 완성 훅 1 · equip 우회 2 · 띠 6 · 아이콘 16**(전부 `cp` 백업 + md5 대조) · **F5/MCP 리드 직접**(실세이브가 마침 「도안 6장·슬롯 만석」이라 문제 상황 그대로 확인 — 실제 마우스 클릭으로 슬롯 교체 성공 · 리포트 잘림은 세85가 본 **정확히 그 문자열**로 재현). ⚠ 세이브는 건드리기 전 백업하고 끝나고 복원했다.
🔴 **다음 세션 이월**: **문양 모티프를 예쁘게**(사용자: *"문양이 이쁘면 좋겠는데 그건 나중에"* — 🔴 판·아이콘·책 셀이 **전부 `RingBoard.glyph_guide_pts` 한 곳**에서 나오므로 그 함수만 고치면 셋이 같이 바뀐다. 세83 「문양 어휘 확장」과 같은 자리) · Tab 마법진 탭 **머리글이 탭 버튼과 겹친다** · 완성 연출이 **은은하다**(`FINISH_*`) · 무성함은 **감속으로 차별화가 안 된다**(상한 포화 — 답은 `status_overgrowth_sec`) · **접힌 보관 도안은 UI로 손댈 수 없다**(정렬·페이지 결정 필요).

🔴 **지지난 = 85 「감사 결정 15건 확정 + 덜어내는 세션 + 검증 수단 전환」** (정본 STATUS「세션 85」):
사용자 *"해나씩물어서 정하는것부터 하자"* → 세84가 남긴 결정 대기를 하나씩 물어 **15건 전부 확정**(원래 12 + 작업 중 3). 커밋 `a4e3028`(코드 57파일 **+1007/−2529**) · `c6f64f8`(문서).
🔴 **성격이 갈렸다 — 15건 중 7건이 은퇴·청소, 새로 만드는 건 3건뿐**이라 **덜어내는 세션**이 됐다. `ring_board.gd` **1673→1255줄(−25%)**.
주요 결정: **지팡이의 「발사 형태」 축 은퇴**(형태는 이미 진·문양·층이 답하는 질문 — 세60 문양본 흡수와 같은 논리. 대신 `wand_speed_mult`·`wand_mana_mult`를 **소비 코드와 함께** 붙여 제작이 순수 손실이 아니게) · **`glyph_slots` 은퇴**(진의 개성은 `band_count`·`rune_slots`) · **취약 이중 증폭 = 사고** · **같은 룬 2개 허용**(실측 +2.6%라 지배 전략 아님) · **해독대 + q05 한 세트 은퇴** · **per-piece 유물 청소** · **F6 벤치 은퇴** · **소비자 0 필드 접기**.
🔴 **되돌릴 수 있어야 하는 것은 보존했다** — `enter_combined_trace`·`coverage`/`accuracy`·펜 보정(세83 스위치를 되돌리면 손 긋기가 살아난다). 청소 대상은 **세70에 은퇴한 per-piece**로 스위치와 무관하다.
🔴🔴 **이번 세션의 교훈 셋 — 전부 「초록불은 근거가 아니다」의 새 판**:
① **`.tres`는 두 가지로 다르게 죽는다**(실측 정정) — 스키마에 없는 **프로퍼티 이름은 조용히 무시되고 리소스는 산다**. **값** 파싱 실패(3인자 `Color`)만 통째로 죽인다. 기존 함정 문구가 둘을 뭉뚱그렸다.
② 🔴 **`git checkout <file>`로 뮤테이션을 되돌리면 남의 미커밋 작업이 날아간다**(리드가 밟아 3파일 소실). **진짜 교훈은 그 다음** — **복구 뒤 전 스위트가 그대로 그린이었다**(날아간 게 「아직 그물이 없는 신규 기능」이라 초록불이 소실을 덮었다). 반면 **범위 밖이라 안 넣을 뻔했던 보상 이관 그물은 30초 만에 소실을 지목**했다.
③ 🔴 **빨간불도 도구가 틀리면 근거가 안 된다** — 리드가 fail 수를 `fail=N`으로 셌는데 이 프로젝트는 `TEST_*_OK`/`_FAIL`로 찍는다 → **뮤테이션 3건이 전부 「검출 실패」로 보였다.** 멀쩡한 그물을 헐렁하다 판정하고 에이전트의 정확한 보고를 뒤집을 뻔했다.
🔴 **검증 수단이 바뀌었다** — 사용자 확정 *"mcp로 니가 확인해 앞으로도 니가 확인해"*. 세71~84의 「리드는 에디터를 켜지 마라」가 **뒤집혔다**(절차·함정 5개 = 아래 「에디터·MCP」 절). **F5 11건 중 10건 통과**: 🔴창 닫기 저장(실행 중 세이브를 지우고 `WM_CLOSE` → 재생성 + 내용 대조) · 리포트 카드(「완성도 0%」가 사라지고 **점수 근거**가 들어감 = 세83 *"허전하다"* 해결) · 판 룬 마커 2곳 · HUD 룬 점 2색 · 보스방 사망 HP · 콘솔 0건 · 지팡이 축(마나 16→11.2) · 적 스폰 5마리 · **좌클릭 발사 도달 실증**(세25 버그 없음).
⚠ **F5가 새 문제 3건을 잡았다**(전부 문구·레이아웃, 좌표는 착수 킷 §B): 「다시 그리기」 버튼이 폐지 모드에 남음 · 리포트 「조립:」 줄 잘림 · 진 탭 *"열리는 칸이 진마다 다르다"*(**우리가 방금 은퇴시킨 축** — 감사 T4 즉시 재현).
✅ 검증: 전 스위트 **28종** 그린(F6 은퇴로 29→28) · SCRIPT ERROR 0 · 리드 뮤테이션 **3/3** + md5 대조 · **검증 목록 기계 대조 완전 일치**(유령 항목 청산 — 세84가 박은 한 줄이 값을 했다) · `test_decode_auto` → **`test_rune_unlock_auto`** 개명.
✅ **세85가 넘긴 (b) 신설 4건 + 문제 3건은 세86에 전부 소진됐다**(위 참조). ⚠ 세85 F5가 남긴 실측 함정 하나는 아직 유효하다: **`Enums.RuneType`은 `[0, 2, 3, 4, 5, 6]`으로 값이 연속이 아니다**(1은 은퇴 자리) — 인덱스로 착각하면 「없는 룬」이 만들어진다. **enum은 값을 먼저 확인해라.**

🔴 **세84 「코드베이스 전면 감사 + 「조용히 죽던 자리」 청산」** (정본 STATUS「세션 84」 · ⚠ 감사 보고서 `scratch_audit_s84.md`는 **결정이 세85·86에 전부 소진돼 삭제했다** — 필요하면 git 이력):
사용자 *"ultracode 로 코드 점검해줘"* → *"그냥 치워도 되는것들은 진행해주고 내 허락이 필요한것만 알려줄래?"*.
워크플로 7갈래 탐색 → **갈래마다 적대적 검증자**(불확실하면 REFUTED로 기울이게) → 발견 75건 중 **12건 반박 탈락·63건 확정** → 구현 8갈래 위임. 커밋 `e8d3313`·`bf74d06`·`4854a96`·`9238489`.
🔴🔴 **가장 큰 성과 = 최다 재발 버그가 처음으로 헤드리스에 걸렸다**(신규 `test_scene_contract_auto`): 세25·26에 **두 번 밟은** `mouse_filter` 버그 — CLAUDE.md가 *"헤드리스는 클릭이 닿는지도 모른다"*고 못 박았지만, **실패 형태가 늘 「씬에서 그 줄이 빠진다」라서 `.tscn` 프로퍼티로 잴 수 있다**(런타임 히트 테스트 불가와 별개다). 씬을 **스캔**하므로 새 씬이 자동으로 든다. 🔴 **F5의 대체가 아니라 1차 방어선.**
🔴 **사용자가 실제로 잃고 있던 것 = 종료 시 저장이 없었다**(트리거 셋 + 실시간 12분, 종료 훅 0건 → 마을 작업이 창 닫으면 **에러 없이 롤백**). ⚠ **이 항목은 헤드리스 검출력 0** → F5 필수.
🔴 **세83 스위치의 안전장치가 검증되지 않았다**: 그리기 갈래가 조기 return으로 **한 줄도 안 돌았고**, 폐지 모드의 **유일한 성장축** `assembled_score`는 tests/에서 **0건**(부품 항을 0.0으로 내려도 전 스위트 그린 = 원정 보상이 전투에 영향 0인데 아무도 모른다). 🔴 실측: **`skip_drawing()`이 컴파일 타임에 굳어**(세24 함정) 한 프로세스에서 두 모드를 못 돌린다 → **모드 무관 층**에서 계약을 잡았다. 성장축은 **단조성**으로(세82 교훈).
🔴🔴 **이번 세션의 방법론 교훈 셋** (다음 세션이 그대로 쓸 것):
① **적대적 검증이 값을 했다** — 반박된 12건을 그대로 실행하면 **살아있는 기능이 죽었다**(`get_analysis()`는 라이브가 부른다 · `flatten_bands` 호출은 `ring_board.gd:721`이 **명시한 합법 호출자** · 중복 룬은 지배 전략이 아니다 실측 +2.6%).
② 🔴 **그런데 감사 보고서 자체도 틀린다 — 보고서도 초록불처럼 근거가 아니다.** 두 건을 구현 갈래가 실측으로 뒤집었다(#30 *"어떤 테스트도 못 잡는다"* → **4건 빨감** · #20 *"3종만 싣는다"* → **8종 전부**). **각 갈래에 실측을 시켜라.**
③ 🔴 **경고문에 의존하지 말고 대조를 기계로 해라** — `takbon-verify` 스킬에 **지난 세 세션의 심장 테스트가 전부 빠져 있었다**(M1·M2·M3). 스킬 자신이 그 경고를 46줄에 적어 뒀는데도 **세 세션 연속** 났다. 세션 종료 전 한 줄: `for t in $(ls tests/*_auto.gd); do grep -q $(basename $t) CLAUDE.md || echo 누락; done`(스킬에도). 박은 직후 실제 누락을 즉시 잡았다 — **경고문 3세션 vs 기계 1회.**
🔴 **구조적 테마 8개**(보고서 §테마 — 개별 버그가 아니라 왜 이 종류가 자꾸 생기나): T1 모드 분기를 일부 자리에만 넣고 **소비 지점 목록을 안 만든다** · T2 **은퇴를 선언 없이 두어 죽은 몸이 살아있는 그물을 갖는다**(= 「27종 그린」이 라이브 경로의 근거로 **과대평가**된다) · T3 소비자가 사라진 데이터 필드가 **거짓 손잡이**로 남는다 · T4 주석이 계약인데 코드와 함께 안 늙는다 · T5 파생 대신 복제(**문구·좌표는 사본이 아니라는 무의식 예외**) · T6 실패를 로그 없이 삼킨다(🔴 **엔진 ERROR·`push_warning`은 `SCRIPT ERROR` grep에 안 걸린다** — 세50 바람 룬이 두 세션 죽어 있던 이유) · T7 **「임시 시드」 딱지가 그물을 안 세우는 면허로 쓰인다** · T8 축이 1→N으로 늘 때 **표시부가 뒤처져 「쏘는 것 ≠ 보이는 것」**이 된다.
✅ 검증: 전 스위트 **29종**(+2 신규) 그린 · SCRIPT ERROR 0 · 크래시 0 · **뮤테이션 잔재 0(md5 대조)** · 커밋 후 재확인 그린. 🔴 리드 뮤테이션 3건이 결론: 성장축 무력화 → **6건 빨감**(감사 시점 그린) · `mouse_filter` 회귀 → **노드를 이름·크기까지 지목** · 표시부 회귀 → **4건 빨감**. ⚠ **정정**: `test_save_auto`의 「ObjectDB 누수」는 리드의 **오독**(save_test 잔재에 따라 갈리는 값 — clean start에선 HEAD와 동일). ⚠ `test_chapter_auto` 종료 segfault는 **기존 flake**(HEAD 8회 중 1회).
⚠ **세84가 넘긴 것은 세85에 전부 소진됐다** — F5 6건 확인 완료 · 결정 11건 확정 · 볼 애니 3종 커밋(`7792bd3`).
  🔴 **남은 잔여 둘**: `_ai_charge`·`_ai_hover`·`_ai_boss_*` 폴백이 실값 사본이다(hound·mist는 스폰 0곳이라 아직 무해 — **살릴 때 같이 고쳐라**) ·
  `display_name`이 **전 도안 "고리 마법진"**이라 1·2·3 키로 무엇을 쏘는지 **이름으로는 구분이 안 된다**(#12 남은 절반 — 세85 F5에서 눈으로 재확인).

🔴 **세83 = 그리기(탁본) 폐지 — 이 게임의 코어 축이 뒤집힌 세션** (정본 STATUS「세션 83」 · memory `takbon-drawing-abolished`):
사용자가 세 번 반복해 확정(*"조립이 맞음 모든걸 조립으로"*), F5 반응 *"쉬워서 좋은거 같아"*. memory `takbon-core-fun-drawing`(세16 「핵심 재미 = 그리는 것」)과 세81 발언을 **뒤집는다**.
🔴 **구현 = 스위치, 코드는 한 줄도 안 지웠다** — `balance.skip_drawing`(기본 **true**)을 false로 되돌리면 손 긋기가 그대로 돌아온다(채점기·펜·잉크 전부 그 자리에 있다).
**그래서 청소할 때 「되돌릴 수 있어야 하는 것」과 「세70에 은퇴한 per-piece」를 갈라야 한다** — 세85가 그 구분으로 청소했다. 위력 = `RingPower.assembled_score(문양수, 층수)`.
🔴 **아직 안 건드린 것**: `docs/GDD.md` 개정(🔒 사용자 허락 필요 — 폐지가 굳을 때까지 일부러 미뤘다) · 「탁본」 이름 변경(사용자: *"미정인 이름이라 변경해도됨"*) · 채점기·펜·잉크 경제 청소.
🔴 **곁가지 둘**: 「진」 탭 셀을 **진 모양 + 룬 자리만**으로 정리(`jin_icon_paths`) · **룬 6종 복원**(기계는 처음부터 살아 있었고 **데이터 3장뿐**이었다 — 반응이 통째로 삶).
📄 **설계 착지·구현 대기** = `docs/takbon-design/jin_shape_image_design.md`(진 모양을 **PNG로 등록**). 🔴 폐지가 굳으면 훨씬 단순해진다 — 점열 추출이 불필요해 레퍼런스급 복잡한 마법진이 비로소 가능.
🔴 **② 다음 큰 축 = 문양 어휘 확장** — 폐지하면 조립이 **유일한 재미 축**이라 부품이 얇으면 곧 「얕다」가 된다. 세82가 토대를 깔아 **새 문양 = 2곳**. **사용자가 직접 그려서 주기로 했다**(64×64 PNG · 배경 투명 · 위쪽=바깥 · 가장자리 여백). 효과 방향 리드 추천 = **지연**(9종이 전부 「즉시」라 시간 축이 통째로 비었다).
⚠ **임시 시드가 불어나 있다** — M1 재료 3종(세79)·융합진(세81)·응축(세82)·**룬 6종(세83)**. 관문을 붙이는 세션은 **해당 시드 줄을 같이 지워야** 한다(안 지우면 관문이 조용히 죽는다 — 세58 교훈). 정본 = `docs/PROGRESSION.md`.
...
- 🔴 **BOLT·EARTH·GRASS 전용 피격음·볼 애니가 없다**(세56 빚 — **세83에 룬이 복원돼 이제 실제로 필요하다**): 피격음 = "새 소리 = wav 한 장"(세33) + `audio.gd` match 세 줄 · 볼 애니 = `takbon-art` 3장 + `ring_carrier.BALL_ANIM` 3줄(지금은 **fireball 폴백**이라 번개를 쏴도 불덩이가 난다).
- **취약 이중 증폭** — 반응 산물에 배수가 두 번 곱한다(세49부터라 회귀는 아님). 의도인지 사고인지 미정.
  🔴 **세84에 실측했다**(감사 #13, `status_holder.gd:129`): 흙→풀(ROOT 1.5)→불이면 `add(BLAZE, 2.25)`가 되어
  **DoT 초당 13.5**(의도 6.75의 **2배**)이고 **clamp가 없어** 특별잉크 `status_mult`까지 그대로 곱한다.
  감속은 상한 포화라 체감이 작아 **레버가 이미 죽었다** — 「반응은 두 배 보상」이 의도라면 감속이 아니라
  DoT·지속으로 표현해야 한다. 곱하는 자리를 하나로 합치는 게 수정(= `_resolve_reaction`이 넘기는 power를
  「증폭 전 원시 세기」로). **사용자 결정 대기.** ⚠ **세81 M2 융합진이 흙을 포함하면 한 발에 닿는다**(지금까지는 취약+반응을 두 발 필요) — 발생 빈도가 오른다. 살릴지/자를지 M3에서 룬 복원과 같이 결정.
...
- **82** **M3① 문양 효과·표현 데이터화 + 응축** — `GlyphDef.behavior`+`params`로 분기가 `.tres`로 이사(새 문양 = **5곳→2곳**) · 🔴응축 = 폭발과 **같은 `blast` 알고리즘 + 부호만 뒤집은 `.tres` 한 장**(옛 구조면 `_explode` 사본이 생겼다) · `MODIFIER_GLYPHS`·`GLYPH_NAMES`/`COLORS`·`BOLT_EFFECTS` 은퇴 · 🔴**사용자 논거(하드코딩 방지 = 작업 규율)가 리드의 비용계산을 이겼다** · 뮤테이션 9/9이 **그물 구멍 + 죽은 코드**를 잡음 · 🟢architect가 치명 3 + 중요 10을 라이브 코드로 잡아 전량 반영 (정본 STATUS「세션 82」, 설계 `docs/takbon-design/glyph_data_design.md`, memory `takbon-glyph-data-driven`)
- **81** 진별 해석 **M2 — 룬 2개 + 융합진**(두 룬을 한 발에 → 원소 반응) · 하드 제약=긋는 양이 곱으로 안 늘게(*"2겹 긋기 지겹다"* — 세83 폐지의 씨앗) · 🔴architect가 심장 주장 3개를 라이브 코드로 반박(적 0-피해 가드·반응 순서 정렬·둘 다 0.7씩) · 뮤테이션 7/7 (정본 STATUS「세션 81」, memory `takbon-jin-fusion-m2`)
...
- 🔴🔴 **`.tres`의 「값」이 틀리면 그 데이터는 조용히 사라진다** (세50) — `Color`를 **3인자**로 쓰면 파서가 **리소스 전체를 거부**하고 `Db`가 말없이 건너뛴다.
  🔴 **세85 실측 정정 — 「한 글자만 틀려도」가 아니다. 둘은 다르게 죽는다**(clean-data가 4.7.1에서 직접 재현):
  **ⓐ 스키마에 없는 프로퍼티 *이름*은 조용히 무시되고 리소스는 산다**(`night_buff` 줄을 도로 넣어도 `ENEMIES=8`).
  **ⓑ *값* 파싱 실패는 리소스를 통째로 죽인다**(3인자 `Color` → 룬 6→5로 실증). 그래서 필드를 스키마에서 걷을 때
  `.tres`의 남은 줄은 **호환을 안 깨지만**, 값 문법을 틀리면 그 파일이 통째로 증발한다. 바람 룬이 두 세션 내내 그렇게 죽어 있었고 **전 스위트가 그린이었다**(검출력 0). ⚠ 그래서 **"파일을 만들었다"를 완료로 치지 마라 — `Db`를 거쳐 실제로 로드되는지 확인해라**(`test_rune_unlock_auto`의 「룬 6종 로드」가 그 그물이다).
...
- 🔴🔴 **뮤테이션 되돌리기에 `git checkout <file>`을 쓰지 마라 — 남의 미커밋 작업까지 날아간다** (세85에 리드가 실제로 밟았다).
  그 명령은 「뮤테이션만」이 아니라 **워킹트리를 통째로 HEAD로 되돌린다.** 병렬 갈래가 도는 세션에선 그 한 줄이
  에이전트 3갈래의 성과를 지운다(실제로 3파일이 날아가 재작업했다). 반드시 **`cp f f.bak` → 뮤테이션 →
  `cp f.bak f` → `rm f.bak`** + md5 대조. ⚠ 더 무서운 건 **복구 뒤 전 스위트가 그대로 그린이라는 것**이다 —
  날아간 게 「아직 그물이 없는 신규 기능」이면 초록불이 소실을 덮는다(이번에 `cast_mana_cost` 신설이 그랬다).
...
- **남은 모듈**: `src/base`(베이스캠프) · `src/field`(숲 원정 — 세션 26) · `src/actors`·`src/hud`
  ...
  - `src/drawing` = **ring_assembly**(조립 상태기계·순수 데이터) · **trace_scorer**(탁본 채점·순수 수학)
    ...
    🔴🔴 **세85 ⑨: per-piece(조각별로 하나씩 긋기) 경로는 세70에 은퇴했고 이번에 걷어냈다** —
    라이브 진입점은 **`enter_combined_trace` 하나**다. `advance`/`finish`/`select_slot`/`choose_jin`/
    `choose_rune`/`ring_summary`를 찾다 헤매지 마라(없다. `test_ring_assembly_auto`가 **재발까지 감지**한다).
    ⚠ `RingAssembly`는 이제 **쓰는 자리가 0곳**이다(조립 상태의 실소유자는 `ring_forge_panel`) — 다음 정리 후보.
  ...
  - 🔴 **보정은 펜이 판다**: `ItemKind.PEN` → `data/items/pen_*.tres`의 `params.correction` →
    `GameState.stroke_correction()` → `ring_board.enter_combined_trace` → `trace_scorer.set_correction`.
...
🔴 **세84: 이 목록과 `takbon-verify` 스킬 목록이 또 갈라져 있었다** — 스킬에 `test_jin_layers`·`test_jin_fusion`·
`test_glyph_data`·`test_ui_text` 넷이 없었다(**지난 세 세션의 심장 테스트가 전부**). 스킬 자신이 46줄에
그 경고를 적어 뒀는데도 세 세션 연속 났다 → **경고문에 의존하지 말고 대조를 기계로 해라**(세션 종료 전 한 줄):
`for t in $(ls tests/*_auto.gd); do grep -q $(basename $t) CLAUDE.md || echo 누락; done` (스킬 파일에도 같이).
...
🔴🔴 **리드가 F5·MCP를 직접 확인한다** (2026-07-26 세85 사용자 확정 — *"mcp로 니가 확인해 앞으로도
니가 확인해 내가 나중에 한번에 볼 수 있도록"*). ⚠ **이건 세71~84의 「리드가 에디터를 켜지 마라」를
뒤집은 것이다** — 그 규칙의 이유(사용자 콘솔에 연결 메시지가 떠 방해)는 아래 절차로 해소된다.
- **1차 방어선은 여전히 헤드리스 + 뮤테이션.** 겉보기·클릭 도달·시간 경과만 MCP로 넘어간다.
