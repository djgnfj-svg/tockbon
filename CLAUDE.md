...
🔴🔴 **직전 세션 = 48 「진 3→8종 — 축을 갈라 새 진 = .tres 한 장을 성립시킴」** (정본 STATUS「48」+ memory `takbon-jin-axes-split`):
세44에 진 3개가 `WandPattern` 3개를 **1:1 소진**해 "새 진 = .tres 한 장"이 깨져 있던 걸 **축을 둘로 갈라** 뚫었다 — **`pattern`(몇 발·언제) × `motion`(어떻게 나는가, 신설 `JinMotion`)**. 직교라 조합이 열린다(나선 산탄 = 데이터 한 장). `body_scale`도 DARK에서 깨웠다(**"크다"의 의미가 진마다 다르다** — 발수/몸집/진폭/사거리).
🔴🔴 **그리고 진 밑그림을 진마다 다른 「닫힌 도형」으로 갈랐다** (사용자 지적으로 발견): *"진의 궤적은 원처럼 **닫힌 도형**이어야 함 — 그 안에 룬이 들어가야 해서."* 세44~48까지 진은 **전부 같은 원**이라 8종으로 늘려도 손 궤적이 똑같았다(= 색만 다른 8지선다). 룬은 세34에 받은 처리를 **진만 못 받고 있었다.**
🔴 **다음 세션 = ⓐ 사용자가 직접 쏴 보고 그려 보기**(연발 간격·부메랑 선회·나선 진폭·큰 진 발수 — 헤드리스도 스샷도 못 잼) → ⓑ **룬 상태이상 배선**(아래 참조 — 지금 가장 큰 구멍). ⚠ 성급히 코드 얹지 말고 플레이부터.
⏸ **보류 = forest_t2(숲2·티어 하강)** — 사용자 확정(세48): *"숲2는 아직 필요없음."* 딸린 숙제인 **「하강 시 회복 스킵」도 같이 보류**다(지금 `forest._ready`가 진입 때 만HP/만마나로 회복 → 출격엔 맞지만 **하강도 같은 코드를 타서 "깊이=위험"이 안 선다**. 하강할 데가 없으면 실효도 없다). 티어를 실제로 지을 때 함께 처리한다 — 코드 주석은 `src/field/forest.gd:131`에 박혀 있다.

🔴🔴 **살아있는 큰 구멍 = 룬 축이 껍데기다** (세47 발견, 아직 안 고침): `forest_enemy.take_hit`이 `_status`를 **밑줄로 버려** 상태이상 전체가 미구현 → 불·물·바람의 실질 차이 = **색 + 데미지 ±15%**뿐. 세34 룬 해금도 세28~29 특별잉크 경제도 그래서 **증폭할 대상이 없다.** 룬을 6종으로 늘리기 **전에** 이걸 먼저 배선해야 색깔만 6개가 되지 않는다.
