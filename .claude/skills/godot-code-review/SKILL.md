...
> 🔴🔴 **이건 제네릭 스킬이다 — 탁본에선 `takbon-rules`가 이긴다.**
> 아래 체크리스트 중 셋(**§1 오토로드** · **§2 `class_name` 예제** · **§6 시그널 방향**)은 탁본 아키텍처와 **정면으로 어긋난다.** 각 절 머리에 경고를 달아 뒀다 — 그 절을 곧이 적용하면 **정상 배선이 지적 대상이 된다.**
> ⚠ **C# 예제는 참고용으로만 남겼다** — 탁본은 **GDScript 전용**이다(typed GDScript 강제). C# 블록은 리뷰에 쓰지 마라.

> **관련 스킬:** 여기 있던 참조 3개(`godot-testing`·`scene-organization`·`godot-optimization`)는 **세107에 은퇴한 스킬**이라 걷어냈다 — 부르면 실패한다.
> 🔴 **남은 스킬 목록을 여기 베끼지 마라**(베끼는 순간 두 벌이 되어 갈라진다). 다른 스킬이 필요하면 **`.claude/skills/`를 직접 훑어라 — 디스크가 정본이다.** 탁본의 검증·테스트 규율은 **`takbon-verify`**가 정본이다.

---

## 1. 노드 & 씬 아키텍처

> ⚠ **탁본 예외 — 오토로드:** 아래 *"오토로드를 아껴 쓴다"*는 탁본에 **적용하지 마라.** 탁본은 오토로드 **6개가 설계**다(`EventBus`·`GameState`·`Clock`·`Db`·`SaveManager`·`Audio`). 이걸 「싱글턴 남용」으로 지적하면 **설계를 지적하는 것**이 된다. 정본은 `takbon-rules`.
...
> ⚠ **탁본 예외 — `class_name`:** 아래 「좋음」 예제가 `class_name PlayerController`로 시작하는데, 탁본 **서브에이전트는 `class_name` 금지**다(→ `const X := preload("res://...")`). 즉 이 스킬의 모범 예제가 **금지 패턴을 정상으로 보여준다** — 그 한 줄만 빼고 읽어라. 정본은 `takbon-rules`.
> ⚠ **`@export` 체크 항목도 주의**: 탁본은 **수치를 `data/balance.tres`에 둔다** — 코드에 밸런스 상수를 `@export`로 여는 것이 오히려 위반이다(연출값 `const`는 예외).
...
> ⚠ **탁본에선 이 절 전체를 건너뛴다** — 탁본은 **GDScript 전용**이다. 절을 지우지 않고 남긴 이유는 **절 번호를 밀지 않기 위해서**다(§4~§9를 당기면 이 스킬을 「§6 시그널」 식으로 가리키는 곳이 전부 어긋난다). 제네릭 지식으로는 맞으니 다른 프로젝트에선 그대로 써라.
...
> 🔴🔴 **탁본 예외 — 시그널 방향(이 절에서 가장 크게 어긋난다):**
> 아래 *"시그널이 트리 **위로** 이동한다"*는 탁본에 **적용하지 마라.** 탁본은 **모듈 간 통신을 트리로 보내지 않는다** — **`EventBus` 오토로드 허브가 유일한 통신로**이고, 페이로드는 `src/core` 스키마를 쓴다. 부모/조상이 자식 시그널에 직접 `connect`하는 것이 오히려 결합이다.
> 🔴 이 체크리스트를 곧이 적용하면 **정상 배선(EventBus 경유)이 지적 대상이 된다.** 살아 있는 명시 예외 둘도 `takbon-rules` §0에 있다 — 지적 전에 열어라. **탁본에선 `takbon-rules`가 이긴다.**
> ✅ 반면 **「시그널 이름은 과거 시제」·「연결은 `_ready()`에서」·「순환 의존 없음」은 탁본에도 그대로 유효하다.**
