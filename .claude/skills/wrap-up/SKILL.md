...
기능을 다 만들고 `active/` 에 그대로 두거나, 파일을 지웠는데 그걸 가리키는 문서를 안 고치거나, 알게 된 걸 메모리에 안 남긴다. 다음 세션은 그걸 사실로 믿고 시작한다.
...
- **`docs/plans/` 상태 이동** — 끝난 문서가 `active/` 에 남아 있지 않은가. 옮겼으면 문서 안의 `**상태**:` 줄도 같이 고쳤는가
- **죽은 참조** — 이번에 지우거나 이름을 바꾼 것을 아직 가리키는 문서·주석이 있는가
- **`CLAUDE.md`** — 이번에 바뀐 구조나 규칙이 반영돼야 하는가. 짧게 유지한다
- **메모리** — 리포에 안 적히는 것만 남긴다. 사용자가 어떤 사람인지, 어떻게 일하라고 했는지. 코드·구조·이번 작업 내용은 남기지 않는다. 그건 git과 문서의 몫이다
