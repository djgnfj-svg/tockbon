...
- 🔴 **`mcp__godot__godot_docs`는 4축(architect·dev·reviewer·vfx)에 열려 있다 — 직접 불러라.** 그 밖의 `mcp__godot__*`은 **도구 층에서 이미 막혀 있어 부를 수 없다**(에디터는 리드가 관리).
  ⚠ **세108 실측 — 이건 산문 규율이 아니라 물리적 제약이다**: 허용목록(`tools:`) 밖의 도구는 `ToolSearch`로 이름을 정확히 찍어도 `No matching deferred tools found`, 직접 호출하면 `No such tool available`이 난다. **즉 「금지」를 글로 다시 적을 필요가 없다 — 필요한 건 「열린 것을 쓰라」는 안내다**(세107에 셰이더·엔진 패턴 레퍼런스 스킬을 지우며 그 대체재로 연 문이다).
  **출처·이유의 정본 = `docs/DECISIONS.md`「일하는 방식」** · §6 참조.
...
- ⚠ **서브에이전트의 `mcp__godot__*` 제한은 유효하다** — 단 **글이 아니라 도구 층이 강제한다**(세108 실측 → §0). 열린 건 `godot_docs` 하나뿐이고 **그건 쓰라고 연 것이다**. ⚠ 이 관행은 사용자가 정한 게 아니라 하네스 첫 커밋에서 왔다 — **출처·이유의 정본 = `docs/DECISIONS.md`「일하는 방식」**. 단 **리드는 세85부터 F5·MCP를 직접 확인한다** — 「리드는 에디터를 켜지 마라」(세71~84)는 뒤집혔다.
