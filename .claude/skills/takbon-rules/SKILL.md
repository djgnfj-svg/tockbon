...
게임 = `src/base/base.tscn`(마법사 학교 마을) + 고리 조립 책 + **챕터(보스방) 루프** + 온보딩 레일. 2D 탑다운 **마법 액션 로그라이크**, Godot 4.7.1, 뷰포트 960×540, 렌더러 Compatibility.
⚠ **숲 원정은 세58-B에 은퇴했다** — **지금 코드의** 메인 루프는 마을 → 챕터 패널 → `boss_room` → 클리어 → 창고다.
🔴🔴 **세110에 장르가 익스트랙션에서 갈아엎혔다 — 구현은 착수 전이라 위 루프가 아직 참이다.** 새 방향(방을 이어서 깨고 챕터가 연쇄 · 탈출·시야·챕터 선택 폐기) = `docs/takbon-design/planned/genre_pivot_design.md` · 경위 = `docs/DECISIONS.md` 「게임 정체성」.
⚠ **그래서 아래 계약 중 탈출(`extraction_success`)·시야(`Vision`)·챕터 선택에 걸린 것은 「곧 걷힐 것」이다** — 그 위에 새 기능을 얹기 전에 리드에게 물어라.

**정본은 역할별로 갈려 있다:**

| 문서 | 역할 |
|---|---|
| 📖 `docs/GDD.md` | **「게임이 무엇인가」 단일 진실원**(세71 신설) · 그래프의 **앵커**(`anchor: true` — 딴 문서와 겹치면 **이쪽이 이긴다**). 🔴 **에이전트는 읽기만 — 고치는 건 리드 몫이다.** ⚠ 세111에 settings.json 잠금은 걷혔다(사용자 확정) — 이제 막는 건 도구가 아니라 이 규율이다 |
...
| `docs/takbon-design/` | 설계 문서 **3단계 보관** — `planned/`(미착수) · `building/`(구현 중) · `done/`(완료) |
...
🔴🔴 **충돌하면 이 스킬이 이긴다.** 계약·모듈 상세·함정의 **정본은 여기**이고, `CLAUDE.md`는 **지도**(무엇이 어디 있나)다 — **지도와 규칙이 어긋나면 규칙이 이긴다.**
⚠ **세107에 이 줄을 뒤집었다.** 그전엔 여기가 *"충돌하면 CLAUDE.md가 이긴다"*였고 CLAUDE.md는 *"규율은 스킬이 정본"*이라 적어 **서로를 정본이라 불렀다** — 내용이 갈리는 순간 어느 쪽을 따를지 결정할 수 없는 상태였다.
🔴 **예외 하나 = §6(위임 축 목록·옛 이름 해소표).** 그 절만은 `CLAUDE.md`「위임 · 일하는 방식」이 정본이다 — §6이 스스로 그렇게 못 박아 뒀고, 실제로 세107에 여기만 낡아 「다섯 축」이라 적고 있었다.
...
- 🔴 **`mcp__godot__*`은 축별로 열려 있다 — 열린 것은 쓰라고 연 것이다**(세108 사용자 확정 *"버그 잡는 데 유용할 듯"*). **이 표가 정본이다**:

  | 축 | **읽기**(`godot_docs`·`editor_read`·`node_read`·`runtime_state`·`animation_read`·`tilemap_read`·`profiler`·`project`) | **실행·조작**(`editor_edit` run/stop·`exec`·`input`·`game_time`) |
  |---|---|---|
  | `takbon-dev` · `takbon-vfx` | ✅ | ✅ |
  | `takbon-architect` · `takbon-reviewer` | ✅ | ❌ |
  | `takbon-art` · `takbon-docs` | ❌ | ❌ |

  ⚠ **각 축 파일의 `tools:` 줄이 실제 정본이다** — 이 표는 「무엇을 왜 열었나」를 읽는 곳이지 허용목록의 사본이 아니다.
  **안 연 것 = `godot_scene`·나머지 `*_edit`(노드·타일맵·애니 편집)** — 씬 파일을 직접 고치면 「에이전트는 자기 모듈 폴더만」과 부딪힌다. 3D 도구도 안 열었다(탁본은 2D).
  🔴🔴 **에이전트 `tools:`를 고쳐도 세션을 다시 켜기 전엔 안 먹는다**(세108 실측 — frontmatter를 고친 **뒤에** 스폰한 둘이 신규 도구를 **0/7·0/4** 전부 `No such tool available`로 받았고, 같은 세션에서 원래 있던 `godot_docs`는 정상 응답했다 = MCP 서버가 아니라 **도구 목록이 세션 시작 시점에 굳는다**). ⚠ **스킬은 즉시 반영된다 — `tools:`만 재시작이 필요하다.** 「하네스를 고쳤는데 왜 안 되지」는 여기다.
  ⚠ 허용목록 밖의 도구는 **글이 아니라 도구 층이 막는다** — `ToolSearch`로 이름을 정확히 찍어도 `No matching deferred tools found`, 직접 호출하면 `No such tool available`이다. **「금지」를 글로 다시 적을 필요가 없다.**
  🔴 **실행·조작에는 사용 규율 둘이 붙는다**(열린 프로젝트가 tockbon인지 확인 · 에디터를 조종하는 에이전트는 한 번에 하나) — **정본 = `.claude/agents/takbon-dev.md`·`takbon-vfx.md`.** ⚠ **읽기는 병렬 안전하다.**
  **출처·이력의 정본 = `docs/DECISIONS.md`「일하는 방식」** · §6 참조.
...
- **`src/base`** — 마법사 학교 마을(진입점, `run/main_scene`).
  ...
  🔴🔴 **세95: `Npc`(길잡이)도 씬에 없다 — 문(`ForestGate`)이 화자·정산을 겸한다.** 퀘스트 정산·오프닝 대사·머리 위 [?] 마크가 전부 문에 붙어 있다(`_on_gate_talk`·`ForestGate/Mark`). ⚠ `src/props/npc_guide.tscn`·PNG는 **남겨 뒀다**(씬에서만 뺐다). 🔴 **문 [E]에 세 번째 일을 얹지 마라** — 이미 대사→정산→챕터선택 체인이고, 모달 규약 때문에 조용히 깨지는 자리가 넷이다(정본 = `docs/takbon-design/building/world_and_visual_design.md` §2 표 ⓐ~ⓓ).
...
- ✅ `docs/GDD.md`는 **세96에 따라왔다** — §2의 심장이 「조립 + 그 결과가 화면에 보이는 것」이고, 손 긋기는 §3의 휴면 각주다. 「그리는 것 = 심장」은 더는 GDD에 없다.
...
- 🔴🔴 **씬끼리 `PackedScene`을 물지 마라 — 경로(`res://…`)를 써서 런타임에 `load()`해라**(세26 · 세107에 memory에서 건져 올렸다).
  씬 A가 씬 B를 `ext_resource`로 물면 **B를 여는 순간 A가 통째로 딸려 로드**되고, 순환이 생기면 **에디터가 조용히 빈 노드를 만든다**(에러가 안 난다).
  ⇒ 무대·프롭처럼 **서로를 참조할 여지가 있는 씬**은 경로 상수로 잇는다.

## 6. 위임 라우팅 (리드용)

- 🔴🔴 **이 절(§6)만은 정본이 `CLAUDE.md`「위임 · 일하는 방식」이다 — 여기 베끼지 마라.** 위 머리말의 「충돌하면 스킬이 이긴다」에 대한 **유일한 예외**가 §6이고, 어긋나면 CLAUDE.md가 이긴다.
...
- 🔴 **기획(무엇을·왜·어떻게 재밌게)은 위임하지 마라** — 리드가 `takbon-design` 스킬로 **사용자와 대화하며** 확정한다(세71: 서브에이전트는 대화를 못 해 혼자 정한다). **정본 = `takbon-design` 스킬의 하드-게이트.**
...
- 🔴 **리드가 안 놓는 것 = 기획·검증·`--import`·커밋.** **에이전트의 "그린 나왔습니다"를 근거로 쓰지 마라** — 리드가 직접 돌리고 뮤테이션으로 검출력을 확인한다. **정본 = `takbon-verify` §4**(넷의 목록 자체는 `CLAUDE.md`).
- ⚠ **서브에이전트의 `mcp__godot__*`은 세108에 축별로 열렸다**(dev·vfx는 실행·조작까지 — **표 = §0**). 제한은 **글이 아니라 도구 층이 강제한다.** 🔴 **에디터는 하나뿐이라 실행·조작을 쓰는 에이전트는 한 번에 하나 — 리드가 부를 때 그렇다고 명시해라**(사용자 확정 · 강제 장치는 없다). **병렬로 여럿 띄울 땐 읽기 전용만 준다.** ⚠ 이 관행의 출처는 사용자가 아니라 하네스 첫 커밋에서 왔다 — **출처·이유의 정본 = `docs/DECISIONS.md`「일하는 방식」**. 단 **리드는 세85부터 F5·MCP를 직접 확인한다** — 「리드는 에디터를 켜지 마라」(세71~84)는 뒤집혔다.
- 🔴 **에이전트에게 「보고서를 파일로 써라」고 지시해라** — **채팅으로 낸 최종 보고는 리드에게 안 온다**(세48~49에 4건이 idle 알림만 남기고 증발했다). 특히 `takbon-architect`·`takbon-reviewer`는 **산출물이 보고서뿐**이라 파일로 안 시키면 작업 전체가 사라진다. **정본 = `CLAUDE.md`「위임 · 일하는 방식」.** 🔴 **이 문장이 여러 파일에 겹쳐 있는 건 지우면 안 되는 중복이다** — 각 에이전트가 **자기 파일에서** 안 읽으면 작업이 통째로 증발한다.
- ⚠ **뮤테이션을 시킬 땐 원상복구까지가 지시다** — `src/`를 되돌린 채 두면 기능이 조용히 죽은 채 커밋된다. **정본 = `takbon-verify` §4**(⚠ 되돌리기에 `git checkout` 금지 = §4-1).
