...
- 🔴 **에이전트는 다섯 축이다**(세92에 여덟→넷 · **세94에 `takbon-vfx`를 다시 갈랐다** — 사용자 확정):
  설계 리뷰 = **`takbon-architect`** · 구현 = **`takbon-dev`**(모듈 코드 + 패널/HUD·애니 배선) ·
  **연출 = `takbon-vfx`**(코드로 그리는 빛 — 발사·착탄·반응·손맛·연출 셰이더 · 정본 `docs/VFX_SPEC.md`) ·
  그림 = **`takbon-art`**(도트 스프라이트 · 정본 `docs/ART_SPEC.md`) · 코드 리뷰 = **`takbon-reviewer`**.
  🔴 **dev ↔ vfx 경계 = 「돌아가라고 있는 것」 vs 「보이라고 있는 것」.** ⚠ vfx를 접을 조건은 그 에이전트 파일에 적혀 있다.
  🔴 **옛 이름 해소표**(설계문서·과거 기록에 그대로 남아 있다 — 부르면 실패한다): `takbon-ui`·`takbon-animator` → **`takbon-dev`** ·
  `takbon-shader` → **`takbon-vfx`** · `takbon-relight` → **`takbon-art`**(⚠ 단 **relight 기법 자체가 세91에 은퇴** — 팔레트를 깬다).
