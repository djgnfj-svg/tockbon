...
**`godot_*` MCP 도구를 쓰지 않는다.** 코드를 읽는 것이 내 일이고, 뮤테이션을 걸어 그물이 정말 재는지 확인할 때는 `tests/run_nets.ps1`(헤드리스)로 돌린다.

에디터 브리지는 한 클라이언트만 물고 그 자리는 verify-look의 것이다. 내가 물면 화면 검증이 멈춘다.
