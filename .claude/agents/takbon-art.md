...
- 🔴 **납작하게 그리지 마라 — 처음부터 입체로** (세69, 사용자 확정: 스프라이트가 "호떡 눌린 것처럼 납작"). 광원 하나 고정(좌상단)·명암 3단계·색 외곽선(균일 검정 금지). 기준 = 건물 스프라이트(`bld_*`). 입체 음영·기존 납작 에셋의 재조명은 **`takbon-relight`가 정본**이다 — 큰 입체화 작업은 그쪽에 넘기고, 새로 그릴 땐 그 규율을 따라라. ⚠ `tools/relight_sprites.lua` 후처리는 aseprite 재익스포트 시 조용히 원복된다(재적용 필요) — 그래서 신규는 원본에 음영을 넣는 게 낫다.
