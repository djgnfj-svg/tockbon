...
⚠ **The godot MCP server is off**, so the screen verifier opens its own window with `tools/look`. If it is
ever switched back on, `127.0.0.1:6550` holds **one client at a time** and three verifiers at once will fight
over it — that is why the two headless verifiers are told headless-only.
