...
## ⚠⚠ **`spec` is gone — planning is a conversation** (the user)
...
## ⚠⚠ **Reading this repo is not an agent's job any more** (the user)

**`lookup` read the inside and `research` reads the outside, and the split was set.** ⚠⚠
...
## ⚠⚠ **`sculpt` is gone — Blender is back in the main session** (the user)

**It was pulled out because the 3D ate the conversation**, and it went back because **twice
...
⚠ **`tools/blender/` is gone.** The meshes are `.blend` originals in `blend/` now, and
that page is where the rule lives.

## ⚠ **`net-tuner` is gone too** (the user: *"when it gets long, we check it then"*)
...
⚠⚠ **`verify` runs TWO PASSES in one agent** — reading, then running. Three separate verifiers on every
ticket was too much ceremony, and the split has to survive inside the one agent:
...
own frontmatter** . Until then this paragraph was the only thing holding the rule: the files
