...
2. **정본은 역할별이다** — 설계 근거는 여기서 고른다:
   ...
   - `CLAUDE.md` 최상단 = 살아있는 함정·계약 · **`docs/DECISIONS.md` = 무엇을 왜 정했나**(세92 신설 — 방향을 뒤집는 설계를 내기 전에 **여기서 그 결정의 이유부터 읽어라**) · `docs/TODO.md` = 아직 안 한 것(⚠ 2026-07-29에 `BACKLOG.md`에서 갈라졌다 — 그쪽은 이제 「어느 세션이 무엇을 했나」다) · `docs/PROGRESSION.md` = 관문 · `docs/takbon-design/` = 설계 **3단계**(`1_planned`·`2_building`·`3_done`).
