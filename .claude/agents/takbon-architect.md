...
2. **정본은 역할별이다** — 설계 근거는 여기서 고른다:
   - 📖 **`docs/GDD.md` = 「게임이 무엇인가」 단일 진실원**(세71 신설). 🔒 **읽기만 해라 — 수정엔 사용자의 명시적 허락이 필요하다.** ✅ **GDD는 현행이다 — 그대로 인용해도 된다**: 그리기 폐지가 세96·97에 §2·§3으로, 세110 장르 전환이 §1 첫 줄·§6으로 반영됐다(둘 다 허락받음). 🔴 **「개정이 허락 대기 중이라 미뤘다」류 서술을 믿지 마라 — 낡은 것이다**(세107이 스킬 둘에서, 세111이 다섯 곳에서 걷어냈고 이 줄이 그 잔재였다).
