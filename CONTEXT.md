...
| **눈금** | **`level`** | ⚠⚠ **반 칸이다** (2026-08-26 에 정의됐다). 칸마다 정수 하나를 들고, **이 게임의 모든 높이가 그 정수에서 나온다** |
| **층** | — | **눈금 둘.** 1층이 0, 2층이 2. **지금은 2층까지가 최대**이고 3층은 눈금 3·4 로 나중에 붙는다 |
| **계단** | — | **눈금 하나.** 곧 **홀수 눈금이 계단**이고, 몸은 눈금 차이 1 까지만 넘으므로 **계단이 층 사이의 유일한 문**이다. ⚠ **계단의 단은 그림이지 판이 아니다** — 판에는 눈금 하나로 적히고 단은 조각 안에 그려진다 |
...
| 인간 넷 | ⚠⚠ **삭제됐다** — 창병·궁수·방패병·전사 넷이 표에서 빠지고 **검사 하나**가 남았다 | ✅ **검사는 새로 지은 몸이 아니다** — 한 판을 통째로 상대해 본 창병과 방패병 **사이에서 잡은 숫자**이고, 사거리만 0 이다. 짐승 넷의 숫자는 **진영이 바뀌면서도 하나도 안 움직였다** |
...
| 한국어 | Code | What it is |
|---|---|---|
| 회차 | **`Run`** | 한 판. **무슨 상태에 서 있고**(섬 · 카드 · 정비 · 승 · 패) **명부가 무엇을 들고 있는가.** ⚠⚠ **노드는 없다** — 아래 줄이 지도를 지웠고 `Run.State` 에 `MAP` 이 남아 있지 않다 |
...
| 정비 판 | **`Loadout`** | **플레이어 병종마다 하나** — `Rules.player_type_count()` 로 잡으므로 **지금은 판도 하나**다. 카드가 여기에 값을 치른다. ⚠ **소환 칸이 아니라 병종에 매달린다**(티켓 11): 칸에 매달았을 때는 판에 선 종만 장비를 받았다 |
...
| 장비가 붙는 자리 | **`Loadout`** | ✅ **찼다**(티켓 11). **몸 하나가 아니라 병종 하나에 판이 하나** — 꽂으면 그 병종 전부가 낀다. **몸이 죽어도 판은 안 죽는다** |
...
판은 칸이 아니라 **병종**에 매달렸고 **칸에서 이름이 떨어졌다.** ~~**「부리」는 그대로 둔다** — 근접
