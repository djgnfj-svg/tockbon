# **The candidates on the REAL island, in the game's own view.**
#
# ⚠⚠ **The wafer lab next door judges a mechanism; this judges a SCREEN.** `lab.gd` builds a thin slab
# with nothing on it so that every pixel that differs between two versions is water. That is the right
# world for asking *what does this shader do* and the wrong one for asking *does the game look better* —
# the island has a second storey, a stair, a keep and a cliff face, and a sea that reads beautifully
# beside a flat slab can read as noise beside all that.
#
# **This file loads the game's own island mesh, the game's sun, the game's water height and the game's
# camera angle**, and then swaps the sea shader under it.
#
# ⚠⚠ **NOTHING IN `src/` IS TOUCHED** (2026-08-29, the user: 「프로토 타입 view를 만들어주는거임 ㅇㅇ
# 본코드 건들지말고」 — *"it is a prototype view you are making, do not touch the main code"*). A probe
# was briefly wired into the shell and the field view and it has been reverted. **This reads `src/` —
# `Look`, `Islands`, and `FieldView`'s own bake — and writes none of it.**
#
# ⚠ **What is NOT here**: bodies, boats, buildings, the 판's white marks, the HUD. This is the island
# and the sea. **The bodies are the one thing a sea has to stay quiet behind**, so if a candidate
# survives this it still has to be looked at with a fight running.
#
#   Godot_v4.7.1-stable_win64.exe --path . -s .prototypes/swash/island_lab.gd
#       opens and stays. **0 is the shipped sea · 1..9 are the candidates · LEFT/RIGHT step ·
...
const OUT := "res://.prototypes/swash/out/island/%s_%d.png"
...
var _turn := 0.0
var _tilt := 0.0
## ⚠ **Ten, not eighteen.** The camera is orthogonal and its `size` is the screen's HEIGHT in tiles;
## at 18 a 16x12 island tilted 40 degrees sat in the middle of the frame with open sea all round it,
## and the line being judged was three pixels wide.
var _zoom := 10.0
...
func _initialize() -> void:
	...
	if f == null:
		push_error("island_lab: no island.json — bake the island first")
		return
	var data: Variant = JSON.parse_string(f.get_as_text())
	if typeof(data) != TYPE_DICTIONARY:
		push_error("island_lab: island.json did not parse")
		return
	var gw := int(data["w"])
	var gh := int(data["h"])
	...
	if packed == null:
		push_error("island_lab: island.glb will not load")
		return
	...
	if _shooting:
		DirAccess.make_dir_recursive_absolute(
			ProjectSettings.globalize_path("res://.prototypes/swash/out/island"))
	else:
		_label = Label.new()
...
func _bake_field(gw: int, gh: int, coast: Array) -> void:
	...
	print("[island_lab] field %dx%d texels, %d coast segments" % [tw, th, coast.size()])
...
func _apply(row: Array) -> void:
	...
	if sh == null:
		push_warning("island_lab: could not load %s" % row[1])
		return
...
func _show(k: int) -> void:
	...
	_apply(_looks[_i])
	if _label != null:
		_label.text = "%d  %s\n0..9 · SHIFT+n = 10s · CTRL+n = 20s (%d in all) · LEFT/RIGHT step · Q/E turn · W/S zoom · R/F tilt · ESC quit" % [
...
func _aim() -> void:
	...
	var eye := _mid + Vector3(sin(_turn) * cos(p), sin(p), cos(_turn) * cos(p)) * r
	_cam.look_at_from_position(eye, _mid, Vector3.UP)
...
func _process(delta: float) -> bool:
	...
	if step == 0:
		...
		root.get_texture().get_image().save_png(
			ProjectSettings.globalize_path(OUT % [str(_looks[k][0]), step - 1]))
		print("[island_lab] %s %d" % [str(_looks[k][0]), step - 1])
	_i += 1
	return false
