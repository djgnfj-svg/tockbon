-- 번개·흙·풀 원소 볼 시트 생성
-- 파이어볼/워터볼/윈드볼과 같은 92×48 혜성 지오메트리 — 세 시트는 필드(실루엣+프레임별 노이즈)가 비트 동일하고 8밴드 램프 + 밴드별 알파 배수만 다르다.
-- 그래서 파이어볼 PNG에서 필드를 읽어 램프만 갈고, 그 위에 원소별 질감을 얹는다.
--   번개 = 지그재그 아크 + 튀는 스파크 / 흙 = 자갈 파편(좌상단 광원) / 풀 = 잎날·덩굴손
-- 🔴 파이어볼 PNG는 읽기만 한다 — 재익스포트하면 relight 음영이 날아간다.
-- 🔴 램프는 불·물의 명도 곡선(≈59,73,93,119,160,205,239,247)을 따라간다 — 색조만 갈고 명도 곡선을 지키는 게 「같은 발사체 계열」의 정체다.
-- 명도를 통째로 올리면 밝은 밴드 4개가 구분이 안 가 몸통이 납작한 단색 덩어리로 읽힌다.
...
local ELEMS = {
	...
	{
		id = "earthball",
		-- 🔴 흙은 빛나지 않는다 — 최상단이 207에서 멈춘다(불은 249). 위쪽 밴드를 눌러 갈색을 지킨다.
...
local function ov_earth(img, f, A, R)
	-- 🔴 돌 몸통은 밴드 2·4, 그늘만 밴드 1이다 — 몸통을 밴드 1로 하면 밝은 머리 위에서 검은 구멍(「짙은 대시」)이 된다. 3단 명암이라야 돌로 선다.
	local dk, md, lt, hi, sh = R[2], R[4], R[6], R[7], R[1]
	if ROCKS == nil then
		ROCKS = {}
		srnd(4471)
		-- 🔴 파편은 크게(4~5×3~4) · 적게(10개) · 윗면을 확실히 밝혀야 「빛 받은 돌」이 된다.
		for i = 1, 10 do
			ROCKS[i] = {
...
-- 🔴 잎을 1px 폭으로 그리면 「낙서 선」이 된다 — 폭 2(가운데 5px) + 짙은 잎몸 · 위 하이라이트 · 아래 어두운 림으로 잎 모양을 세운다.
local LEAVES = nil
