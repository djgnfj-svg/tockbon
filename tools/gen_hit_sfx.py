#!/usr/bin/env python3
"""🔊 룬 피격음 합성기 — BOLT·EARTH·GRASS 세 장을 만든다 (세104 · BACKLOG C2).

    python tools/gen_hit_sfx.py            # assets/audio/sfx/ 에 3장 생성 + 규격 표 출력
    python tools/gen_hit_sfx.py --dry-run  # 파일을 안 쓰고 규격만 재본다

기존 SFX 17종과 마찬가지로 **합성 플레이스홀더**다. 외부 패키지 없이 표준 라이브러리
(`wave`·`struct`·`math`·`random`)만 쓴다.

## 규격은 추측이 아니라 실측이다 (기존 hit_fire·hit_water·hit_wind를 열어 읽었다)
  22050Hz · 16bit · mono · 캐논 44바이트 헤더 · **피크를 정확히 0.85로 정규화**
  (기존 4장 전부 피크가 0.8499 = round(0.85*32768)/32768 — 같은 정규화를 거쳤다는 뜻이다).

## 🔴 왜 이 셋이 서로 구분되는가 — 겹치는 축을 일부러 갈랐다
`_on_enemy_hit`은 룬 6종을 소리로 갈라야 하는데, 그냥 "노이즈 버스트"를 셋 더 만들면
기존 fire·wind와 뭉개진다. 그래서 **기존 셋이 이미 점유한 자리**를 실측하고 빈 자리에 앉혔다:

    id          지배음    어택     성격                     이미 쓰는 자리와의 거리
    hit(기본)   188Hz    2.5ms   저역 톤 감쇠
    hit_fire      —      0.1ms   광대역 노이즈 급감쇠
    hit_water   312Hz     68ms   거의 순톤 스웰
    hit_wind    668Hz     76ms   중고역 노이즈 느린 스웰
    ─────────────────────────────────────────────────────────────────
    hit_bolt      —        0ms   **다중 크랙 + 하강 처프**   fire보다 짧고(0.10s) 훨씬 높다
    hit_earth    ~75Hz    ~2ms   **저역 처프 임팩트 + 그릿** hit보다 한 옥타브 아래 + 거칠다
    hit_grass     —       ~18ms  **입자형 러슬(그레인 26)**  wind보다 짧고 6k 위가 잘렸다

즉 구분 축은 ⓐ 스펙트럼 무게중심(75Hz ↔ 7kHz) ⓑ 어택(0ms 즉발 ↔ 18ms 스웰)
ⓒ 엔벨로프 결(매끈한 감쇠 ↔ 계단형 크랙 ↔ 톱니형 그레인) 셋이다.

## ⚠ 길이는 기존 최장(hit_wind 0.200s)을 넘기지 않는다
피격음은 연타되고 `randf_range(0.95, 1.05)` 피치 흔들림까지 얹히므로 길면 겹쳐 뭉갠다.
...
def make_bolt() -> list[float]:
    ...
    hit_fire도 노이즈 버스트라 뭉갤 위험이 제일 크다. 셋으로 갈랐다:
      ① 길이 0.070s — 기존 피격음 중 압도적으로 가장 짧다(fire 0.180s·hit 0.120s).
         ⚠ 처음엔 0.100s로 잡았는데 실측 엔벨로프상 50ms 뒤가 전부 0이었다(절반이 침묵).
         꼬리를 잘라 '순간'이라는 정체성을 더 세웠다.
      ② 3500Hz 하이패스 — 무게중심을 fire(3955Hz) 위로 밀어올린다.
      ③ **임펄스 셋**(0·11·26ms)이라 엔벨로프가 매끈한 감쇠가 아니라 계단이다.
      ④ 5200→1400Hz 하강 처프 — 순수 노이즈인 fire엔 없는 '음정'이 실린다.
    """
...
def make_earth() -> list[float]:
    ...
    기본음 hit(188Hz·저역 55%)과 겹칠 위험이 크다. 셋으로 갈랐다:
      ① 지배음을 ~75Hz로 — hit보다 한 옥타브 아래라 '더 무겁다'로 읽힌다.
      ② **그릿**(350Hz 대역 노이즈 급감쇠) — hit의 매끈한 톤과 달리 부스러지는 결.
      ③ 1600Hz 로우패스 — 위를 잘라 '둔탁'을 만든다(BOLT의 정반대 자리).
    """
    rng = random.Random(1042)
    buf = silence(0.140)

    # 몸통: 190→72Hz로 떨어지는 처프 = 무거운 '쿵'
    # ⚠ 바닥을 55Hz까지 내렸더니 0-200Hz에 90%가 몰려 **노트북 스피커에서 안 들릴** 위험이 생겼다
    #   (실측 dom 59Hz). 72Hz로 올리고 아래 배음·그릿을 키워 200-500Hz에 들을 거리를 남긴다 —
    #   그래도 일곱 중 가장 어둡다(hit의 dom 188Hz보다 한 옥타브 아래).
...
def make_grass() -> list[float]:
    ...
    hit_wind(중고역 노이즈 스웰)와 겹칠 위험이 크다. 셋으로 갈랐다:
      ① **그레인 26개** — 잘게 끊겨 엔벨로프가 톱니다(wind는 매끈한 스웰).
      ② 5200Hz 로우패스 — wind는 6kHz 위가 18.8%인데 여기는 위를 잘라 '쉭'이 아닌 '사삭'.
      ③ 길이 0.150s(wind 0.200s)·어택 ~18ms — 부드럽되 늘어지지 않는다.
    """
