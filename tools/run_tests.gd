...
##   ... -s res://tools/run_tests.gd -- realtime      # 전부 실시간(가장 보수적)
##   ... -s res://tools/run_tests.gd -- ring          # 이름에 "ring"이 든 것만
##
## 🔴🔴 **테스트마다 자식 프로세스를 따로 띄운다 — 「부팅 1회」는 다시 시도하지 마라(실측).**
##  ① 테스트는 전부 `extends SceneTree`이고 **프로세스당 메인 루프는 하나**다.
##  ② 두 번째 SceneTree를 만들면 **자기만의 빈 root**가 생겨 오토로드가 없고 `await process_frame`이 영영 안 깨어난다.
##  ③ 실 tree에 `set_script`를 걸면 돌긴 하지만 테스트 끝의 `quit()`이 **프로세스를 죽인다.**
##  ④ 래퍼로 `quit()`을 가로채는 것도 안 된다 — GDScript가 **네이티브 직접 호출로 컴파일**해 오버라이드를 지나친다.
##  🔴 게다가 값이 작다: 부팅은 1회 ≈ 0.70s로 전체의 14%뿐이고, 시간을 먹는 건 **`await physics_frame`이**
##   **벽시계 60Hz에 묶여 있는 것**이다.
##
## 🔴 그래서 실제 지렛대 = **`--fixed-fps`** — 델타를 1/60로 고정해 프레임을 CPU 속도로 돌린다
##  (게임 시간은 같고 벽시계만 준다. 실측: 25.0s → 1.35s, 결과 동일).
## 🔴🔴 단, **벽시계를 읽는 테스트는 여기서 거짓 빨강이 난다** — 게임은 가상 시간으로 흐르는데
##  테스트가 `Time.get_ticks_msec()`로 실시간을 재면 둘이 갈린다.
## 🔴 그 목록을 **손으로 적지 않는다** — 소스를 스캔해 파생시킨다(`_reads_wall_clock`).
##  목록을 박으면 낡고, **새 벽시계 테스트가 조용히 샌다.**

const TESTS_DIR := "res://tests"
const REALTIME_MARKERS: Array[String] = ["Time.get_ticks_", "Time.get_unix_"]

## 🔴🔴 엔진 `ERROR:`는 `SCRIPT ERROR:`가 **아니다** — 그래서 이 러너를 오래 그냥 통과했다.
##  물리 flush 중 `add_child` · 씬 노드가 사라진 뒤의 `push_error` · 방 밖 스폰이 **연달아** 이 사각에
##  걸렸다(전부 **엔진이 짖는데 전 스위트가 그린**).
## 🔴 그런데 판정에는 안 넣는다 — **세되 보고만 한다.** 종료 잡음이 거의 모든 테스트에 뜨는 자리라
##  바로 판정에 넣으면 매번 무더기가 빨개지고, 그러면 사람이 빨강을 무시하기 시작한다(그게 그물을 죽인다).
## ⚠ **`push_error`도 여기 걸린다**(`USER ERROR:`가 아니라 `ERROR:`로 찍힌다) — 테스트가 **일부러 태우는 것까지** 잡힌다.
## 🔴 **기준선 = 0건이다**(2026-08-02 실측). **1건이라도 뜨면 그게 새로 생긴 것이다.**
##  ⚠ **기준선을 잡음 목록으로 옮겨 없애지 마라** — 문구가 바뀌면 필터가 낡고, 그 필터가 **진짜 에러를 가린다.**
##  ⚠ **일부러 `push_error`를 태우는 그물을 세우면 기준선이 오른다.** 그렇게 올린 기준선은
##   그만큼의 진짜 에러를 가리므로, 그 검증은 **일회성 스크립트로 하고 그물로 남기지 마라.**
##  ⚠ 기준선을 적을 때 **무엇이 그 건수를 만드는지**까지 같이 적어라 — 원인이 사라져도 숫자만 남으면
##   그 차이만큼 새 에러가 조용히 통과한다(실제로 그렇게 낡아 있었다).
...
## `tests/*_auto.gd`를 파일시스템에서 파생시킨다 — 하드코딩 목록 금지(새 테스트가 자동으로 든다).
...
## 한 테스트의 출력에서 판정을 뽑는다.
## 🔴 집계 형식은 이 프로젝트 것 — `TEST_*_OK` / `TEST_*_FAIL`이지 `fail=N`이 아니다.
func _score(name: String, realtime: bool, ms: int, code: int, text: String) -> Dictionary:
	...
	# 🔴🔴 **판정은 마커 + `SCRIPT ERROR:`로 한다 — 종료 코드로 하지 마라**(실측).
	# `code == 0`을 OK의 필요조건에 넣었더니 전 스위트를 세 번 돌려 세 번 다 **「서로 다른」 테스트가**
	# **하나씩 FAIL**로 찍혔다(셋 다 단독 실행에선 종료 코드 0 · 마커 `_OK` · `SCRIPT ERROR` 0).
	# 즉 종료 코드가 간헐적으로 흔들린다(엔진 종료 시 "resources still in use"와 맞물린 것으로 보인다).
