...
A = {
    ...
    "n9": (235, 237, 233),
    # ── 세91 보강 14 (takbon.gpl — Apollo 46을 부분집합으로 품는다) ──────────
...
## 🔴 코어 흰은 전부 `w`(F7F7F2) — Apollo 최명부 EBEDE9는 회색빛이라 빛이 안 산다(세91 정본).
RAMPS = {
    #             0(코어) 1      2      3      4      5      6      7(테)  8~(악센트)
    "fireball":  ["w",   "o5",  "o4",  "r5",  "r4",  "r3",  "r2",  "s2",  "r1"],
    "waterball": ["w",   "n8",  "b5",  "b4",  "b3",  "b2",  "b1",  "b0",  "n7"],
    # 바람 = 청록. 회색 잡탕이던 걸 c1/c0/k1로 갈아 「바람 마법 몸」을 준다
    "windball":  ["w",   "c1",  "b5",  "b4",  "c0",  "n5",  "k1",  "s0",  "n6"],
    # 🔴 번개 = F2CB3F. Apollo엔 없던 색이라 여태 E8C170으로 버텨 불과 안 갈렸다
...
def rim(lv, dark):
    """실루엣 가장자리를 한 단계 어둡게 — 균일 검정이 아니라 색 외곽선(세69 규율)."""
    edge = [p for p in lv
...
def earthball(f):
    ...
    #    (세69 「색 외곽선」 규율: 검정 한 겹이 아니라 빛 받는 면과 그늘 면이 갈린다)
...
def grassball(f):
    ...
    outline(lv, 6)   # 🔴 k0(16281C) 최암 초록 — 잔디에 안 묻히는 윤곽 (세91 팔레트)
    return lv
