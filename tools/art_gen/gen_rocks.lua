-- 숲 바위 2종 (rock_big · rock)
-- 🔴 밑그림 생성기다 — 실루엣은 손으로 적은 꼭짓점 목록이고, 면(facet) 경계도 손으로 그은 두 직선이다(표 밖의 값을 수식으로 만들지 않는다).
...
-- 🔴 색조는 사다리(명암 축)에 실으면 안 된다 — 명도가 같아도 채도가 다르면 얼룩이 색 반점으로 튄다. 색조는 넓은 얼룩으로 따로 얹는다.
...
-- 🔴 광원이 위쪽이면 V홈의 위쪽 벽은 빛을 등지고(어둡다) 아래쪽 벽이 빛을 받는다 — 반대로 그리면 금이 「흰 실」이 된다. 고정색을 박지 말고 사다리 한 칸만 움직인다.
  ...
  for i = 1, #nodes - 1 do
    ...
    for s = 0, n do
      ...
      if buf:get(x, y) ~= nil then
        ...
        -- ⚠ 금은 「선」이 아니라 「띄엄띄엄 빛이 걸리는 홈」이다 — 깊은 금(deep)은 아랫벽을 아예 안 밝힌다.
...
-- 🔴 겹쳐 놓은 돌은 자기 테두리를 그어야 큰 돌에 안 흡수된다.
--   빛 쪽(왼위)은 밝게 · 그늘 쪽은 어둡게 — 한 색으로 두르면 스티커가 되고, 어두운 색으로만 두르면 큰 바위의 그늘 면 위에서 아예 안 보인다.
  ...
  for y = 0, h - 1 do
    for x = 0, w - 1 do
      if mask[y * w + x] then
        ...
        -- 🔴 반사광 — 그늘 면 아래쪽은 땅에서 튕겨 온 빛을 받는다(없으면 오른쪽 면이 통째로 검은 덩어리가 돼 형태가 죽는다).
...
-- 광물 얼룩 — 격자로 찍으면 화면에서 체크무늬로 읽힌다. 흩뿌리고, 얼룩은 사다리 한 칸만 움직여 면의 평평함을 안 깬다.
...
local function rock_big()
  ...
  -- ⚠ 꼭짓점이 많고 각도 변화가 완만하면 「식빵 돔」이 된다 — 9개로 줄이고 긴 직선을 뒀다:
  --   납작한 윗면 · 완만한 왼 어깨 · 뚝 떨어지는 오른쪽 = 좌우가 안 맞는다.
  -- 🔴 곁돌은 뺐다 — 80x64에선 큰 바위가 거의 전부라 겹친 실루엣이 흡수된다. 실루엣 다양성은 오른쪽 아래 각진 단(shelf) (76,38)→(69,42)→(74,50)이 낸다.
  -- ⚠ 「화면 키」(64px)는 눈이 아니라 자로 재라 — 표만 보고는 3px 모자란 것을 못 잡는다.
  local MAIN = {
  ...
  -- ⚠ 톤을 밝게 잡으면 밝은 풀 위에서 바위만 하얗게 뜬다 — 나무는 어두운데 혼자 밝으면 한 결로 안 읽힌다.
  shade_facets(buf, mask, W, H, { 2, 44 }, { 58, 2 }, { 52, 0 }, { 66, 58 },
    { 3.6, 5.2, 6.0 }, 3.7)
  mottle(buf, mask, W, H, 3.7, 55)

  -- 금 — ⚠ 면 경계를 따라 긴 금을 겹치지 마라. 면 경계가 이미 그 일을 한다(겹치면 「흰 실을 붙인 것」이 된다).
  ...
  -- ⚠ 흩어진 초록 점은 이물질로 보인다 — 덩어리로 뭉치고 밑동은 통째로 두른다(풀에 파묻힌 자리).
  ...
  -- 🔴 "lichen" = 넓은 지의류 얼룩 — 바위를 숲 물건으로 만드는 색조를 넓은 면적으로 준다(점으로 흩으면 색 반점으로 튄다).
  -- 🔴 코어를 확정으로 채우지 마라 — d<0.45를 무조건 채웠더니 얼룩이 완벽한 타원(곰팡이 자국)이 됐다.
  --   중심에서 멀어질수록 확률만 떨어뜨리면 경계가 저절로 너덜해지고 덩어리 모양이 매번 달라진다.
  for _, k in ipairs({ { 17, 25, 7, 5, "top" }, { 31, 11, 6, 4, "top" }, { 44, 7, 5, 3, "top" },
    ...
    for dy = -k[4], k[4] do
      for dx = -k[3], k[3] do
        ...
        local d = nx * nx + ny * ny
        -- ⚠ 확률만 쓰면 가장자리가 1px 별/가시로 성긴다 — 해시를 2px 블록으로 물려 최소 덩어리를 2x2로 보장한다.
        if d <= 1.0 and (d < 0.28
          ...
          if cur ~= nil and is_stone(cur) then
            -- ⚠ 이끼는 「초록 얼룩」이지 「잎」이 아니다 — 밝은 초록을 쓰면 뭉쳐서 나뭇잎처럼 보인다(m3/m4 둘로만 간다).
            if k[5] == "top" then
