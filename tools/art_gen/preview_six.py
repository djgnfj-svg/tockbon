#!/usr/bin/env python3
"""여섯 장을 **한 장에 나란히** 놓고 결이 맞나 본다 (세100 · takbon-art).

🔴 리드가 정한 합격선: *"음영 두께·외곽선·채도가 한 결로 읽히나"* —
   낱장으로는 절대 안 보이고, 여섯이 같은 바닥 위에 서야 드러난다.
   바닥은 실제 `tileset_ground.png` GRASS_PLAIN + `Daylight(1, 0.99, 0.96)`.
