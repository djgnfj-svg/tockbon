...
-- 🔴 평평한 초록 위의 5~9px 짙은 덩어리는 양이 아니라 크기 때문에 「벌레」로 튄다 — 지우는 대신 크게(22x18) 만들면 「그늘진 자리」로 읽힌다.
local HUGE = {
  { "........####..........", ".....#########........", "...############.......",
    "..##############......", ".################.....", "##################....",
    "###################...", "####################..", "####################..",
...
------------------------------------------------------------------ 질감 레시피
-- 🔴 넓은 면은 조용해야 한다. 큰 저대비 패치가 몸통이고 고대비는 뭉쳐서 아주 조금.
-- 🔴 작은 점 위주 질감은 이어 붙인 판에서 반복이 그대로 읽힌다 — 점은 모양이 또렷해 눈이 같은 배열을 알아본다(부드러운 얼룩은 반복해도 안 들킨다).
-- ⚠ 얼룩은 타일 모서리에 걸쳐 둔다 — 감싸서 찍히므로 이웃 칸과 이어져 한 덩어리가 되고, 격자로 반복되는 「점」이 안 된다.
...
local function paintTile(ox, oy, mask, tox, toy, opt)
  ...
  if not opt.flat then
    ...
    -- 🔴 타일 밖은 감싸지 말고 붙들어라(clamp) — 감싸면 한쪽만 풀인 타일이 자기 반대편을 이웃으로 착각해 흙 한복판에 검은 세로줄을 긋는다.
    -- ⚠ 예외: 자기 자신과 이어 깔리는 얼룩 칸만 감싼다(wrapShade) — clamp하면 좌우 끝에서 음영이 끊겨 이음매가 보인다.
    ...
    for y = 0, T-1 do
      for x = 0, T-1 do
        if snap[y][x] then
          ...
          local hit = 0
          for d = 1, th do
            -- 🔴 왼쪽에 풀이 있다고 다 그늘이 아니다 — 아래 둑(길 건너편 풀)도 경계가 굽으면 왼쪽에 걸려 아래 경계에까지 검은 줄이 그어진다.
          ...
          -- 🔴 두께만이 아니라 진하기도 같이 변한다 — 두께만 흔들면 굵기가 같은 검은 아이라인이 그대로 남는다. 얕은 자리는 연하게 끝난다.
          if hit > 0 then
          ...
          -- 🔴 두 자리를 다 만족할 때만 = 점선이 아니라 끊긴 토막이 된다
...
do
  ...
  local ox, oy = 2*T, 4*T
  -- 🔴 풀 위에는 밝은 자국만 얹는다 — 어두운 자국은 크기와 무관하게 구멍·벌레·돌로 읽힌다
  --   (TAKBON 60의 초록은 간격이 넓어 base(#468232)보다 어두우면 Δ휘도 23↑다). 그래서 풀B는 「짙은 풀」이 아니라 「볕에 마른 풀」이다.
...
-- 🔴 바탕은 흙과 같은 색으로 두고 「알갱이가 드러난 자리」로 만든다 — 톤이 갈리면 옆 흙 타일과 같은 땅으로 안 읽힌다.
do
...
-- 🔴 흙은 길에 붙어 있을 때만 흙이다 — 풀밭 한복판에 뜬 다른 색 섬은 크기·밝기와 무관하게 물체로 읽힌다.
--   그래서 풀 위의 흙이 아니라 흙 위의 풀이다 — 가장 많이 반복되는 면(길)을 깨 준다.
do
