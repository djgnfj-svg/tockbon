-- 숲 나무 3종 + 덤불 — 세100 · takbon-art
--
-- 🔴 이건 **밑그림 생성기**다 (ART_SPEC §4 = 「생성기는 밑그림까지, 완성은 손질이 한다」).
--    수관은 **손으로 적은 잎덩이 좌표표**(CLUMPS)고, 가지는 **손으로 적은 마디 사슬**(NODES)이다.
--    돌린 뒤엔 반드시 export → Read → 국소 수정 루프를 돈다.
--
-- 🔴🔴 이 그림이 놓일 바닥은 **#468232 밝은 풀**이다(boss_room GRASS_TILE_BASE).
--    그래서 수관 지배색을 #468232로 두면 **나무가 바닥에 녹는다**(옛 tree_round이 정확히 그 상태였다:
--    #468232가 최다색 343px). 새 그림은 **바닥보다 어둡고 차가운 덩어리 + 어두운 테두리**로 간다.
--
...
--
-- 7·8회차에 두 번 틀렸다. 죽은 나무가 「살아 있는 나무처럼 붉다」의 답을 두 번 다 **사다리**에서
-- 찾았는데, 사다리는 광원이 정하는 축이라 무채를 끼우면 **그늘 단계 전체가 회색**이 된다:
--   · 8-a: 무채 셋을 명도순으로 끼움 → 가지마다 오른쪽(그늘)에 **균일한 청회색 줄**이 그어졌다.
--   · 8-b: 하나 뺐지만 index 6이 394A50이라 여전히 모든 가지의 그늘 쪽이 파랬다.
...
--   각 덩이가 **자기 중심 기준으로** 음영을 받는다(= 실루엣 오프셋 복사가 아니다 · 진단 ④).
--   base = 그 덩이의 기본 어둠(뒤에 묻힌 덩이일수록 크다) — 손으로 적었다.
----------------------------------------------------------------------
local LX, LY = 0.56, 0.83  -- 광원 = 왼쪽 위

-- 잎 뭉치 도장 4종 (손으로 그린 5~8px 덩어리 — 1px 노이즈 금지 · ART_SPEC §3-5)
local TUFT = {
...
--   himin = 이 덩이가 닿을 수 있는 **가장 밝은 단계**. 🔴 하이라이트를 몇 덩이에만 허용해
--   시선을 몰아준다(진단 ⑤ — 1회차의 「균등한 뭉게 텍스처」가 이걸 안 해서 났다).
local function canopy(buf, clumps, seed, opt)
  ...
  for y = 0, buf.h - 1 do
    for x = 0, buf.w - 1 do
      ...
      if best then
        ...
        --   2회차에 골만 넣었더니 텍스처에 묻혀 안 보였다 — 밝은 쪽이 있어야 덩이가 선다.
        --
        -- ⚠ 3회차 실측: 밴드 폭을 0.11로 **고정**했더니 덩이마다 **똑같은 굵기의 초승달**이 붙어
        --   오히려 기계로 보였다(진단 ②). → 밴드 폭이 **각도에 따라 출렁이고 덩이마다 위상이 다르다.**
        local ang = math.atan(ny, nx)
        local band = 0.065 + 0.06 * math.abs(math.sin(ang * 3.0 + k[1] * 0.7))
        local rim = 1.0 - math.min(bd / band, 1.0)
        -- k[7] = 이 덩이의 하이라이트 세기. 🔴 좌상단 덩이만 세다 = 시선이 거기 몰린다(진단 ⑤).
  ...
  -- 잎 뭉치 texture — 🔴 **격자를 버렸다.** 2회차에 셀당 하나씩 지터로 놨더니
  --   `(cx%2)*2` 오프셋이 수관 전체에 **왼쪽위→오른쪽아래 사선 빗금**을 그었다(진단 ③).
  --   잎이 아니라 무늬로 읽혔다 → 흩뿌리기(Park-Miller)로 바꾸고 진폭도 낮췄다
  --   (덩이 테두리가 형태를 쥐므로 텍스처는 거들기만 하면 된다).
  ...
  for _ = 1, math.floor(buf.w * buf.h / (sp * sp) * 1.35) do
    ...
    local r = rnd()
    -- ⚠ 3회차: 어둡히는 도장이 0.44로 잦아 수관 전체에 **깨알 후추**가 뿌려졌다(진단 ⑤) → 0.30.
    local d = (r < 0.30) and 0.95 or ((r < 0.62) and -0.65 or 0.0)
    if d ~= 0.0 then
      for _, q in ipairs(m) do
        local i = (py + q[2]) * buf.w + (px + q[1])
        -- 🔴 볕 받는 덩이(cap<=2)엔 어둡히는 도장을 안 찍는다 — 하이라이트에 후추가 뿌려지면
        --   시선이 갈 곳이 지저분해진다(진단 ⑤). 그늘 쪽에만 잎 그림자를 준다.
    ...
    for x = 0, w - 1 do
      if mask_of(buf, x, y) then
        ...
        if open_up or open_dn or open_l or open_r then
          ...
          -- ⚠ 2회차엔 `lit<=0.15`를 전부 2px로 깔아 아래 둘레에 **균일한 검은 띠**가 생겼다(진단 ②).
...
-- 위쪽 왼쪽 가장자리에서 몇 칸 안쪽인가 = 그 자리의 밝기. 형태를 따라간다(진단 ④).
  ...
  for y = 0, h - 1 do
    for x = 0, w - 1 do
      if mask[y * w + x] then
        ...
        --   광원과 무관한 3x4 덩어리 해시라 **줄이 안 생긴다**(위 GREY_OF 머리말의 8-a·8-b 참조).
        -- ⚠ 8-c: 얼룩만으로는 줄기 한복판에 회색이 몰려 또 세로 띠로 보였다. **두 조건을 곱한다** —
        --   ⓐ 볕 받는 쪽(k<=2)이어야 하고 ⓑ 얼룩 해시에 걸려야 한다.
        --   ⓐ만 쓰면 4회차의 「테이프」, ⓑ만 쓰면 8-c의 「가운데 띠」. 둘을 곱해야 **볕에 바랜 자리**가 된다.
        if bleach and k <= 2
            and hsh(math.floor(x / 3) + 3, math.floor(y / 4), seed + 2.0) < 0.55 then
          -- ⚠ 여섯 장을 한 줄로 세워 보니 죽은 나무만 **채도가 튀어** 혼자 붉었다.
          --   밝기를 두 번 내려도 안 잡혔는데 당연하다 — **밝기가 아니라 채도 문제**였다.
          --   base를 더 내리는 대신 풍화 얼룩 비율을 0.42 → 0.55로 올려 무채 면적을 늘렸다.
          col = GREY_OF[n]
        end
        buf:set(x, y, col)
      end
...
----------------------------------------------------------------------
-- 접지 그림자 — 🔴 광원이 왼쪽 위니 그림자는 **오른쪽 아래**로 흐른다(대칭 파괴 · 진단 ①)
----------------------------------------------------------------------
...
-- 🔴 고립 조각 청소 — ART_SPEC §3-5(고립 1px 고대비 노이즈 금지). 침엽수 옆선을 솎다가
--   본체에서 **떨어져 떠 있는 부스러기**가 남았다(4회차 실측). 이웃 1개 이하는 지운다.
...
--   5회차에 침엽수 옆선을 3px 열 단위로 솎았더니 이웃이 잘린 열이 통째로 고립돼
--   **허공에 뜬 세로 막대**가 대여섯 개 남았다 — despeckle(이웃 1개 이하)은 이걸 못 잡는다.
...
local function tree_round()
  ...
  local mask = {}
  -- ⚠ 4회차: 128px 수관에 밑동 반지름 15는 **가늘어서 못 버틸 것처럼** 보였다 → 18.
  ...
  -- ⚠ 3회차: 중앙에 rx27짜리 큰 덩이 하나를 뒀더니 그 테두리가 **수관을 대각선으로 자르는 굵은 띠**가
  --   됐다(preview에서 사선 빗금으로 보이던 것). → **둘로 쪼갰다**(51,50)+(72,57).
  -- 🔴🔴 4회차 — 덩이 중심을 **대각선에서 흩었다.** (34,38)→(51,50)→(72,57)→(81,76)이 거의
  --   일직선이라 각 덩이의 밝은 호가 **이어붙어 수관을 가로지르는 사선 띠**가 됐다(preview 3회 내내
  --   보이던 그것). 덩이를 지그재그로 흔들면 호가 끊긴다.
  local CL = {
  ...
  -- ⚠ 2회차엔 그림자 중심이 71이라 밑동(59)보다 12px 오른쪽이어서 **나무가 왼쪽으로 기운 듯** 보였다.
...
local function tree_pine()
  ...
  -- 🔴🔴 3회차 — **잎덩이(타원)를 버렸다.** 타원 열일곱 개로는 침엽수가 안 됐다:
  --   매끈한 크리스마스 트리가 나왔고 활엽수와 실루엣이 안 갈렸다.
  --   침엽수의 몸은 **아래로 처진 층(tier)**이다 → 층마다 밑변이 톱니로 내려앉고,
  --   그 밑변 그늘이 아래 층 위에 떨어져 **가로 결**이 생긴다. 그게 「침엽수」의 읽힘이다.
  --   { ytop, ybot, cx, 위폭, 아래폭, base, himin } — 손으로 적었다. cx가 60~63으로 흔들린다(진단 ①).
  -- ⚠ 4회차: 층 위끝이 78/62/46/30/15/2 = **딱 16 간격**이라 밑변 그늘이 균일한 가로 줄무늬가
  --   됐다(진단 ③). 간격을 18·13·17·14·13으로 흔들고 층 두께도 다르게 했다.
  --   himin도 한 단 올렸다 — 침엽수에 밝은 올리브(7A8C2E)가 잦아 「후추」로 뿌려졌다.
  local TIER = {
  ...
  for _, T in ipairs(TIER) do
    ...
    for x = math.floor(cx - hwb) - 6, math.ceil(cx + hwb) + 6 do
      ...
      if math.abs(dxn) <= 1.02 then
        ...
        -- ⚠ 5회차: x **한 칸마다** 길이를 굴렸더니 밑변이 뭉개져 「흩어진 짧은 대시」가 됐고
        --   층이 안 읽혔다. 4~5px **구간마다** 같은 길이를 줘야 톱니가 「가지 한 다발」로 읽힌다.
        ...
        --   5회차엔 열을 통째로 솎았다가 살아남은 열이 고립돼 허공에 뜬 막대가 됐다.
        ...
        for y = yt, yb do
          ...
          -- ⚠ 4회차: 가로 계수 0.95라 오른쪽 절반(dxn>0)이 통째로 어두워져 **층이 안 읽히고
          --   덩어리로 뭉갰다.** 0.62로 낮춰 오른쪽에도 형태가 남게 한다.
          local t = (-dxn) * 0.62 + (1.0 - 2.0 * vy) * 0.5
          local v = base - t * 2.1   -- ⚠ 1.7이면 명암 폭이 좁아 청록 덩어리 하나로 보였다
          -- 🔴 층 밑변 3행이 **가장 어둡다** = 위 층이 아래 층에 드리우는 가로 결.
          --   3회차엔 +2.3인데 아래 층 윗부분도 같이 어두워서(+0.9) 대비가 죽어 층이 안 읽혔다.
  ...
  -- ⚠ 5회차: 좌표를 손으로 적어 놓고 **본체에 붙었는지 안 봤다** → 층 밖 허공에 세로 막대 대여섯이
  --   떠 있었다. 이제 **시작점 바로 위가 몸이어야만** 그린다(붙음 보장).
...
local function tree_dead()
  ...
  -- 줄기 — 🔴 1회차엔 밑동 반지름이 30이라 「나무」가 아니라 **굵은 통나무**로 읽혔다. 22로 줄였다.
  ...
  -- ⚠ 4회차: 오른쪽 큰 가지가 왼쪽(B1)과 **같은 각도로 같은 높이까지** 뻗어 거울상이었다(진단 ①).
  --   낮고 완만하게 · 짧게 끊어 「부러진 쪽」으로 만든다.
  ...
  -- 🔴🔴 밑동 세로 균열 — **손으로 찍은 좌표를 여기로 옮겨 적었다**(7회차에 MCP draw_pixels로
  --   찍어 보고 고른 경로다). 램프를 바꾸느라 다시 생성해야 했는데, 표에 옮기지 않으면 손질이
  --   그대로 날아간다. ⚠ 앞으로 이 나무를 다시 생성해도 이 균열은 남는다.
  --   x가 몇 행마다 한 칸씩 어긋난다 = 갈라진 틈은 곧게 안 간다(진단 ③).
  local CRACK_MAIN = {
  ...
  -- ⚠ 8회차: 입술을 884B2B(주황 갈색)로 찍었더니 어두운 밑동에 **붉은 반점**이 튀어 상처처럼 보였다.
  --   무채(577277)로 바꿨더니 이번엔 세로 회색 줄에 가담했다 → 개수를 반으로 줄였다.
...
----------------------------------------------------------------------
-- 🔴 크기 판단의 근거(9회차): 리드가 「32px 안팎, 정확한 값은 나무와 나란히 놓고 정해라」고 남겼다.
--   실제로 한 화면(960x540)에 3~4그루와 같이 깔아 보니 몸 30px은 128px 나무 옆에서
--   **「덤불」이 아니라 「이끼 덩어리」**로 읽혔다 → 몸 ~35px(프레임 56x48)로 한 단 올렸다.
--   접지선 = row 40.
local function bush()
  local W, H = 56, 48
  local buf = Buf.new(W, H)
  ...
  -- ⚠ 8회차: base 5.2면 잔가지가 밝은 갈색이라 잎 사이에서 **붉은 점(꽃? 상처?)**으로 튀었다.
  shade_mask(buf, mask, W, H, WOOD, 6.8, 3, 6.6)

  -- ⚠ 6회차: 덩이가 크고 밝아 **바닥보다 밝은 빵 덩어리**가 됐다(나무는 어둡게 갔는데 덤불만 밝으면
  --   같은 숲으로 안 읽힌다). 덩이를 잘게 쪼개 rim이 먹게 하고, 전체를 한 단 내렸다.
  local CL = {
...
--   **다른 셋에 이미 얹은 손질(MCP draw_pixels)을 날리지 않기 위한 안전장치다** —
--   전량 재생성은 손질 루프를 통째로 되감는다(ART_SPEC §4-3 「전체 재생성 금지」).
