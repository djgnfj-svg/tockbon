...
func _process(_delta: float) -> bool:
	...
	match here:
		...
		"fight":
			...
			_game.field_view._process(FRAME_SEC)
		"reward":
			# The beak: the panel picks a soldier. Whatever is under the first roster row.
			_click(Look.PANEL_ORIGIN_PX + Look.ROSTER_ORIGIN_OFFSET_PX
				+ Look.ROSTER_ENTRY_SIZE_PX * 0.5)
		"cards":
