...
# ⚠ 세112: 여기 있던 `AimStub`(조준을 고정해 돌려주던 「플레이어」)과 `_fog`·`_fog_stub`이
#  `fog` 프리셋과 함께 사라졌다 — 시야 안개가 은퇴했다.
...
# ⚠ 세112: 여기 있던 `_setup_fog`·`_occlude_radius`·`_aim_fog`(= `fog` 프리셋의 무대·엄폐물·조준)가
#  시야 안개와 함께 사라졌다. ⚠ 그 셋은 이미 **세107 차폐 은퇴 뒤 죽어 있었다** — `boss_room`의
#  `OCCLUDER_GROUP`·`OCCLUDE_R_META`를 읽는데 그 상수가 그때 걷혔다(부르면 런타임 에러였다).
