...
static func bake() -> bool:
	...
	# 🔴 산출물 머리글도 낡는다. 2026-08-07까지 여기가 「`build_terrain_tileset.gd`의 표」를 가리켰는데
	#  **그 표는 없어졌고**(`terrain_palette.gd` 파생) `NAME_BY_MAT` 은 안 적혀 있었다.
	out += "## 🔴 재질을 새로 추가했으면 `terrain_baker.gd`의 CHAR_BY_MAT·NAME_BY_MAT을 늘리고\n"
	out += "## **이 파일을 다시 구워야 한다** — 도구만 고치면 이 산출물은 안 따라온다.\n"
	out += "## (붓 그림과 타일셋 표는 `terrain_palette.gd` 파생이라 손댈 것이 없다.)\n\n"
