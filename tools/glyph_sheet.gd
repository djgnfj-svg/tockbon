...
##
## 왜: `RingBoard`의 밑그림은 점열을 **코드로 계산한다** — 열어볼 그림 파일이 없다. 이 도구는
##   그 static 함수를 직접 불러 격자로 그린다.
##
## 🔴 `--headless`를 쓰지 마라 — 렌더를 안 해서 빈 이미지가 나온다(`shot.gd`와 같은 이유).
## 🔴 `load()`로 지연 로드한다 — `-s` 스크립트의 `_init`은 오토로드 등록보다 먼저 돌아서
##   `preload`로 물면 그 스크립트가 참조하는 오토로드 식별자에서 컴파일 에러가 난다.
##
## 사용:
